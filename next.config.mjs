...
const contentSecurityPolicy = [
  ...
  // Nerve is reached from a non-loopback host. Same-origin HTTP fetches stay covered
...
// NERVE_BUILD_PROFILE=minimal physically removes four optional privileged
...
// The resulting artifact is intended to be published as `nerve-secure`
// for security-sensitive environments. See docs/security/SOCKET_DEV_FINDINGS.md.
const isMinimalBuild = process.env.NERVE_BUILD_PROFILE === "minimal";
...
const nextConfig = {
  ...
  // Nerve under https://host/nerve/). Empty by default so root-path
  ...
  basePath: normalizeBasePath(process.env.NERVE_BASE_PATH),
  ...
  env: {
    NEXT_PUBLIC_NERVE_BASE_PATH: normalizeBasePath(process.env.NERVE_BASE_PATH),
  },
  distDir,
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // (Docker sets NERVE_MITM_STUB=1 — #3390 graceful degradation). The
  ...
  // Nerve is a proxy for AI APIs — request bodies routinely include
  ...
  experimental: {
    serverActions: {
      bodySizeLimit: process.env.NERVE_SERVER_ACTIONS_BODY_LIMIT || "50mb",
    },
    ...
    // Next's internal router proxy defaults to 30s when this is unset. Nerve
    ...
    // PR-2 of diegosouzapw/Nerve#3932: tree-shake barrel re-exports so
    ...
    // the internal `@nerve/open-sse` workspace here: optimizePackageImports
  ...
  transpilePackages: ["@nerve/open-sse", "@lobehub/icons", "fumadocs-ui", "fumadocs-core"],
  ...
  webpack(config, { webpack }) {
    ...
    config.optimization.splitChunks = {
      ...
      cacheGroups: {
        ...
        // PR-2 of diegosouzapw/Nerve#3932: isolate the heavy long-tail
  ...
  async headers() {
    return [
      ...
      // G-10: allow Nerve's own dashboard to embed the 9Router UI via our reverse proxy.
  ...
  async rewrites() {
    return [
      ...
      // so they hit the /api/[...nerveApiCatchAll] route (#6424) — which
