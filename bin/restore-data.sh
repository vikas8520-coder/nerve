#!/usr/bin/env bash
# bin/restore-data.sh — restore the Nerve SQLite data volume from a snapshot
...
Stop Nerve before running, and restart it afterwards.
EOF
}
...
ops_log "restore source: $snap → $NERVE_DATA_DIR"
ops_confirm "Overwrite storage.sqlite at $NERVE_DATA_DIR from $snap?" || ops_die "aborted"

# Pre-restore safety copy of the live data.
if [ -f "$NERVE_SQLITE" ]; then
  safety="$NERVE_BACKUPS_DIR/pre-restore_$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$safety"
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$NERVE_SQLITE" "VACUUM INTO '$safety/storage.sqlite'" \
      || cp -a "$NERVE_SQLITE" "$safety/storage.sqlite"
  else
    cp -a "$NERVE_SQLITE" "$safety/storage.sqlite"
  fi
...
mkdir -p "$NERVE_DATA_DIR"
# Drop stale WAL/SHM so the restored DB is authoritative, then copy in.
rm -f "$NERVE_SQLITE" "${NERVE_SQLITE}-wal" "${NERVE_SQLITE}-shm"
cp -a "$snap/storage.sqlite" "$NERVE_SQLITE"

# Restore sibling DBs captured in the snapshot.
for f in "$snap"/*.sqlite; do
  [ -e "$f" ] || continue
  [ "$(basename "$f")" = "storage.sqlite" ] && continue
  cp -a "$f" "$NERVE_DATA_DIR/"
done

ops_log "restore complete — restart Nerve to pick up the restored data"
