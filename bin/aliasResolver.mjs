 ...
 *
 * Problem (#7791): when Nerve is installed via `npm i -g nerve`, the
 * package files live under `node_modules/nerve/`. tsx's tsconfig-path
 * resolution does not apply there, so specifiers like `@/shared/utils/featureFlags`
 * (declared in tsconfig.json `paths` as `@/* → ./src/*`) or
 * `@nerve/open-sse/services/usage` fail with `ERR_MODULE_NOT_FOUND`.
 ...
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@nerve/open-sse/"`)
 ...
 *             prefix *without* a trailing slash (e.g. `@nerve/open-sse` →
...
export const ALIAS_MAP = [
  ...
  { prefix: "@nerve/open-sse/", target: "open-sse", exact: false },
  { prefix: "@nerve/open-sse", target: "open-sse", exact: true },
];
 ...
 *   "@/..."                     → <root>/src/...
 *   "@nerve/open-sse/..."   → <root>/open-sse/...
 *   "@nerve/open-sse"       → <root>/open-sse/index.*
 *
...
export function resolveAlias(specifier, root) {
  ...
  // Exact match (e.g. `@nerve/open-sse` with no trailing path) →
