 ...
 *   nerve reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | nerve reset-password
 *   printf 'NewPass123' | nerve reset-password --password-stdin
 *
 * Resets the admin password for Nerve.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure Nerve has been started at least once.`);
  ...
  console.log("   Restart Nerve for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/nerve.mjs
    // routes `nerve reset-password` here) terminates cleanly instead of
