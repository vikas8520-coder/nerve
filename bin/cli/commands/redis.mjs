...
const DEFAULT_NAME = "nerve-redis";
const DEFAULT_PORT = "6379";
const DEFAULT_VOLUME = "nerve-redis-data";
...
export function registerRedis(program) {
  const redis = program
    ...
    .description(
      t("redis.description") ||
        "Launch a 1-click local Redis container (Podman or Docker) for Nerve caching and quota tracking"
    );
...
export async function runRedisUpCommand(opts = {}) {
  ...
  try {
    ...
    info(`Set NERVE_REDIS_URL=redis://127.0.0.1:${port} in your .env to wire Nerve to it.`);
    return 0;
  } catch (err) {
...
export async function runRedisStatusCommand(opts = {}) {
  ...
  if (!running) {
    info(`Run 'nerve redis up' to launch it.`);
  }
  return 0;
}