/**
 * nerve setup-crush — configure Crush (charmbracelet/crush) for Nerve.
 *
 ...
 * include /v1; the api_key may reference an env var (`$NERVE_API_KEY`) so the
...
const API_KEY_REF = "$NERVE_API_KEY";
...
export function resolveCrushTarget(opts = {}) {
  ...
  if (opts.remote) root = String(opts.remote).replace(/\/+$/, "");
    ...
    try {
      root = resolveActiveContext(opts.context ?? process.env.NERVE_CONTEXT)?.baseUrl;
    } catch {
      /* none */
    }
...
export function buildCrushProvider(modelIds, baseUrl) {
  ...
  for (const id of modelIds) {
    ...
    if (!cfg) continue;
    models.push({ id, name: `Nerve: ${id}`, context_window: cfg.ctx });
  }
  return {
    type: "openai-compat",
    base_url: baseUrl,
...
/** Merge the Nerve provider into an existing crush.json (preserve the rest). */
export function mergeCrushConfig(existing, provider) {
  const cfg = existing && typeof existing === "object" ? { ...existing } : {};
  cfg.providers = { ...(cfg.providers || {}), nerve: provider };
  return cfg;
}
...
export async function runSetupCrushCommand(opts = {}) {
  ...
  printHeading("Nerve → Crush (openai-compat)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${provider.models.length} model(s) under providers.nerve → ${configPath}`);
    return 0;
  }
  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${provider.models.length} models under providers.nerve)`);
  printInfo("Provide the key (config references $NERVE_API_KEY):  export NERVE_API_KEY=...");
...
export function registerSetupCrush(program) {
  program
    .command("setup-crush")
    .description("Generate the Nerve openai-compat provider in ~/.config/crush/crush.json")
