/**
 * nerve setup-cline — configure the Cline AI coding agent to use Nerve.
 *
 ...
 * writes the CLI-mode files (matching the Nerve dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(resolveActiveContext(opts.context ?? process.env.NERVE_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
/** Merge Nerve openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_nerve" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("Nerve → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_nerve"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for Nerve: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
