/**
 * nerve setup-roo — configure Roo Code (RooVeterinaryInc.roo-cline) for Nerve.
 *
 ...
 * must support native OpenAI tool-calling (Nerve does).
 */
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "Nerve",
      apiConfigs: {
        Nerve: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_nerve",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath = opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".nerve", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ?? opts["vscode-settings"] ?? join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("Nerve → Roo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    console.log(`\n── [dry-run] ${importPath} ──`);
    console.log(JSON.stringify({ ...importDoc, providerProfiles: { ...importDoc.providerProfiles, apiConfigs: { Nerve: { ...importDoc.providerProfiles.apiConfigs.Nerve, openAiApiKey: apiKey ? "set" : "sk_nerve" } } } }, null, 2));
  ...
  printInfo(`  API Key:   <your NERVE_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for Nerve: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.nerve/roo-settings.json)")
