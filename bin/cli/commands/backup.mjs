...
export function registerBackup(program) {
  ...
  // Legacy: `nerve backup` without a subcommand still creates a backup
  ...
  // (#8512), and no doc shows `nerve backup` invoked with flags.
...
async function pruneBackups(backupDir, retention) {
  ...
  try {
    const dirs = readdirSync(backupDir)
      .filter((f) => f.startsWith("nerve-backup-"))
      .sort()
      .reverse();
...
export async function runBackupCommand(opts = {}) {
  ...
  const backupName = safeName ? `nerve-backup-${safeName}` : `nerve-backup-${timestamp}`;
...
async function _uploadBackupToCloud(backupPath, info) {
  ...
  try {
    const boundary = `nerve-backup-${Date.now().toString(36)}-${randomBytes(8).toString("hex")}`;
    ...
    const cliToken = process.env.NERVE_CLI_TOKEN ?? (await getCliToken());
...
export async function runRestoreCommand(backupId, opts = {}) {
  ...
  if (opts.list || !backupId) {
    ...
    if (!backupId) console.log("\nUsage: nerve restore <backup-id>");
    return 0;
  }

  const safeBackupId = String(backupId).replace(/[/\\]/g, "_");
  const backupPath = join(backupDir, `nerve-backup-${safeBackupId}`);
