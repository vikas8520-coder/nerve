/**
 * nerve setup-cursor — guide Cursor to use Nerve.
 *
...
export function buildCursorInstructions({ apiBase, models }) {
  const lines = [
    ...
    "  3. Set the OpenAI API Key to your Nerve key (NERVE_API_KEY)",
...
export function registerSetupCursor(program) {
  program
    ...
    .description("Print the steps to point Cursor at Nerve (chat panel; Cursor config is not file-writable)")
