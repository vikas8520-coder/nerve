...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or NERVE_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `nerve setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("Nerve Setup");
    const { db, dbPath } = await openNerveDb();
