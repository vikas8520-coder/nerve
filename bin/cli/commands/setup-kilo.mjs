/**
 * nerve setup-kilo — configure Kilo Code to use Nerve.
 *
 ...
 * This writes BOTH (matching the Nerve dashboard) and prints the UI settings.
 *
...
/** Merge the Nerve openai-compatible provider into Kilo's CLI auth.json. */
export function buildKiloAuth(existing, { apiKey, baseUrl, model }) {
  const auth = { ...(existing || {}) };
  auth["openai-compatible"] = {
    ...(auth["openai-compatible"] || {}),
    apiKey: apiKey || "sk_nerve",
    baseUrl,
    model,
  };
  return auth;
...
export function buildKiloVscodeSettings(existing, { apiKey, baseUrl, model }) {
  const s = { ...(existing || {}) };
  s["kilocode.customProvider"] = { name: "Nerve", baseURL: baseUrl, apiKey: apiKey || "sk_nerve" };
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("Nerve → Kilo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(
        { "openai-compatible": { ...auth["openai-compatible"], apiKey: apiKey ? "set" : "sk_nerve" } },
        null,
        2
      )
    );
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for Nerve: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
