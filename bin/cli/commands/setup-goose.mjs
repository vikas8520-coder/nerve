/**
 * nerve setup-goose — configure Goose (block/goose) for Nerve.
 *
...
export async function runSetupGooseCommand(opts = {}) {
  ...
  printHeading("Nerve → Goose (openai-compatible)");
...
export function registerSetupGoose(program) {
  program
    .command("setup-goose")
    .description("Configure Goose for Nerve: write ~/.config/goose/config.yaml + print the env recipe")
