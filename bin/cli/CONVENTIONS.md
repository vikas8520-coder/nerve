# Nerve CLI — Internal Conventions
...
```
nerve keys add openai sk-xxx
nerve combo switch fastest
nerve memory search "react hooks"
```

**Not allowed**:

```
nerve --add-key openai sk-xxx     # ❌ flag-as-verb
nerve add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `NERVE_BASE_URL` env or `~/.nerve/config.json`
  (active profile).
- Injects `Authorization: Bearer ${NERVE_API_KEY}` when available.
- Injects `x-nerve-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `NERVE_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `NERVE_LANG` to `~/.nerve/.env`.
