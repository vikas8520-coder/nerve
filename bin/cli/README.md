# bin/cli — Nerve CLI internals

This directory contains the CLI runtime, helpers, and commands for the `nerve` binary.
...
├── sqlite.mjs              ← openNerveDb() — DB bootstrap
...
All HTTP calls to the Nerve server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `NERVE_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `NERVE_API_KEY`)
...
Locale detection order: `NERVE_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `NERVE_LANG` environment variable
...
```bash
nerve config lang set pt-BR       # saves to ~/.nerve/.env
nerve config lang list            # show all 42 available locales
nerve config lang get             # show currently active locale
```

**One-time override:**

```bash
nerve --lang de providers list    # run in German, not persisted
NERVE_LANG=ja nerve status    # same effect via env
```
