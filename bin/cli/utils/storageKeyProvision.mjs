 ...
 *
 * Purely informational invocations must NOT create `~/.nerve/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `nerve --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `nerve` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `nerve` runs the default `serve` command → must provision.
