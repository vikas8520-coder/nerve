...
// ~/.nerve/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `Nerve  •  port ${port}`, tooltip: "Server running", enabled: false },
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[nerve][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
