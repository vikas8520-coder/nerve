...
const APP_LABEL = "com.nerve.autostart";
const WIN_REG_VALUE = "Nerve";
const WIN_STARTUP_FILE = "Nerve.vbs";
const LINUX_SERVICE_NAME = "nerve.service";
const LINUX_DESKTOP_NAME = "nerve.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v nerve 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "nerve.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("nerve.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".nerve", ".env");
  const lines = [
    "[Unit]",
    "Description=Nerve AI proxy router",
 ...
 * running process. When the running Nerve cli was itself spawned by the
...
/**
 * Builds the VBScript source that launches Nerve with WSH's Run method
