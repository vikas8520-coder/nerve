...
const NERVE_IPC_PORT_BASE = 29128;

export function initWinTray({ port, onQuit, onOpenDashboard, onShowLogs }) {
  if (process.platform !== "win32") return null;

  const ipcPort = NERVE_IPC_PORT_BASE + (port % 1000);
  const scriptPath = join(tmpdir(), `nerve-tray-${process.pid}.ps1`);
...
$mQuit = $menu.Items.Add("Quit Nerve")
