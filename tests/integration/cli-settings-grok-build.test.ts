 ...
 * rebuilt on top of Nerve's existing "custom" configType settings pattern
 ...
 * the `[model.nerve]` section and preserves the rest of the file.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-grok-build-settings-"));
...
// ── Test 4: POST with valid body → surgically upserts [model.nerve] ─────

test("grok-build-settings POST: writes [model.nerve] section and preserves existing content", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("[model.nerve]"), "Config should have [model.nerve]");
      assert.ok(content.includes("http://localhost:20128/v1"), "Config should contain base URL");
      assert.ok(content.includes('default = "nerve"'), "Default should point at our slot");
      ...
      assert.ok(
        content.includes('nerve-prev-default = "grok-build"'),
...
test("grok-build-settings DELETE: removes our section, preserves the rest, restores default", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      const configPath = path.join(tmpHome, ".grok", "config.toml");
      const content = fs.readFileSync(configPath, "utf-8");
      assert.ok(!content.includes("[model.nerve]"), "Our section should be removed");
