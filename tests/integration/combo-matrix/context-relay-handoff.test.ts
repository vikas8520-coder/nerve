...
//      - request carries x-nerve-session-id header
...
// reads "x-session-id" (not "x-nerve-session-id") and prefixes the result with "ext:".
...
//     internal summary request (identified by _nerveInternalRequest flag in body)
...
function installHandoffAwareFetch() {
  ...
  globalThis.fetch = async (url: any, init: any = {}) => {
    ...
    if (bodyObj._nerveInternalRequest === "universal-handoff") {
...
// Without the x-nerve-session-id header, sessionId = null → block is skipped.
test("context-relay universal handoff: does NOT fire when x-nerve-session-id header is absent", async () => {
  ...
  assert.equal(
    handoff,
    null,
    "handoff must NOT be written when x-nerve-session-id header is absent (sessionId gate)"
  );
