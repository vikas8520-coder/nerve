...
// completely independent of the per-request `x-nerve-compression` header/mode. The internal
...
// a request carrying `x-nerve-compression: off` must NOT get the styles system message
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-testmodel-compression-"));
...
test("chatCore: x-nerve-compression: off suppresses Output Styles injection even with the operator's global style enabled (#6240)", async () => {
  ...
  assert.match(plainFirstMessage?.content ?? "", /Nerve Output Styles/);

  // The "Test model" connection test sends x-nerve-compression: off — must be clean.
  const testModelBody = await runChatCore({
    provider,
    model,
    connectionId: connection.id,
    headers: new Headers({ "x-nerve-compression": "off" }),
  });
  ...
  const anyMessageHasMarker = (testModelBody?.messages ?? []).some((m) =>
    (m?.content ?? "").includes("Nerve Output Styles")
  );
  assert.equal(
    anyMessageHasMarker,
    false,
