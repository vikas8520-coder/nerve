 ...
 *
 * Letta configures Nerve as its "lmstudio" provider (localModelDiscovery:
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-letta-settings-"));
...
// ── Test 6: POST refuses to overwrite an existing non-Nerve lmstudio config ──
...
// ── Test 7: DELETE → removes the Nerve lmstudio config ──────────────────
