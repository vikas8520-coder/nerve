...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-jcode-settings-"));
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        assert.equal(written._managedBy, "nerve");
...
// ── Test 5: DELETE → removes Nerve fields ────────────────────────────────

test("jcode-settings DELETE: removes Nerve fields from existing config", async () => {
