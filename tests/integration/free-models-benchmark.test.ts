 ...
 *
 * Live benchmark: how well do the free-tier models Nerve exposes handle
 ...
 *
 * Environment:
 *   NERVE_API_KEY  — required (else test skips)
...
test(
  ...
  async () => {
    ...
    // Nerve's routing) is broken, not that every free provider failed at
    // once.
    assert.ok(
      totalSucceeded > 0,
