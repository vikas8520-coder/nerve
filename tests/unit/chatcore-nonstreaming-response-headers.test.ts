...
// #3501). attachNerveMetaHeaders + now are injected so the static headers, the meta payload,
...
function makeDeps(now = 1000) {
  ...
  const deps = {
    attachNerveMetaHeaders: (headers: Record<string, string>, meta: Record<string, unknown>) => {
      metaCalls.push({ headers, meta });
      headers["x-nerve-meta"] = "attached";
    },
    now: () => now,
