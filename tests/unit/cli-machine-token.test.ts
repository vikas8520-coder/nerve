...
test("cliToken.mjs pode ser importado sem erro", async () => {
  ...
  assert.equal(mod.CLI_TOKEN_HEADER, "x-nerve-cli-token");
});
...
test("NERVE_CLI_TOKEN env sobrescreve token gerado em apiFetch", async () => {
  const orig = process.env.NERVE_CLI_TOKEN;
  process.env.NERVE_CLI_TOKEN = "test-override-token-12345";
  try {
    // Re-import api.mjs não funciona por cache ESM — validamos apenas que env é lido.
    assert.equal(process.env.NERVE_CLI_TOKEN, "test-override-token-12345");
  } finally {
    if (orig === undefined) delete process.env.NERVE_CLI_TOKEN;
    else process.env.NERVE_CLI_TOKEN = orig;
  }
});
...
test("NERVE_DISABLE_CLI_TOKEN desabilita auth (estrutura verificada)", async () => {
  const { readFileSync } = await import("node:fs");
  const { join, dirname } = await import("node:path");
  const { fileURLToPath } = await import("node:url");
  const dir = dirname(fileURLToPath(import.meta.url));
  const src = readFileSync(join(dir, "../../src/lib/middleware/cliTokenAuth.ts"), "utf8");
  assert.ok(src.includes("NERVE_DISABLE_CLI_TOKEN"));
});
