 ...
 * Helper.app bundles from build.productName ("Nerve"). The old code used
 * app.getName() (package.json `name` = "nerve-desktop"); the two diverged, so it
...
const EXEC = "/Applications/Nerve.app/Contents/MacOS/Nerve";

describe("#7941 — resolveDarwinHelperExecutable()", () => {
  it("resolves the productName-derived framework Helper even though app.getName() would say 'nerve-desktop'", () => {
  ...
  it("prefers the sibling (unsuffixed) Helper next to the main binary when present", () => {
    const sibling = path.join(path.dirname(EXEC), "Nerve Helper");
    const result = resolveDarwinHelperExecutable({
      execPath: EXEC,
