...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-combo-test-route-"));
...
test("combo test route marks a model healthy only when it returns assistant text", async () => {
  ...
  assert.equal(fetchCalls[0].init.headers["X-Nerve-No-Cache"], "true");
...
test("combo test route preserves structured step metadata for repeated model/account targets", async () => {
  ...
  assert.equal(fetchCalls[0].init.headers["X-Nerve-Connection"], "conn-openai-a");
  assert.equal(fetchCalls[1].init.headers["X-Nerve-Connection"], "conn-openai-b");
