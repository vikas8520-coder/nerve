...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-image-route-auth-"));
...
test("v1 image generation POST rejects an invalid presented API key", async () => {
  const originalNerveApiKey = process.env.NERVE_API_KEY;
  const originalRequireApiKey = process.env.REQUIRE_API_KEY;
  process.env.NERVE_API_KEY = "valid-image-route-key";
  process.env.REQUIRE_API_KEY = "true";
  ...
  try {
    ...
    if (originalNerveApiKey === undefined) {
      delete process.env.NERVE_API_KEY;
    } else {
      process.env.NERVE_API_KEY = originalNerveApiKey;
    }
    if (originalRequireApiKey === undefined) {
      delete process.env.REQUIRE_API_KEY;
    } else {
