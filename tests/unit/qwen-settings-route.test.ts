...
const TEST_HOME = path.join(os.tmpdir(), `nerve-qwen-route-${process.pid}-${Date.now()}`);
...
test("POST writes the shared V4 contract and preserves unrelated credentials", async () => {
  ...
  assert.match(env, /^NERVE_API_KEY="sk-route-secret"$/m);
});

test("DELETE removes only Nerve-owned settings and env lines", async () => {
  const configured = {
    ui: { theme: "dark" },
