...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/nerve" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/nerve");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/nerve",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/nerve/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "nerve-base",
    ...
    assert.equal(config.repoDir, "/srv/nerve");
    assert.equal(config.composeFile, "/srv/nerve/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "nerve-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart nerve --update-env \|\| true/);
  });
});
