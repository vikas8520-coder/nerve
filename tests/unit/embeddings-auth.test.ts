...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.NERVE_API_KEY;

  await t.test("should return true if key matches NERVE_API_KEY", async () => {
    process.env.NERVE_API_KEY = "test-key";
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.NERVE_API_KEY = originalOmniKey;
});
