...
test("C2 header contract — server.cjs intercept must inject x-nerve-source and x-nerve-agent", async () => {
  ...
  assert.match(
    src,
    /"x-nerve-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-nerve-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-nerve-agent":\s*agentId/,
    'server.cjs must inject "x-nerve-agent: <id>" from the host→agent map'
  );
