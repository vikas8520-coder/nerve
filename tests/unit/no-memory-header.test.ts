...
// `x-nerve-no-memory` header (mirrors `x-nerve-no-cache`).
...
test("isNoMemoryRequested is true for truthy header values (plain object)", () => {
  for (const v of ["true", "1", "yes", "TRUE", "Yes", " true "]) {
    assert.equal(
      isNoMemoryRequested({ "x-nerve-no-memory": v }),
      true,
...
test("isNoMemoryRequested is case-insensitive on the header NAME", () => {
  assert.equal(isNoMemoryRequested({ "X-Nerve-No-Memory": "true" }), true);
});

test("isNoMemoryRequested works with a Headers instance", () => {
  const h = new Headers();
  h.set("x-nerve-no-memory", "1");
...
test("isNoMemoryRequested is false when the header is absent / empty / falsy", () => {
  ...
  assert.equal(isNoMemoryRequested({ "x-nerve-no-memory": "" }), false);
  assert.equal(isNoMemoryRequested({ "x-nerve-no-memory": "false" }), false);
  assert.equal(isNoMemoryRequested({ "x-nerve-no-memory": "0" }), false);
  assert.equal(isNoMemoryRequested({ "x-nerve-no-memory": "no" }), false);
});

test("getHeaderValueCaseInsensitive still resolves the header (sanity)", () => {
  assert.equal(
    getHeaderValueCaseInsensitive({ "x-nerve-no-memory": "true" }, "x-nerve-no-memory"),
    "true"
  );
});
