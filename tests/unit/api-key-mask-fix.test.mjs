...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_nerve when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_nerve");
  });

  it("falls back to sk_nerve when both are undefined", async () => {
    mockKeyStore.clear();
