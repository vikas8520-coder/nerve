...
test("Test 3: handleChat returns cached response directly for Semantic Cache hits", async () => {
  ...
  assert.equal(res2.headers.get("X-Nerve-Cache"), "HIT");
});

test("Test 4: handleChat supports X-Nerve-Progress tracking header for streams", async () => {
  ...
  assert.equal(response.headers.get("X-Nerve-Progress"), "enabled");
...
test("handleChat returns cached response directly for Idempotency hits", async () => {
  ...
  assert.equal(response2.headers.get("X-Nerve-Idempotent"), "true");
...
test("handleChat returns Semantic Cache hit", async () => {
  ...
  const json2 = (await response2.json()) as any;
  assert.equal(response2.status, 200);
  assert.equal(response2.headers.get("X-Nerve-Cache"), "HIT");
