...
test("wantsProgress detects X-Nerve-Progress header correctly", () => {
  const headersObj = { "x-nerve-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-nerve-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-nerve-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-nerve-progress": "false" }), false);
