...
test("buildCodexEnv strips stale OpenAI/Codex creds from the child env (defense-in-depth)", () => {
  ...
  assert.equal(env.NERVE_API_KEY, "oma_live_x");
...
test("buildCodexEnv uses a no-auth sentinel when no token is given", () => {
  ...
  assert.equal(env.NERVE_API_KEY, "nerve-no-auth");
});
...
test("buildCodexProviderArgs defines the nerve provider inline (works without config.toml)", () => {
  const args = buildCodexProviderArgs("http://vps:20128");
  const joined = args.join(" ");
  assert.ok(joined.includes('model_provider="nerve"'));
  assert.ok(joined.includes('model_providers.nerve.base_url="http://vps:20128/v1"'));
  assert.ok(joined.includes('model_providers.nerve.env_key="NERVE_API_KEY"'));
  assert.ok(joined.includes('model_providers.nerve.wire_api="responses"'));
  assert.ok(joined.includes("model_providers.nerve.requires_openai_auth=false"));
