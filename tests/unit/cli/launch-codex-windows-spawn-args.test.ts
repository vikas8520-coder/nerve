...
test("quoteCodexArgs: exact win32 encoding (golden)", () => {
  const golden: Array<[string, string]> = [
    ["-c", '^^^"-c^^^"'],
    [
      'model_providers.nerve.base_url="http://localhost:20128/v1"',
      '^^^"model_providers.nerve.base_url=\\^^^"http://localhost:20128/v1\\^^^"^^^"',
    ],
    [
      "model_providers.nerve.requires_openai_auth=false",
      '^^^"model_providers.nerve.requires_openai_auth=false^^^"',
    ],
...
test("quoteCodexArgs does not mutate the caller's array", () => {
  const input = ["-c", "model_provider=\"nerve\""];
  quoteCodexArgs(input, "win32");
  assert.deepEqual(input, ["-c", 'model_provider="nerve"']);
});
...
test(
  ...
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "nerve-codex-argv-"));
    try {
