...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "nerve-autostart-win-"));
...
test("Windows enable/disable writes and removes VBS in Startup folder", async () => {
  ...
  const vbsPath = join(startupDir, "Nerve.vbs");
  ...
  assert.match(vbs, /serve --no-open --tray/, "launches Nerve tray server");
