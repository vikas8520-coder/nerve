...
test("runFabricatedDocsCheck: index contains real Nerve routes", () => {
  const result = runFabricatedDocsCheck();
  ...
  const found = findingsFor({
    files: {
      "open-sse/config/constants.ts":
        'const t = envInt("NERVE_CIRCUIT_BREAKER_OAUTH_THRESHOLD", 8);\n',
    },
    docs: { "cfg.md": "Override with `NERVE_CIRCUIT_BREAKER_OAUTH_THRESHOLD`.\n" },
  });
  assert.ok(
    !found.has("env-var::NERVE_CIRCUIT_BREAKER_OAUTH_THRESHOLD"),
...
test("cli-cmd: an arg-bearing `.command('connect <host>')` registration is NOT flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "You log in once with `nerve connect <host>`.\n" },
  });
  assert.ok(
    !found.has("cli-cmd::nerve connect"),
...
test("ANTI-OVER-SUPPRESSION: an unregistered subcommand IS still flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "Then run `nerve teleport <host>` to finish.\n" },
  });
  assert.ok(
    found.has("cli-cmd::nerve teleport"),
