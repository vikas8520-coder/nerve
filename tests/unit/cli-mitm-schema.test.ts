...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_nerve',
...
test("sentinel guard condition catches sk_nerve and null", () => {
  const SENTINEL = "sk_nerve";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_nerve')
  ...
  assert.equal(shouldReject("sk_nerve"), true, "sentinel must be rejected");
