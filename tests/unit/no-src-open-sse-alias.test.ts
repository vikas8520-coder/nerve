 ...
 * workspace (alias `@nerve/open-sse/*`). So an import of `@/open-sse/...`
...
test("src/ never imports the open-sse workspace via the @/ alias (use @nerve/open-sse)", () => {
  ...
  assert.deepEqual(
    offenders,
    [],
    `@/open-sse/* resolves to the non-existent src/open-sse/* and throws "Cannot find module" at runtime. Use @nerve/open-sse/* instead. Offenders:\n${offenders.join("\n")}`
  );
});
