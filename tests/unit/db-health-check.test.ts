...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-db-health-check-"));
...
test("getDbInstance can auto-repair persisted broken rows when startup repair is forced", async () => {
  ...
  const previousForce = process.env.NERVE_FORCE_DB_HEALTHCHECK;
  process.env.NERVE_FORCE_DB_HEALTHCHECK = "1";
  try {
    db = core.getDbInstance();
  } finally {
    if (previousForce === undefined) {
      delete process.env.NERVE_FORCE_DB_HEALTHCHECK;
    } else {
      process.env.NERVE_FORCE_DB_HEALTHCHECK = previousForce;
    }
  }
