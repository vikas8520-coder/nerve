...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-codex-ws-policy-6564-"));
process.env.DATA_DIR = TEST_DATA_DIR;
process.env.API_KEY_SECRET = process.env.API_KEY_SECRET || "issue-6564-api-key-secret";
process.env.NERVE_WS_BRIDGE_SECRET = "issue-6564-bridge-secret";
...
type CodexWsPrepareBody = {
  ...
  response: {
    ...
    _nerveReasoningRule?: unknown;
    _nerveReasoningRouteTrace?: unknown;
  };
  reasoningRouting: {
    ruleId: string;
    sourceModel: string;
...
function buildPrepareRequest(apiKey: string, model: string): Request {
  return new Request("http://localhost/api/internal/codex-responses-ws", {
    ...
    headers: {
      ...
      "x-nerve-ws-bridge-secret": process.env.NERVE_WS_BRIDGE_SECRET as string,
    },
    body: JSON.stringify({
      action: "prepare",
...
test("WS reasoning rules apply Codex-to-Codex effort and reject non-Codex targets", async () => {
  ...
  assert.equal(allowedBody.response._nerveReasoningRule, undefined);
  assert.equal(allowedBody.response._nerveReasoningRouteTrace, undefined);
