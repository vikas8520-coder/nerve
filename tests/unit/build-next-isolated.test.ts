...
async function withTempDir(fn) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nerve-build-next-isolated-"));
...
test("resolveNextBuildEnv honors the NERVE_BUILD_MEMORY_MB override", () => {
  const env = resolveNextBuildEnv({ NERVE_BUILD_MEMORY_MB: "6144" });
...
test("getTransientBuildPaths only moves _tasks when explicitly enabled", () => {
  const paths = getTransientBuildPaths("/repo", { NERVE_BUILD_MOVE_TASKS: "1" });
