...
describe("ProviderPageHeader — Kimi partner-link note", () => {
  ...
  it.each([
    ["moonshot", "Kimi", "https://platform.kimi.ai?aff=nerve"],
    ["kimi-coding", "Kimi Code CLI", "https://www.kimi.com/code?aff=nerve"],
    ["kimi-web", "Kimi Web", "https://www.kimi.com/code?aff=nerve"],
    ...
    expect(link?.getAttribute("title")).toBe(
      "Partner link — supports Nerve at no extra cost to you"
    );
    expect(link?.getAttribute("aria-label")).toBe(
      `${name} — Partner link — supports Nerve at no extra cost to you`
    );
    expect(el.textContent).toContain("Partner link — supports Nerve at no extra cost to you");
  });
