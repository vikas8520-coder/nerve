...
//
// NERVE_SKIP_SYSTEM_TRUST=1 (set globally in tests/_setup/isolateDataDir.ts)
...
test("isolateDataDir setup exports the system-trust guard for every test process", () => {
  assert.equal(process.env.NERVE_SKIP_SYSTEM_TRUST, "1");
});
...
test("installCert under the guard skips the OS mutation but keeps input contracts", async () => {
  ...
  await assert.rejects(() => installCert("", "/nonexistent/nerve-guard-test.pem"));
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-trust-guard-"));
  const pem = path.join(dir, "nerve-guard-test.pem");
  fs.writeFileSync(
    pem,
