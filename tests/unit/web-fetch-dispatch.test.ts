...
// #7339 — proves the synthetic nerve_web_fetch tool call (emitted by
// webFetchInterception.ts, Phase 3-4 of #3384) is routed through the same
// generic dispatch path already proven for nerve_web_search
// (handleToolCallExecution -> builtinSkills.web_fetch), including the
// error/abort-not-swallowed case (Hard Rule #6).
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-web-fetch-dispatch-"));
...
const { NERVE_WEB_FETCH_FALLBACK_TOOL_NAME } = await import(
...
const contextWithFetchBuiltin = {
  ...
  sessionId: "session-1",
  requestId: "request-1",
  builtinToolNames: [NERVE_WEB_FETCH_FALLBACK_TOOL_NAME],
  provider: "openai",
  model: "gpt-5",
};

test("handleToolCallExecution routes nerve_web_fetch to the web_fetch builtin handler and returns the tool-result envelope", async () => {
            ...
            tool_calls: [
              {
                id: "call-fetch-1",
                function: {
                  name: NERVE_WEB_FETCH_FALLBACK_TOOL_NAME,
