...
test("i18n.mjs detecta locale por NERVE_LANG", async () => {
  const { resetForTests, detectLocale } = await import("../../bin/cli/i18n.mjs");
  const orig = process.env.NERVE_LANG;
  process.env.NERVE_LANG = "pt-BR";
  resetForTests();
  const locale = detectLocale();
  assert.equal(locale, "pt-BR");
  if (orig === undefined) delete process.env.NERVE_LANG;
  else process.env.NERVE_LANG = orig;
  resetForTests();
});
...
test("i18n.mjs usa fallback en quando locale não existe", async () => {
  const { resetForTests, detectLocale } = await import("../../bin/cli/i18n.mjs");
  const orig = process.env.NERVE_LANG;
  process.env.NERVE_LANG = "xx-FAKE";
  resetForTests();
  const locale = detectLocale();
...
test("t() usa pt-BR quando disponível", async () => {
  ...
  assert.ok(result.includes("nerve serve"), `Esperava mensagem pt-BR, obteve: ${result}`);
  resetForTests();
});
