 ...
 *      providerSpecificData._nerveForceResponsesUpstream === true.
 *   3. The default chat path is preserved for non-responses requests.
 */
import test from "node:test";
import assert from "node:assert/strict";

import { DefaultExecutor } from "@nerve/open-sse/executors/default.ts";
...
test("DefaultExecutor.buildUrl forces /responses path when _nerveForceResponsesUpstream=true", () => {
  const executor = new DefaultExecutor("azure-ai");

  const forced = executor.buildUrl("gpt-4.1", true, 0, {
    providerSpecificData: {
      baseUrl: "https://example-resource.services.ai.azure.com/openai/v1",
      _nerveForceResponsesUpstream: true,
    },
  });
