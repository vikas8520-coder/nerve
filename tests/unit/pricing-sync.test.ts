...
describe("transformToNerve", () => {
  test("converts LiteLLM per-token pricing to Nerve per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToNerve(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToNerve(raw);
