...
// forwards. On the stdio transport `nerve_ccr_retrieve` therefore fell back
...
// the registered nerve_ccr_retrieve handler with an `extra` carrying that
...
test("static tool loops forward `extra` so stdio callers keep their scope/identity (#6178)", async () => {
  ...
  const retrieve = getRegisteredHandler(server, "nerve_ccr_retrieve");
