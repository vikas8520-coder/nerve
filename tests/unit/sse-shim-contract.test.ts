...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "nerve-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@nerve\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@nerve\/open-sse\/services\/model\.ts/);
