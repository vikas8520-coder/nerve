...
const PACKAGE_ROOT = path.join(GLOBAL_ROOT, "nerve");
...
test("resolves the package root when its package.json exists (Bug3)", async () => {
  ...
  const result = await mod.resolveGlobalNervePath(execStub(`${GLOBAL_ROOT}\\n`), exists);
...
test("falls back to the legacy app/ layout when only it has a package.json", async () => {
  ...
  const result = await mod.resolveGlobalNervePath(execStub(GLOBAL_ROOT), exists);
