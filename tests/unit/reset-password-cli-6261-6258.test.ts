...
const NERVE_BIN = path.join(ROOT, "bin", "nerve.mjs");
...
// ~/.nerve so DATA_DIR is the only data directory in play.
function baseEnv(dataDir: string, isolatedHome: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DATA_DIR: dataDir,
    HOME: isolatedHome,
    // Give the CLI a key so bin/nerve.mjs never warns/provisions.
...
function mkHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "nerve-reset-home-"));
}
function mkDataDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "nerve-reset-data-"));
}

// #6261: `nerve reset-password` must be a real subcommand (not "unknown
...
test("nerve reset-password subcommand applies the reset over piped stdin (#6261, #6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
    const res = spawnSync("node", [NERVE_BIN, "reset-password"], {
...
test("nerve-reset-password applies the reset over piped two-line stdin (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
...
test("nerve-reset-password --password-stdin reads the whole stdin as the password (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
