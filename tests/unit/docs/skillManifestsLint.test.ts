...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `nerve*`, mas os skills
...
// invariante de uso é "referencia as env vars ($NERVE_URL/NERVE_KEY) OU
// comandos da CLI (`nerve …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("NERVE_") || content.includes("nerve "),
        `${dir}: missing usage references (NERVE_* env vars or nerve CLI commands)`
      );
    }
  }
});
