...
describe("formatCompressionAnnotation", () => {
  ...
  it("is ASCII-only so it survives HTTP header (X-Nerve-Compression) construction", () => {
    // Regression: the annotation is appended to the X-Nerve-Compression response
    ...
    assert.doesNotThrow(() => new Headers({ "X-Nerve-Compression": value }));
    const res = new Response(null, { headers: { "X-Nerve-Compression": value } });
    assert.equal(res.headers.get("X-Nerve-Compression"), value);
  });
  ...
  it("bounds high-cardinality rule telemetry before it reaches an HTTP response header", () => {
    ...
    assert.doesNotThrow(
      () => new Response(null, { headers: { "X-Nerve-Compression": annotation } })
    );
  });
