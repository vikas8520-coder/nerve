...
// re-imported. The cap is now operator-tunable via NERVE_DB_IMPORT_MAX_MB, with the
...
test("honors NERVE_DB_IMPORT_MAX_MB (#4719)", () => {
  assert.equal(resolveMaxUploadSizeBytes({ NERVE_DB_IMPORT_MAX_MB: "256" }), 256 * MB);
});

test("clamps absurd values to the 4 GB ceiling (#4719)", () => {
  assert.equal(resolveMaxUploadSizeBytes({ NERVE_DB_IMPORT_MAX_MB: "999999" }), 4096 * MB);
});

test("falls back to default for invalid / out-of-range values (#4719)", () => {
  assert.equal(resolveMaxUploadSizeBytes({ NERVE_DB_IMPORT_MAX_MB: "abc" }), 100 * MB);
  assert.equal(resolveMaxUploadSizeBytes({ NERVE_DB_IMPORT_MAX_MB: "0" }), 100 * MB);
  assert.equal(resolveMaxUploadSizeBytes({ NERVE_DB_IMPORT_MAX_MB: "-5" }), 100 * MB);
});

test("floors fractional MB values (#4719)", () => {
  assert.equal(resolveMaxUploadSizeBytes({ NERVE_DB_IMPORT_MAX_MB: "150.9" }), 150 * MB);
});
