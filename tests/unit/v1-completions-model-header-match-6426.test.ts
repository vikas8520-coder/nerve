...
// `X-Nerve-Model` header. Some upstreams (notably legacy text-completions)
...
test("#6426 body.model rewritten to resolved backend model aligns with header value", () => {
  ...
  const resolvedModel = "gpt-5.5"; // this is what attachNerveMetaHeaders wrote to X-Nerve-Model
  ...
  assert.equal(
    ...
    resolvedModel,
    "body.model must equal the resolved backend model that is in X-Nerve-Model header"
  );
});
