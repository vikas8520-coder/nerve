...
const argv = (...args: string[]) => ["node", "nerve", ...args];
...
test("nerve CLI --version fast-path prints ONLY the version, skipping bootstrap output", async () => {
  const pkg = JSON.parse(
    readFileSync(join(process.cwd(), "package.json"), "utf8")
  ) as { version: string };

  const { stdout } = await execFileAsync(process.execPath, ["bin/nerve.mjs", "--version"], {
    cwd: process.cwd(),
