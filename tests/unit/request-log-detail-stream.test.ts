...
test("status discrepancy shows both Nerve and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("Nerve returned 504"),
    -1,
    "Should indicate Nerve returned its own status"
  );
});
