...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-model-sync-"));
...
test("modelSyncScheduler resolves only loopback origins and uses the dashboard port", async () => {
  const previous = {
    NERVE_PORT: process.env.NERVE_PORT,
    PORT: process.env.PORT,
    DASHBOARD_PORT: process.env.DASHBOARD_PORT,
    BASE_URL: process.env.BASE_URL,
    NEXT_PUBLIC_BASE_URL: process.env.NEXT_PUBLIC_BASE_URL,
    NEXT_PUBLIC_APP_URL: process.env.NEXT_PUBLIC_APP_URL,
    NERVE_BASE_PATH: process.env.NERVE_BASE_PATH,
    NERVE_INTERNAL_SCHEME: process.env.NERVE_INTERNAL_SCHEME,
    NERVE_TLS_CERT: process.env.NERVE_TLS_CERT,
    NERVE_TLS_KEY: process.env.NERVE_TLS_KEY,
  };
  process.env.NERVE_PORT = "20128";
  process.env.PORT = "22128";
  process.env.DASHBOARD_PORT = "22128";
  process.env.BASE_URL = "https://attacker.example";
  delete process.env.NEXT_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.NERVE_BASE_PATH = "/nerve/";
  delete process.env.NERVE_INTERNAL_SCHEME;
  delete process.env.NERVE_TLS_CERT;
  delete process.env.NERVE_TLS_KEY;

  try {
    const scheduler = await loadScheduler("trusted-loopback-origin");
    assert.equal(scheduler.getModelSyncInternalBaseUrl(), "http://127.0.0.1:22128/nerve");
    assert.equal(
      scheduler.resolveModelSyncInternalBaseUrl("https://attacker.example/steal"),
      "http://127.0.0.1:22128/nerve"
    );
    assert.equal(
...
test("modelSyncScheduler uses the listener-declared TLS scheme without trusting candidates", async () => {
  ...
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.NERVE_BASE_PATH = "/nerve";
  process.env.NERVE_INTERNAL_SCHEME = "https";
  delete process.env.NERVE_TLS_CERT;
  delete process.env.NERVE_TLS_KEY;
...
test("runtime launchers publish the actual internal listener scheme", () => {
  ...
  assert.match(runNext, /NERVE_INTERNAL_SCHEME\s*=\s*["']http["']/);
  assert.match(standalone, /NERVE_INTERNAL_SCHEME\s*=\s*tlsOptions\s*\?\s*["']https["']/);
});
...
test("initCloudSync skips auto initialization during build and test processes unless explicitly re-enabled", () => {
  ...
  assert.equal(
    initCloudSync.shouldSkipCloudSyncInitialization(
      {
        NODE_ENV: "test",
        NERVE_ENABLE_RUNTIME_BACKGROUND_TASKS: "1",
      },
      ["node", "--test"]
    ),
    false
