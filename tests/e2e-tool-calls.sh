...
NERVE_URL="${NERVE_URL:-http://localhost:20128}"
MODEL="${1:-${NERVE_MODEL:-}}"
AUTH_TOKEN="${NERVE_AUTH_TOKEN:-dummy}"
FAILURES=0
...
echo "=== Nerve Tool Call Integrity E2E Test ==="
echo "URL: $NERVE_URL | Model: $MODEL"

echo
echo "[TEST 1] Non-streaming tool call integrity..."
response=$(curl -fsS -X POST "$NERVE_URL/v1/chat/completions" \
...
stream_output=$(curl -fsS -X POST "$NERVE_URL/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -d @- <<JSON
{
...
models=$(curl -fsS "$NERVE_URL/v1/models" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
