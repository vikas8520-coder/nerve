...
# escape hatch: `--build-arg`/-e NERVE_USE_TURBOPACK=0.
# See docs/ops/QUALITY_GATE_PLAYBOOK.md Parte 6.
ENV NERVE_USE_TURBOPACK=1

# Next.js basePath is fixed at build time; pass NERVE_BASE_PATH here when the
# image should serve under a reverse-proxy subpath without a runtime patch.
ARG NERVE_BASE_PATH=""
ENV NERVE_BASE_PATH=$NERVE_BASE_PATH
...
# (NERVE_MEMORY_MB). Override: `--build-arg NERVE_BUILD_MEMORY_MB=6144`.
ARG NERVE_BUILD_MEMORY_MB=4096
ENV NODE_OPTIONS="--max-old-space-size=${NERVE_BUILD_MEMORY_MB}"
...
LABEL org.opencontainers.image.title="nerve" \
  org.opencontainers.image.description="Unified AI proxy — route any LLM through one endpoint" \
  org.opencontainers.image.url="https://nerve.online" \
  org.opencontainers.image.source="https://github.com/diegosouzapw/Nerve" \
...
# `-e NERVE_MEMORY_MB=2048` (or higher) if you raise fusionTuning.maxPanel
# above the default cap.
ENV NERVE_MEMORY_MB=1024
ENV NODE_OPTIONS="--max-old-space-size=${NERVE_MEMORY_MB}"
...
ENV NERVE_MIGRATIONS_DIR=/app/migrations
...
#
#  Two image flavors:
#    runner-base  →  nerve:VERSION        Lean base (~500 MB). No browsers.
#    runner-web   →  nerve:VERSION-web    +Chromium/Playwright (~800 MB).
#
...
#
#  Build:
#    docker build --target runner-web -t nerve:web .
#  Compose:
#    build:
#      context: .
#      target: runner-web
