...
import { getUsageForProvider } from "@nerve/open-sse/services/usage.ts";
...
import { getCodexQuotaWindowFilterForModel } from "@nerve/open-sse/config/codexQuotaScopes.ts";
import { getAntigravityQuotaFamily } from "@nerve/open-sse/services/antigravityQuotaFamily.ts";
...
declare global {
  var __nerveQuotaCacheState: QuotaCacheState | undefined;
}

function getState(): QuotaCacheState {
  if (!globalThis.__nerveQuotaCacheState) {
    globalThis.__nerveQuotaCacheState = {
      cache: new Map(),
  ...
  return globalThis.__nerveQuotaCacheState;
}
