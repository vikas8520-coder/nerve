...
export const QWEN_CODE_ENV_KEY = "NERVE_API_KEY";
...
/**
 * Identifies only entries owned by Nerve. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "nerve") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (Nerve)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (Nerve)`,
...
/**
 * Merge one Nerve model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "nerve") delete next.selectedProvider;

  return next;
};

export const findNerveQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasNerveQwenCodeConfig = (settings: unknown): boolean =>
  findNerveQwenCodeModel(settings) !== undefined;

/** Remove only Nerve-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?NERVE_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_nerve"))}`);
  return `${lines.join("\n")}\n`;
};
