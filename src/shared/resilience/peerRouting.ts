...
export const NERVE_PEER_TRACE_HEADER = "X-Nerve-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  NERVE_INSTANCE_ID?: string;
  NERVE_PEER_URLS?: string;
  NERVE_PEER_MAX_HOPS?: string;
};
...
export function isConfiguredNervePeer(
  targetUrl: string,
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, NERVE_PEER_TRACE_HEADER));
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted Nerve URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredNervePeer(targetUrl, env)) return false;

  const trace = parsePeerTrace(readHeader(clientHeaders, NERVE_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = NERVE_PEER_TRACE_HEADER.toLowerCase();
  for (const key of Object.keys(outgoingHeaders)) {
    if (key.toLowerCase() === traceHeaderLower) delete outgoingHeaders[key];
  }
  outgoingHeaders[NERVE_PEER_TRACE_HEADER] = trace.join(",");
  return true;
}
