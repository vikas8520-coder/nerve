...
/**
 * Resolve the deploy basePath for display URLs (e.g. `/nerve`).
 *
 * Priority:
 * 1. Explicit `envBasePath` argument / `NEXT_PUBLIC_NERVE_BASE_PATH`
  ...
  const fromEnv = normalizeBasePath(
    envBasePath ??
      (typeof process !== "undefined"
        ? process.env.NEXT_PUBLIC_NERVE_BASE_PATH || process.env.NERVE_BASE_PATH
        : undefined)
  );
 ...
 * @param envBasePath       `NEXT_PUBLIC_NERVE_BASE_PATH` / `NERVE_BASE_PATH`
 */
export function resolveDisplayBaseUrl(
  ...
  // Example: NEXT_PUBLIC_BASE_URL=https://host/nerve must not be collapsed to
  ...
  // reverse-proxy subpaths (NERVE_BASE_PATH).
 ...
 *   1. Public browser origin (+ NERVE_BASE_PATH when set) — proves the
 ...
 * `src/shared/utils/resolveNerveBaseUrl.ts` instead.
 */
