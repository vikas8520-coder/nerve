...
describe("useDisplayBaseUrl", () => {
  ...
  it("appends NERVE basePath when resolving a public browser origin", () => {
    expect(
      resolveDisplayBaseUrl(
  ...
  it("derives basePath from NEXT_PUBLIC_BASE_URL when env basePath is unset", () => {
    expect(
      resolveDisplayBaseUrl("https://api.example.com/nerve", "https://api.example.com", "/")
    ).toBe("https://api.example.com/nerve");
  });

  it("normalizes basePath values without a leading slash", () => {
    expect(normalizeBasePath("nerve")).toBe("/nerve");
    expect(normalizeBasePath("/nerve/")).toBe("/nerve");
