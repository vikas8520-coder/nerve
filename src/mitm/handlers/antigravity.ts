 ...
 * `thinkingConfig`, …). The Nerve router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the Nerve router,
 ...
 * @param model      resolved Nerve model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`Nerve ${upstream.status}: ${errText}`);
      }
