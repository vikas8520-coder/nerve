...
const LINUX_CERT_NAME = "nerve-mitm.crt";
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.NERVE_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] NERVE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
 ...
 * cert-path-agnostic and keep writing to the same `nerve-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `nerve-tproxy-ca.crt` slot
...
// They install / remove the Nerve MITM root CA from the OS trust store and
