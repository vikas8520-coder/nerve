...
// ─── Provider mapping: LiteLLM provider → Nerve aliases ─────
...
/**
 * Transform LiteLLM raw data → Nerve PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (Nerve format).
 ...
 */
export function transformToNerve(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to Nerve providers
    const litellmProvider = info.litellm_provider || "";
    const nerveFetchProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (nerveFetchProviders) {
      for (const provider of nerveFetchProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToNerve(raw);
