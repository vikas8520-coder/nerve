...
function resolveMigrationsDir(): string {
  ...
  throw new Error(
    "[Migration] Could not resolve migrations directory. Set NERVE_MIGRATIONS_DIR."
  );
}
...
function getAppliedRecords(db: SqliteAdapter): Array<{ version: string; name: string }> {
  return db
    .prepare("SELECT version, name FROM _nerve_migrations ORDER BY version")
    .all() as Array<{
    version: string;
    name: string;
  }>;
...
function reconcileRenumberedMigrations(
  ...
  for (const compatibility of RENAMED_MIGRATION_COMPATIBILITY) {
    ...
    if (residualRow) {
      ...
      db.prepare("DELETE FROM _nerve_migrations WHERE version = ?").run(
        compatibility.fromVersion
      );
    }
  }
    ...
    try {
      ...
      if (message.includes("duplicate column name")) {
        const applyMarkerOnly = db.transaction(() => {
          db.prepare(
            "INSERT OR IGNORE INTO _nerve_migrations (version, name) VALUES (?, ?)"
