 ...
 * can be iframed inside the Nerve dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between Nerve and the embedded service.
 ...
 * Prevents Nerve session cookies and Authorization from leaking upstream.
 */
 ...
 *   Nerve origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the Nerve iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (Nerve controls framing via its own CSP).
