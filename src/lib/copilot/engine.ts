...
Use \`createCombo\` tool or \`runNerveCli\` to create them.`;
  }
...
Nerve supports 212+ providers across categories:
...
I can use CodeGraph to explore the Nerve codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## Nerve Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`nerve health\`
- "CLI list-combos" → ejecuta \`nerve list-combos\`
- "CLI set-budget 10" → ejecuta \`nerve set-budget 10\`

### Conocimiento
- "Cómo funciona Nerve?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with Nerve.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runNerveCli") {
    ...
    return {
      message: result,
      toolCalls: [{ name: "runNerveCli", args: intent.args, result }],
    };
  }
