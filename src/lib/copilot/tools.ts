/**
 * Nerve Copilot — Tool definitions
 *
 * Tools the copilot can execute to configure Nerve on behalf of the user,
...
// ── Helper: check if nerve CLI is available ──────────────────────────────

function getNerveCliPath(): string | null {
  try {
    const result = execSync("which nerve 2>/dev/null || command -v nerve 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the Nerve codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within Nerve.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runNerveCli",
    description:
      "Execute an 'nerve' CLI command to configure or query the Nerve app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: nerve list-keys, nerve switch-combo [id], nerve set-budget 10, nerve set-strategy [id] priority, nerve health, nerve mcp (starts MCP server), nerve db-health, nerve reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'nerve'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getNerveCliPath();
      if (!cliPath) return "nerve CLI not found in PATH. Install Nerve first.";
