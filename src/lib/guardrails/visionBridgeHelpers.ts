 ...
 *      Nerve itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      Nerve's own `/v1` so the vision model can be any provider
 *      registered in Nerve (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. Nerve self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known Nerve-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require Nerve self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through Nerve's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via NERVE_PORT / PORT env vars, falling back to the default 20128.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // Nerve itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the Nerve self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so Nerve can resolve the
      ...
      // When routing through Nerve's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_nerve as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_nerve";
