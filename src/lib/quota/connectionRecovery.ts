...
import { cooldownUntilMs } from "@nerve/open-sse/services/accountFallback.ts";
...
declare global {
  var __nerveConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__nerveConnRecovery) {
    globalThis.__nerveConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__nerveConnRecovery;
}
...
function isRecoverySchedulerDisabled(): boolean {
  return (
    isEnvFlagEnabled("NERVE_DISABLE_CONNECTION_RECOVERY") ||
    isEnvFlagEnabled("NERVE_DISABLE_BACKGROUND_SERVICES") ||
    isBuildProcess() ||
...
/**
 * Resolve the tick interval (ms) from NERVE_CONNECTION_RECOVERY_INTERVAL_MS,
...
export function resolveConnectionRecoveryIntervalMs(
  rawValue: string | undefined = typeof process !== "undefined"
    ? process.env.NERVE_CONNECTION_RECOVERY_INTERVAL_MS
    : undefined
): number {
