...
async function fetchCodexSaturation(
  ...
  const mod = await import("@nerve/open-sse/services/codexQuotaFetcher");
...
async function fetchBailianSaturation(
  ...
  const mod = await import("@nerve/open-sse/services/bailianQuotaFetcher");
...
async function defaultAnthropicDeps(): Promise<AnthropicSaturationDeps> {
  const [localDbMod, usageMod] = await Promise.all([
    import("@/lib/localDb"),
    import("@nerve/open-sse/services/usage"),
  ]);
  return {
...
async function defaultGenericUsageFetch(
  ...
  const mod = await import("@nerve/open-sse/services/usage");
