...
import { withRateLimit } from "@nerve/open-sse/services/rateLimitManager";

const INTERNAL_ORIGIN = "http://nerve.internal";
...
export function buildInternalChatRequest(
  ...
  return new Request(`${INTERNAL_ORIGIN}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "X-Nerve-Compression": "off",
      "X-Request-Id": `model-test-${randomUUID()}`,
      ...(connectionId ? { "X-Nerve-Connection": connectionId } : {}),
    },
...
export async function runSingleModelTest(
  ...
  const testBody = isRerank
    ? {
        ...
        query: "What is Nerve?",
        documents: [
          "Nerve routes AI requests across configured providers.",
