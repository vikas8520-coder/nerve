import { handleEmbedding } from "@nerve/open-sse/handlers/embeddings.ts";
import {
  parseEmbeddingModel,
...
} from "@nerve/open-sse/config/embeddingRegistry.ts";
import { errorResponse, unavailableResponse } from "@nerve/open-sse/utils/error.ts";
...
import { runWithProxyContext } from "@nerve/open-sse/utils/proxyFetch.ts";
import { handleComboChat } from "@nerve/open-sse/services/combo.ts";
...
import { attachNerveMetaHeaders } from "@/domain/nerveResponseMeta";
...
export async function createEmbeddingResponse(
  ...
  if (result.success) {
    ...
    attachNerveMetaHeaders(responseHeaders, {
      provider,
