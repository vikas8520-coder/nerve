...
import { hasNerveQwenCodeConfig } from "@/shared/services/qwenCodeConfig";
import { getRuntimePorts } from "@/lib/runtime/ports";

const { apiPort } = getRuntimePorts();

/**
 * Check if a tool has Nerve configured by reading its config file directly.
...
export async function checkToolConfigStatus(
  ...
  try {
    ...
    if (toolId === "codex") {
      ...
      if (!hasNerve) return "not_configured";
    ...
    if (toolId === "hermes") {
      ...
      const hasNerve =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasNerve ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores Nerve config differently
    switch (toolId) {
      case "claude":
        return (config?.env as Record<string, unknown>)?.ANTHROPIC_BASE_URL
          ...
          : "not_configured";
      case "qwen":
        return hasNerveQwenCodeConfig(config) ? "configured" : "not_configured";
      case "droid":
      case "openclaw":
      case "cline":
      case "kilo": {
        // Generic check: look for Nerve-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("nerve") ||
          configStr.includes("sk_nerve") ||
