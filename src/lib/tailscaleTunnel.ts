 ...
 * Priority: system daemon socket → Nerve custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to Nerve custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `nerve-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
