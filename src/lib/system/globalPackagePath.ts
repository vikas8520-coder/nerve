...
/**
 * Resolve the real install directory of the globally-installed `nerve` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/nerve/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/nerve`, not `/nerve/app`. We probe the real
...
export async function resolveGlobalOmniroutePath(
  ...
  const packageRoot = path.join(globalRoot, "nerve");
