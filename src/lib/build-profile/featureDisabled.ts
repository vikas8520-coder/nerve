/**
 * Helper used by the `NERVE_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (NERVE_BUILD_PROFILE=minimal). ` +
        `Install the full nerve artifact instead of nerve-secure if you need this feature.`
    );
