...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API Nerve
...
export const API_KEY_PLACEHOLDER = "$NERVE_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export NERVE_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["NERVE_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
