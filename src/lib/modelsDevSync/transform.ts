 ...
 * types, the provider-id mapping table, and the raw→Nerve transform
...
// ─── Provider mapping: models.dev provider ID → Nerve provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// Nerve uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL Nerve identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But Nerve's catalog & combo targets
  ...
  // Additional providers that may overlap with Nerve
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to Nerve provider IDs.
...
/**
 * Transform models.dev raw data → Nerve PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as Nerve format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    const nerveFetchProviders = mapProviderId(providerId);
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped Nerve providers
      for (const omniProvider of nerveFetchProviders) {
