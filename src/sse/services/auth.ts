...
import { getQuotaScopeLabelForProvider } from "@nerve/open-sse/services/antigravityQuotaFamily.ts";
import { getCreditsMode } from "@nerve/open-sse/services/antigravityCredits.ts";
import {
...
import { isLocalProvider } from "@nerve/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@nerve/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@nerve/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@nerve/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@nerve/open-sse/services/errorClassifier.ts";
...
} from "@nerve/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-nerve-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / Nerve / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against Nerve keys.
 *
 ...
 * Feature #1350: Supports NERVE_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.NERVE_API_KEY || process.env.ROUTER_API_KEY;
