...
import { handleChatCore } from "@nerve/open-sse/handlers/chatCore.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@nerve/open-sse/utils/error.ts";
...
} from "@nerve/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@nerve/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@nerve/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-Nerve-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-Nerve-Session-Id", sessionId);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-Nerve-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
