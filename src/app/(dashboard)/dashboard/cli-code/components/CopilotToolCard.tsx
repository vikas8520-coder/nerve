 ...
 *
 * Feature request: https://github.com/vikas8520-coder/nerve/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("nerve-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("nerve-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem("nerve-copilot-selected-models", JSON.stringify([...selectedModels]));
    }
  }, [selectedModels]);
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "Nerve",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.nerve}`,
      models,
    };

    const responsesConfig = {
      name: "Nerve-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.nerve}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("nerve-cli-key-copilot", value);
  };
