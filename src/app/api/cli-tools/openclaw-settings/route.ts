...
// Check if settings has Nerve config
const hasNerveConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["nerve"];
};
...
// POST - Update Nerve settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `nerve/${model}`;

    // Update models.providers.nerve
    settings.models.providers["nerve"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Nerve from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["nerve"];
    ...
    // Reset agents.defaults.model.primary if it uses nerve
    if (settings.agents?.defaults?.model?.primary?.startsWith("nerve/")) {
