...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.nerve]
...
// Check if config has Nerve settings
const hasNerveConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "nerve"') ||
    config.includes("[model_providers.nerve]")
  );
};
...
// POST - Update Nerve settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only Nerve related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use NERVE_API_KEY
    parsed._root.model_provider = "nerve";
    parsed._sections["model_providers.nerve"] = {
      name: "Nerve",
...
// DELETE - Remove Nerve settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Nerve related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "nerve") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove nerve provider section
    delete parsed._sections["model_providers.nerve"];
    ...
    return NextResponse.json({
      success: true,
      message: "Nerve settings removed successfully",
    });
  } catch (error) {
