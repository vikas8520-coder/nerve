...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasNerve: !!(ymlProvider || creds.hasNerve),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 2. Write auth_credentials — so omp sees nerve as "logged in"
    saveOmpCredentials(PROVIDER_ID, keyRef, normalizedBaseUrl);

    return NextResponse.json({
      success: true,
      message:
        "Oh My Pi settings applied! Run omp and all Nerve models appear under nerve in /model.",
      configPath: getOmpModelsYmlPath(),
    });
  } catch (error) {
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Nerve removed from Oh My Pi",
    });
  } catch (error) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(error) } },
