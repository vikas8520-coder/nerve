...
// Check if settings has Nerve customModels.
// Multi-model entries are stored as `custom:Nerve-0`, `custom:Nerve-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasNerveConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isNerveCustomModel);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasNerve: hasNerveConfig(settings),
...
// POST - Update Nerve customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing Nerve config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isNerveCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend Nerve entries (one per requested model)
...
// DELETE - Remove Nerve customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Nerve customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isNerveCustomModel(m));
