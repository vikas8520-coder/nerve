...
// Check if Nerve OpenAI-compatible provider is configured
const hasNerveConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["nerve"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("nerve")
  );
};
...
// POST - Configure Kilo Code to use Nerve as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update Nerve as openai-compatible provider
...
// DELETE - Remove Nerve config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Nerve settings removed from Kilo Code",
    });
  } catch (error) {
