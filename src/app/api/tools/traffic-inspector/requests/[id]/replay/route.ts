 ...
 *
 * Re-issues the captured request through the local Nerve instance and
...
import { buildErrorBody, sanitizeErrorMessage } from "@nerve/open-sse/utils/error.ts";
...
const NERVE_BASE = process.env.NERVE_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${NERVE_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-nerve-source": "inspector-replay",
  };
