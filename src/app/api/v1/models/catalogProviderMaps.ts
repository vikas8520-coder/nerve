...
import { parseModel } from "@nerve/open-sse/services/model";
...
/** True when `${prefix}/__nerve_probe__` parses back to the given providerId. */
export function prefixRoutesToProvider(prefix: string, providerId: string): boolean {
  const parsed = parseModel(`${prefix}/__nerve_probe__`);
