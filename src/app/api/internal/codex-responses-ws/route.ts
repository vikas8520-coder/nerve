...
import { z } from "zod";
import { CodexExecutor } from "@nerve/open-sse/executors/codex.ts";
...
import { logger } from "@nerve/open-sse/utils/logger.ts";
import { resolveProxy } from "@nerve/open-sse/utils/networkProxy.ts";
import { proxyConfigToUrl } from "@nerve/open-sse/utils/proxyDispatcher.ts";
import {
...
function getBridgeSecret(): string {
  return process.env.NERVE_WS_BRIDGE_SECRET || "";
}
...
function getAuthRequest(body: JsonRecord): Request {
  ...
  const url = new URL(requestUrl, "http://nerve.local");
...
async function resolveCodexRequestContext(body: JsonRecord) {
  if (!isFeatureFlagEnabled("NERVE_CODEX_WS_ENABLED")) {
    return {
...
async function prepare(body: JsonRecord) {
  ...
  if (reasoningDecision) {
    ...
    reasoningRouting = isRecord(withDirective._nerveReasoningRouteTrace)
      ? withDirective._nerveReasoningRouteTrace
      : null;
    responseBodyWithMemory = applyReasoningRuleDirective(withDirective) as JsonRecord;
    delete responseBodyWithMemory._nerveReasoningRouteTrace;
  }
  ...
  // Responses WebSocket too. The downstream client→Nerve hop works, but the
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-nerve-ws-bridge-secret") || "";
