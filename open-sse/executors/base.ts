...
/**
 * Strip the Nerve provider prefix from versioned built-in tool model
 ...
 * leaked Nerve prefix makes Anthropic reject the request. Mutates in place.
 */
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip Nerve provider prefix from versioned built-in tool
          ...
          //   x-nerve-effort:   low | medium | high | xhigh | max | off
          //   x-nerve-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-nerve-effort"] ?? clientHeaders?.["X-Nerve-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-nerve-thinking"] ?? clientHeaders?.["X-Nerve-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-nerve-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one Nerve.
          ...
          // is pinned to the captured CLI wire image, not Nerve's Node.
