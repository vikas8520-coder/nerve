 ...
 * cache-MISS JSON response — the static Content-Type + cache marker, the Nerve meta headers
...
import { attachNerveMetaHeaders as defaultAttachMeta } from "@/domain/nerveResponseMeta";
...
export function buildNonStreamingResponseHeaders(
  ...
  deps: { attachNerveMetaHeaders: typeof defaultAttachMeta; now: () => number } = {
    attachNerveMetaHeaders: defaultAttachMeta,
    now: Date.now,
  }
  ...
  deps.attachNerveMetaHeaders(responseHeaders, {
  ...
  if (args.compressionResponseMeta) {
    responseHeaders[NERVE_RESPONSE_HEADERS.compression] = args.compressionResponseMeta;
  }
