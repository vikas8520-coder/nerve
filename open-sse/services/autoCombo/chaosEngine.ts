 ...
 * not reliable here because Nerve may force a streaming envelope internally.
 */
 ...
 *
 * Supports BOTH wire formats Nerve may emit:
 ...
 * `x-nerve-chaos-error` header and an error final chunk (status stays 200
...
export async function handleChaosChat(opts: {
  ...
  return new Response(stream, {
    ...
    headers: {
      ...
      "X-Nerve-Chaos": "true",
      "X-Nerve-Chaos-Panel": String(panel.length),
      "X-Nerve-Chaos-Primary": primaryModel ?? "",
    },
  });
}
