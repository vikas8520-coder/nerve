 ...
 *
 *   X-Nerve-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-Nerve-Budget:          <max USD per request>                             (#6023)
 *   X-Nerve-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-Nerve-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-Nerve-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-Nerve-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-nerve-mode")?.trim() || null;
  const budgetHeader = headers.get("x-nerve-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-nerve-budget-fallback")?.trim() || null;
