...
/**
 * Interprets the `x-nerve-compression` request header into a plan, or null when the
...
/** Renders the X-Nerve-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-Nerve-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-Nerve-Compression HTTP header,
