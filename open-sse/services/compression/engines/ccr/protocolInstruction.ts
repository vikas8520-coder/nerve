 ...
 * content — it can only call `nerve_ccr_retrieve` if it (a) knows the tool
 ...
 * `nerve_ccr_retrieve` (an MCP-capable caller). Plain OpenAI-compatible
 * callers that cannot reach the tool must never be told to call it.
 */

const CCR_RETRIEVE_TOOL_NAME = "nerve_ccr_retrieve";
 ...
 * but only when:
 *  - the caller can actually reach `nerve_ccr_retrieve` (see
