 ...
 *
 * Goal: ensure the final request body Nerve sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - nerve-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "nerve-daystamp";
  /**
 ...
 * proven against the live Nerve deployment (call log
...
/**
 * Compute the `cc_version` suffix per the Nerve native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "nerve-daystamp"
