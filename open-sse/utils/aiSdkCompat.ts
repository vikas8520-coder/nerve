 ...
 * `stream`. Without this hint, Nerve previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // Nerve accumulates the provider stream and converts it to JSON for the client
  ...
  // Nerve fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve Nerve's streaming default unless
