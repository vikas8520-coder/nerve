 ...
 * `x-nerve-combo-*` headers and a `diagnostics` field in the OpenAI-shaped
 ...
 * carries a `recovery` hint it is mirrored as `x-nerve-recovery-action` /
 * `x-nerve-recovery-next-step` / `x-nerve-retry-after-seconds` headers and as a
...
export function errorResponseWithComboDiagnostics(
  ...
  const headers: Record<string, string> = {
    ...
    "x-nerve-combo-pool-size": String(safe.poolSize),
    "x-nerve-combo-attempted": String(safe.attempted),
    "x-nerve-combo-excluded": excludedHeader,
    "x-nerve-combo-terminal-reason": toHeaderSafeAscii(safe.terminalReason.slice(0, 200)),
  };

  if (safe.recovery) {
    headers["x-nerve-recovery-action"] = safe.recovery.action;
    ...
    headers["x-nerve-recovery-next-step"] = toHeaderSafeAscii(safe.recovery.next_step).slice(
      0,
      128
    );
    if (
      ...
      headers["x-nerve-retry-after-seconds"] = String(safe.recovery.retry_after_seconds);
    }
  }
