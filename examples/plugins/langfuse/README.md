...
Emits [Langfuse](https://langfuse.com/) generation traces for every LLM completion routed through Nerve.
...
Copy the `examples/plugins/langfuse/` directory to your Nerve plugins path, or install directly from the marketplace UI.
...
- **Trace:** `nerve:<model>` with `userId`, `provider`, `requestId` metadata
...
- API keys are stored in the plugin config, encrypted at rest by Nerve
...
- Nerve plugin SDK: `docs/frameworks/PLUGIN_SDK.md`
