# Quick Start: Get Nerve Running in 3 Minutes

> **TL;DR**: Install → Connect a free provider → Point your IDE to Nerve. Done.

---

## Step 1: Install Nerve
...
Nerve starts at `http://localhost:20128`. The dashboard opens automatically.

---

## Step 3: Connect a Free Provider

You can use Nerve **without paying anything** by connecting a free provider.
...
From [API Keys](http://localhost:20128/dashboard/api-manager), create a new key. Store this key since it will not appear again. Do note that this key is for tools to access Nerve, not to access upstream providers.
...
## Step 5: Point Your IDE or CLI to Nerve
...
That's it! Your IDE now uses Nerve with automatic provider selection.
...
3. In the Continue.dev chat pane, select `Nerve - Auto` and you will make requests to Nerve.
...
2. Now let's launch Codex, but configured for Nerve. Run:

```
nerve launch-codex --model auto
```

You can do this manually via `codex` and command line parameters to specify endpoint and api key, but with the above command, Nerve takes care of everything for you.

3. The CLI should be sending requests to Nerve now.

### Confirm your tool is routing to Nerve
...
- **[Auto-Combo Guide](./AUTO-COMBO-GUIDE.md)** — Let Nerve pick the best AI for you
...
`auto` tells Nerve to automatically pick the best provider for each request. It considers speed, cost, quality, and availability. See the [Auto-Combo Guide](./AUTO-COMBO-GUIDE.md) for details.

### "How much does it cost?"

Nerve itself is **free and open-source**. You only pay for the providers you use. Many providers have free tiers — see the [Free Tiers Guide](./FREE-TIERS-GUIDE.md).

### "Can I use it with Claude Code / Cursor / Copilot?"

**Yes!** Nerve works with any tool that supports OpenAI format. Just set the base URL to `http://localhost:20128/v1`. See the [CLI Tools Guide](../reference/CLI-TOOLS.md) for specific setup instructions.

### "What if a provider goes down?"

Nerve automatically skips failed providers and tries the next one. You don't need to do anything. See the [Auto-Combo Guide](./AUTO-COMBO-GUIDE.md) for details.
...
- **[GitHub Issues](https://github.com/diegosouzapw/Nerve/issues)** — Report bugs
