...
adapter the primary transport. Set `NERVE_BROWSER_POOL` to one of the same values to
...
`X-Nerve-Claude-Web-*` headers (`open-sse/executors/claude-web/stream.ts:364`).
...
1. Start Nerve with `npm run dev` or a built installation.
