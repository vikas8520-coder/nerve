...
Nerve includes a full-featured proxy management system that lets you route upstream AI provider traffic through HTTP, HTTPS, or SOCKS5 proxies. Whether you're in a blocked region, need IP rotation, or want stealth fingerprinting — this guide covers everything.
...
Nerve supports proxy configuration at **four independent scopes**, resolved in priority order:
...
When Nerve sends a request to an upstream provider, it calls `resolveProxyForConnectionFromRegistry()` which checks each level in order:
...
Proxies are included in the **Backup/Restore** system. When you export your Nerve configuration:
...
If you configured proxies in an older version (pre-registry), Nerve automatically migrates them:
...
> 🆕 **Contributed by [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/diegosouzapw/Nerve/pull/1847) (Issue [#1788](https://github.com/diegosouzapw/Nerve/issues/1788))

Nerve integrates with the **[1proxy](https://1proxy-api.aitradepulse.com)** community platform to provide access to **hundreds of free, validated proxies** from around the world. This is perfect for users who don't have their own proxy infrastructure.
...
1. **Sync** — Nerve fetches validated proxies from the 1proxy API
...
Nerve doesn't just route traffic through a proxy — it makes the traffic look legitimate:
...
For providers that use the CLIProxyAPI pattern, Nerve supports three upstream proxy modes:

| Mode          | Description                                        |
| ------------- | -------------------------------------------------- |
| `native`      | Nerve handles proxy routing directly (default) |
...
For exposing your Nerve instance to the public internet (Cloudflare/ngrok/Tailscale) instead of routing outbound through a proxy, see [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md). The tunnel REST API lives under `/api/tunnels/{cloudflared,ngrok,tailscale}/*` and is orthogonal to the outbound proxy chain documented above.
...
This is normal with cheap proxies that drop idle connections. Nerve already handles this by:
...
Make sure the proxy is configured **before** starting the OAuth flow. Nerve routes OAuth token exchange through the configured proxy. Set a global or provider-level proxy first, then connect.
...
Nerve's **proxy fast-fail** mechanism (`src/lib/proxyHealth.ts`) detects dead proxies in <2s via a quick TCP connection check, then **caches the result** to avoid per-request overhead.
...
```ts
import { getAllProxyHealthStatuses, invalidateProxyHealth } from "nerve/proxyHealth";
...
Nerve tracks per-proxy usage to help operators diagnose routing patterns, latency spikes, and recurring failures.

### What's Tracked

For every request through a configured proxy, Nerve records:
...
```bash
# Recent proxy events
curl -H "Authorization: Bearer $NERVE_KEY" \
...
When multiple proxies are assigned to a scope, Nerve uses a **rotation strategy** to pick which one to use for each request. The strategy is configured at the scope level (global, per-provider, per-account, per-combo).
...
```ts
import { rotateOneproxyProxy } from "nerve/oneproxyRotator";
...
```ts
import { resetSequentialIndex } from "nerve/oneproxyRotator";
...
```ts
import { failOneproxyProxy } from "nerve/oneproxyRotator";
