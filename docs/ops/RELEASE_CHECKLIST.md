...
1. `npm stage list nerve` — find the stage id (also printed in the workflow summary).
...
`nerve` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate nerve@<bad> "<reason> — use <fixed>"`
...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `NERVE_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.nerve/nerve.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/nerve/app/`.
...
- [ ] `nerve --tray` boots on macOS (systray2 installed into `~/.nerve/runtime/`)
- [ ] `nerve --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `nerve --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `nerve config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g nerve@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `nerve update --apply` and the auto-updater
...
- [ ] `nerve status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/nerve*/SKILL.md` files are publicly fetchable via raw GitHub URL
