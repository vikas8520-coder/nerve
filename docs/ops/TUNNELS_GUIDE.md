...
Nerve can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — Nerve starts/stops the underlying
...
- **Auto-install.** On first use, Nerve downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  Nerve. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, Nerve uses it instead of downloading. |
...
must be configured in the ngrok dashboard — Nerve itself only forwards the
...
1. Install Tailscale (or let Nerve do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via Nerve's `login` endpoint).
...
Nerve's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose Nerve through a tunnel, the dashboard and OAuth flows must
...
and restart Nerve before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; Nerve will track each
independently.
...
Nerve attempts to auto-install on first use. If the install is blocked
