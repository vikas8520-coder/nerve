...
```bash
sudo mkdir -p /opt/nerve-ops
sudo cp scripts/ops/runner-janitor.sh /opt/nerve-ops/
sudo chmod +x /opt/nerve-ops/runner-janitor.sh
( sudo crontab -l 2>/dev/null; echo '*/30 * * * * /opt/nerve-ops/runner-janitor.sh >> /var/log/runner-janitor.log 2>&1' ) | sudo crontab -
```
