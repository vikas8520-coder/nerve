...
> **TL;DR**: Nerve ships with built-in health monitoring, provider autopilot, quota tracking, and observability hooks. This guide covers the dashboard, alerts, and troubleshooting.
...
Nerve has **3 layers of monitoring**:
...
│  └─ MCP tools: nerve_get_health, nerve_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
Nerve supports **3 alert channels**:
...
- Nerve's quota tracking may be **stale** — the provider's truth is upstream
...
- Run `sqlite3 ~/.nerve/storage.sqlite "PRAGMA integrity_check;"`
- If "ok" — false alarm, the health check is being too strict
- If anything else — **stop Nerve** and follow the [disaster recovery guide](./DATABASE_GUIDE.md#disaster-recovery)
