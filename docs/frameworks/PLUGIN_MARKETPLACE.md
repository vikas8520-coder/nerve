...
Nerve ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the Nerve data directory:
...
`<home>/.nerve/plugins`, where `<home>` is taken from the `HOME` /
