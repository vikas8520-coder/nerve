...
Nerve can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through Nerve (with retry, timeout, and error classification)
...
> Nerve to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
Nerve saves the token and immediately runs a 1-result test search; if Notion
...
`open-sse/mcp-server/server.ts` only when `NERVE_MCP_ENFORCE_SCOPES=true`; the
caller's allowed scopes come from `NERVE_MCP_SCOPES` (comma-separated) or the
