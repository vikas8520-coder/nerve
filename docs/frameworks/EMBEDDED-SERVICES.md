...
Embedded services are locally-installed process sidecar tools that Nerve installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as Nerve and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that Nerve can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- Nerve installs them under `DATA_DIR/services/{name}/` (isolated from Nerve's own `package.json`)
- Nerve spawns and monitors them as child processes
- Nerve injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | Nerve generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with Nerve's own deps.
...
the next time Nerve boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of Nerve session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs Nerve.
...
- `DATA_DIR` or its parent is not writable by the Nerve process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.nerve/`): `ls -la ~/.nerve/`
2. Ensure the Nerve process user owns the directory: `chown -R $USER ~/.nerve/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.nerve/services/{name}/`.
...
Yes. Both services follow the same local-first model as Nerve itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where Nerve is deployed, not a remote cloud provider.
...
2. Check structured logs in Nerve's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.nerve/nerve.db "SELECT * FROM version_manager WHERE tool='9router'"`.
