...
TPROXY transparent decrypt is Nerve's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA Nerve
...
> and installs under the pre-existing `nerve-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `nerve-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"Nerve MITM CA"`,
...
`nerve-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`nerve-mitm.crt`) so the two never clobber each
other.
...
host means anything Nerve intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `nerve-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if Nerve was killed mid-session, use the AgentBridge
