...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to Nerve,
...
## How Nerve Solves This (v3.8.0+)

Starting with v3.8.0, Nerve calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each Nerve connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; Nerve reads credentials from the
...
4. Nerve validates the key and saves the connection.
...
(`clientSecretExpiresAt`). Nerve stores this timestamp in `providerSpecificData`
...
- Ensure Nerve can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise Nerve cannot
