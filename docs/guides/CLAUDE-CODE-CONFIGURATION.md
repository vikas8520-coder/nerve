---
title: "Claude Code CLI — Configuration with Nerve"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — Configuration with Nerve

Point the **Claude Code** CLI (`claude`) at Nerve — local or a remote VPS —
...
| `ANTHROPIC_AUTH_TOKEN`                       | Sent as `Authorization: Bearer …` — use your Nerve access token / API key          |
...
`nerve launch` sets all of these for you: it resolves the base URL + token
from the active context (so `nerve connect <vps>` then `nerve launch`
...
`/model` picker normally shows **only** Nerve's Claude/Anthropic models — a
`kimi/kimi-k2.6` or `glm/glm-5.2` never appears, even though it routes fine.

Nerve can mirror any enabled model (and combo) under a `claude/…` id so it
...
When you pick one of these in Claude Code, Nerve strips the `claude/` wrapper
...
wins), so a plain Nerve never doubles its catalog for clients that don't use
Claude Code:
...
`nerve setup-claude` fetches the live `/v1/models` catalog and writes one
...
> `nerve launch --profile <name>` (it injects `ANTHROPIC_AUTH_TOKEN` from the
> active context), or export `ANTHROPIC_AUTH_TOKEN` yourself and run
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync after model discovery (opt-in).** Nerve can regenerate these same
...
auto-sync" → Claude Code), or set `NERVE_AUTO_SYNC_CLAUDE_PROFILES=true` (it also honors
...
Claude Code routes to capability tiers. Map each to an Nerve model via env /
...
Once you've run `nerve connect <host>` (see
[Remote Mode](./REMOTE-MODE.md)), `nerve launch` and `nerve setup-claude`
...
`/v1`** and restart `claude` (env is read once at startup). `nerve launch`
...
Nerve refuses to guess. Fix it either way: pin a prefixed id with
...
`NERVE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (default off;
...
**Auth errors** — the profile holds no token. Use `nerve launch --profile`
