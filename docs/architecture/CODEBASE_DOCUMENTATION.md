---
title: "Nerve Codebase Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Nerve Codebase Documentation

> **Version:** v3.8.0
> **Last updated:** 2026-06-28
> **Audience:** Engineers contributing to Nerve or building integrations on top of it.
>
...
- `@/*` → `src/*`
- `@nerve/open-sse` → `open-sse/index.ts`
- `@nerve/open-sse/*` → `open-sse/*`

Default HTTP port: **`20128`** (API and dashboard share the same process). Data
directory is `DATA_DIR` env var, defaulting to `~/.nerve/`.
...
├── open-sse/             Streaming engine workspace (@nerve/open-sse)
├── electron/             Desktop wrapper (Electron 41 main + preload)
├── bin/                  CLI entry points (nerve, reset-password)
...
| `responses.ts`, `nerveResponseMeta.ts` | Response shape helpers                            |
...
Separate npm workspace published as `@nerve/open-sse`. Owns request
...
```
bin/
├── nerve.mjs           Main CLI entry (Node ESM)
...
- `nerve` → `bin/nerve.mjs`
- `nerve-reset-password` → `bin/reset-password.mjs`
