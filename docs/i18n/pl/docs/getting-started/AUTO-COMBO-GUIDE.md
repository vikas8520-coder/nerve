# Auto-Combo: niech Nerve wybierze najlepsze AI za Ciebie

> **TL;DR**: Ustaw model na `auto`, a Nerve automatycznie wybierze najlepszego providera AI dla każdego żądania. Nie wymaga konfiguracji.

---

## Co robi

Zamiast wybierać konkretny model AI (np. GPT-4o lub Claude), możesz pozwolić Nerve **automatycznie wybrać najlepszy** dla każdego żądania. Bierze pod uwagę:
...
Nerve ocenia wszystkich podłączonych providerów i wybiera najlepszego. Jeśli ten zawiedzie, automatycznie próbuje kolejnego.
...
**Krok 2**: To wszystko! Nerve zajmie się resztą.
...
Gdy wysyłasz żądanie z `model: "auto"`, Nerve:
...
4. **Automatycznie odzyskuje się** — jeśli ten zawiedzie, Nerve automatycznie próbuje kolejnego
...
Nerve ma **trzy warstwy ochrony**:

### 1. Auto-Fallback

Jeśli najlepszy provider zawiedzie, Nerve automatycznie próbuje kolejnego. Nie musisz nic robić.
...
Jeśli wszyscy providerzy zawiodą, Nerve jako ostateczność kieruje ruch do stabilnych darmowych providerów (np. Kiro lub Qoder).

---

## Obsługa wielu kont

Jeśli masz wiele kont u tego samego providera (np. dwa klucze OpenAI), Nerve traktuje każde jako **osobnego kandydata**. To oznacza:
...
Nerve od czasu do czasu **eksploruje** nowych providerów, żeby odkryć lepsze opcje:
...
Dzięki temu Nerve uczy się, którzy providerzy najlepiej pasują do Twojego wzorca użycia.
...
Nerve automatycznie go pomija i próbuje kolejnego. Jeśli provider wciąż zawodzi, jest tymczasowo wykluczany (5–30 minut). Nie musisz nic robić.

### „Czy mogę zobaczyć, który provider został użyty?”

Sprawdź nagłówki odpowiedzi — Nerve dołącza użytego providera i model w każdej odpowiedzi.
