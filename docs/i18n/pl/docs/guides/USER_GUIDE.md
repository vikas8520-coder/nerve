...
Kompletny przewodnik po konfiguracji providerów, tworzeniu combo, integracji narzędzi CLI i wdrażaniu Nerve.
...
**Wskazówka:** Używaj Opus do złożonych zadań, Sonnet dla szybkości. Nerve śledzi limit per model!

Trasy zgodne z Claude i Claude Code zachowują poziom myślenia `max` dla modeli Opus i Sonnet
Modele Haiku nie akceptują poziomu wysiłku `max`, więc Nerve obniża to
...
1. Subskrybuj: [Moonshot AI](https://platform.kimi.ai?aff=nerve)
2. Pobierz klucz API → Dashboard → Add API Key
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-nerve-api-key"
  }
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "nerve/if/kimi-k2.7-code" }
    }
  },
  "models": {
    "providers": {
...
CLI automatycznie ładuje `.env` z `~/.nerve/.env` lub `./.env`.

### Odinstalowywanie

Gdy nie potrzebujesz już Nerve, mamy dwa szybkie skrypty do czystego usunięcia:

| Polecenie                | Działanie                                                                        |
| ------------------------ | -------------------------------------------------------------------------------- |
| `npm run uninstall`      | Usuwa aplikację systemową, ale **zachowuje DB i konfiguracje** w `~/.nerve`. |
| `npm run uninstall:full` | Usuwa aplikację ORAZ trwale **kasuje wszystkie konfiguracje, klucze i bazy**.    |

> Uwaga: Aby uruchomić te polecenia, przejdź do folderu projektu Nerve (jeśli klonowałeś) i je wykonaj. Alternatywnie, przy instalacji globalnej możesz po prostu uruchomić `npm uninstall -g nerve`.

### Wdrożenie na VPS

```bash
git clone https://github.com/diegosouzapw/Nerve.git
cd Nerve && npm install && npm run build

export JWT_SECRET="your-secure-secret-change-this"
export INITIAL_PASSWORD="your-password"
export DATA_DIR="/var/lib/nerve"
export PORT="20128"
...
npm run start
# Or: pm2 start npm --name nerve -- start
```
...
pm2 start npm --name nerve -- start

# Or with custom memory limit
NERVE_MEMORY_MB=512 pm2 start npm --name nerve -- start
...
docker build -t nerve:cli .

# Portable mode (recommended)
docker run -d --name nerve -p 20128:20128 --env-file ./.env -v nerve-data:/app/data nerve:cli
```
...
Użytkownicy Void Linux mogą spakować i zainstalować Nerve natywnie przez framework cross-kompilacji `xbps-src`. Automatyzuje to build standalone Node.js wraz z wymaganymi natywnymi bindingami `better-sqlite3`.
...
| --------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `JWT_SECRET`                            | `nerve-default-secret-change-me` | Sekret podpisu JWT (**zmień w produkcji**)                                                                               |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | Hasło pierwszego logowania                                                                                               |
| `DATA_DIR`                              | `~/.nerve`                       | Katalog danych (db, usage, logi)                                                                                         |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://nerve.dev`              | Bazowy URL endpointu cloud sync (zastępuje legacy `CLOUD_URL`)                                                           |
...
| `NERVE_MEMORY_MB`                   | `512`                                | Limit sterty Node.js w MB                                                                                                |
...
### Łączenie peerów Nerve

Inną bramę Nerve możesz dodać jako provider **Custom OpenAI-compatible**. Użyj
...
```bash
# gateway-a
NERVE_INSTANCE_ID=gateway-a
NERVE_PEER_URLS=http://gateway-b:20128/v1
NERVE_PEER_MAX_HOPS=4
```

```bash
# gateway-b
NERVE_INSTANCE_ID=gateway-b
NERVE_PEER_URLS=http://gateway-a:20128/v1
NERVE_PEER_MAX_HOPS=4
```

Tylko żądania wysyłane na jawnie dozwolony URL peeru otrzymują nagłówek
`X-Nerve-Peer-Trace`. Brama odrzuca powtórzone ID instancji lub wyczerpany budżet hopów
...
do wielu uruchomionych instancji Nerve.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie Nerve lub kontenera; włącz je ponownie z dashboardu w razie potrzeby
...
- **Semantic Cache** — automatycznie cache'uje odpowiedzi non-streaming z temperature=0 (pomiń przez `X-Nerve-No-Cache: true`)
- **Idempotencja żądań** — deduplikuje żądania w ciągu 5s przez nagłówek `Idempotency-Key` lub `X-Request-Id`
- **Śledzenie postępu** — opcjonalne zdarzenia SSE `event: progress` przez nagłówek `X-Nerve-Progress: true`

---

### Translator Playground

Dostęp: **Dashboard → Translator**. Debuguj i wizualizuj, jak Nerve tłumaczy żądania API między providerami.
...
Nerve akceptuje też `x_session_id` i zwraca efektywny klucz sesji w `X-Nerve-Session-Id`.
...
Nerve implementuje odporność na poziomie providera w pięciu komponentach:
...
4. **Wait For Cooldown** — jeśli każde połączenie-kandydat jest już w cooldown, Nerve może poczekać na najwcześniejszy cooldown i automatycznie ponowić to samo żądanie klienta.
...
Nerve obsługuje transkrypcję audio przez endpoint zgodny z OpenAI:
...
Nerve obcina go do tego limitu przed wysłaniem żądania upstream.
...
Nerve ma wbudowany **auto-router oparty na score**, który wybiera najlepszy model dla każdego żądania spośród podłączonych providerów — bez utrzymywania combo. Wystarczy wysłać żądanie z jednym z prefiksów `auto/*`, a Nerve złoży wirtualne combo w locie, oceniając kandydatów pod kątem opóźnienia, kosztu, success rate, dopasowania kontekstu, przydatności modelu do zadania, ostatnich błędów, quoty i stanu circuit breakera.
...
Nerve jest jednocześnie **serwerem MCP** (Model Context Protocol) i **serwerem A2A** (Agent-to-Agent JSON-RPC 2.0). Każde IDE lub host agentów zgodny z MCP może wywoływać narzędzia Nerve bezpośrednio — bez dodatkowego wrappera.

### Transporty MCP

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `nerve --mcp` (dla wtyczek IDE preferujących stdio)
...
Nerve udostępnia rozszerzalny **framework skills** (`src/lib/skills/`), dzięki czemu agenci i endpoint A2A mogą uruchamiać rutyny domenowe (np. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
Nerve przechowuje **długoterminową pamięć konwersacyjną** z hybrydowym retrieval:
...
Subskrybuj zdarzenia Nerve do monitoringu i automatyzacji w czasie rzeczywistym.
...
- Każdy payload zawiera `X-Nerve-Signature` (HMAC-SHA256) do weryfikacji
...
Nerve integruje się z cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**), dzięki czemu możesz wysyłać długotrwałe zadania z tego samego dashboardu, który obsługuje lokalny routing.

- Twórz zadania w **Dashboard → Cloud Agents** lub przez `POST /api/v1/agents/tasks`
- Śledź status, logi i artefakty per zadanie
- Własny klucz API per provider — credentials nigdy nie opuszczają instancji Nerve
...
Możesz zarządzać każdym zasobem Nerve (providerzy, combo, klucze, ustawienia) przez HTTP, używając klucza zarządzania.
...
curl http://localhost:20128/api/providers \
  -H "Authorization: Bearer $NERVE_MANAGE_KEY"
...
curl http://localhost:20128/api/keys -H "Authorization: Bearer $NERVE_MANAGE_KEY"
curl -X POST http://localhost:20128/api/keys -H "Authorization: Bearer $NERVE_MANAGE_KEY" \
...
Nerve dostarcza wewnętrzne CLI (`nerve …`) do setupu, diagnostyki i kontroli runtime.

```bash
nerve setup                    # Interactive wizard (password, providers, combos)
nerve setup --non-interactive  # CI-friendly
nerve doctor                   # Health diagnostics (data dir, DB, providers, ports)
nerve providers available      # List supported providers
nerve providers list           # List configured connections
nerve providers test <id>      # Live test a provider connection
nerve combos list              # List combos
nerve combos switch <name>     # Set default combo
nerve models                   # List available models (--json, --search)
nerve keys add | list | remove # Manage API keys from the terminal
nerve backup                   # Snapshot config + DB
nerve restore [<timestamp>]    # Restore from a snapshot
nerve health                   # Detailed health (breakers, cache, memory)
nerve quota                    # Provider quota usage
nerve mcp status               # MCP server status
nerve a2a status               # A2A server status
nerve tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
nerve reset-password           # Reset the admin password
nerve --mcp                    # Start MCP server over stdio
nerve --port 3000              # Start the server on a custom port
```

Wskazówka: połącz `nerve doctor --json` z narzędziem monitoringu, by alertować o niezdrowych providerach.

---

## 🖥️ Aplikacja desktopowa (Electron)

Nerve jest dostępny jako natywna aplikacja desktopowa na Windows, macOS i Linux.
...
| Zmienna               | Domyślnie | Opis                               |
| --------------------- | --------- | ---------------------------------- |
| `NERVE_PORT`      | `20128`   | Port serwera                       |
| `NERVE_MEMORY_MB` | `512`     | Limit sterty Node.js (64–16384 MB) |
