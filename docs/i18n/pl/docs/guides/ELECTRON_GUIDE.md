...
Nerve dostarcza wieloplatformową aplikację desktopową (Windows / macOS / Linux) opartą na
...
| Id aplikacji       | `online.nerve.desktop` |
| Nazwa produktu     | `Nerve`                |
...
- Windows: `%APPDATA%\nerve`
- Linux: `$XDG_CONFIG_HOME/nerve` lub `~/.nerve`
- macOS: `~/.nerve`
...
- Przycisk zamknięcia minimalizuje do trayu; menu trayu ma **Open Nerve**, **Open Dashboard** (zewnętrzna przeglądarka), podmenu **Server Port**, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.nerve.online https://*.nerve.dev`
- Tryb dev dodaje `'unsafe-eval'` tylko do `script-src`

## Auto-aktualizacja

Używa `electron-updater` z providerem GitHub (`diegosouzapw/Nerve`).
...
- `Nerve Setup X.Y.Z.exe`, `Nerve-X.Y.Z-portable.exe` (Windows)
- `Nerve-X.Y.Z-mac.dmg`, `Nerve-X.Y.Z-arm64-mac.dmg` (macOS)
- `Nerve-X.Y.Z.AppImage`, `nerve-desktop_X.Y.Z_amd64.deb` (Linux)

Wydania są publikowane w GitHub Releases (`diegosouzapw/Nerve`), skąd `electron-updater` sprawdza też nowe wersje.
