---
title: "Claude Code CLI — konfiguracja z Nerve"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — konfiguracja z Nerve

Skieruj CLI **Claude Code** (`claude`) na Nerve — lokalnie lub na zdalny VPS —
...
```bash
# Launch Claude Code against a local Nerve (auto-detects the active context)
nerve launch

# Against a remote Nerve (after `nerve connect <host>`, this is automatic)
nerve launch --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Generate per-model profiles, then launch one
nerve setup-claude            # writes ~/.claude/profiles/<name>/settings.json
nerve launch --profile glm52  # Claude Code using glm/glm-5.2 via Nerve
```
...
| `ANTHROPIC_AUTH_TOKEN`                       | Wysyłany jako `Authorization: Bearer …` — użyj tokenu dostępu / klucza API Nerve       |
...
`nerve launch` ustawia to wszystko za Ciebie: rozwiązuje bazowy URL + token
z aktywnego kontekstu (więc `nerve connect <vps>`, a potem `nerve launch`
...
picker `/model` zwykle pokazuje **tylko** modele Claude/Anthropic z Nerve — `kimi/kimi-k2.6`
lub `glm/glm-5.2` się nie pojawią, mimo że routują poprawnie.

Nerve może odzwierciedlić dowolny włączony model (i combo) pod identyfikatorem `claude/…`,
aby przeszedł ten filtr i pojawił się w pickerze:

```
kimi/kimi-k2.6            →  claude/kimi/kimi-k2.6      "Kimi K2.6 (Nerve)"
glm/glm-5.2              →  claude/glm/glm-5.2         "GLM 5.2 (Nerve)"
<combo "custo-otimizado"> →  claude/combo/custo-otimizado
```

Gdy wybierzesz jeden z nich w Claude Code, Nerve zdejmuje opakowanie `claude/`
...
wygrywa), więc zwykły Nerve nie podwaja katalogu dla klientów, które nie używają
Claude Code:
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "http://<your Nerve>:20128",
...
`nerve setup-claude` pobiera żywy katalog `/v1/models` i zapisuje jeden
...
> `nerve launch --profile <name>` (wstrzykuje `ANTHROPIC_AUTH_TOKEN` z
> aktywnego kontekstu) albo sam wyeksportuj `ANTHROPIC_AUTH_TOKEN` i uruchom
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync po discovery modeli (opt-in).** Nerve może regenerować te same
...
auto-sync” → Claude Code) albo ustaw `NERVE_AUTO_SYNC_CLAUDE_PROFILES=true` (honoruje też
...
nerve setup-claude --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Only some providers
nerve setup-claude --only glm,kimi

# Preview without writing
nerve setup-claude --dry-run

# Launch a profile
nerve launch --profile kimi-k27
```
...
Claude Code routuje do poziomów możliwości. Zmapuj każdy na model Nerve przez env /
...
Po uruchomieniu `nerve connect <host>` (zobacz
[Tryb zdalny](./REMOTE-MODE.md)), `nerve launch` i `nerve setup-claude`
...
`/v1`**, i zrestartuj `claude` (env jest odczytywane raz przy starcie). `nerve launch`
robi to za Ciebie.
...
Nerve odmawia zgadywania. Napraw na jeden z dwóch sposobów: przypnij prefiksowane id przez
...
`NERVE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (domyślnie wyłączone;
...
**Błędy auth** — profil nie przechowuje tokenu. Użyj `nerve launch --profile`
