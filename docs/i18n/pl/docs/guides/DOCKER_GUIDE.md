---
title: "🐳 Przewodnik Docker — Nerve"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Przewodnik Docker — Nerve
...
Nerve dostarcza cztery profile Compose. Wybierz ten, który pasuje do Twojego środowiska.

| Profil            | Usługa           | Kiedy używać                                                                                                                              | Polecenie                                    |
| ----------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (domyślny) | `nerve-base` | Serwer headless / minimalne runtime, bez dołączonych CLI providerów                                                                       | `docker compose --profile base up -d`        |
| `cli`             | `nerve-cli`  | Przepływy agentowe wywołujące `nerve providers/setup/doctor` oraz dołączone CLI (Codex, Claude Code, Droid, OpenClaw)                 | `docker compose --profile cli up -d`         |
| `host`            | `nerve-host` | Hosty Linux z dostępem do CLI hosta w stylu `network_mode` przez montowanie `~/.local/bin`, `~/.codex`, `~/.claude` itd. tylko do odczytu | `docker compose --profile host up -d`        |
...
Nerve korzysta z Redis jako zaplecza rozproszonego rate limitera i współdzielonej pamięci podręcznej. Usługa `redis` jest **zawsze zdefiniowana** w `docker-compose.yml` (bez bramki profilu) i startuje razem z każdym innym profilem.

| Szczegół             | Wartość                           |
| -------------------- | --------------------------------- |
| Image                | `redis:7-alpine`                  |
| Container name       | `nerve-redis`                 |
| Internal port        | `6379`                            |
| Host port (override) | `REDIS_PORT` (defaults to `6379`) |
| Volume               | `nerve-redis-data` → `/data`  |
...
| Image                  | `nerve:prod` (built from `runner-cli` target)                                  |
| Redis container        | `nerve-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)         |
| Data volume            | `nerve-prod-data` (named, persisted across rebuilds)                           |
...
```bash
docker build --target runner-base -t nerve:base .
docker build --target runner-cli  -t nerve:cli  .
```

Domyślne wartości eksportowane przez `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `NERVE_MIGRATIONS_DIR=/app/migrations`.

Zachowanie pamięci w Dockerze:

- `NODE_OPTIONS=--max-old-space-size=512` jest wbudowane w obraz jako fallback.
- Właściwy proces serwera uruchamia launcher standalone, który czyta `NERVE_MEMORY_MB` i dopisuje `--max-old-space-size=<NERVE_MEMORY_MB>`.
- Node używa ostatniej powtórzonej wartości `--max-old-space-size`, więc ustawienie `NERVE_MEMORY_MB` kontroluje efektywny limit heapa w Dockerze.
- Gdy `NERVE_MEMORY_MB` nie jest ustawione, launcher używa `512`.
...
| ----------------------------- | -------------------------------------------------------------------------------------------- | ------------------------ |
| `NERVE_WS_BRIDGE_SECRET`  | Współdzielony sekret mostu WebSocket. **Wymagany w produkcji** — ustaw silny, losowy ciąg.   | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Ścieżka hosta montowana w profilu `cli` pod `/workspace/nerve` na potrzeby self-update   | `.` (current directory)  |
| `NERVE_MEMORY_MB`         | Sufit heapa Node w runtime dla serwera standalone Dockera; nadpisuje fallback obrazu powyżej | `512`                    |
| `DASHBOARD_PORT` / `API_PORT` | Nadpisanie eksponowanych portów dashboardu (20128) i API (20129)                             | `20128` / `20129`        |
| `NERVE_BASE_PATH`         | Podścieżka URL, gdy aplikacja jest publikowana za reverse proxy (np. `/nerve`)           | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Publiczny origin przeglądarki wraz z podścieżką (np. `https://host/nerve`)               | unset                    |
...
Next.js `basePath` jest kompilowany do bundla standalone. Nerve zapisuje wbudowaną
...
`NERVE_BASE_PATH` przy starcie kontenera. Gdy się różnią, a obraz był
...
NEXT_PUBLIC_BASE_URL=https://myhostname.example.com/nerve
```
...
`docker-compose.yml` przekazuje `NERVE_BASE_PATH` jako Docker build-arg oraz jako
zmienną środowiskową runtime.

### Wstępnie zbudowany obraz root + podścieżka w runtime

Opublikowane obrazy `diegosouzapw/nerve:*` są budowane pod root domeny. Nadal możesz
ustawić `NERVE_BASE_PATH` w runtime; kontener jednorazowo patchuje bundel przy starcie.
...
prefiksu). Traefik powinien routować `PathPrefix(`/nerve`)` do kontenera bez
`StripPrefix`, żeby Next.js otrzymywał `/nerve/...` i serwował assety z
`/nerve/_next/...`.
...
Nerve można bezpiecznie udostępnić dzięki automatycznemu provisionowaniu SSL w Caddy. Upewnij się, że rekord DNS A domeny wskazuje na IP Twojego serwera.
...
Caddy ustawia standardowe nagłówki forwardingu dla kontenera upstream. Nerve używa
...
związanej z sesją. Włączaj `NERVE_TRUST_PROXY` tylko w zaawansowanych wdrożeniach, gdzie świadomie
chcesz, by Nerve wyprowadzał publiczny origin z zaufanych nagłówków forwarded zamiast z jawnej
konfiguracji.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie Nerve ani kontenera. Włącz je ponownie z dashboardu, gdy potrzeba.
...
- Ustaw `CLOUDFLARED_BIN=/absolute/path/to/cloudflared`, jeśli chcesz, by Nerve używał istniejącego binarium zamiast pobierać własne.

## Tagi obrazów

| Obraz                    | Tag      | Rozmiar | Opis                       |
| ------------------------ | -------- | ------- | -------------------------- |
| `diegosouzapw/nerve` | `latest` | ~250MB  | Najnowsze stabilne wydanie |
| `diegosouzapw/nerve` | `3.8.0`  | ~250MB  | Bieżąca wersja             |
...
- **Tryb SQLite WAL:** `docker stop` powinien móc się dokończyć, żeby Nerve mógł zrobić checkpoint najnowszych zmian z powrotem do `storage.sqlite`. Dołączone pliki Compose ustawiają już 40s grace period stopu. Przy bezpośrednim uruchomieniu obrazu zachowaj `--stop-timeout 40`.
