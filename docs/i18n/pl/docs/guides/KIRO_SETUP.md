...
Ten przewodnik opisuje dodawanie kont Kiro (hostowany w AWS asystent AI do kodowania) do Nerve,
...
## Jak Nerve to rozwiązuje (v3.8.0+)

Od v3.8.0 Nerve wywołuje `registerClient()` (AWS SSO OIDC) przy każdym
imporcie połączenia Kiro. Dzięki temu każde połączenie Nerve ma własną, dedykowaną
...
2. Wybierz jedną z opcji:
   ...
   - **Auto-Import** — kliknij przycisk; Nerve odczyta poświadczenia z
...
4. Nerve waliduje klucz i zapisuje połączenie.
...
(`clientSecretExpiresAt`). Nerve przechowuje ten znacznik czasu w `providerSpecificData`
...
- Upewnij się, że Nerve może dotrzeć do `https://oidc.us-east-1.amazonaws.com` (lub
...
- Klucz musi móc wywołać `ListAvailableProfiles`; w przeciwnym razie Nerve nie może
