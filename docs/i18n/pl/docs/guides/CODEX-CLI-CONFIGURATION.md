---
title: "Codex CLI — konfiguracja z Nerve"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — konfiguracja z Nerve

Kompletny przewodnik po używaniu Codex CLI skierowanego na Nerve jako backend zgodny z OpenAI.
...
**Nerve rozwiązuje to w sposób przezroczysty:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (Nerve)
    → Nerve Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

Przy Nerve nie potrzebujesz osobnego proxy tłumaczącego. **Wszystkie modele używają `wire_api = "responses"`** — resztę obsługuje Nerve.
...
| Model                                | ID Nerve                         | Okno kontekstu           | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ------------------------ | -------------- | ------------------- |
...
Wszystkie modele Codex w Nerve używają prefiksu `cx/`:

| Nazwa w Codex CLI       | Model Nerve    |
| ----------------------- | ------------------ |
...
Inni providerzy używają własnego prefiksu (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — prefiks odpowiada aliasowi providera w Nerve.
...
## Automatyczne generowanie profili przez `nerve setup-codex`

Gdy Nerve działa na VPS, możesz automatycznie wygenerować pliki profili z żywego katalogu modeli:

```bash
# From a VPS (uses local Nerve on port 20128)
nerve setup-codex

# From any machine — point at your VPS
nerve setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
nerve setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
nerve setup-codex --only glm,kimi

# Write to a custom directory
nerve setup-codex --codex-home /path/to/.codex
```
...
Nerve może też **automatycznie synchronizować** te same pliki profili po udanym discovery/import modeli providera, gdy zmienia się żywy katalog. To opcja **opt-in, domyślnie wyłączona**: włącz ją w **dashboardzie CLI Code** („CLI profile auto-sync” → Codex) albo ustaw `NERVE_AUTO_SYNC_CODEX_PROFILES=true` (uwzględnia też `CLI_ALLOW_CONFIG_WRITES`, domyślnie włączone). Po włączeniu zapisuje tylko osobne pliki profili `~/.codex/*.config.toml`; nigdy nie zmienia aktywnego/domyślnego `~/.codex/config.toml`, ustawień Codex-lb, auth ani wyboru providera.

---

## Uruchamianie Codex przez `nerve launch-codex`

Sprawdza health instancji Nerve przed uruchomieniem Codex:

```bash
# Launch against local Nerve (default port 20128)
nerve launch-codex

# Launch with a specific profile
nerve launch-codex --profile kimi-k27

# Launch against a remote VPS
nerve launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
nerve launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.nerve.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.nerve.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.nerve.query_params]
...
```toml
[model_providers.nerve-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "NERVE_API_KEY"

[model_providers.nerve-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "NERVE_API_KEY"
```
...
Dwa domyślne ustawienia Nerve mogą cicho psuć wielogodzinne sesje Codex CLI. Żadne nie jest ustawieniem Codex CLI — oba leżą po stronie Nerve. Użytkownicy migrujący konfigurację z upstreamowych proxy, które pinują konta i wyłączają idle cutoff, często trafiają na oba i wnioskują, że Nerve „nie utrzyma długiej sesji”.
...
Powiązane dyskusje: [#7126](https://github.com/diegosouzapw/Nerve/discussions/7126) (zrywanie długich zadań), [#5718](https://github.com/diegosouzapw/Nerve/discussions/5718) (dlaczego affinity jest domyślnie wyłączone). Tracking: [#7287](https://github.com/diegosouzapw/Nerve/issues/7287).
...
Krytyczny szczegół: syntetyczny SSE **heartbeat Nerve nie resetuje** zegara idle. Tylko prawdziwy chunk body z upstream aktualizuje `lastChunkTime`. Cichy model, który nadal „myśli”, wygląda dla watchdogu tak samo jak zawieszony upstream.
...
W środowisku procesu Nerve (`.env` / compose / systemd):
...
Zrestartuj Nerve po zmianie tych zmiennych env.
...
2. **Podnieś / wyłącz idle cutoff** w środowisku Nerve:
...
3. Zostaw zwykły Codex `config.toml` (`wire_api = "responses"`, poprawny `base_url`, `NERVE_API_KEY`) — po stronie Codex nie ma pokręteł affinity/idle dla tych dwóch zachowań.
4. Zrestartuj Nerve, potem uruchom długie zadanie Codex.
...
Gdy odpala się idle watchdog, Nerve loguje linię w stylu:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grepuj `Idle timeout: no data from` (albo kod `stream_idle_timeout` / nazwę błędu `StreamIdleTimeoutError`). Segment providera to to, czego Nerve użyło dla tego requestu (`codex`, inny id providera albo `provider`, gdy nieznany) — nie zawsze jest to dosłowny string `codex`.
...
Sprawdź, czy model istnieje w Nerve z poprawnym prefiksem. Użyj `nerve models list` albo otwórz `/dashboard/providers/<provider>`.

**`Authentication error`**
Potwierdź, że `NERVE_API_KEY` jest wyeksportowany: `echo $NERVE_API_KEY`.

**`Connection refused`**
Sprawdź, czy Nerve działa i że host/port w `base_url` jest poprawny dla Twojej sieci (lokalnie vs Tailscale vs VPS).
...
Zobacz [Zadania długotrwałe](#zadania-długotrwałe). Włącz session affinity (TTL powyżej długości zadania) i podnieś lub wyłącz `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grepuj logi Nerve pod kątem `Idle timeout: no data from`.
