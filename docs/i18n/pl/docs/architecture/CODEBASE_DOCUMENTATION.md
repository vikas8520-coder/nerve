---
title: "Dokumentacja bazy kodu Nerve"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dokumentacja bazy kodu Nerve

> **Wersja:** v3.8.0
> **Ostatnia aktualizacja:** 2026-06-28
> **Odbiorcy:** Inżynierowie współtworzący Nerve lub budujący na nim integracje.
>
...
danych to zmienna środowiskowa `DATA_DIR`, domyślnie `~/.nerve/`.
...
├── open-sse/             Workspace silnika streamingu (@nerve/open-sse)
├── electron/             Opakowanie desktopowe (Electron 41 main + preload)
├── bin/                  Punkty wejścia CLI (nerve, reset-password)
...
| `responses.ts`, `nerveResponseMeta.ts` | Helpery kształtu odpowiedzi                       |
...
Osobny npm workspace publikowany jako `@nerve/open-sse`. Odpowiada za
...
```
bin/
├── nerve.mjs           Główne wejście CLI (Node ESM)
...
- **Importy**: external → internal (`@/`, `@nerve/open-sse`) → relative.
