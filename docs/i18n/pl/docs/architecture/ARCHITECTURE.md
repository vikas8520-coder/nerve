...
Nerve to lokalna brama routingu AI i panel (dashboard) zbudowane na Next.js.
...
flowchart LR
    ...
    subgraph Router[Nerve Local Process]
...
Nerve może instalować, nadzorować i routować do lokalnie działających procesów narzędzi AI
...
- Builder metadanych odpowiedzi Nerve: `src/domain/nerveResponseMeta.ts`
...
- plik: `${DATA_DIR}/storage.sqlite` (lub `$XDG_CONFIG_HOME/nerve/storage.sqlite` gdy ustawione, w przeciwnym razie `~/.nerve/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Nerve Runtime]
        Next[Next.js Server\nPORT=20128]
...
> Nerve v3.8.0. Kanoniczna i stale aktualizowana lista: zob.
...
- końcowa odpowiedź klienta zwrócona przez Nerve; odpowiedzi streamowane są przechowywane w tej samej zwartej formie podsumowania
...
1. `usageDb` i `localDb` współdzielą tę samą politykę katalogu bazowego (`DATA_DIR` -> `XDG_CONFIG_HOME/nerve` -> `~/.nerve`) z migracją plików legacy.
...
5. Katalog `open-sse/` jest publikowany jako **pakiet npm workspace** `@nerve/open-sse`. Kod źródłowy importuje go przez `@nerve/open-sse/...` (rozwiązywane przez Next.js `transpilePackages`). Ścieżki plików w tym dokumencie nadal używają nazwy katalogu `open-sse/` dla spójności.
...
- Build obrazu Docker: `docker build -t nerve .`
