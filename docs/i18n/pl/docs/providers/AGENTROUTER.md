...
Dla większości użytkowników **nie jest wymagana żadna specjalna konfiguracja**. Nerve dostarcza wbudowany
...
Nerve obsługuje też AgentRouter (i podobne przekaźniki) przez typ providera **Claude Code
...
- Nerve uruchomione z włączoną flagą funkcji `ENABLE_CC_COMPATIBLE_PROVIDER`
  (patrz poniżej).
...
ustawiając zmienną środowiskową przed uruchomieniem Nerve:
...
**`Invalid JSON response from provider (reset after Ns)` w logach nerve** —
...
providera `agentrouter`, który już ma poprawny obraz sieciowy. Sprawdź w logach nerve,
