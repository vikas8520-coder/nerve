...
Wszystkie trasy management API Nerve są klasyfikowane do jednego z trzech
...
operator wystawił Nerve za tunelem nginx/Cloudflare/Tailscale i wyciekł JWT,
albo auth był źle skonfigurowany — atakujący zamienia „wywołaj API” na „uruchom
komendę na hoście” (remote code execution). Nerve zamyka to, egzekwując
...
Jeśli uruchamiasz Nerve za reverse proxy lub tunelem (nginx, Caddy, Cloudflare
...
- `403 LOCAL_ONLY` w logach Nerve dla jednej z tych ścieżek to strażnik
