...
TPROXY transparent decrypt to **5. tryb przechwytywania** Nerve w stosie MITM
...
zmiany konfiguracji** — ale musi ufać dynamicznemu CA instalowanemu przez Nerve
...
"TPROXY decrypt requires Linux + root + the native addon", a reszta Nerve
działa normalnie.
...
> i instaluje pod istniejącym slotem trust-store `nerve-mitm.crt`
> (zastępując stary pojedynczy leaf — bez potrzeby dual-trust cleanup),
> w pełni oddzielony od slotu TPROXY `nerve-tproxy-ca.crt` opisanego poniżej.
...
- Generuje długowieczne CA przez `generateMitmCa()` (CN `"Nerve MITM CA"`,
...
`nerve-tproxy-ca.crt` (stała `TPROXY_CA_CERT_NAME`) — oddzielonym od
slotu statycznego certu MITM (`nerve-mitm.crt`), żeby te dwa nigdy się nie nadpisywały.
...
| **Dedicated CA slot**            | The dynamic CA installs to `nerve-tproxy-ca.crt`, never clobbering the static MITM cert.                                                                                                                                                                                                                                                                       |
...
host, oznacza, że wszystko przechwycone przez Nerve może zostać odszyfrowane. Jest bramkowane przez
...
- Potwierdź, że proces ufa dynamicznemu CA. CA jest instalowane pod
  `nerve-tproxy-ca.crt`; aplikacje z własnym trust store (Firefox/Chrome NSS)
...
trybu cofa reguły; jeśli Nerve zostało zabite w trakcie sesji, użyj akcji AgentBridge
