...
Komendy CLI Nerve uwierzytelniają się względem lokalnego management API za pomocą
tokenu `HMAC-SHA256(machine-id, salt)` wysyłanego w nagłówku żądania
`x-nerve-cli-token`.

Dzięki temu podkomendy CLI (`nerve status`, `nerve providers` itd.)
...
3. CLI wysyła token jako `x-nerve-cli-token` w każdym żądaniu do
...
Ustaw `NERVE_CLI_SALT`, aby obrócić wyprowadzony token bez zmian w kodzie.
...
Domyślna sól: `nerve-cli-auth-v1`
...
**Opt-out:** ustaw `NERVE_DISABLE_CLI_TOKEN=true` (env lub `.env`), aby całkowicie wyłączyć
