...
> **TL;DR**: Nerve dostarcza wbudowane monitorowanie kondycji, autopilota providerów, śledzenie limitów (quota) oraz haki obserwowalności. Ten przewodnik obejmuje dashboard, alerty i rozwiązywanie problemów.
...
Nerve ma **3 warstwy monitorowania**:
...
│  └─ Narzędzia MCP: nerve_get_health, nerve_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
Nerve obsługuje **3 kanały alertów**:
...
- Śledzenie quota w Nerve może być **nieaktualne** — prawda providera jest upstream
...
- Uruchom `sqlite3 ~/.nerve/storage.sqlite "PRAGMA integrity_check;"`
- Jeśli "ok" — fałszywy alarm, health check jest zbyt rygorystyczny
- Jeśli cokolwiek innego — **zatrzymaj Nerve** i postępuj według [przewodnika disaster recovery](./DATABASE_GUIDE.md#disaster-recovery)
