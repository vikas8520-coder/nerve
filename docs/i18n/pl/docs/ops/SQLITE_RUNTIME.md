...
Nerve wybiera sterownik SQLite przy starcie w 5-stopniowym łańcuchu fallback:
...
2. **`better-sqlite3` zainstalowany w runtime** (w `~/.nerve/runtime/`)
...
- **Windows EBUSY**: `npm install -g nerve@latest` może się nie udać, jeśli
  `better_sqlite3.node` poprzedniej wersji jest zablokowany przez działający proces. Instalacja
  runtime w `~/.nerve/runtime/` omija globalną pamięć podręczną npm.
- **Brak narzędzi do budowania**: Niektóre środowiska (korporacyjny Windows bez VS Build
  ...
  gwarantują, że Nerve i tak się uruchomi, nawet gdy to się nie uda.
...
Przed załadowaniem pliku `.node` zainstalowanego w runtime Nerve odczytuje pierwsze 8
...
NERVE_SKIP_POSTINSTALL=1 npm install -g nerve

# Force-reinstall runtime better-sqlite3
rm -rf ~/.nerve/runtime
nerve  # will reinstall on next start

# Check what driver is active
nerve config db-info  # (if CLI command exists)
```
