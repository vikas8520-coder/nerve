---
title: "🗜️ Przewodnik po kompresji promptów — Nerve"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🗜️ Przewodnik po kompresji promptów — Nerve
...
Nerve implementuje modularny potok kompresji promptów, który działa **proaktywnie** zanim żądania trafią do upstreamowych providerów. Oszczędności tokenów zachodzą więc przejrzyście — bez zmian w Twoim workflow.
...
Nerve dokumentuje oszczędności kompresji z dwóch źródeł: benchmarków projektów upstream oraz
własnej kompozycji silników Nerve.
...
Dla nakładających się payloadów narzędzi/kontekstu domyślne combo Nerve układa silniki w stos:
...
Wyślij nagłówek żądania `x-nerve-compression`, aby nadpisać plan kompresji dla pojedynczego
...
Zastosowany plan jest zwracany w nagłówku odpowiedzi `X-Nerve-Compression: <mode>; source=<source>`,
...
Poza 7 standardowymi trybami Nerve zawiera kilka zaawansowanych systemów kompresji,
...
} from "@nerve/open-sse/services/compression/cachingAware";
...
```ts
import { applyAging } from "@nerve/open-sse/services/compression/progressiveAging";
