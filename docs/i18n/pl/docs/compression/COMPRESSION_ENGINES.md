...
Kompresja Nerve opiera się na kontraktach silników. Tryb może uruchomić jeden silnik bezpośrednio
...
dowodzą, że faktycznie może dotrzeć do `nerve_ccr_retrieve`** (`callerSupportsCcrRetrieve()` w
...
z zakresem `22-87%` oraz narzędzie kompresji wejścia na poziomie `~46%`. Nerve używa liczby
...
całą instalację Nerve; gdy opcjonalny stos jest nieobecny, LLMLingua nadal fail-openuje. Tylko trzy
...
- **Global npm (`npm i -g nerve`) / standalone** — uruchom powyższą komendę install wewnątrz
...
| Tool                                | Scope               | Purpose                                 |
| ----------------------------------- | ------------------- | --------------------------------------- |
| `nerve_compression_status`      | `read:compression`  | Ustawienia, analytics, statystyki cache |
| `nerve_compression_configure`   | `write:compression` | Aktualizacja ustawień globalnych        |
| `nerve_set_compression_engine`  | `write:compression` | Ustawienie trybu i opcjonalnego potoku  |
| `nerve_list_compression_combos` | `read:compression`  | Lista combo kompresji                   |
| `nerve_compression_combo_stats` | `read:compression`  | Odczyt analytics combo/silnika          |
