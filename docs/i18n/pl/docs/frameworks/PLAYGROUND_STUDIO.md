...
z bieżącego `PlaygroundState`. Placeholder klucza API to zawsze `$NERVE_API_KEY` (D11).
...
| Eksport kodu pokazuje `MISSING_API_KEY`        | Placeholder nie wstawiony               | `codeExport.ts` zawsze używa `API_KEY_PLACEHOLDER = "$NERVE_API_KEY"`          |
