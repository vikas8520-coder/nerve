...
Nerve łączy się z vaultem **Obsidian** jako **źródłem kontekstu** — lokalną bazą
...
wskazuje Nerve na lokalny katalog vaultu i generuje losową parę username/password WebDAV:
...
w **serwer sync** Nerve (cztery narzędzia `obsidian_sync_*`) dodatkowo wymagają
tokenu auth sync skonfigurowanego w ustawieniach Nerve.
...
| `obsidian_sync_status`       | Status serwera sync Nerve: running, nazwa vaultu, port, uptime, last sync.               |
...
`open-sse/mcp-server/server.ts`, bramkowane przez `NERVE_MCP_ENFORCE_SCOPES=true`, z
dozwolonymi scope pochodzącymi z `NERVE_MCP_SCOPES` lub kontekstu scope klucza API. Zobacz
