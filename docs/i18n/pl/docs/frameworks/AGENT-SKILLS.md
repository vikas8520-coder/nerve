---
title: "Katalog Agent Skills Nerve"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Katalog Agent Skills Nerve

> **Source of truth:** `src/lib/agentSkills/` (catalog, generator, parsers) + `skills/` directory (SKILL.md files)
> **Last updated:** 2026-06-28 — v3.8.40

Agent Skills to ustrukturyzowane pliki SKILL.md, które uczą zewnętrzne agenty, klientów MCP oraz orkiestratory A2A, jak korzystać z REST API i CLI Nerve. W przeciwieństwie do [Omni Skills](./SKILLS.md) (które są definicjami narzędzi LLM wykonywanymi wewnątrz Nerve), Agent Skills to _katalog dokumentacji_ — statyczny markdown, który można wstrzyknąć bezpośrednio do kontekstu agenta.
...
- **Surowy URL GitHub** do pobrania na żywo: `https://raw.githubusercontent.com/diegosouzapw/Nerve/refs/heads/main/skills/{id}/SKILL.md`
...
| Tool                              | Description                                        |
| :-------------------------------- | :------------------------------------------------- |
| `nerve_agent_skills_list`     | List skills (optional `category` / `area` filters) |
| `nerve_agent_skills_get`      | Get metadata + SKILL.md for one skill by `id`      |
| `nerve_agent_skills_coverage` | Coverage stats (API/CLI have/total)                |
...
| :------------------- | :----------------- | :---------------------- |
| `cli-serve`          | cli-serve          | `nerve serve`       |
| `cli-health`         | cli-health         | `nerve health`      |
| `cli-providers`      | cli-providers      | `nerve providers`   |
| `cli-keys`           | cli-keys           | `nerve keys`        |
| `cli-models`         | cli-models         | `nerve models`      |
| `cli-chat`           | cli-chat           | `nerve chat`        |
| `cli-routing`        | cli-routing        | `nerve routing`     |
| `cli-resilience`     | cli-resilience     | `nerve resilience`  |
| `cli-compression`    | cli-compression    | `nerve compression` |
| `cli-contexts`       | cli-contexts       | `nerve contexts`    |
| `cli-cost-usage`     | cli-cost-usage     | `nerve cost`        |
| `cli-mcp`            | cli-mcp            | `nerve mcp`         |
| `cli-a2a`            | cli-a2a            | `nerve a2a`         |
| `cli-tunnel`         | cli-tunnel         | `nerve tunnel`      |
| `cli-backup-sync`    | cli-backup-sync    | `nerve backup`      |
| `cli-policy-audit`   | cli-policy-audit   | `nerve policy`      |
| `cli-batches`        | cli-batches        | `nerve batch`       |
| `cli-eval`           | cli-eval           | `nerve eval`        |
| `cli-plugins-skills` | cli-plugins-skills | `nerve plugins`     |
| `cli-setup`          | cli-setup          | `nerve setup`       |
...
curl "http://your-nerve/api/agent-skills" | jq '.skills[] | {id, name, category}'

# Get SKILL.md for context injection
curl "http://your-nerve/api/agent-skills/omni-providers/raw" > omni-providers.md
```
...
const result = await client.callTool("nerve_agent_skills_list", { category: "api" });
...
resp = requests.post("http://your-nerve/a2a", json={
    "jsonrpc": "2.0", "id": "1",
    "method": "message/send",
...
```bash
BASE="https://raw.githubusercontent.com/diegosouzapw/Nerve/refs/heads/main/skills"
...
- [MCP-SERVER.md](./MCP-SERVER.md) — katalog narzędzi MCP (narzędzia `nerve_agent_skills_*`)
