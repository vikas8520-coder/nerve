...
Usługi wbudowane to lokalnie instalowane procesy sidecar, które Nerve instaluje, nadzoruje i
udostępnia jako pełnoprawne cele routingu. W przeciwieństwie do zewnętrznych providerów (osiąganych przez internet
przez klucze API), usługi wbudowane działają na tej samej maszynie co Nerve i komunikują się przez loopback.
...
| --------------- | ---------------------------------------------- | :-----------: | -------------------------------------------------------------------------------------------------------------------------- |
| **9Router**     | `9router`                                      |     20130     | Router AI, którego Nerve może używać jako sub-providera. Modele jako `9router/{sub}/{model}`                           |
...
- Nerve instaluje je pod `DATA_DIR/services/{name}/` (odizolowane od własnego `package.json` Nerve)
- Nerve uruchamia je i monitoruje jako procesy potomne
- Nerve wstrzykuje efemeryczny klucz API do środowiska potomka i rotuje go bez przestoju (gdzie dotyczy)
...
| Zarządzanie kluczem API                   | Nerve generuje, szyfruje at-rest (AES-256-GCM) i wstrzykuje przez env |
...
`package.json` i `node_modules/`. Nie koliduje z zależnościami Nerve.
...
przy następnym boot Nerve (jeśli jest zainstalowana).
...
- Usuwa przychodzące nagłówki `cookie` i `authorization` (brak wycieku sesji Nerve)
...
2. Zweryfikuj, że `npm` jest w PATH: `which npm` z tego samego konta użytkownika, które uruchamia Nerve.
...
- `DATA_DIR` lub jego rodzic nie jest zapisywalny przez proces Nerve.
- Uruchomienie w Docker rootless bez zapisu do zamapowanego volume.

**Naprawa:**

1. Sprawdź `DATA_DIR` (domyślnie: `~/.nerve/`): `ls -la ~/.nerve/`
2. Upewnij się, że użytkownik procesu Nerve jest właścicielem katalogu: `chown -R $USER ~/.nerve/`
...
3. Spróbuj instalacji ręcznie: `npm install {package}@latest --prefix ~/.nerve/services/{name}/`.
...
Tak. Obie usługi idą tym samym modelem local-first co sam Nerve. Działają
na tej samej maszynie i komunikują się przez loopback. „Production” oznacza tu VPS
lub lokalny serwer, na którym wdrożono Nerve, a nie zdalnego providera chmurowego.
...
2. Sprawdź strukturalne logi w output pino Nerve filtrowane po
   namespace `service:supervisor`.
3. Podejrzyj wiersz DB: `sqlite3 ~/.nerve/nerve.db "SELECT * FROM version_manager WHERE tool='9router'"`.
