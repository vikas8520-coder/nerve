...
> **TL;DR**: `open-sse/` to rdzeń silnika strumieniowania, który obsługuje każde żądanie LLM w Nerve. Zawiera ~900 plików implementujących pipeline żądań, executory, serwisy, serwer MCP oraz warstwę tłumaczenia formatów. Ten przewodnik wyjaśnia, jak te elementy współgrają ze sobą.
...
`open-sse/` to **samodzielny workspace** w monorepo Nerve z kilku powodów:

1. **Reużywalność** — `open-sse` jest publikowany jako `@nerve/open-sse` na npm, więc inne projekty mogą go używać niezależnie
2. **Czyste granice** — silnik strumieniowania jest odseparowany od warstwy UI/DB specyficznej dla Nerve
...
```ts
import { getExecutor } from "@nerve/open-sse/executors";
...
```ts
import { needsTranslation, translateRequest } from "@nerve/open-sse/translator";
...
Responses API to nowy format OpenAI ze **stateful conversations** (`previous_response_id`). Gdy klient wysyła żądanie Responses, Nerve:
