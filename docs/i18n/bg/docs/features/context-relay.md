...
- before the active account is exhausted, Nerve generates a compact structured summary
- after authentication selects a different account for the same session, Nerve injects
...
If the active provider is enabled in `handoffProviders`, Nerve generates a structured
...
Nerve prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, Nerve converts that payload into a `<context_handoff>` system
