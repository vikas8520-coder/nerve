# 🚀 Nerve — Бесплатный AI-шлюз (Русский)
...
<img src="../../screenshots/MainNerve.png" alt="Панель Nerve" width="820"/>

<br/>

# 🚀 Nerve — Бесплатный AI-шлюз
...
**Вопросы, советы по провайдерам, roadmap и поддержка → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/nerveOficial) · WhatsApp [🌍 Global](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 Brasil](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[**🚀 Быстрый старт**](#-быстрый-старт) • [**🎯 Комбо**](#-комбо--главная-фича) • [**🌐 Провайдеры**](#-278-ai-провайдеров--90-бесплатных) • [**🔌 CLI и MCP**](#-полный-cli--a2a-и-mcp) • [**🗜️ Сжатие**](#️-экономьте-1595-токенов--автоматически) • [**🌍 Сайт**](https://nerve.online)

[💥 Обещание](#-обещание) • [🤔 Зачем](#-зачем-nerve) • [🏆 Чем отличается](#-чем-nerve-отличается) • [🤖 Совместимые CLI](#-совместимые-cli-и-агенты) • [🖥️ Где запускать](#️-где-запускается-nerve--везде) • [🔒 Приватность](#-приватно-и-local-first) • [🎬 В деле](#-nerve-в-деле) • [📚 Дальше](#-узнать-больше) • [📧 Поддержка](#-поддержка-и-сообщество)
...
> Собирать free-tier вручную — боль: десятки SDK, лимиты и непонятный остаток. Nerve сводит **документированные** free-tier **43 пулов / 460+ моделей** в одно честное число и показывает его live на `/dashboard/free-tiers`.
>
...
> Один endpoint. **278 провайдеров.** Код не останавливается — Nerve сам выбирает самый дешёвый рабочий вариант.
...
| ❌ Боль каждый день | ✅ Как решает Nerve |
|---|---|
...
> **Combo** — цепочка моделей, по которой Nerve ходит **сам**. Квота кончилась, провайдер упал, цена взлетела — комбо тихо уходит на следующий шаг. **Именно это делает Nerve «неубиваемым».** 🛡️

### ⚡ Без настройки — просто `auto`

Комбо создавать не обязательно. Поставьте модель `auto` (или вариант) — Nerve соберёт виртуальное комбо из подключённых провайдеров:
...
<sub>📊 Сравнение с LiteLLM, OpenRouter, Portkey → [`docs/comparison/NERVE_VS_ALTERNATIVES.md`](../../comparison/NERVE_VS_ALTERNATIVES.md)</sub>
...
- **🤖 One-command CLI setup** — `setup-*` для 12+ tools; `nerve launch` / `launch-codex`.
...
- **💸 Cost telemetry** — `X-Nerve-*` headers, cache-HIT savings, per-key USD quotas.
- **🧠 Memory** — opt-in, int8 quantization, `x-nerve-no-memory`.
...
<sub>📖 Setup 33 tools → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · OpenCode plugin → [`@nerve/opencode-provider`](https://www.npmjs.com/package/@nerve/opencode-provider)</sub>
...
# 🖥️ Где запускается Nerve — везде

</div>

| Платформа | Установка | Плюсы |
|---|---|---|
| 📦 **npm (global)** | `npm install -g nerve` | Одна команда, любая ОС |
| 🐳 **Docker** | `docker run … diegosouzapw/nerve` | **AMD64 + ARM64** |
| 🖥️ **Desktop (Electron)** | `npm run electron:build` | Окно + tray — Win/macOS/Linux |
| 💪 **ARM** | native `arm64` | Pi, ARM servers, Apple Silicon |
| 📱 **Android (Termux)** | `pkg install nodejs && npx -y nerve` | На телефоне 24/7, без root |
| 📲 **PWA** | «Add to Home Screen» | Fullscreen, offline |
| 🧩 **OpenCode plugin** | `@nerve/opencode-provider` | Нативная интеграция |
...
> Ваши ключи, ваша машина, ваши данные. Nerve — **локальный прокси**, без «звонков домой».

- 🏠 **100% на вашем железе** — npm, Docker, desktop или телефон. Нет cloud-hop Nerve.
...
> Это не «просто сервер» — **CLI-кокит** с **80+ командами** и открытыми agent-протоколами, чтобы AI управлял Nerve **сам**.

### ⌨️ Настоящий CLI

```bash
nerve               # gateway + dashboard (порт 20128)
nerve chat          # TUI-чат (/model /combo /skill /memory)
nerve setup         # мастер первого запуска
nerve doctor        # диагностика провайдеров, портов, native deps
```

### 🛰️ Remote mode — CLI здесь, Nerve на VPS

```bash
nerve connect 192.168.0.15            # пароль → scoped token
nerve models list                     # ← на REMOTE
nerve configure codex                 # remote model → local Codex profile
nerve tokens create --name ci --scope read
nerve contexts use default            # ← обратно на local
```
...
|---|---|---|
| 🧰 **MCP (stdio)** | `nerve --mcp` | Claude Desktop, Cursor, любой MCP client |
...
Precedence (high → low): header `x-nerve-compression` › combo override › named profile › adaptive › panel default › off.
...
Должны появиться подключённые модели. 🎉 Дальше пишите код — Nerve сам роутит и делает fallback.
...
| Команда | Описание |
|---|---|
| `nerve` | Сервер (`PORT=20128`, API + dashboard) |
| `nerve --port 3000` | Порт 3000 |
| `nerve --mcp` | MCP server (stdio) |
| `nerve --no-open` | Не открывать браузер |
| `nerve --help` | Справка |
...
| Команда | Действие |
|---|---|
| `npm run uninstall` | Убирает app, **сохраняет** `~/.nerve` |
| `npm run uninstall:full` | Удаляет app **и** все ключи/БД |
| `npm uninstall -g nerve` | Глобальный npm uninstall |
...
> 🎬 **Сняли видео про Nerve?** Откройте [issue](https://github.com/diegosouzapw/Nerve/issues/new) или [discussion](https://github.com/diegosouzapw/Nerve/discussions) — добавим в этот раздел.
...
> 💡 «Cost» в дашборде — **tracker экономии**, не счёт Nerve. Nerve вам **не** выставляет счета.
...
| `DATA_DIR` | `~/.nerve` | БД и конфиги |
| `REQUEST_TIMEOUT_MS` | `600000` | Базовый timeout |
| `STREAM_IDLE_TIMEOUT_MS` | inherits | Idle gap SSE |

**Nerve берёт деньги?** Нет — open-source на вашей машине. Платите только платным провайдерам.  
...
- 🌍 **Сайт:** [nerve.online](https://nerve.online)
- 🐙 **GitHub:** [github.com/diegosouzapw/Nerve](https://github.com/diegosouzapw/Nerve)
- 🐛 **Issues:** [сообщить о баге](https://github.com/diegosouzapw/Nerve/issues) (с `npm run system-info`)
...
| [Remote Mode](../../guides/REMOTE-MODE.md) | CLI с ноутбука → Nerve на VPS |
...
**Сделано с ❤️ open-source сообществом · MIT License · [nerve.online](https://nerve.online)**

⭐ Если Nerve помог сэкономить — поставьте star репозиторию.
