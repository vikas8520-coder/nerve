# Nerve MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `nerve_get_health`          | Gateway health, circuit breakers, uptime |
| `nerve_list_combos`         | All configured combos with models        |
| `nerve_get_combo_metrics`   | Performance metrics for a specific combo |
| `nerve_switch_combo`        | Switch active combo by ID/name           |
| `nerve_check_quota`         | Quota status per provider or all         |
| `nerve_route_request`       | Send a chat completion through Nerve |
| `nerve_cost_report`         | Cost analytics for a time period         |
| `nerve_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `nerve_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `nerve_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `nerve_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `nerve_test_combo`             | Live-test all models in a combo via a real upstream request |
| `nerve_get_provider_metrics`   | Detailed metrics for one provider                           |
| `nerve_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `nerve_explain_route`          | Explain a past routing decision                             |
| `nerve_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
