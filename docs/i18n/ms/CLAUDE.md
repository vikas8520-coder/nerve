...
**Nerve** — proksi/router AI yang bersatu. Satu titik akhir, 160+ penyedia LLM, auto-fallback.
...
Nerve mempunyai tiga mekanisme kegagalan sementara yang berkaitan tetapi berbeza. Pastikan skop mereka terpisah semasa menyahpepijat tingkah laku penghalaan. Lihat
...
- **Import**: luaran → dalaman (`@/`, `@nerve/open-sse`) → relatif
...
- **Alias laluan**: `@/*` → `src/`, `@nerve/open-sse` → `open-sse/`, `@nerve/open-sse/*` → `open-sse/*`
- **Port lalai**: 20128 (API + papan pemuka pada port yang sama)
- **Direktori data**: `DATA_DIR` env var, lalai kepada `~/.nerve/`
