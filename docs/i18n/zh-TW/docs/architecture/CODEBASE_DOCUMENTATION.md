...
> **版本：** v3.8.0
> **最後更新：** 2026-06-28
> **讀者對象：** 為 Nerve 貢獻程式碼或在其之上建立整合的工程師。
>
...
├── open-sse/             串流引擎 workspace（@nerve/open-sse）
├── electron/             桌面包裝程式（Electron 41 main + preload）
├── bin/                  CLI 進入點（nerve、reset-password）
...
| `responses.ts`、`nerveResponseMeta.ts` | 回應形狀輔助程式             |
...
獨立的 npm workspace，發布為 `@nerve/open-sse`。負責請求
