# Nerve — Panduan Mencopot Pemasangan (Bahasa Indonesia)
...
Panduan ini menjelaskan cara mencopot pemasangan Nerve dari sistem Anda secara bersih.

---

## Mencopot Pemasangan dengan Cepat (v3.6.2+)

Nerve menyediakan dua skrip bawaan untuk penghapusan yang bersih:
...
Perintah ini menghapus aplikasi Nerve tetapi **mempertahankan** basis data, konfigurasi, kunci API, dan pengaturan penyedia Anda di `~/.nerve/`. Gunakan ini jika Anda berencana memasang ulang nanti dan ingin menyimpan pengaturan yang ada.
...
- Berkas cadangan
- Berkas log
- Semua berkas di direktori `~/.nerve/`
...
- Buka `Settings → Apps → Nerve → Uninstall`
- Atau jalankan uninstaller NSIS dari direktori instalasi

**macOS:**

- Seret `Nerve.app` dari `/Applications` ke Trash
- Hapus data: `rm -rf ~/Library/Application Support/nerve`

**Linux:**

- Hapus berkas AppImage
- Hapus data: `rm -rf ~/.nerve`
...
Nerve menyimpan data di lokasi-lokasi berikut secara default:
