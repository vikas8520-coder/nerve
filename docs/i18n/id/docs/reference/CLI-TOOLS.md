# Panduan Pengaturan Alat CLI — Nerve (Bahasa Indonesia)
...
untuk menggunakan **Nerve** sebagai backend terpadu, memberikan manajemen kunci terpusat,
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Kiro / Cursor / Copilot
           │
           ▼  (semua mengarah ke Nerve)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (Nerve meneruskan ke penyedia yang tepat)
...
| **Antigravity**    | `antigravity` | internal   | mitm            | Nerve        |
...
## Langkah 1 — Dapatkan API Key Nerve

1. Buka dashboard Nerve → **API Manager** (`/dashboard/api-manager`)
...
export ANTHROPIC_API_KEY="sk-your-nerve-key"
...
Atau gunakan dashboard Nerve → **CLI Tools → Cline → Apply Config**.
...
Atau gunakan dashboard Nerve → **CLI Tools → KiloCode → Apply Config**.
...
# CLI ini menggunakan autentikasinya sendiri — Nerve tidak diperlukan sebagai backend untuk Kiro CLI itu sendiri.
# Gunakan kiro-cli bersama Nerve untuk alat lainnya.
kiro-cli status
```
...
> **Catatan:** Cursor merutekan permintaan melalui cloudnya sendiri. Untuk integrasi Nerve,
> aktifkan **Cloud Endpoint** di Pengaturan Nerve dan gunakan URL domain publik Anda.

Melalui GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: kunci Nerve Anda

---

## Konfigurasi Otomatis Dashboard

Dashboard Nerve mengotomatiskan konfigurasi untuk sebagian besar alat:
...
**Droid** dan **OpenClaw** adalah agen AI yang dibangun langsung ke dalam Nerve — tidak perlu instalasi.
Keduanya berjalan sebagai rute internal dan menggunakan perutean model Nerve secara otomatis.
...
| ------------------------- | --------------------------------- | ------------------------------------------ |
| `Connection refused`      | Nerve tidak berjalan          | `pm2 start nerve`                      |
...
```bash
# Instal semua CLI dan konfigurasi untuk Nerve (ganti dengan kunci dan URL server Anda)
NERVE_URL="http://localhost:20128/v1"
NERVE_KEY="sk-your-nerve-key"
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$NERVE_URL\",\"apiKey\":\"$NERVE_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $NERVE_KEY\napiBaseUrl: $NERVE_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$NERVE_URL"
export OPENAI_API_KEY="$NERVE_KEY"
export ANTHROPIC_BASE_URL="$NERVE_URL"
export ANTHROPIC_API_KEY="$NERVE_KEY"
EOF

source ~/.bashrc
echo "✅ All CLIs installed and configured for Nerve"
```
