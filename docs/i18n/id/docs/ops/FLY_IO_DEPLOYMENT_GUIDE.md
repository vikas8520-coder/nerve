# Panduan Deployment Nerve di Fly.io
...
Dokumen ini menjelaskan metode deployment Nerve di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `nerve`.
...
- Alamat akses: `https://nerve.fly.dev/`
...
- `app = 'nerve'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `nerve`
...
Jika Anda sudah mengganti nama aplikasi, ganti `nerve` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `nerve` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `nerve`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a nerve
```
...
- Apakah aplikasi yang sedang dilihat adalah `nerve`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/diegosouzapw/Nerve`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `nerve`
