...
**Problem it solves:** Nerve proxies many API keys against the same upstream
...
- All persistence is in the existing Nerve SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("nerve:quota-share:pools")` is non-empty.
