...
Nerve compression is built around engine contracts. A mode can run one engine directly
...
actually reach `nerve_ccr_retrieve`** (`callerSupportsCcrRetrieve()` in
...
with a `22-87%` range, and a `~46%` input-compression tool. Nerve uses the Caveman input-side
...
Nerve install; when the optional stack is absent, LLMLingua still fail-opens. Only the three
...
- **Global npm (`npm i -g nerve`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `nerve_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `nerve_compression_configure`   | `write:compression` | Update global settings           |
| `nerve_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `nerve_list_compression_combos` | `read:compression`  | List compression combos          |
| `nerve_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
