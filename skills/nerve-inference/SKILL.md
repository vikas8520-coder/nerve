...
Mistral OCR–compatible document OCR endpoint. Accepts a JSON body referencing a document/image and returns extracted text. Success responses carry the `X-Nerve-*` cost-telemetry headers.
...
OpenAI Whisper–compatible audio translation (multipart/form-data). Unlike `/api/v1/audio/transcriptions`, output is always English regardless of the source language. Success responses carry the `X-Nerve-*` cost-telemetry headers.
...
<!-- skill:custom-start -->
<!-- Aggregated from: nerve-chat, nerve-image, nerve-tts, nerve-stt, nerve-embeddings, nerve-web-search, nerve-web-fetch -->
...
- `POST $NERVE_URL/v1/chat/completions` — OpenAI format
- `POST $NERVE_URL/v1/messages` — Anthropic Messages format
- `POST $NERVE_URL/v1/responses` — OpenAI Responses API

### Discover

```bash
curl $NERVE_URL/v1/models | jq '.data[].id'
```
...
```bash
curl -X POST $NERVE_URL/v1/chat/completions \
...
```bash
curl -X POST $NERVE_URL/v1/messages \
  -H "Authorization: Bearer $NERVE_KEY" \
...
- `POST $NERVE_URL/v1/images/generations` — Text-to-image
- `POST $NERVE_URL/v1/images/edits` — Image edit (mask)
- `POST $NERVE_URL/v1/images/variations` — Variations

### Discover

```bash
curl $NERVE_URL/v1/models/image | jq '.data[]'
```
...
```bash
curl -X POST $NERVE_URL/v1/images/generations \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "dall-e-3",
...
- `POST $NERVE_URL/v1/audio/speech` — returns binary audio (mp3/opus/wav/flac)

### Discover

```bash
curl $NERVE_URL/v1/models/tts | jq '.data[]'
```
...
```bash
curl -X POST $NERVE_URL/v1/audio/speech \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tts-1",
    "input": "Hello from Nerve.",
    "voice": "alloy",
    "response_format": "mp3"
  }' --output speech.mp3
```
...
- `POST $NERVE_URL/v1/audio/transcriptions` — multipart upload, returns text
- `POST $NERVE_URL/v1/audio/translations` — transcribe + translate to English

### Discover

```bash
curl $NERVE_URL/v1/models/stt | jq '.data[]'
```

### Example

```bash
curl -X POST $NERVE_URL/v1/audio/transcriptions \
  -H "Authorization: Bearer $NERVE_KEY" \
  -F "file=@audio.mp3" \
  -F "model=whisper-1" \
...
- `POST $NERVE_URL/v1/embeddings`

### Discover

```bash
curl $NERVE_URL/v1/models/embedding | jq '.data[]'
```
...
```bash
curl -X POST $NERVE_URL/v1/embeddings \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
- `POST $NERVE_URL/v1/web/search` — unified search format

### Discover

```bash
curl $NERVE_URL/v1/models/web | jq '.data[] | select(.kind == "webSearch")'
```

### Example

```bash
curl -X POST $NERVE_URL/v1/web/search \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "tavily/search",
    "query": "Nerve github latest release",
    "max_results": 5,
    "include_answer": true
  }'
```
...
- `POST $NERVE_URL/v1/web/fetch`

### Discover

```bash
curl $NERVE_URL/v1/models/web | jq '.data[] | select(.kind == "webFetch")'
```

### Example

```bash
curl -X POST $NERVE_URL/v1/web/fetch \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "jina/reader",
