...
<!-- skill:custom-start -->
<!-- Migrated from skills/nerve-monitoring/SKILL.md (preserved curated content) -->

# Nerve — Monitoring & Health
...
```bash
curl $NERVE_URL/api/monitoring/health \
  -H "Authorization: Bearer $NERVE_KEY"
```
...
```bash
curl $NERVE_URL/api/providers/metrics \
  -H "Authorization: Bearer $NERVE_KEY"
```
...
## Via MCP (if Nerve is your MCP server)

```
nerve_get_health            → full system health snapshot
nerve_get_provider_metrics  → p50/p95/p99 + circuit state per provider
nerve_get_session_snapshot  → cost, tokens, errors for current session
nerve_check_quota           → quota balance + percent remaining + reset time
nerve_db_health_check       → diagnose + auto-repair database drift
```

## Quota check

```bash
curl $NERVE_URL/api/quota \
  -H "Authorization: Bearer $NERVE_KEY"
```
...
```bash
curl -X POST $NERVE_URL/api/budget/guard \
  -H "Authorization: Bearer $NERVE_KEY" \
...
Nerve logs every MCP tool call to `mcp_audit` table. Query via API:

```bash
curl "$NERVE_URL/api/mcp/status" \
  -H "Authorization: Bearer $NERVE_KEY"
```
...
- `503` on health endpoint → Nerve is starting up; retry in 5s
