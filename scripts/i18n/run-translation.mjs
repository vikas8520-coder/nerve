#!/usr/bin/env node
/**
 * Nerve — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   NERVE_TRANSLATION_API_URL     e.g. https://cloud.nerve.dev/v1
 *   NERVE_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   NERVE_TRANSLATION_MODEL       e.g. cx/gpt-5.6-sol
 *   NERVE_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   NERVE_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("NERVE_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("NERVE_TRANSLATION_API_KEY");
  const model = requireEnv("NERVE_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.NERVE_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.NERVE_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.NERVE_TRANSLATION_API_URL || "").replace(/\/$/, "");
