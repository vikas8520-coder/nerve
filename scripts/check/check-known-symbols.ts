 ...
 *
 * Sources:
 *   - MCP_TOOLS (33 base tools: nerve_* + compression + agent_skills)
 *   - memoryTools (3): nerve_memory_*
 *   - skillTools (4): nerve_skills_*
...
export const KNOWN_MCP_TOOL_NAMES: readonly string[] = [
  ...
  "nerve_cache_stats",
  "nerve_cache_flush",
  "nerve_compression_status",
  "nerve_compression_configure",
  "nerve_set_compression_engine",
  "nerve_list_compression_combos",
  "nerve_compression_combo_stats",
  ...
  "nerve_skills_list",
  "nerve_skills_enable",
  "nerve_skills_execute",
  "nerve_skills_executions",
  // gamificationTools (8)
...
async function main(): Promise<void> {
  ...
  const executorsMod = await import("@nerve/open-sse/executors/index.ts");
  ...
  // ── (3) Translator pairs ──────────────────────────────────────────────────
  await import("@nerve/open-sse/translator/bootstrap.ts").then((m) =>
    (m.bootstrapTranslatorRegistry as () => void)()
  );
  const formatsMod = await import("@nerve/open-sse/translator/formats.ts");
  const registryMod = await import("@nerve/open-sse/translator/registry.ts");
  ...
  const { MCP_TOOLS } = await import("@nerve/open-sse/mcp-server/schemas/tools.ts");
  const { memoryTools } = await import("@nerve/open-sse/mcp-server/tools/memoryTools.ts");
  const { skillTools } = await import("@nerve/open-sse/mcp-server/tools/skillTools.ts");
  const { gamificationTools } =
    await import("@nerve/open-sse/mcp-server/tools/gamificationTools.ts");
  const { pluginTools } = await import("@nerve/open-sse/mcp-server/tools/pluginTools.ts");
  const { notionTools } = await import("@nerve/open-sse/mcp-server/tools/notionTools.ts");
  const { obsidianTools } = await import("@nerve/open-sse/mcp-server/tools/obsidianTools.ts");
