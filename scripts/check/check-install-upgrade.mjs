...
function findDb(dataDir) {
  const candidates = ["storage.sqlite", "nerve.sqlite", "data.sqlite"];
...
async function bootAndProbe({ prefix, dataDir, port, expectVersion, label }) {
  const binPath = path.join(prefix, "bin", "nerve");
...
function resolvePreviousVersion(current, explicit) {
  ...
  const out = execFileSync("npm", ["view", "nerve", "dist-tags.latest"], { encoding: "utf8" });
  const latest = out.trim();
  if (!latest) throw new Error("could not resolve nerve@latest from npm");
  if (latest === current) {
    ...
    const all = JSON.parse(execFileSync("npm", ["view", "nerve", "versions", "--json"], { encoding: "utf8" }));
...
async function main() {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-install-upgrade-"));
  const failures = [];
  const warnings = [];
