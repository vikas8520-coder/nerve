...
set -euo pipefail
echo "[verify-published] npm i -g nerve@\${VERIFY_VERSION} (public registry)"
npm install -g "nerve@\${VERIFY_VERSION}"
export DATA_DIR=/tmp/nerve-data JWT_SECRET=verify-published-secret-with-sufficient-length API_KEY_SECRET=verify-published-api-key-secret DISABLE_SQLITE_AUTO_BACKUP=true NERVE_SKIP_SYSTEM_TRUST=1
mkdir -p "\$DATA_DIR"
nerve serve --port "\$VERIFY_PORT" &
node -e '
...
function main() {
  ...
  console.log(`[verify-published] clean-container verify of nerve@${version}…`);
  ...
  console.error(`[verify-published] ❌ FAILED (exit ${r.status}) — consider: npm deprecate nerve@${version} "<reason>"`);
  process.exit(1);
}
