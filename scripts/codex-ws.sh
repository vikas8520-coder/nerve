#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um Nerve LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O Nerve expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O Nerve, no
...
#   NERVE_WS_BASE   base URL do Nerve local   (default abaixo)
#   NERVE_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   NERVE_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
NERVE_WS_BASE="${NERVE_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do Nerve local
NERVE_WS_MODEL="${NERVE_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export NERVE_LOCAL_KEY="${NERVE_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do Nerve.
model = "$NERVE_WS_MODEL"
model_provider = "nerve-local"

[model_providers.nerve-local]
name = "Nerve Local (WS)"
base_url = "$NERVE_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "NERVE_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$NERVE_WS_MODEL" -c model_provider="nerve-local")
