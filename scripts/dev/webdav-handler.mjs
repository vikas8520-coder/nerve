/**
 * WebDAV file server handler for Nerve.
 *
...
// Key derivation: scryptSync(secret, "nerve-field-encryption-v1", 32)
// ─────────────────────────────────────────────────────────────────────────────

const ENC_PREFIX = "enc:v1:";
const STATIC_SALT = "nerve-field-encryption-v1";
const KEY_LENGTH = 32;
...
/**
 * Decrypt a value that may be encrypted with the Nerve enc:v1: format.
...
function getDefaultDataDir() {
  ...
  const legacyDir = path.join(homeDir, ".nerve"); // getLegacyDotDataDir()

  // 2) Preserve the legacy ~/.nerve path if it already exists (avoid data loss).
  try {
  ...
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    return path.join(path.resolve(xdgConfigHome.trim()), "nerve");
  }

  // 5) Default → legacy ~/.nerve (even if it does not exist yet).
  return legacyDir;
}
...
function sendUnauthorized(res) {
  sendError(res, 401, "Unauthorized", {
    "WWW-Authenticate": 'Basic realm="Nerve WebDAV"',
  });
}
...
async function handlePut(req, res, absPath) {
  ...
  const tmpPath = absPath + ".nerve-webdav-tmp-" + Date.now();
  let writeStream;
  try {
