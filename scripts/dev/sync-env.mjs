...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/nerve.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.nerve/.env to survive across upgrades. This prevents credential loss
  // when upgrading Nerve (issue #1622).
  MACHINE_ID_SALT: () => `nerve-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/nerve.mjs and persisted to
 * ~/.nerve/.env to survive across upgrades.
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "nerve");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "nerve");

  return join(homedir(), ".nerve");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.nerve/server.env, ~/.nerve/.env, ` +
