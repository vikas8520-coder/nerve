 ...
 *
 * Phase-3 VPS HTTP scenario driver for Nerve combo routing.
...
// Known approximate input cost ($/M tokens) from Nerve's default-pricing constants.
...
//
// Nerve's sortModelsByCost uses getPricingForModel which merges
...
async function scenarioCostOptimized(healthy) {
  ...
  try {
    ...
    // Both direct match and provider-substring match are accepted since Nerve
...
//   404, Nerve calls recordProviderCooldown(providerA, undefined) — a provider-
...
//   1. SSH to read STORAGE_ENCRYPTION_KEY from /root/.nerve/.env
//   2. Encrypt a wrong API key using scryptSync(key,"nerve-field-encryption-v1",32)+AES-256-GCM
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== Nerve combo-live-vps scenario driver ===");
