 ...
 *   NERVE_ENABLE_LIVE_WS — Set to "0" or "false" to disable
 */
...
const BOOTSTRAPPED_ENV = "NERVE_LIVE_WS_BOOTSTRAPPED";
...
async function main() {
  ...
  // child is re-spawned with NERVE_ENABLE_LIVE_WS="0" purely to stop liveServer.ts
  ...
  if (
    ...
    (process.env.NERVE_ENABLE_LIVE_WS === "0" ||
      process.env.NERVE_ENABLE_LIVE_WS?.toLowerCase() === "false")
  ) {
    console.log("[LiveWS] Disabled via NERVE_ENABLE_LIVE_WS");
    process.exit(0);
  }
