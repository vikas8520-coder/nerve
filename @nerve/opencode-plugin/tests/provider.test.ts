 ...
 *
 * Covers `createNerveProviderHook(opts, deps)`:
...
const FIXTURE: NerveRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: NerveRawModelEntry[]): NerveModelsFetcher & {
  callCount: () => number;
...
test("createNerveProviderHook: default providerId is 'nerve'", () => {
  const hook = createNerveProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-nerve");
});

test("createNerveProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createNerveProviderHook(
    { providerId: "nerve-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createNerveProviderHook(
    { providerId: "nerve-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-nerve-preprod");
  assert.equal(b.id, "opencode-nerve-local");
});
...
test("models: extracts apiKey from ctx.auth (type=api) and calls fetcher with it", async () => {
  ...
  // ("nerve"), not the OC-gate-prefixed hook.id ("opencode-nerve") —
  // that prefix must never leak into anything Nerve's server parses.
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createNerveProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  // nerveProviderId ("nerve") — the OC-gate prefix ("opencode-")
  // must stay OC-internal (hook.id / AuthHook.provider) and never leak into
  // anything Nerve's own server parses for credential lookup.
  const claude = out["nerve/claude-primary"];
  ...
  assert.equal(claude.id, "nerve/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "nerve");
  ...
  // cost is zeroed (Nerve /v1/models has no pricing)
