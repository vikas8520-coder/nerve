 ...
 *
 * Validates that two `NervePlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@nerve/opencode-plugin", {"providerId": "nerve-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@nerve/opencode-plugin", {"providerId": "nerve-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { NervePlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof NervePlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await NervePlugin(fakeInput, {
    providerId: "nerve-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await NervePlugin(fakeInput, {
    providerId: "nerve-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-nerve-prod");
  assert.equal(b.auth?.provider, "opencode-nerve-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await NervePlugin(fakeInput, { ...opts });
  const second = await NervePlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => NervePlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await NervePlugin(fakeInput, {
