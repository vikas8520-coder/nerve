 ...
 *
 * Covers `createNerveFetchInterceptor` (URL-prefix gating, header merge,
...
import { createNerveAuthHook, createNerveFetchInterceptor } from "../src/index.js";
...
test("createNerveFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNerveFetchInterceptor: path-prefixed baseURL scopes auth to its normalized inference paths", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const prefixedBase = "https://or.example.com/tenant-a/v1";
    const f = createNerveFetchInterceptor({
      apiKey: KEY,
    ...
    const suffixingInterceptor = createNerveFetchInterceptor({
      apiKey: KEY,
...
test("createNerveFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createNerveFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createNerveFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNerveFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNerveFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNerveFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNerveFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNerveFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({
      apiKey: KEY,
...
test("createNerveFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNerveFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createNerveAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createNerveAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createNerveAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
