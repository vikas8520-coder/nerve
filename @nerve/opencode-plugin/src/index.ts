...
> & {
  ...
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${NERVE_PROVIDER_KEY}`
      ? "Nerve"
      : `Nerve (${providerId})`);
  const modelCacheTtl =
 ...
 */
export function invalidateNerveFetchCache(
...
export async function forceSyncNerveModels(args: {
  ...
  const compressionMetaFetcher =
    args.compressionMetaFetcher ?? defaultNerveCompressionMetaFetcher;
  ...
  const auth = await resolveNerveRuntimeAuth(
    resolved,
...
export const defaultNerveModelsFetcher: NerveModelsFetcher = async (
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `@nerve/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`
      );
    }
...
export const defaultNerveCombosFetcher: NerveCombosFetcher = async (
  ...
  if (!baseURL)
    throw new Error("@nerve/opencode-plugin: baseURL required to fetch /api/combos");
...
export const defaultNerveAutoCombosFetcher: NerveAutoCombosFetcher = async (
  ...
  try {
    ...
    if (res.status === 404) {
      console.warn(
        `[nerve-plugin] /api/combos/auto not available (404) — auto combos disabled`
      );
      return [];
    }
    if (!res.ok) {
...
export function mapAutoComboToStaticEntry(
  autoCombo: NerveRawAutoCombo
): NerveStaticModelEntry {
...
export function createNerveProviderHook(
  ...
  const compressionMetaFetcher =
    deps.compressionMetaFetcher ?? defaultNerveCompressionMetaFetcher;
      ...
      if (cached && cached.expiresAt > t) {
        ...
        if (wantEnrichment) {
          try {
            ...
            console.warn(
              "[nerve-plugin] enrichment fetch failed, falling back to raw ids",
              err
            );
          }
        }
