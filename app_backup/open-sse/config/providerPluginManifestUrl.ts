export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-Nerve-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.NERVE_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.NERVE_PUBLIC_PROTOCOL || "http";
