/**
 * Nerve MCP Compression Tools — Manage and monitor prompt compression.
 *
 * Tools:
 *   1. nerve_compression_status   — Get compression config, analytics, and cache stats
 *   2. nerve_compression_configure — Update compression settings
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("nerve_compression_status", args, result, duration, true);
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("nerve_compression_configure", args, result, duration, true);
...
export async function handleCcrStoreTool(
  ...
  await logToolCall("nerve_ccr_store", auditInput, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrRetrieveTool(
  ...
  if (!metadata) {
    ...
    await logToolCall("nerve_ccr_retrieve", args, output, Date.now() - start, false, "NOT_FOUND");
    return output;
  }
  if ((!args.mode || args.mode === "full") && metadata.bytes > MAX_CCR_MCP_FULL_BYTES) {
    ...
    await logToolCall("nerve_ccr_retrieve", args, output, Date.now() - start, true);
    return output;
  }
...
export async function handleCcrInspectTool(
  ...
  await logToolCall("nerve_ccr_inspect", args, output, Date.now() - start, Boolean(metadata));
  return output;
}
...
export async function handleCcrListTool(
  ...
  await logToolCall("nerve_ccr_list", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrDeleteTool(
  ...
  await logToolCall("nerve_ccr_delete", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrStatsTool(
  ...
  await logToolCall("nerve_ccr_stats", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleRtkDiscover(
  ...
  await logToolCall("nerve_rtk_discover", args, result, Date.now() - start, true);
  return result;
}
...
export async function handleRtkLearn(
  ...
  await logToolCall("nerve_rtk_learn", args, result, Date.now() - start, true);
  return result;
}
