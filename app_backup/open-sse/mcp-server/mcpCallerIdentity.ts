 ...
 * `extra.authInfo` (Nerve authenticates with API keys, not OAuth client ids —
 ...
 *
 * On stdio transport (`nerve --mcp`) there is no HTTP context, so we fall
 * back to the NERVE_API_KEY / ROUTER_API_KEY environment variable.
 *
 ...
 *
 * Falls back to NERVE_API_KEY / ROUTER_API_KEY env vars for stdio transport
...
/**
 * Resolve the principal id from the NERVE_API_KEY (or ROUTER_API_KEY) env var.
...
async function resolvePrincipalFromEnv(): Promise<string | undefined> {
  const rawKey = process.env.NERVE_API_KEY || process.env.ROUTER_API_KEY;
  if (!rawKey) return undefined;
  try {
