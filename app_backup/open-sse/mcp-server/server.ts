...
import { resolveNerveBaseUrl } from "../../src/shared/utils/resolveNerveBaseUrl.ts";
import { sanitizeErrorMessage } from "../utils/error.ts";
import { getMcpModelsCatalog } from "./catalog.ts";
export { getMcpModelsCatalog } from "./catalog.ts";

const NERVE_BASE_URL = resolveNerveBaseUrl();
const MCP_ENFORCE_SCOPES = process.env.NERVE_MCP_ENFORCE_SCOPES === "true";
const MCP_ALLOWED_SCOPES = new Set(
  (process.env.NERVE_MCP_SCOPES || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
);
...
function getNerveApiKey(): string {
  return process.env.NERVE_API_KEY || "";
}

export async function nerveFetchFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  const url = `${NERVE_BASE_URL}${path}`;
  const apiKey = getNerveApiKey();
  ...
  if (!response.ok) {
    ...
    throw new Error(`Nerve API error [${response.status}]: ${errorText}`);
  }
...
async function handleGetHealth() {
  ...
  try {
    const [healthRaw, resilienceRaw, rateLimitsRaw] = await Promise.allSettled([
      nerveFetchFetch("/api/monitoring/health"),
      nerveFetchFetch("/api/resilience"),
      nerveFetchFetch("/api/rate-limits"),
    ]);
    ...
    await logToolCall("nerve_get_health", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_get_health", {}, null, Date.now() - start, false, msg);
...
async function handleListCombos(args: { includeMetrics?: boolean }) {
  const start = Date.now();
  try {
    const combosRaw = await nerveFetchFetch("/api/combos");
    ...
    if (args.includeMetrics) {
      metrics = toRecord(await nerveFetchFetch("/api/combos/metrics").catch(() => ({})));
    }
    ...
    await logToolCall("nerve_list_combos", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_list_combos", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleGetComboMetrics(args: { comboId: string }) {
  const start = Date.now();
  try {
    const result = await nerveFetchFetch(
      `/api/combos/metrics?comboId=${encodeURIComponent(args.comboId)}`
    );
    await logToolCall("nerve_get_combo_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_get_combo_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleSwitchCombo(args: { comboId: string; active: boolean }) {
  const start = Date.now();
  try {
    const result = await nerveFetchFetch(`/api/combos/${encodeURIComponent(args.comboId)}`, {
      method: "PUT",
      body: JSON.stringify({ isActive: args.active }),
    });
    await logToolCall("nerve_switch_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_switch_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCheckQuota(args: { provider?: string; connectionId?: string }) {
  ...
  try {
    ...
    const result = normalizeQuotaResponse(await nerveFetchFetch(path), {
      provider: args.provider || null,
      connectionId: args.connectionId || null,
    });

    await logToolCall("nerve_check_quota", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_check_quota", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleRouteRequest(args: {
  ...
  try {
    ...
    const raw = (await nerveFetchFetch("/v1/chat/completions", {
      method: "POST",
...
async function handleCostReport(args: { period?: string }) {
  ...
  try {
    ...
    const raw = toRecord(
      await nerveFetchFetch(`/api/usage/analytics?range=${encodeURIComponent(range)}`)
    );
    ...
    await logToolCall("nerve_cost_report", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_cost_report", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleListModelsCatalog(args: { provider?: string; capability?: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_list_models_catalog", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebSearch(args: {
  ...
  try {
    ...
    const result = await nerveFetchFetch("/v1/search", {
      method: "POST",
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(60000),
    });
    await logToolCall("nerve_web_search", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_web_search", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebFetch(args: {
  ...
  try {
    ...
    const result = await nerveFetchFetch("/v1/web/fetch", {
      method: "POST",
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(60000),
    });
    await logToolCall("nerve_web_fetch", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("nerve_web_fetch", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Returns Nerve health status including uptime, memory, circuit breakers, rate limits, and cache stats",
      inputSchema: getHealthInput,
    },
    withScopeEnforcement("nerve_get_health", async (args) => {
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_switch_combo", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_check_quota", (args) =>
  ...
  server.registerTool(
    "nerve_route_request",
    {
      description: "Sends a chat completion request through Nerve intelligent routing",
      inputSchema: routeRequestInput,
    },
    withScopeEnforcement("nerve_route_request", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_cost_report", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_set_resilience_profile", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_test_combo", (args) => handleTestCombo(testComboInput.parse(args)))
  );

  server.registerTool(
    "nerve_get_provider_metrics",
    {
      description:
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_get_session_snapshot", async (args) => {
  ...
  server.registerTool(
    "nerve_db_health_check",
    {
      description:
        "Diagnoses or repairs Nerve database drift, including broken combo references and orphan quota/domain rows",
      inputSchema: dbHealthCheckInput,
    },
    withScopeEnforcement("nerve_db_health_check", (args) =>
  ...
  server.registerTool(
    ...
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into Nerve without overwriting user-set prices",
      inputSchema: syncPricingInput,
    },
    withScopeEnforcement("nerve_sync_pricing", (args) =>
  ...
  server.registerTool(
    ...
    {
      description:
        "Performs a web search using Nerve's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
      inputSchema: webSearchInput,
    },
    withScopeEnforcement("nerve_web_search", (args) => handleWebSearch(webSearchInput.parse(args)))
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Fetches and extracts content from a URL using Nerve's web fetch gateway. Supports multiple providers (Firecrawl, Jina Reader, Tavily) with automatic failover. Returns the page content as markdown, HTML, links, or screenshot, along with metadata.",
      inputSchema: webFetchInput,
    },
    withScopeEnforcement("nerve_web_fetch", (args) => handleWebFetch(webFetchInput.parse(args)))
  );
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_cache_stats", () => handleCacheStats())
  );
  ...
  server.registerTool(
    ...
    withScopeEnforcement("nerve_cache_flush", (args) =>
      handleCacheFlush(cacheFlushInput.parse(args))
    )
  );

  server.registerTool(
    "nerve_oneproxy_fetch",
    {
      description:
    ...
    withScopeEnforcement("nerve_oneproxy_fetch", (args) =>
      handleOneproxyFetch(oneproxyFetchInput.parse(args))
    )
  );

  server.registerTool(
    "nerve_oneproxy_rotate",
    {
      description:
    ...
    withScopeEnforcement("nerve_oneproxy_rotate", (args) =>
      handleOneproxyRotate(oneproxyRotateInput.parse(args))
    )
  );

  server.registerTool(
    "nerve_oneproxy_stats",
    {
      description:
    ...
    withScopeEnforcement("nerve_oneproxy_stats", (args) =>
...
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] Nerve MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] Nerve MCP Server connected and ready.");
  } finally {
    if (closeAuditDb()) {
