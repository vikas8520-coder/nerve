...
const FALLBACK_KIE_CALLBACK_URL = "https://nerve.local/api/kie/callback";
...
function getConfiguredKieCallbackUrl(): string {
  const explicit =
    process.env.KIE_CALLBACK_URL?.trim() || process.env.NERVE_KIE_CALLBACK_URL?.trim();
  if (explicit) return explicit;

  return (
    callbackUrlFromBaseUrl(process.env.NERVE_PUBLIC_URL) ||
