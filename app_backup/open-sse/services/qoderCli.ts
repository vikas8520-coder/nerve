...
export function getQoderCliWorkspace(): string {
  const explicit = String(
    process.env.QODER_CLI_WORKSPACE || process.env.NERVE_QODER_WORKSPACE || ""
  ).trim();
...
/**
 * Isolated `--config-dir` for Nerve-driven qodercli runs. Keeping it separate
...
export function getQoderCliConfigDir(): string {
  ...
  const base = dataDir || path.join(os.homedir() || os.tmpdir(), ".nerve");
...
export async function runQoderCli(options: QoderCliRunOptions): Promise<QoderCliRunResult> {
  ...
  const args = [
    ...
    "--model",
    level,
    // Disable all built-in tools — Nerve only wants a plain LM reply, never
...
/**
 * Resolve an Nerve model id to the exact value to pass to `qodercli -m`.
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an Nerve OpenAI-compatible request through the Qoder CLI transport.",
...
// Nerve mis-surfaced as "PAT may not be valid for the chat API". We mirror the
...
export async function validateQoderCliPat({
  ...
  if (run.timedOut) {
    return {
      valid: false,
      error:
        "qodercli timed out while validating the token. Check network/proxy access from the Nerve host.",
      unsupported: false,
    };
  }
