 ...
 * with the connection's OAuth access token, falling back to Nerve's own
...
export async function getXaiOauthUsage(
  ...
  try {
    ...
    const used =
      ...
      0;
    return {
      plan: "xAI OAuth (Grok) · Nerve-tracked",
      message: "Live weekly quota unavailable; showing Nerve-routed token totals only.",
      quotas: {
        monthly: {
          used,
          total: 0,
