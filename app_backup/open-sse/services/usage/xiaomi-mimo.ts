 ...
 * cannot reach the `tokenPlan/usage` endpoint), so Nerve self-tracks the
 ...
 * API to call. Instead we count the tokens Nerve itself routed to this
 ...
 * through Nerve, not the provider's own dashboard figure.
 */
export async function getXiaomiMimoUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "Xiaomi MiMo Token Plan (Nerve-tracked)",
      quotas: {
