 ...
 * synthetic `nerve_web_fetch` function tool; the actual `/v1/web/fetch` call
...
export const NERVE_WEB_FETCH_FALLBACK_TOOL_NAME = "nerve_web_fetch";
...
function buildFallbackTool(targetFormat?: string | null): JsonRecord {
  const name = NERVE_WEB_FETCH_FALLBACK_TOOL_NAME;
  const description = [
...
export function prepareWebFetchFallbackBody<T extends WebFetchFallbackBody>(
  ...
  if (!toolNames.has(NERVE_WEB_FETCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebFetchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: NERVE_WEB_FETCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: NERVE_WEB_FETCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: NERVE_WEB_FETCH_FALLBACK_TOOL_NAME,
