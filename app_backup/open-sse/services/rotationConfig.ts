 ...
 *
 * Nerve's account-fallback engine (accountFallback.ts) historically rotated accounts using
 ...
 *   - master enable                            NERVE_ROTATION_ENABLED               (default true)
 *   - rate-limit reset/cooldown seconds        NERVE_ROTATION_RATE_LIMIT_RESET_SECONDS (0 => engine default)
 *   - per-status fallback enable               NERVE_ROTATE_ON_{429,500,502,400}    (429/500/502 default true, 400 default false)
 *   - per-status threshold (errors in window)  NERVE_ROTATE_{status}_THRESHOLD      (default 1 => immediate, current behavior)
 *   - per-status window seconds                NERVE_ROTATE_{status}_WINDOW_SECONDS (default 120)
 *
...
const GLOBAL_KEY = "__nerve_rotation_config__";
...
function buildFromEnv(): RotationConfig {
  return {
    enabled: envBool("NERVE_ROTATION_ENABLED", true),
    rateLimitResetMs: envInt("NERVE_ROTATION_RATE_LIMIT_RESET_SECONDS", 0, 0) * 1000,
    disableTagWithoutReset: envBool("NERVE_ROTATION_DISABLE_TAG_WITHOUT_RESET", true),
    rateLimit429: buildClass(
      "NERVE_ROTATE_ON_429",
      "NERVE_ROTATE_429_THRESHOLD",
      "NERVE_ROTATE_429_WINDOW_SECONDS",
      true
    ),
    serverError500: buildClass(
      "NERVE_ROTATE_ON_500",
      "NERVE_ROTATE_500_THRESHOLD",
      "NERVE_ROTATE_500_WINDOW_SECONDS",
      true
    ),
    badGateway502: buildClass(
      "NERVE_ROTATE_ON_502",
      "NERVE_ROTATE_502_THRESHOLD",
      "NERVE_ROTATE_502_WINDOW_SECONDS",
      true
    ),
    badRequest400: buildClass(
      "NERVE_ROTATE_ON_400",
      "NERVE_ROTATE_400_THRESHOLD",
      "NERVE_ROTATE_400_WINDOW_SECONDS",
      false
    ),
  };
}
...
const COUNTER_KEY = "__nerve_rotation_counters__";
