...
function getModuleDir(): string {
  ...
  return path.join(os.homedir(), ".nerve");
}
...
function getDataDir(): string {
  return process.env.DATA_DIR || path.join(os.homedir(), ".nerve");
}
...
function projectFiltersTrusted(
  ...
  if (process.env.NERVE_RTK_TRUST_PROJECT_FILTERS === "1") return true;
...
function collectProjectFilterSources(sources: FilterSource[], options: RtkFilterLoadOptions): void {
  const projectCandidates = [
    ...
    { path: path.join(process.cwd(), ".rtk", "filters.json"), format: "nerve-json" as const },
  ];
...
function collectGlobalFilterSources(sources: FilterSource[]): void {
  const globalCandidates = [
    ...
    { path: path.join(getDataDir(), "rtk", "filters.json"), format: "nerve-json" as const },
  ];
...
export function loadRtkFilters(options: RtkFilterLoadOptions = {}): RtkFilterDefinition[] {
  const currentCacheKey = [
    ...
    process.env.NERVE_RTK_TRUST_PROJECT_FILTERS === "1" ? "env-trust" : "env-normal",
  ].join("|");
  ...
  const formatRank = { "rtk-toml-v1": 2, "nerve-json": 1 } as const;
  const sorted = filters.sort(
    (a, b) =>
      sourceRank[b.source ?? "builtin"] - sourceRank[a.source ?? "builtin"] ||
      formatRank[b.sourceFormat ?? "nerve-json"] - formatRank[a.sourceFormat ?? "nerve-json"] ||
