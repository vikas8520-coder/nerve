...
import { wasRefreshTokenRotated } from "@nerve/open-sse/services/refreshSerializer.ts";
...
export async function handleChatCore({
  ...
  const traceEnabled = process.env.NERVE_TRACE === "true" || process.env.DEBUG === "true";
  ...
  if (webSearchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to Nerve fallback for ${provider}`
    );
  }
  ...
  if (webFetchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webFetchFallbackPlan.convertedToolCount} web_fetch tool(s) to Nerve fallback for ${provider}`
    );
  }
  ...
  // (#5312 / #5245): `x-nerve-thinking-marker: off` suppresses it for
  ...
  // They are accepted for compatibility at the Nerve API boundary only.
  if (body && typeof body === "object") {
  ...
  const reasoningRouteDecision =
    body && typeof body === "object"
      ? (body as Record<string, unknown>)._nerveReasoningRouteTrace
      : null;
  if (reasoningRouteDecision) {
    reqLogger.logRouteDecision(reasoningRouteDecision);
    body = { ...(body as Record<string, unknown>) };
    delete (body as Record<string, unknown>)._nerveReasoningRouteTrace;
  }
  ...
  // `x-nerve-no-memory: true` to skip memory+skills injection (a null owner
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    try {
      ...
      if (compressionHeader) {
        log?.debug?.("COMPRESSION", `x-nerve-compression header: ${compressionHeader}`);
      }
  ...
  // rejects unknown/extra params with a strict "400 Param Incorrect". Map Nerve's
  ...
  if (!stream) {
    ...
    if (clientResponseFormat === FORMATS.OPENAI_RESPONSES) {
      ...
      // Port of decolua/9router#517: opt-in `x-nerve-strip-reasoning` header
    ...
    // #6426: align response body `model` with the `X-Nerve-Model` header
  ...
  if (needsResponsesTranslation) {
    ...
    transformStream = createSSETransformStreamWithLogger(
      ...
      // (e.g. OpenCode by UA; any client via `x-nerve-thinking-marker: off`);
