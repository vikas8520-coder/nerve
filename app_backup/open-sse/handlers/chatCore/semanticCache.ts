  ...
  if (semanticCacheEnabled && isCacheableForRead(body, clientRawRequest?.headers)) {
    ...
    if (cached) {
      ...
      const headers: Record<string, string> = {
        ...
        [NERVE_RESPONSE_HEADERS.cache]: "HIT",
      };
      // A cache HIT serves WITHOUT an upstream call, so the incremental cost billed to
      // the client is 0 (consumers that sum X-Nerve-Response-Cost must not charge for
      // hits). The original/would-have-been cost is surfaced via X-Nerve-Cost-Saved.
