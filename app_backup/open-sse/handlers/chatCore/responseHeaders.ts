import { attachNerveMetaHeaders, buildNerveResponseMetaHeaders } from "@/domain/nerveResponseMeta";
import { NERVE_RESPONSE_HEADERS } from "@/shared/constants/headers";
import { defaultLogger } from "@nerve/open-sse/utils/logger";
 ...
 * This budget includes each header name, separator, value, and trailing CRLF. Nerve's own
...
function isNerveInternalHeader(headerName: string): boolean {
  return headerName.toLowerCase().startsWith("x-nerve-");
}
 ...
 *
 * If Nerve re-emits those headers from an App Router route handler, Next
...
export function buildStreamingResponseHeaders(
  ...
  providerHeaders.forEach((value, key) => {
    ...
    if (
      ...
      isNerveInternalHeader(normalized)
    ) {
      return;
    }
    candidates.push({
  ...
  const responseHeaders: Record<string, string> = {
    ...
    [NERVE_RESPONSE_HEADERS.cache]: "MISS",
  };
  attachNerveMetaHeaders(responseHeaders, meta);
