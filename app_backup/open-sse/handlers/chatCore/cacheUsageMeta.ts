 ...
 * shapes), and attach an `_nerve` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _nerve: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._nerve && typeof payload._nerve === "object" && !Array.isArray(payload._nerve)
      ? payload._nerve
      : {};
  return {
    ...payload,
