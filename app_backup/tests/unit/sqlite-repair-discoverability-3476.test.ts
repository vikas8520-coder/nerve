...
// ─── #3476: discoverability of `nerve runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, Nerve prints
...
// A self-heal command already exists (`nerve runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `nerve runtime repair` as a recovery hint"
  );
});
