...
test("ipFilter: tempBanIP blocks then expires", async () => {
  const { configureIPFilter, checkIP, tempBanIP, removeTempBan, resetIPFilter } =
...
test("ipFilter: blacklist blocks listed IPs", async () => {
  const { configureIPFilter, checkIP, addToBlacklist, resetIPFilter } =
    await import("../../open-sse/services/ipFilter.ts");
...
test("ipFilter: whitelist mode only allows listed IPs", async () => {
  const { configureIPFilter, checkIP, addToWhitelist, resetIPFilter } =
    await import("../../open-sse/services/ipFilter.ts");
...
test("ipFilter: getIPFilterConfig reflects current state", async () => {
  const { configureIPFilter, tempBanIP, getIPFilterConfig, resetIPFilter } =
    await import("../../open-sse/services/ipFilter.ts");
