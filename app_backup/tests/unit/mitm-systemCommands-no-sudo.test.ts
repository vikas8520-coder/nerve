 ...
 *
 * Nerve funnels every privileged MITM command through the single choke
 ...
 * feature adds a third opt-out trigger — the `NERVE_NO_SUDO` env flag — so
 * Nerve starts cleanly in a root-less / user-namespace deployment where
...
test("resolveSudoSpawn: with NERVE_NO_SUDO=1, strips `sudo -S` and needs no password", () => {
  const prev = process.env.NERVE_NO_SUDO;
  process.env.NERVE_NO_SUDO = "1";
  try {
    ...
    if (prev === undefined) delete process.env.NERVE_NO_SUDO;
    else process.env.NERVE_NO_SUDO = prev;
  }
});

test("resolveSudoSpawn: with the flag OFF + non-root + sudo available, preserves `sudo -S`", () => {
  const prev = process.env.NERVE_NO_SUDO;
  delete process.env.NERVE_NO_SUDO;
  try {
    const { finalCommand, finalArgs, stripSudo, needsPassword } = resolveSudoSpawn(
      "sudo",
      ["-S", "cp", "/tmp/ca.pem", "/usr/local/share/ca-certificates/ca.crt"],
...
test("resolveSudoSpawn: a non-sudo command is never touched, regardless of the flag", () => {
  ...
  try {
    ...
    assert.deepEqual(finalArgs, ["-A", "-n", "Nerve"]);
  } finally {
    if (prev === undefined) delete process.env.NERVE_NO_SUDO;
    else process.env.NERVE_NO_SUDO = prev;
  }
...
test("isNoSudoEnv: truthiness matrix matches isTruthyEnvFlag semantics", () => {
  ...
  try {
    for (const [value, expected] of cases) {
      if (value === undefined) delete process.env.NERVE_NO_SUDO;
      else process.env.NERVE_NO_SUDO = value;
      assert.equal(isNoSudoEnv(), expected, `NERVE_NO_SUDO=${JSON.stringify(value)}`);
    }
  } finally {
    if (prev === undefined) delete process.env.NERVE_NO_SUDO;
    else process.env.NERVE_NO_SUDO = prev;
