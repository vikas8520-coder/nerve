...
const { hasStreamReadinessSignal } = await import("@nerve/open-sse/utils/streamReadiness");
const { buildErrorBody } = await import("@nerve/open-sse/utils/error");
const { AuggieExecutor } = await import("@nerve/open-sse/executors/auggie");
...
test("hasStreamReadinessSignal wrongly reports readiness for a choices-less mid-stream error frame", () => {
  ...
  assert.equal(
    "choices" in errorBody,
    false,
    "sanity check: buildErrorBody() output really has no choices key"
  );
