...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "nerve-autostart-linux-"));
...
test("resolveCliPath finds nerve.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "nerve.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "nerve.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*nerve\.mjs.*serve --no-open/m);
