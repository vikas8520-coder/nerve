 ...
 * `Cannot find package '@/shared'` when Nerve is installed globally
 * (issue #7791).
 *
...
describe("aliasResolver.resolveAlias — @nerve/open-sse aliases", () => {
  it("exposes ALIAS_MAP with three entries matching tsconfig paths", () => {
    ...
    // @nerve/open-sse/ (subpath)
    assert.equal(ALIAS_MAP[1].prefix, "@nerve/open-sse/");
    assert.equal(ALIAS_MAP[1].target, "open-sse");
    assert.equal(ALIAS_MAP[1].exact, false);
    // @nerve/open-sse (exact package name)
    assert.equal(ALIAS_MAP[2].prefix, "@nerve/open-sse");
    assert.equal(ALIAS_MAP[2].target, "open-sse");
    assert.equal(ALIAS_MAP[2].exact, true);
  });

  it("resolves @nerve/open-sse (bare) to open-sse/index.ts", () => {
    const got = resolveAlias("@nerve/open-sse", REPO_ROOT);
    assert.ok(got, "expected non-null URL for @nerve/open-sse");
    assert.ok(got.startsWith("file://"), "must be a file URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("resolves @nerve/open-sse/services/usage to open-sse/services/usage.ts", () => {
    const got = resolveAlias("@nerve/open-sse/services/usage", REPO_ROOT);
  ...
  it("resolves @nerve/open-sse/utils/proxyFetch to open-sse/utils/proxyFetch.ts", () => {
    const got = resolveAlias("@nerve/open-sse/utils/proxyFetch", REPO_ROOT);
    assert.ok(got, "expected non-null URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("returns null for non-existent @nerve/open-sse/* paths", () => {
    assert.equal(resolveAlias("@nerve/open-sse/does/not/exist", REPO_ROOT), null);
  });

  it("returns null for @nerve/other (unmatched scope)", () => {
    assert.equal(resolveAlias("@nerve/other", REPO_ROOT), null);
    assert.equal(resolveAlias("@nerve/other/pkg", REPO_ROOT), null);
  });

  it("rejects path-traversal via @nerve/open-sse/../../etc/passwd", () => {
    assert.equal(resolveAlias("@nerve/open-sse/../../etc/passwd", REPO_ROOT), null);
  });
});
...
describe("aliasResolver end-to-end (#7791 regression)", () => {
  ...
  it("resolves @/shared/network/outboundUrlGuard from a child process", () => {
    ...
    // bin/nerve.mjs does (await import("tsx/esm") first, then register).
    const script = `
