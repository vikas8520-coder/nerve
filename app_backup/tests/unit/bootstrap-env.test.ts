...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".nerve") });
  } finally {
...
test("bootstrapEnv prefers ~/.nerve/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
