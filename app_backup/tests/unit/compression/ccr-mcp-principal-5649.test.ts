 ...
 * `nerve_ccr_retrieve` tool used to resolve the caller via `extra.authInfo.clientId`
...
test("#7883 resolveMcpCallerApiKeyId env-var fallback executes without throwing", async () => {
  ...
  try {
    ...
    if (prev) process.env.NERVE_API_KEY = prev;
    else delete process.env.NERVE_API_KEY;
  }
});
