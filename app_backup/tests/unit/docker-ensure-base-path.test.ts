...
function makeStandaloneRoot(basePath = "") {
  const appRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-docker-base-"));
  ...
  fs.writeFileSync(path.join(appRoot, "BUILD_NERVE_BASE_PATH"), `${basePath}\n`);
  return appRoot;
}

test("ensureDockerBasePath is a no-op when runtime matches the baked sentinel", () => {
  const appRoot = makeStandaloneRoot("");
  const result = ensureDockerBasePath({ appRoot, env: { NERVE_BASE_PATH: "" } });
  assert.equal(result.action, "noop");
});

test("ensureDockerBasePath patches a root image when a subpath is configured", () => {
  const appRoot = makeStandaloneRoot("");
  const result = ensureDockerBasePath({ appRoot, env: { NERVE_BASE_PATH: "/nerve" } });
  assert.equal(result.action, "patched");
  assert.equal(fs.readFileSync(path.join(appRoot, "BUILD_NERVE_BASE_PATH"), "utf8"), "/nerve\n");
  const manifest = JSON.parse(
    fs.readFileSync(path.join(appRoot, ".build", "next", "routes-manifest.json"), "utf8")
  );
  assert.equal(manifest.basePath, "/nerve");
});

test("ensureDockerBasePath rejects unset runtime on subpath images", () => {
  const appRoot = makeStandaloneRoot("/nerve");
  assert.throws(() => ensureDockerBasePath({ appRoot, env: {} }), /built for subpath/);
});
