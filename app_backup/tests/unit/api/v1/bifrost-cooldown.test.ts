...
test("bifrost cooldown defaults to a short retry suppression window", () => {
  ...
  assert.equal(getBifrostFailureCooldownMs({ NERVE_BIFROST_FAILURE_COOLDOWN_MS: "250" }), 250);
  assert.equal(getBifrostFailureCooldownMs({ NERVE_BIFROST_FAILURE_COOLDOWN_MS: "bad" }), 5000);
});
