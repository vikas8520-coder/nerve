...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-token-healthcheck-"));
...
test("checkConnection skips providers listed in NERVE_HEALTHCHECK_SKIP_PROVIDERS (#kimi-15)", async () => {
  await resetStorage();

  const providerId = "custom-oauth-skip-list";
  const refreshRequests: string[] = [];
  const prevSkip = process.env.NERVE_HEALTHCHECK_SKIP_PROVIDERS;
    ...
    async (tokenServer) => {
      await withPatchedProvider(
        ...
        async () => {
          ...
          process.env.NERVE_HEALTHCHECK_SKIP_PROVIDERS = `foo, ${providerId} ,bar`;
          await tokenHealthCheck.checkConnection(connection);
          assert.equal(
          ...
          process.env.NERVE_HEALTHCHECK_SKIP_PROVIDERS = "some-other-provider";
  ...
  if (prevSkip === undefined) delete process.env.NERVE_HEALTHCHECK_SKIP_PROVIDERS;
  else process.env.NERVE_HEALTHCHECK_SKIP_PROVIDERS = prevSkip;
});
