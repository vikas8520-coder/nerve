...
test("runMigrations applies pending files sequentially in version order", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(db.prepare("SELECT version FROM _nerve_migrations ORDER BY version").all(), [
      { version: "001" },
      { version: "002" },
      { version: "010" },
    ]);
...
test("runMigrations skips versions that are already tracked as applied", serial, async () => {
  ...
  try {
    ...
    assert.equal(
      (
        db
          .prepare("SELECT COUNT(*) AS count FROM _nerve_migrations WHERE version = ?")
          .get("001") as any
      ).count,
      1
    );
...
test(
  ...
  async () => {
    ...
    `);
      ...
      assert.deepEqual(
        db.prepare("SELECT version, name FROM _nerve_migrations WHERE version = ?").get("032"),
...
test("getMigrationStatus reports applied and pending migrations", serial, async () => {
  ...
  try {
    db.exec(`
      CREATE TABLE _nerve_migrations (
        version TEXT PRIMARY KEY,
        name TEXT NOT NULL,
        applied_at TEXT NOT NULL DEFAULT (datetime('now'))
      );
    `);
    db.prepare("INSERT INTO _nerve_migrations (version, name) VALUES (?, ?)").run("001", "first");
...
test("invalid file names are ignored while valid migrations still run", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version, name FROM _nerve_migrations ORDER BY version").all(),
...
test(
  ...
  async () => {
    ...
    `);
      db.prepare("INSERT INTO _nerve_migrations (version, name) VALUES (?, ?)").run("999", "ghost");
...
test(
  ...
  async () => {
    ...
    `);
      db.prepare("INSERT INTO _nerve_migrations (version, name) VALUES (?, ?)").run(
        "021",
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        const row028 = db
          .prepare("SELECT version, name FROM _nerve_migrations WHERE version = ?")
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        db.prepare("SELECT name FROM _nerve_migrations WHERE version = ?").get("041")?.name,
        "compression_receipts"
      );
      assert.equal(
        db.prepare("SELECT name FROM _nerve_migrations WHERE version = ?").get("050")?.name,
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(
          db.prepare("SELECT name FROM _nerve_migrations WHERE version = ?").get("056")?.name,
          "mcp_accessibility_compression"
        );
        assert.equal(
          db.prepare("SELECT name FROM _nerve_migrations WHERE version = ?").get("059")?.name,
          "manifest_routing"
        );
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(
          db.prepare("SELECT name FROM _nerve_migrations WHERE version = ?").get("051")?.name,
          "hot_path_db_indexes"
        );
        assert.equal(
          db.prepare("SELECT name FROM _nerve_migrations WHERE version = ?").get("054")?.name,
          "usage_history_service_tier"
        );
  ...
  async () => {
    ...
    try {
      ...
      for (const [v, n] of oldMigrations) {
        db.prepare("INSERT INTO _nerve_migrations (version, name) VALUES (?, ?)").run(v, n);
      }
      ...
      try {
        ...
        const row034 = db
          .prepare("SELECT name FROM _nerve_migrations WHERE version = ?")
...
// ── #3416: NERVE_MAX_PENDING_MIGRATIONS env override ─────────────────────
...
test(
  "runMigrations aborts when NERVE_MAX_PENDING_MIGRATIONS lowers the threshold (#3416)",
  serial,
  async () => {
    const runner = await importFresh("src/lib/db/migrationRunner.ts");
    const db = createDb();
    const original = process.env.NERVE_MAX_PENDING_MIGRATIONS;

    try {
      seedExistingDbWithoutPhysicalBaseline(db);
      process.env.NERVE_MAX_PENDING_MIGRATIONS = "5";
      ...
      if (original === undefined) delete process.env.NERVE_MAX_PENDING_MIGRATIONS;
      else process.env.NERVE_MAX_PENDING_MIGRATIONS = original;
      db.close();
    }
  }
);
...
test(
  "runMigrations allows a large pending set when NERVE_MAX_PENDING_MIGRATIONS raises the threshold (#3416)",
  serial,
  async () => {
    const runner = await importFresh("src/lib/db/migrationRunner.ts");
    const db = createDb();
    ...
    try {
      seedExistingDbWithoutPhysicalBaseline(db);
      process.env.NERVE_MAX_PENDING_MIGRATIONS = "500";
...
test(
  "runMigrations keeps the default 50 threshold when NERVE_MAX_PENDING_MIGRATIONS is unset or invalid (#3416)",
  serial,
  async () => {
    const runner = await importFresh("src/lib/db/migrationRunner.ts");
    const original = process.env.NERVE_MAX_PENDING_MIGRATIONS;
    ...
    try {
      ...
      delete process.env.NERVE_MAX_PENDING_MIGRATIONS;
      ...
      process.env.NERVE_MAX_PENDING_MIGRATIONS = "abc";
