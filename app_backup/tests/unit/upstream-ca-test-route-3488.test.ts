...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-ca-datadir-"));
...
// A throwaway self-signed cert (CN=Nerve Test CA), valid to 2036.
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-ca-test-"));
...
test("#3488 valid PEM cert → 200 ok with subject", async () => {
  ...
  assert.match(json.subject, /Nerve Test CA/);
});
