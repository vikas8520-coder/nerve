...
test("6A.8: discoverManifests finds root and workspace package.json files", () => {
  ...
  assert.ok(
    manifests.includes("@nerve/opencode-plugin/package.json"),
    "@nerve/opencode-plugin/package.json must be included"
  );
  assert.ok(
    manifests.includes("@nerve/opencode-provider/package.json"),
    "@nerve/opencode-provider/package.json must be included"
  );
});
