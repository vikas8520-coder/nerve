...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-codex-stream-false-"));
...
test("non-stream chat success carries cost-telemetry meta headers (cost/version/tokens)", async () => {
  ...
  // through attachNerveMetaHeaders, which must always emit the cost-telemetry
  ...
  const cost = headers.get(NERVE_RESPONSE_HEADERS.responseCost);
  ...
  const version = headers.get(NERVE_RESPONSE_HEADERS.version);
  assert.equal(typeof version, "string");
  assert.ok(String(version).length > 0);

  // Real usage from the upstream body must surface as token-count headers.
  assert.equal(headers.get(NERVE_RESPONSE_HEADERS.tokensIn), "10");
  assert.equal(headers.get(NERVE_RESPONSE_HEADERS.tokensOut), "5");
});
