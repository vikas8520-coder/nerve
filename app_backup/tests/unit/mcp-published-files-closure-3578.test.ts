...
// #3578 — `nerve --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  else if (spec.startsWith("@nerve/open-sse/"))
    base = path.join("open-sse", spec.slice("@nerve/open-sse/".length));
  else if (spec === "@nerve/open-sse") base = path.join("open-sse", "index");
