...
// NERVE_BUILD_BACKEND_ONLY=1 so `npm run build:cli`'s fallback full build
...
for (const { file, jobName, stepName } of TARGETS) {
  test(`${file} :: ${jobName} '${stepName}' step sets NERVE_BUILD_BACKEND_ONLY=1 (skips dashboard UI build the API-only smoke job never exercises)`, () => {
    ...
    assert.equal(
      isBackendOnly(step),
      true,
      `${file}'s '${jobName}' -> '${stepName}' step must set NERVE_BUILD_BACKEND_ONLY=1 or NERVE_BUILD_PROFILE=backend`
    );
  });
}
