...
test("isCodexResponsesWebSocketRequired: NERVE_CODEX_WS_ENABLED=false forces HTTP even with codexTransport=websocket", () => {
  ...
  const prev = process.env.NERVE_CODEX_WS_ENABLED;
  process.env.NERVE_CODEX_WS_ENABLED = "false";
  try {
    ...
    if (prev === undefined) delete process.env.NERVE_CODEX_WS_ENABLED;
    else process.env.NERVE_CODEX_WS_ENABLED = prev;
    __setCodexWebSocketTransportForTesting(undefined);
  }
});
...
test("CodexExecutor.transformRequest preserves store-enabled responses state when explicitly enabled", () => {
  ...
  assert.equal(result._nerveResponsesStore, undefined);
...
test("Codex internal websocket bridge rejects non-object JSON payloads", async () => {
  await withEnv({ NERVE_WS_BRIDGE_SECRET: "bridge-secret" }, async () => {
    const { POST } = await import("../../src/app/api/internal/codex-responses-ws/route.ts");

    const response = await POST(
      new Request("http://nerve.local/api/internal/codex-responses-ws", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-nerve-ws-bridge-secret": "bridge-secret",
        },
