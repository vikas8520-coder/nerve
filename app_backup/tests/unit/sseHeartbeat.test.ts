...
function withEnv(value: string | undefined, fn: () => void) {
  const prev = process.env.NERVE_SSE_COMMENTS;
  try {
    if (value === undefined) delete process.env.NERVE_SSE_COMMENTS;
    else process.env.NERVE_SSE_COMMENTS = value;
    fn();
  } finally {
    if (prev === undefined) delete process.env.NERVE_SSE_COMMENTS;
    else process.env.NERVE_SSE_COMMENTS = prev;
  }
}
...
test("createSseHeartbeatTransform suppresses COMMENT heartbeats when NERVE_SSE_COMMENTS=off", async () => {
  const prev = process.env.NERVE_SSE_COMMENTS;
  process.env.NERVE_SSE_COMMENTS = "off";
  try {
