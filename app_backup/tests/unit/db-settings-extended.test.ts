...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__nerveDb?.open) {
      (globalThis as any).__nerveDb.close();
    }
  } catch {}
  delete (globalThis as any).__nerveDb;
}
