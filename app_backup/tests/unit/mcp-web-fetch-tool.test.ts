...
test("webFetchTool has the required McpToolDefinition shape", () => {
  assert.equal(webFetchTool.name, "nerve_web_fetch");
...
test("webFetchTool is registered in MCP_TOOLS and MCP_TOOL_MAP", () => {
  ...
  assert.ok(toolNames.includes("nerve_web_fetch"), "webFetchTool must be in MCP_TOOLS array");
  assert.ok("nerve_web_fetch" in MCP_TOOL_MAP, "webFetchTool must be in MCP_TOOL_MAP");
});

// ── Scope mapping ──

test("nerve_web_fetch is mapped in MCP_TOOL_SCOPES with execute:search", () => {
  const scopes = MCP_TOOL_SCOPES["nerve_web_fetch"];
  assert.ok(scopes != null, "nerve_web_fetch must have a scope mapping");
  assert.ok(scopes.includes("execute:search"), "nerve_web_fetch must require execute:search scope");
});
