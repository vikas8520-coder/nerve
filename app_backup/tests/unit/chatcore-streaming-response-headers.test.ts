...
// x-nerve-request-id, and the compression header only when meta is present.
...
test("merges upstream headers and sets x-nerve-request-id", () => {
  const { build } = makeBuild();
  const h = assembleStreamingResponseHeaders(baseArgs(), build);
  assert.equal(h["x-upstream"], "kept");
  assert.equal(h["x-nerve-request-id"], "preq-1");
});
