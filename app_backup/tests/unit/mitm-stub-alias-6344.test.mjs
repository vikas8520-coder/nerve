...
// opt-in (Docker sets NERVE_MITM_STUB=1); a default production build must
...
describe("mitm manager stub alias (#6344)", () => {
  ...
  it("NERVE_MITM_STUB=1 opts into the stub (Docker graceful degradation, #3390)", () => {
    assert.equal(shouldStubMitmManager({ NERVE_MITM_STUB: "1" }), true);
    assert.deepEqual(mitmManagerAliasFor({ NERVE_MITM_STUB: "1" }), {
  ...
  it("the Dockerfile keeps Docker on the stub via NERVE_MITM_STUB=1", () => {
    const dockerfile = readFileSync(new URL("../../Dockerfile", import.meta.url), "utf8");
    assert.match(dockerfile, /^ENV NERVE_MITM_STUB=1$/m);
  });
});
