 ...
 *
 * `nerve setup opencode` wires the bundled @nerve/opencode-plugin into a
 * local OpenCode install: copies the built plugin into `<config>/plugins/nerve/`
 * and registers a tuple entry in `opencode.json` (idempotent, replacing the legacy
 * `opencode-nerve-auth` entry from issue #3711).
 *
 * The command resolves the bundled plugin at module load, so the
 * NERVE_OPENCODE_PLUGIN_DIR fixture override MUST be set before the import.
...
process.env.NERVE_OPENCODE_PLUGIN_DIR = FAKE_PLUGIN_DIR;
...
function makeFakePluginDist() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@nerve/opencode-plugin", version: "0.0.0-test" })
  );
...
describe("nerve setup opencode", () => {
  ...
  it("installs the plugin and registers a tuple entry honouring --base-url (camelCased by Commander)", async () => {
    ...
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "nerve", "dist", "index.js")));
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "nerve", "package.json")));

    const cfg = readConfig();
    assert.ok(Array.isArray(cfg.plugin));
    assert.equal(cfg.plugin.length, 1);
    const [modulePath, options] = cfg.plugin[0];
    assert.equal(modulePath, "./plugins/nerve/dist/index.js");
    assert.equal(options.providerId, "nerve");
    assert.equal(
      options.baseURL,
      "http://10.0.0.5:20128",
  ...
  it("is idempotent: re-running updates the entry in place instead of duplicating it", async () => {
    ...
    const omniEntries = cfg.plugin.filter(
      (p: unknown) => Array.isArray(p) && (p[1] as { providerId?: string })?.providerId === "nerve"
    );
  ...
  it("removes the legacy opencode-nerve-auth entry (#3711) and preserves unrelated plugins", async () => {
    const cfgPath = path.join(CONFIG_DIR, "opencode.json");
    fs.writeFileSync(
      cfgPath,
      JSON.stringify({
        plugin: ["opencode-nerve-auth", ["./plugins/other/dist/index.js", { providerId: "other" }]],
      })
    );
    ...
    assert.ok(!flat.includes("opencode-nerve-auth"), "legacy entry must be dropped");
    assert.ok(flat.includes('"providerId":"other"'), "unrelated plugin entries must survive");
    assert.equal(cfg.plugin.length, 2, "other + nerve");
  });
