...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-provider-model-routes-codex-"));
...
test("provider models route merges live Codex models with the local catalog then filters denylist", async () => {
  ...
  // larger, so a stale/inflated live number can never make Nerve promise
