...
test("renameProcessTitle renames a bare 'next-server' title to 'nerve'", () => {
  assert.equal(renameProcessTitle("next-server"), "nerve");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "nerve (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "nerve (v16.2.9)");
  assert.equal(twice, "nerve (v16.2.9)");
});
