...
test("header can force goal policy and env can tune or disable recovery", () => {
  const forced = resolveAgentGoalPolicy(
    ...
    { "x-nerve-agent-goal": "true" },
    {
      NERVE_AGENT_GOAL_READINESS_MAX_TIMEOUT_MS: "900000",
      NERVE_AGENT_GOAL_STREAM_RECOVERY: "false",
    }
  );
...
test("NERVE_AGENT_GOAL_POLICY_ENABLED defaults to true — heuristic stays active", () => {
  const policy = resolveAgentGoalPolicy({ messages: [{ content: "/goal ship it" }] }, null, {});
...
test("NERVE_AGENT_GOAL_POLICY_ENABLED=false disables the heuristic entirely (no-op)", () => {
  const policy = resolveAgentGoalPolicy(
    { messages: [{ content: "/goal ship it" }] },
    { "x-nerve-agent-goal": "true" },
    {
      NERVE_AGENT_GOAL_POLICY_ENABLED: "false",
      NERVE_AGENT_GOAL_READINESS_MAX_TIMEOUT_MS: "900000",
    }
  );
