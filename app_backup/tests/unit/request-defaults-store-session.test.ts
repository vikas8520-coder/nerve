...
test("buildOpenAIStoreSessionId normalizes external and generated session ids", () => {
  assert.equal(
    ...
    "nerve-session-client-session-abc"
  );
  assert.equal(buildOpenAIStoreSessionId(" internal:session "), "nerve-session-internal:session");
...
test("ensureOpenAIStoreSessionFallback injects session_id only when no stable cache key exists", () => {
  ...
  assert.equal(injected.session_id, "nerve-session-session-1");
