 ...
 *    `x-session-id`, `x-nerve-session`) are forwarded upstream by
 *    providers that use custom executors with no client-header passthrough.
 *    `x-codex-session-id` and `x-nerve-session` are never forwarded by
...
// the real ~/.nerve data dir instead of this test's isolated tmp dir.

const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-session-affinity-7274-"));
...
test("#7274 x-codex-session-id and x-nerve-session are never forwarded upstream by DefaultExecutor", () => {
  ...
  const headers = executor.buildHeaders({ accessToken: "test-key" }, true, {
    ...
    "x-nerve-session": "another-internal-correlation-id",
  ...
  assert.ok(
    !lowerKeys.includes("x-nerve-session"),
    "x-nerve-session must never reach the upstream request"
  );
});
