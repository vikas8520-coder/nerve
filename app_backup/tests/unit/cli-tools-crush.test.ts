...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-crush-settings-"));
...
test("crush-settings POST: writes crush.json with an openai-compat providers.nerve block", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        const provider = written.providers?.nerve;
        assert.ok(provider, "providers.nerve must be written");
...
test("crush-settings DELETE: removes only the nerve provider entry", async () => {
  ...
  try {
      ...
      JSON.stringify({
        providers: {
          nerve: {
            ...
            api_key: "sk-test",
            models: [{ id: "openai/gpt-5", name: "Nerve: openai/gpt-5", context_window: 128000 }],
          },
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        const written = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        assert.equal(written.providers?.nerve, undefined);
