 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `nerve serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`nerve autostart on` / `... true`) via
...
test("status is the default action (bare `nerve autostart` is a safe read-only)", () => {
