...
test("a 'latest' path segment is NOT a test run (the bug that disabled backups)", () => {
  assert.equal(isAutomatedTestProcess(["node", "/opt/nerve/latest/server.js"], NO_ENV), false);
  assert.equal(
    isAutomatedTestProcess(["node", "C:\\apps\\latest\\bin\\nerve.mjs", "serve"], NO_ENV),
    false
  );
});
...
test("other words merely containing 'test' are not test runs either", () => {
  for (const argv of [
    ...
    assert.equal(
      isAutomatedTestProcess(argv, NO_ENV),
      false,
      `argv should not read as a test run: ${argv.join(" ")}`
    );
  }
});
...
test("the real runners are still detected", () => {
  assert.equal(
    isAutomatedTestProcess(["node", "--test", "tests/unit/foo.test.ts"], NO_ENV),
    true,
    "node --test"
  );
  assert.equal(
    isAutomatedTestProcess(["node", "/repo/tests/unit/foo.test.ts"], NO_ENV),
    true,
    "a tests/ path"
  );
  assert.equal(
    isAutomatedTestProcess(["node", "/repo/src/foo.test.ts"], NO_ENV),
    true,
    "a *.test.ts file"
  );
  assert.equal(
    isAutomatedTestProcess(["node", "/repo/node_modules/.bin/vitest", "run"], NO_ENV),
    true,
    "vitest binary"
  );
  assert.equal(
    isAutomatedTestProcess(["node", "/repo/node_modules/.bin/jest"], NO_ENV),
    true,
    "jest binary"
  );
});
...
test("env still wins, as before", () => {
  assert.equal(
    isAutomatedTestProcess(["node", "server.js"], { NODE_ENV: "test" } as NodeJS.ProcessEnv),
    true
  );
  assert.equal(
    isAutomatedTestProcess(["node", "server.js"], { VITEST: "1" } as NodeJS.ProcessEnv),
    true
  );
  assert.equal(
    isAutomatedTestProcess(["node", "server.js"], { NODE_ENV: "production" } as NodeJS.ProcessEnv),
    false
  );
});
