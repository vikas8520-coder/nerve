 ...
 *
 * Assertions cover:
 * - exec permission gates child_process behind NERVE_PLUGINS_ALLOW_EXEC==="1"
...
describe("pluginWorker sandbox — exec permission gating", () => {
  it("gates child_process behind NERVE_PLUGINS_ALLOW_EXEC === '1'", () => {
    assert.ok(
      source.includes('NERVE_PLUGINS_ALLOW_EXEC !== "1"') ||
        source.includes("NERVE_PLUGINS_ALLOW_EXEC !== '1'"),
      'exec block must check process.env.NERVE_PLUGINS_ALLOW_EXEC !== "1"'
    );
  });

  it("checks the env flag inside the exec permission block", () => {
    // The env guard must appear inside the exec permission block.
    // Use the SECOND occurrence of NERVE_PLUGINS_ALLOW_EXEC (the first is in
    ...
    const envIdxInBlock = source.indexOf("NERVE_PLUGINS_ALLOW_EXEC", execIdx);
    assert.ok(
      envIdxInBlock !== -1,
      "NERVE_PLUGINS_ALLOW_EXEC check must appear inside the exec permission block"
    );
  });
  ...
  it("does NOT wire child_process without the env guard in place", () => {
    ...
    // The NERVE_PLUGINS_ALLOW_EXEC check must come before sandbox.child_process.
    const envIdx = source.indexOf("NERVE_PLUGINS_ALLOW_EXEC");
...
describe("pluginWorker sandbox — trust-model comment", () => {
  ...
  it("references the NERVE_PLUGINS_ALLOW_EXEC opt-in in the comment", () => {
    assert.ok(
      source.includes("NERVE_PLUGINS_ALLOW_EXEC"),
      "trust-model comment must reference NERVE_PLUGINS_ALLOW_EXEC"
    );
  });
});
