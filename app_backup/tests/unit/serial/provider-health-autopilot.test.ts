...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-health-autopilot-"));
...
const accountFallback = await import("@nerve/open-sse/services/accountFallback");
...
test("provider health autopilot action accepts LAN dashboard requests (#6277)", async () => {
  ...
  assert.equal(request.headers.get("x-nerve-peer-ip"), null);
