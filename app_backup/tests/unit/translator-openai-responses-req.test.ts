...
test("Responses -> Chat strips background flag and degrades to synchronous execution", () => {
  // Previously this threw 400 unsupported_feature. Nerve is a forward proxy
...
test("Chat -> Responses preserves prompt_cache_key and session affinity fields", () => {
  ...
  (assert as any).equal((result as any).session_id, "nerve-session-abc");
...
test("Responses -> Chat: tool_search does not throw (issue #2766)", () => {
  ...
  // Nerve must not return 400 — it should silently drop the tool_search entry.
  assert.doesNotThrow(() =>
    openaiResponsesToOpenAIRequest(
      "gpt-4o",
      {
