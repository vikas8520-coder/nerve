...
import { IMAGE_PROVIDERS, parseImageModel } from "@nerve/open-sse/config/imageRegistry.ts";
...
test("cheaperinference image models are NOT in the chat registry", async () => {
  const { REGISTRY } = await import("@nerve/open-sse/config/providers/index.ts");
