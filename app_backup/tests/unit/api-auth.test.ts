...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-api-auth-"));
...
test("getApiKeyMetadata recognizes NERVE_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.NERVE_API_KEY = envKey;
...
test("requireManagementAuth returns null for NERVE_API_KEY env passthrough", async () => {
  await setupAuth();
