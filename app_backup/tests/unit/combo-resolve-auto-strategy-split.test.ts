...
import { resolveAutoStrategyOrder } from "@nerve/open-sse/services/combo/resolveAutoStrategy.ts";
...
// also supports a per-request `X-Nerve-Mode` override (relayOptions.mode) that can
...
test("per-request X-Nerve-Mode override changes the EFFECTIVE weights used for fallback ranking, not just selection", async () => {
