...
const BASE_URL = process.env.NERVE_BASE_URL || "http://localhost:20128";
const API_KEY = process.env.NERVE_API_KEY || "";
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect(toolNames).toContain("nerve_get_health");
        expect(toolNames).toContain("nerve_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=nerve_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "nerve_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
