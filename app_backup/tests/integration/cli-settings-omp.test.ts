...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-omp-settings-"));
...
test("omp-settings GET: treats an existing agent.db as installed", async () => {
  ...
  assert.equal(body.hasNerve, false);
});
...
test("omp-settings POST: writes models.yml and persists credentials for a seeded DB", async () => {
  ...
  assert.equal(getBody.hasNerve, true);
});

// ── Test 6: DELETE → removes Nerve provider entry ────────────────────────

test("omp-settings DELETE: removes the Nerve provider from models.yml and credentials", async () => {
  seedOmpDb();
  await POST(
    req({
      method: "POST",
  ...
  const getRes = await GET(req());
  const getBody = await getRes.json();
  assert.equal(getBody.hasNerve, false);
});
