 ...
 *  2. Zero nerve-* folders remain (post-prune: old nerve-* skill dirs were removed).
...
// ── §2: No nerve-* directories remain ────────────────────────────────────

test("skills/ has zero nerve-* directories (all pruned)", () => {
  if (!fs.existsSync(SKILLS_DIR)) {
    // If skills dir doesn't exist at all, nothing to prune
    return;
  }
  const entries = fs.readdirSync(SKILLS_DIR, { withFileTypes: true });
  const nerveFetchDirs = entries
    .filter((e) => e.isDirectory() && e.name.startsWith("nerve-"))
    .map((e) => e.name);
  assert.deepEqual(
    nerveFetchDirs,
    [],
    `Found nerve-* directories that should have been pruned: ${nerveFetchDirs.join(", ")}`
  );
});
