#!/usr/bin/env bash
# bin/rollback.sh — roll Nerve back to a previous release to mitigate a bad
# deploy. Part of the deploy-rollback incident-recovery flow.
#
# Methods (auto-detected; override with --method):
#   • npm    — `npm install -g nerve@<version>` and, if PM2 manages it,
#              `pm2 restart nerve`. This is how the VPS deploy runs.
#   • docker — re-tag the local image nerve:<version> to nerve:prod and
...
Rolls Nerve back to <version> (e.g. 3.8.35 or v3.8.35). With no version,
...
if [ -z "$VERSION" ]; then
  ...
  VERSION="$(npm view nerve versions --json 2>/dev/null | node -e '
    let s = "";
...
ops_log "target: nerve@$VERSION via $METHOD"
ops_confirm "Roll Nerve back to $VERSION via $METHOD?" || ops_die "aborted"

case "$METHOD" in
  npm)
    ops_require_cmd npm
    npm install -g "nerve@$VERSION"
    if command -v pm2 >/dev/null 2>&1 && pm2 jlist 2>/dev/null | grep -q '"name":"nerve"'; then
      pm2 restart nerve --update-env
      ops_log "pm2 restarted nerve"
    else
      ops_log "installed nerve@$VERSION — restart the service to apply (no PM2 'nerve' process found)"
    fi
    ;;
  docker)
    ops_require_cmd docker
    if ! docker image inspect "nerve:$VERSION" >/dev/null 2>&1; then
      ops_die "local image nerve:$VERSION not found — build it from the $VERSION checkout first (this compose builds the 'prod' tag, it does not pull a registry tag)"
    fi
    docker tag "nerve:$VERSION" nerve:prod
    docker compose -f "$REPO_ROOT/docker-compose.prod.yml" up -d --no-build
    ops_log "recreated prod service from nerve:$VERSION"
    ;;
