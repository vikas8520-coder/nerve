# bin/_ops-common.sh — shared helpers for the Nerve ops runbook scripts.
#
...
ops_set_data_dir() {
  NERVE_DATA_DIR="$1"
  NERVE_SQLITE="${NERVE_DATA_DIR}/storage.sqlite"
  NERVE_BACKUPS_DIR="${DB_BACKUPS_DIR:-${NERVE_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.nerve}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$NERVE_BACKUPS_DIR/$id" \
    "$NERVE_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$NERVE_BACKUPS_DIR" ]; then
    for cand in "$NERVE_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $NERVE_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
