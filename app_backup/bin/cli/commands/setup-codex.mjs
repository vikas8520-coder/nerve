/**
 * nerve setup-codex — Remote-aware Codex CLI profile generator.
 *
 * Connects to a running Nerve instance (local or remote VPS), fetches the
 ...
 *   nerve setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  printHeading(`Nerve → Codex CLI profile generator`);
...
export function registerSetupCodex(program) {
  program
    ...
    .option(
      "--remote <url>",
      "Remote Nerve URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "Nerve API key for the remote instance (defaults to NERVE_API_KEY env var)"
    )
