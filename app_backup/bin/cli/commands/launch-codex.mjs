 ...
 *  in the shell can't shadow the nerve provider (defense-in-depth). Mirrors
...
const NO_AUTH_SENTINEL = "nerve-no-auth";
 ...
 * (`model_providers.nerve.name="Nerve"` arrives unquoted and no longer
...
/**
 * Resolve the Nerve root base URL + auth for codex, honouring (in order):
...
export function resolveCodexTarget(opts = {}) {
  ...
  if (explicit) {
    ...
    let fromCtx;
    try {
      fromCtx = resolveActiveContext(opts.context ?? process.env.NERVE_CONTEXT)?.baseUrl;
    } catch {
      /* no context */
    }
    baseUrl = fromCtx
  ...
  if (!authToken) authToken = process.env.NERVE_API_KEY;
  return { baseUrl, authToken };
}

/** Health-check an Nerve root URL before launching Codex. */
 ...
 * NERVE_API_KEY (the provider env_key) to the resolved token or a sentinel.
...
export function buildCodexEnv(baseEnv, authToken) {
  ...
  env.NERVE_API_KEY = (authToken && String(authToken).trim()) || NO_AUTH_SENTINEL;
  return env;
}

/**
 * Codex `-c` flags that define the `nerve` provider inline, so launch works
 * WITHOUT a pre-existing ~/.codex/config.toml. Mirrors free-claude-code.
 * @param {string} baseUrl  Nerve root URL (no /v1)
 * @returns {string[]}
 */
export function buildCodexProviderArgs(baseUrl) {
  return [
    "-c",
    tomlAssign("model_provider", "nerve"),
    "-c",
    tomlAssign("model_providers.nerve.name", "Nerve"),
    "-c",
    tomlAssign("model_providers.nerve.base_url", `${baseUrl}/v1`),
    "-c",
    tomlAssign("model_providers.nerve.env_key", "NERVE_API_KEY"),
    "-c",
    tomlAssign("model_providers.nerve.wire_api", "responses"),
    "-c",
    tomlAssign("model_providers.nerve.requires_openai_auth", false),
  ];
}
...
export async function runLaunchCodexCommand(opts = {}, codexArgs = []) {
  ...
  if (!(await healthCheck(baseUrl))) {
    console.error(
      (
        t("launch.notRunning") || "Nerve is not reachable at {port}. Start it with 'nerve serve'."
...
export function registerLaunchCodex(program) {
  program
    ...
    .description(
      t("launchCodex.description") || "Launch Codex CLI pointed at Nerve (local or remote VPS)"
    )
    .option("--port <port>", "Local Nerve port (ignored when --remote is set)", "20128")
    .option(
      "--remote <url>",
      "Remote Nerve base URL, e.g. http://192.168.0.15:20128 (overrides --port + context)"
    )
    .option("--profile <name>", "Codex profile to activate (passed as --profile <name>)")
    .option("-p, --p <name>", "Alias for --profile")
    .option(
      "--api-key <key>",
      "Nerve API key (overrides NERVE_API_KEY env var for this invocation)"
    )
