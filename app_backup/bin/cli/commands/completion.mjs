...
function readCache() {
  try {
    ...
    if (process.env.NERVE_DEBUG_COMPLETION) {
      console.error("[nerve completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.NERVE_DEBUG_COMPLETION) {
      console.error("[nerve completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.NERVE_DEBUG_COMPLETION) {
      console.error("[nerve completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_nerve");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "nerve.fish");
  return join(home, ".bash_completion.d", "nerve");
}

function generateZshScript() {
  return `#compdef nerve

# Nerve zsh completion (dynamic)
_nerve_get_cache() {
  local key="$1"
  local cache="$HOME/.nerve/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    nerve completion refresh --quiet >/dev/null 2>&1
  fi
...
_nerve() {
  local -a commands
  commands=(
    'serve:Start the Nerve server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure Nerve'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_nerve_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_nerve_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_nerve_get_cache models))
...
compdef _nerve nerve
`;
}

function generateBashScript() {
  return `#!/bin/bash
# Nerve CLI bash completion (dynamic)
...
complete -F _nerve nerve
`;
}

function generateFishScript() {
  return `# Nerve CLI fish completion (dynamic)
complete -c nerve -f
...
for cmd in $commands
  complete -c nerve -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c nerve -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c nerve -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c nerve -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c nerve -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c nerve -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c nerve -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __nerve_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.nerve/completion-cache.json"
...
complete -c nerve -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__nerve_cache_get combos)'
complete -c nerve -l model -a '(__nerve_cache_get models)'
complete -c nerve -l combo -a '(__nerve_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `nerve completion <shell>` (positional arg form)
  comp
    .command("<shell>")
