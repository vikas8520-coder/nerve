...
export function registerPlugin(program) {
  const plugin = program
    .command("plugin")
    .description(t("plugin.description") || "Manage CLI plugins (nerve-cmd-*)");
  ...
  plugin
    ...
    .action(async (opts, cmd) => {
      ...
      if (plugins.length === 0) {
        ...
        process.stdout.write(`Install: nerve plugin install <name>\n`);
      }
    });
  ...
  plugin
    ...
    .action(async (name, opts) => {
      ...
      const pkgName = isLocal ? name : `nerve-cmd-${name}`;

      if (!opts.yes) {
        process.stderr.write(
          `⚠ WARNING: Plugins run with the same privileges as nerve CLI.\n` +
  ...
  plugin
    ...
    .action(async (name, opts) => {
      const pkgName = name.startsWith("nerve-cmd-") ? name : `nerve-cmd-${name}`;
      if (!opts.yes) {
  ...
  plugin
    ...
    .action(async (name, opts, cmd) => {
      const plugins = await discoverPlugins();
      const p = plugins.find((x) => x.name === name || x.name === `nerve-cmd-${name}`);
      if (!p) {
  ...
  plugin
    ...
    .action(async (query) => {
      const q = query ? `nerve-cmd-${query}` : "nerve-cmd";
      ...
      try {
        ...
        if (rows.length === 0) {
          process.stdout.write(`No plugins found for '${query || "nerve-cmd"}'.\n`);
        } else {
          rows.forEach((r) =>
  ...
  plugin
    ...
    .action(async (name) => {
      ...
      const dir = join(process.cwd(), `nerve-cmd-${safeName}`);
      if (existsSync(dir)) {
      ...
      writeFileSync(
        ...
        JSON.stringify(
          {
            name: `nerve-cmd-${safeName}`,
            version: "0.1.0",
            type: "module",
            main: "index.mjs",
            description: `Nerve CLI plugin: ${safeName}`,
            engines: { nerve: ">=4.0.0" },
      ...
      writeFileSync(
        join(dir, "README.md"),
        `# nerve-cmd-${safeName}\n\nAn Nerve CLI plugin.\n\n## Install\n\n\`\`\`bash\nnerve plugin install ${safeName}\n\`\`\`\n`
      );
      process.stdout.write(`✓ Scaffolded: ${dir}\n`);
      process.stdout.write(`  Run: cd ${dir} && nerve plugin install .\n`);
    });
}
