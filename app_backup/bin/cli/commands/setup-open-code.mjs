/**
 * nerve setup opencode — Wire the bundled @nerve/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g nerve` ships the plugin
 * inside the nerve package (`@nerve/opencode-plugin/dist/`) but
...
// We walk up from this file to find the nerve package root. The script
// lives at `<nerve>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@nerve/opencode-plugin/
// (see root package.json `files`: ["@nerve/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.NERVE_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@nerve", "opencode-plugin");
...
/**
 * Locate the bundled @nerve/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published nerve tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @nerve/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means nerve was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling nerve (npm install -g nerve) ` +
        `or run \`cd @nerve/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@nerve/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/nerve/`. We
 ...
 * resolve the peer deps from the nerve package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "nerve");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`nerve setup opencode\`.`
      );
    }
  }
  ...
  // drop a legacy `opencode-nerve-auth` entry if present — that
  // package is the obsolete predecessor of @nerve/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-nerve-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-nerve-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="nerve"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  ...
  printHeading("Nerve → OpenCode Plugin Setup");
...
/**
 * Register the `nerve setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `nerve setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @nerve/opencode-plugin with a local OpenCode install"
    )
    .option("--provider-id <id>", "OpenCode provider id to register (default: nerve)", "nerve")
    .option(
      "--base-url <url>",
      "Nerve base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote Nerve URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
