/**
 * nerve setup-continue — configure Continue (continue.dev) for Nerve.
 *
 ...
 * `apiKey: ${{ secrets.NERVE_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.NERVE_API_KEY }}";
...
/**
 * Merge Nerve models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "Nerve Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("Nerve → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} Nerve model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} Nerve models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export NERVE_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'NERVE_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the Nerve model catalog"
    )
