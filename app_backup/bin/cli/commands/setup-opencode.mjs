/**
 * nerve setup-opencode — Remote-aware OpenCode provider generator
 * (openai-compatible). Distinct from `nerve setup opencode` (which wires the
 * @nerve/opencode-plugin). This writes the `nerve` provider into
 * ~/.config/opencode/opencode.json with every catalog model, so you can run
 * `opencode -m nerve/<model>`.
 *
...
const ENV_KEY_REF = "{env:NERVE_API_KEY}";
...
export function resolveOpencodeTarget(opts = {}) {
  ...
  if (opts.remote) {
    ...
    try {
      const c = resolveActiveContext(opts.context ?? process.env.NERVE_CONTEXT);
      baseUrl = c?.baseUrl;
    } catch {
      /* no context */
    }
  ...
  if (!apiKey) apiKey = process.env.NERVE_API_KEY || "";
...
export function postProcessOpencodeConfig(rawJson, opts = {}) {
  ...
  const prov = config.provider?.nerve;
...
export async function runSetupOpencodeCommand(opts = {}) {
  ...
  printHeading("Nerve → OpenCode provider (openai-compatible)");
  printInfo(`Connecting to ${baseUrl} …`);

  // Deferred import: opencode.ts is TypeScript; tsx is registered by
  // bin/nerve.mjs before any command runs, so importing here is safe.
  let raw;
  try {
    const { generateOpencodeConfig } =
      await import("../../../src/lib/cli-helper/config-generator/opencode.ts");
    raw = await generateOpencodeConfig({ baseUrl, apiKey, model: opts.model, providerId: "nerve" });
  } catch (err) {
    printError(`Failed to generate opencode.json: ${err?.message || err}`);
    printInfo("Make sure Nerve is running and --remote/--api-key are correct.");
    return 1;
  }
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${modelCount} model(s) under provider 'nerve' → ${configPath}`);
    return 0;
  }

  if (!existsSync(configDir)) mkdirSync(configDir, { recursive: true });
  writeFileSync(configPath, json, "utf8");
  printSuccess(`opencode.json updated at ${configPath} (${modelCount} models under 'nerve')`);
  printInfo('Use it:  opencode -m nerve/<model> "..."   (export NERVE_API_KEY first)');
  return 0;
}
...
export function registerSetupOpencode(program) {
  program
    ...
    .description(
      "Generate the Nerve openai-compatible provider in ~/.config/opencode/opencode.json " +
        "from the live model catalog (local or remote VPS)"
    )
    .option("--port <port>", "Local Nerve port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote Nerve URL, e.g. http://192.168.0.15:20128")
    .option("--api-key <key>", "Nerve API key (defaults to NERVE_API_KEY env var)")
    .option("--model <id>", "Set the default top-level model (nerve/<id>)")
