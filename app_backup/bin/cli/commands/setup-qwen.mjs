/** Configure Qwen Code's OpenAI-compatible provider for Nerve. */
...
export function resolveQwenTarget(opts = {}) {
  ...
  if (!root || !(opts.apiKey ?? opts["api-key"])) {
    try {
      context = resolveActiveContext(opts.context ?? process.env.NERVE_CONTEXT);
    } catch {
...
export async function runSetupQwenCommand(opts = {}) {
  ...
  printHeading("Nerve → Qwen Code (OpenAI-compatible)");
  ...
  try {
    ...
    if (dryRun) {
      ...
      printInfo(`[dry-run] credential → ${envPath} (NERVE_API_KEY)`);
      return 0;
    }
    ...
    printSuccess(`Updated ${envPath} (NERVE_API_KEY only)`);
...
export function registerSetupQwen(program) {
  program
    .command("setup-qwen")
    .description("Configure Qwen Code's upstream V4 modelProviders format for Nerve")
    .option("--port <port>", "Local Nerve port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote Nerve URL")
    .option("--api-key <key>", "Nerve API key")
