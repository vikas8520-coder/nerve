...
// ahead of expensive self-heal hooks), adapted to Nerve's Commander CLI where the
...
// #7791: tsx's tsconfig-path resolution does not apply when Nerve is
// installed globally (files live under node_modules/nerve/), so bare
...
function loadEnvFile() {
  ...
  if (process.env.NERVE_CLI_SKIP_REPO_ENV !== "1") {
...
// Generate STORAGE_ENCRYPTION_KEY if not set (persisted to ~/.nerve/.env)
// This ensures the key survives across upgrades and is not regenerated on each install.
// See: https://github.com/diegosouzapw/Nerve/issues/1622
//
...
// key or write ~/.nerve/.env — running a read-only command should never
// mutate the data dir.
if (shouldProvisionStorageKey(process.argv)) {
  ...
  if (!process.env.STORAGE_ENCRYPTION_KEY) {
    ...
    // Writing only to ~/.nerve (the container home, not a volume) silently lost the key on
    // container recreation, leaving the persisted encrypted DB undecryptable (regression of #1622).
    const dataDir = process.env.DATA_DIR || join(homedir(), ".nerve");
    const envPath = join(dataDir, ".env");
...
// Apply --lang before Commander parses (program descriptions call t() during setup)
{
  ...
  const langEnv = process.env.NERVE_LANG;
...
process.on("exit", () => {
  if (process.env.NERVE_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g nerve` or `nerve update --apply`",
    });
  }
});
