 ...
 *   - forwarding to the Nerve router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@nerve/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@nerve/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the Nerve router round-trip.
  ...
  /**
   * Forward the prepared body to the Nerve router (Next.js API).
   * Adds AgentBridge correlation headers (`x-nerve-source`, `x-nerve-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.NERVE_BASE_URL ?? "http://127.0.0.1:20128";
