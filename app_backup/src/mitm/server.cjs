...
// Inspector. The token is injected by manager.ts (same value the Nerve
 ...
 * shape. The route-only namespace is reserved for client-facing Nerve model ids;
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-nerve-source header is the
...
async function intercept(req, res, bodyBuffer, override, sourceModel) {
  ...
  // The Nerve router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    const response = await fetch(forward.url, {
      ...
      headers: {
        ...
        "x-nerve-source": "agent-bridge",
        "x-nerve-agent": agentId,
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`Nerve ${response.status}: ${errText}`);
    }
...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (req.headers["x-nerve-source"] === "nerve") {
      vlog(1, `[MITM] → PASSTHROUGH (Nerve source loop)`);
