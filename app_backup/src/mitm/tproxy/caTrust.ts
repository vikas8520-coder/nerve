 ...
 * trust store under a DEDICATED slot (`nerve-tproxy-ca.crt`), separate from
 * the static MITM cert (`nerve-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "nerve-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.NERVE_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] NERVE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
