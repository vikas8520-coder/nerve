...
} from "@nerve/open-sse/services/accountFallback.ts";
import { getModelInfo, getComboForModel } from "../services/model";
import { resolveBareModelToConnectionDefault } from "@nerve/open-sse/services/model.ts";
import { errorResponse } from "@nerve/open-sse/utils/error.ts";
import { getImageModelEntry } from "@nerve/open-sse/config/imageRegistry.ts";
import { acceptHeaderForcesStream } from "@nerve/open-sse/utils/aiSdkCompat.ts";
import { applyNoThinkingAlias } from "@nerve/open-sse/utils/noThinkingAlias.ts";
import { resolveCcDiscoveryAliasStrip } from "@/lib/ccDiscoveryAliasResolve";
import { handleComboChat, shouldSkipConnDisable } from "@nerve/open-sse/services/combo.ts";
import { mergeAbortSignals } from "@nerve/open-sse/executors/base.ts";
import { resolveRequestAutoControls } from "@nerve/open-sse/services/autoCombo/requestControls.ts";
import { resolveComboConfig } from "@nerve/open-sse/services/comboConfig.ts";
import { injectHandoffIntoBody } from "@nerve/open-sse/services/contextHandoff.ts";
import {
  HTTP_STATUS,
  ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE,
} from "@nerve/open-sse/config/constants.ts";
import { getTargetFormat, detectFormatFromUrl } from "@nerve/open-sse/services/provider.ts";
import {
  getModelsByProviderId,
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@nerve/open-sse/config/providerModels.ts";
...
import { connectionHasExtraKeys } from "@nerve/open-sse/services/apiKeyRotator.ts";
import {
...
import { isSubscriptionQuotaText } from "@nerve/open-sse/services/quotaTextCooldowns.ts";
...
import { cloneBoundedForLog } from "@nerve/open-sse/utils/requestLogger.ts";
import { handleInternalUsageCommand } from "@/lib/usage/internalUsageCommand";
import {
  applyTaskAwareRouting,
  getTaskRoutingConfig,
} from "@nerve/open-sse/services/taskAwareRouter.ts";
import {
  hasNativeWebSearchTool,
  resolveWebSearchRouteOverride,
} from "@nerve/open-sse/services/webSearchRouting.ts";
import {
...
} from "@nerve/open-sse/services/sessionManager.ts";
import { startQuotaMonitor } from "@nerve/open-sse/services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@nerve/open-sse/services/emergencyFallback.ts";
import {
  registerCodexConnection,
  registerCodexQuotaFetcher,
} from "@nerve/open-sse/services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "@nerve/open-sse/services/bailianQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "@nerve/open-sse/services/crofUsageFetcher.ts";
import { registerDeepseekQuotaFetcher } from "@nerve/open-sse/services/deepseekQuotaFetcher.ts";
import { registerOpenrouterQuotaFetcher } from "@nerve/open-sse/services/openrouterQuotaFetcher.ts";
import { registerOpencodeQuotaFetcher } from "@nerve/open-sse/services/opencodeQuotaFetcher.ts";
import { registerGrokWebQuotaFetcher } from "@nerve/open-sse/services/grokQuotaFetcher.ts";
import { registerGenericQuotaFetchers } from "@nerve/open-sse/services/genericQuotaFetcher.ts";
import "@nerve/open-sse/services/quotaTrackersBatch.ts";
import {
...
export async function handleChat(
  ...
  // Nerve-level 400 before any routing or upstream call (#5110, #6402).
  ...
  const requestedConnectionId = request.headers.get("x-nerve-connection")?.trim() || null;
  if (sessionId) {
  ...
  requestAttemptLoop: while (true) {
    ...
    while (true) {
      ...
      if (
        ...
        body?._nerveSkipContextRelay !== true
      ) {
      ...
      if (runtimeOptions.sessionId && body?._nerveInternalRequest !== "context-handoff") {
