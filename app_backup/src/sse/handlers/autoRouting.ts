...
import type { AutoVariant } from "@nerve/open-sse/services/autoCombo/autoPrefix.ts";
import {
  AUTO_TEMPLATE_VARIANTS,
  VALID_AUTO_VARIANTS,
} from "@nerve/open-sse/services/autoCombo/builtinCatalog.ts";
import {
  parseAutoSuffix,
  type AutoCategory,
  type AutoTier,
} from "@nerve/open-sse/services/autoCombo/suffixComposition.ts";
import {
  isValidModelFamily,
  type ModelFamily,
} from "@nerve/open-sse/services/autoCombo/modelFamily.ts";
...
export async function createVirtualAutoCombo(
  ...
  try {
    const { createVirtualAutoCombo: createVirtual } =
      await import("@nerve/open-sse/services/autoCombo/virtualFactory.ts");
