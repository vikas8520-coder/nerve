...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "Nerve") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "Nerve" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as Nerve
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingNerve: true };
  ...
  // Count of roles that are (or will be) routed via Nerve
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as Nerve intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingNerve) and settings shape (provider + base_url)
    if (typeof info.usingNerve === "boolean") {
      return info.usingNerve;
    }
    return (
      info?.provider === "nerve" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via Nerve
              // - Else if current from disk: show real provider name + "(not Nerve)" or "Nerve"
              let badge: { label: string; pending: boolean; outsideNerve: boolean } | null = null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via Nerve
                const prov = sel.provider || "Nerve";
                badge = {
                  label: t("hermesViaNerve", { provider: prov }),
                  pending: true,
                  outsideNerve: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "nerve" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "Nerve", pending: false, outsideNerve: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotNerve", { provider: realProvider }),
                    pending: false,
                    outsideNerve: true,
                  };
                }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "Nerve";
