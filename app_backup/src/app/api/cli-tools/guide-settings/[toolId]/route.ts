  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the Nerve settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Check if Nerve entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.nerveManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("nerve") ||
 ...
 * keys, but make sure the Nerve provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      nerve: {
        ...((existingConfig.providers && existingConfig.providers.nerve) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers.nerve &&
            existingConfig.providers.nerve.api_key) ||
          "",
      },
    },
  };
