...
import { sanitizeErrorMessage } from "@nerve/open-sse/utils/error.ts";
...
// the `nerve setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. Nerve is registered under
 * the `nerve` provider id as an `openai-compat` provider — same shape
...
const hasNerveConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const nerve = providers?.nerve as Record<string, unknown> | undefined;
  return (
    !!nerve &&
    nerve.type === "openai-compat" &&
    typeof nerve.base_url === "string" &&
    nerve.base_url.length > 0
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasNerve: hasNerveConfig(config),
...
// POST — write Nerve settings to crush.json (providers.nerve)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `Nerve: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove Nerve provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Crush Nerve settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
