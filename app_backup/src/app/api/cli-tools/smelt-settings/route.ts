...
/**
 * Check if the config file contains Nerve settings.
 */
...
// POST — write Nerve settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge Nerve settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove Nerve settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt Nerve settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
