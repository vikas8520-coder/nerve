...
const getBackupPath = () => path.join(getLocalBackendDir(), "providers", "auth.json.nerve-backup");
...
// ── Check if a base_url points to Nerve ──────────────────────────────
const isNerveUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("nerve");
};

// ── Check if Nerve is configured ─────────────────────────────────────
const hasNerveConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return isNerveUrl(provider.base_url);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-Nerve endpoint
    let lmstudioConflict = false;
    if (provider && !isNerveUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasNerve: hasNerveConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply Nerve as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-Nerve provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isNerveUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isNerveUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-Nerve conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-nerve provider if present
    if (authFile.providers?.["lc-nerve"]) {
      delete authFile.providers["lc-nerve"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a Nerve model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(error) } }, { status: 500 });
...
// ── DELETE - Remove Nerve configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "Nerve config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "Nerve config removed. Restart Letta CLI to take effect.";
