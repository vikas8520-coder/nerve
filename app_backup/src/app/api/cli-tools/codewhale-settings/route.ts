...
/**
 * Render the Nerve config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by Nerve (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
 ...
 */
const hasNerveConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by Nerve");
};
...
// POST — write Nerve settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove Nerve CodeWhale config (primary + legacy, if present)
