...
// [model.nerve] ... until the next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# nerve-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
 ...
 * only the flat string fields Nerve itself writes.
 */
...
const buildModelSection = (model: string, baseUrl: string, apiKey: string): string => {
  const lines = [
    ...
    `name = "Nerve"`,
    `description = "Routed via Nerve gateway"`,
...
/** Insert/replace the `[model.nerve]` section, preserving the rest of the file. */
...
const rememberPrevDefault = (toml: string): string => {
  ...
  const marker = `# nerve-prev-default = "${current}"\n`;
...
const hasNerveConfig = (modelCfg: GrokModelSection | null): boolean => Boolean(modelCfg?.base_url);
...
// GET — check Grok Build CLI and return current [model.nerve] config
...
// POST — write the [model.nerve] section into ~/.grok/config.toml and set it default
...
// DELETE — remove the [model.nerve] section and restore the previous default
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Nerve model slot removed from Grok Build",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
