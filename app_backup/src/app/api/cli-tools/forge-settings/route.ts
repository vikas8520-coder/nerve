...
/**
 * Render the Nerve provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by Nerve (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-Nerve marker comment.
 */
...
// POST — write Nerve settings to Forge config.toml
...
// DELETE — remove Forge Nerve config
