...
import { getRegistryEntry } from "@nerve/open-sse/config/providerRegistry.ts";
...
import { getStaticQoderModels } from "@nerve/open-sse/services/qoderCli.ts";
import { deriveConfigFromRegistryModelsUrl } from "./discoveryConfig";
import {
  fetchGitHubCopilotModels,
  fetchGheCopilotModels,
} from "@nerve/open-sse/services/githubCopilotModels.ts";
import { fetchKiroAvailableModels } from "@nerve/open-sse/services/kiroModels.ts";
import { buildGlmCodingHeaders, buildGlmModelsUrl } from "@nerve/open-sse/config/glmProvider.ts";
import { getImageProvider } from "@nerve/open-sse/config/imageRegistry.ts";
import { getVideoProvider } from "@nerve/open-sse/config/videoRegistry.ts";
import {
  discoverBedrockNativeModels,
  isBedrockNativeApiError,
} from "@nerve/open-sse/services/bedrock.ts";
import {
  discoverPromptQlModels,
  PROMPTQL_FALLBACK_MODELS,
} from "@nerve/open-sse/services/promptqlModels.ts";
import {
  discoverNotionWebModels,
  NOTION_WEB_FALLBACK_MODELS,
} from "@nerve/open-sse/services/notionWebModels.ts";
import {
  AZURE_AI_DEFAULT_BASE_URL,
  buildAzureAiModelsUrl,
} from "@nerve/open-sse/config/azureAi.ts";
import {
  DATAROBOT_DEFAULT_BASE_URL,
  buildDataRobotCatalogUrl,
  isDataRobotDeploymentUrl,
} from "@nerve/open-sse/config/datarobot.ts";
import { OCI_DEFAULT_BASE_URL, buildOciModelsUrl } from "@nerve/open-sse/config/oci.ts";
import {
  SAP_DEFAULT_BASE_URL,
  buildSapModelsUrl,
  getSapResourceGroup,
} from "@nerve/open-sse/config/sap.ts";
import { WATSONX_DEFAULT_BASE_URL, buildWatsonxModelsUrl } from "@nerve/open-sse/config/watsonx.ts";
import { getEmbeddingProvider } from "@nerve/open-sse/config/embeddingRegistry.ts";
import { getRerankProvider } from "@nerve/open-sse/config/rerankRegistry.ts";
import {
  getSpeechProvider,
  getTranscriptionProvider,
} from "@nerve/open-sse/config/audioRegistry.ts";
import {
...
export async function GET(
  ...
  try {
    ...
    const buildDiscoveryErrorFallbackResponse = (
      ...
      // the local/cached catalog Nerve ships instead of surfacing a raw 503.
    ...
    if (provider === "vertex" || provider === "vertex-partner") {
      ...
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@nerve/open-sse/executors/vertex.ts");
        if (accessToken) {
