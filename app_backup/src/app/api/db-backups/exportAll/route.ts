...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `nerve-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `nerve-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="nerve-full-backup-${timestamp}.tar.gz"`,
