...
import { estimateTokens } from "@nerve/open-sse/services/contextManager.ts";
import { adaptBodyForCompression } from "@nerve/open-sse/services/compression/bodyAdapter.ts";
import type {
  CompressionConfig,
  CompressionResult,
} from "@nerve/open-sse/services/compression/types.ts";
import { resolveCompressionSettings } from "@nerve/open-sse/handlers/chatCore/compressionSettings.ts";
import {
  writeCompressionAnalytics,
  writeCompressionSkip,
} from "@nerve/open-sse/handlers/chatCore/compressionAnalyticsWrite.ts";
...
export async function applyResponsesWsCompression(
  ...
  try {
    ...
    const { selectCompressionStrategy, applyCompressionAsync } =
      await import("@nerve/open-sse/services/compression/strategySelector.ts");
