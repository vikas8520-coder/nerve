...
import { handleAudioTranscription } from "@nerve/open-sse/handlers/audioTranscription.ts";
import {
...
import { attachNerveMetaToResponse } from "@/domain/nerveResponseMeta";
...
export async function POST(request) {
  ...
  if (response?.ok) {
    ...
    response = attachNerveMetaToResponse(response, {
      provider,
      model: resolvedModel,
      costUsd: 0,
