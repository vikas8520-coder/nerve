 ...
 *
 * When a request carries `x-nerve-compression`, docs promise the response echoes
 * `X-Nerve-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-nerve-compression";
const COMPRESSION_RESPONSE_HEADER = NERVE_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-Nerve-Compression: <mode>; source=request-header`
 * when the request supplied `x-nerve-compression` and the inner pipeline did not
