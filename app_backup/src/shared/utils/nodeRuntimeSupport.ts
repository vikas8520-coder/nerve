/**
 * Shared Node.js runtime support policy for Nerve.
 *
...
export function getNodeRuntimeWarning(version: string = process.versions.node): string | null {
  ...
  if (support.reason === "unreleased-major") {
    return `Node.js ${support.nodeVersion} is outside the supported LTS lines. Nerve currently supports Node.js 22.x, 24.x, 25.x, and 26.x.`;
  }

  return `Node.js ${support.nodeVersion} is outside Nerve's approved secure runtime policy.`;
}
