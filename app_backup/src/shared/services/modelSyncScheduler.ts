...
export function resolveModelSyncInternalBaseUrl(_candidate?: string): string {
  ...
  const nativeTls = process.env.NERVE_INTERNAL_SCHEME === "https";
  const origin = nativeTls
    ? `https://localhost:${dashboardPort}`
    : `http://127.0.0.1:${dashboardPort}`;
  return `${origin}${normalizeInternalBasePath(process.env.NERVE_BASE_PATH)}`;
}
...
const globalState = globalThis as typeof globalThis & {
  __nerveModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__nerveModelSyncInternalAuthToken || randomUUID();
    globalState.__nerveModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__nerveModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__nerveModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call Nerve's own API
