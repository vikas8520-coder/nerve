...
const HEADER_NAME = "x-nerve-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local nerve CLI.
 * Only accepted from loopback IPs. Disabled via NERVE_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.NERVE_DISABLE_CLI_TOKEN === "true") return false;
