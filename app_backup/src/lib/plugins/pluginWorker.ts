 ...
 *   2. The `exec` permission additionally requires NERVE_PLUGINS_ALLOW_EXEC=1
...
function createSandbox(permissions: string[], pluginDir: string): Record<string, unknown> {
  ...
  if (permissions.includes("exec")) {
    if (process.env.NERVE_PLUGINS_ALLOW_EXEC !== "1") {
      throw new Error(
        `Plugin '${name}' requested the 'exec' permission, which is disabled. Set NERVE_PLUGINS_ALLOW_EXEC=1 to enable (local operator only).`
      );
    }
