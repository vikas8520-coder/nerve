...
declare global {
  var __nerveShutdown: { init: boolean; shuttingDown: boolean; activeRequests: number } | undefined;
}

function getShutdownState() {
  if (!globalThis.__nerveShutdown) {
    globalThis.__nerveShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
  }
  return globalThis.__nerveShutdown;
}
...
async function cleanup(): Promise<void> {
  try {
    const [{ closeAuditDb }, { closeDbInstance }, { flushSpendBatchWriter }, { closeLogRotation }] =
      await Promise.all([
        import("@nerve/open-sse/mcp-server/audit.ts"),
        import("@/lib/db/core"),
