import { PROVIDER_ID_TO_ALIAS, PROVIDER_MODELS } from "@nerve/open-sse/config/providerModels.ts";
import { parseModel, resolveCanonicalProviderModel } from "@nerve/open-sse/services/model.ts";
import {
  MODEL_SPECS,
...
import { getUnsupportedParams } from "@nerve/open-sse/config/providerRegistry.ts";
import {
  getLearnedThinkingCap,
  GEMINI_FALLBACK_THINKING_CAP,
} from "@nerve/open-sse/services/learnedThinkingCaps.ts";
...
function reverseModelsDevProviders(provider: string): string[] {
  // models.dev may store capabilities under a different Nerve provider id
