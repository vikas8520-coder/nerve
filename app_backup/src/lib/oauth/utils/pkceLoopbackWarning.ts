 ...
 * browser, not on whatever host serves the Nerve dashboard.
 *
 ...
 *   Nerve *server's* loopback while the provider redirects the *browser's* loopback.
 */
...
export function buildPkceLoopbackMismatchWarning(provider: string): string {
  ...
  return (
    `Nerve is being accessed from a LAN IP, not true localhost. ${provider}'s OAuth app ` +
    `is registered with a fixed loopback redirect (${redirect}) that only resolves on the ` +
    "machine running this browser tab, not on the Nerve server — the login will silently " +
    "fail on the provider's side. Open the Nerve dashboard from true localhost instead " +
    "(SSH port-forward: ssh -L <port>:127.0.0.1:<port> <user>@<nerve-host>, then browse to " +
