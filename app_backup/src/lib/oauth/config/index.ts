 ...
 * with the running Nerve server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.NERVE_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.NERVE_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.NERVE_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
