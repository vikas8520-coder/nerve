 ...
 *   model: string;                    // the model name the user chose from Nerve
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the Nerve base URL
...
export async function generateHermesAgentConfig(
  ...
  const resolvedKey = apiKey || "YOUR_NERVE_API_KEY_HERE";
  ...
  // Build the providers.nerve entry (shared)
