 ...
 * source is the user's OWN Nerve instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from Nerve. The catalog is the
 ...
 */
export async function fetchNerveCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Nerve /v1/models returned ${response.status} ${response.statusText}`);
    }
...
/**
 * Generate a full `opencode.json` document for Nerve. The catalog is the
...
export async function generateOpencodeConfig(options: GenerateOpencodeOptions): Promise<string> {
  ...
  const providerId = options.providerId?.trim() || "nerve";
  ...
  if (fetchCatalog) {
    const result = await fetchNerveCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "Nerve",
