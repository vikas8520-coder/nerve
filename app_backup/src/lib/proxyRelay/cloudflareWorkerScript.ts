/**
 * Cloudflare Worker source emitter for the Nerve proxy relay.
 *
 ...
 *
 * The Nerve variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----NerveCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// Nerve Cloudflare Worker proxy relay — generated at deploy time.
