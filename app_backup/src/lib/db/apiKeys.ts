...
import { resolveModelAlias } from "@nerve/open-sse/services/modelDeprecation.ts";
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.NERVE_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal Nerve operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`NERVE_API_KEY` / `ROUTER_API_KEY`)
