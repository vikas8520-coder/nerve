  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir nerve`
    );
  }
}
...
declare global {
  var __nerveDb: SqliteAdapter | undefined;
  ...
  var __nerveDbProbeRestoreCount: number | undefined;
  ...
  var __nerveDbOomFailureCount: number | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__nerveDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__nerveDb = db;
  } else {
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.NERVE_FORCE_DB_HEALTHCHECK === "1") return true;
...
function getDbHealthCheckIntervalMs(): number {
  const rawValue = process.env.NERVE_DB_HEALTHCHECK_INTERVAL_MS;
...
function startDbHealthCheckScheduler(db: SqliteDatabase) {
  ...
  dbHealthCheckTimer = setInterval(() => {
    try {
      ...
      runDbHealthCheck(db, {
        autoRepair: true,
        skipIntegrityCheck: process.env.NERVE_SKIP_DB_HEALTHCHECK === "1",
...
export function getDbInstance(): SqliteDatabase {
  ...
  if (!fs.existsSync(sqliteFile) && probeFailureBackups.length > 0) {
    ...
    if (
      (globalThis.__nerveDbProbeRestoreCount = (globalThis.__nerveDbProbeRestoreCount || 0) + 1) > 3
    ) {
      throw new Error(
  ...
  if (fs.existsSync(sqliteFile)) {
    try {
      ...
      if (
        ...
        if (
          (globalThis.__nerveDbOomFailureCount = (globalThis.__nerveDbOomFailureCount || 0) + 1) > 3
        ) {
          throw new Error(
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _nerve_migrations (version, name)
  ...
  if (shouldRunStartupDbHealthCheck()) {
    const skipIntegrityCheck = process.env.NERVE_SKIP_DB_HEALTHCHECK === "1";
    if (skipIntegrityCheck) {
      console.log("[DB] Health check skipped (NERVE_SKIP_DB_HEALTHCHECK=1)");
    }
    runDbHealthCheck(db, {
      autoRepair: true,
      expectedSchemaVersion: "1",
 ...
 * can place it in ~/.nerve/runtime/ without blocking a synchronous caller.
 *
