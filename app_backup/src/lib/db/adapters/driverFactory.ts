...
declare global {
  var __nerveSqlJsAdapters: Map<string, SqliteAdapter> | undefined;
  var __nerveSqlJsInitPromises: Map<string, Promise<SqliteAdapter>> | undefined;
  var __nerveSqlJsPreInitErrors: Map<string, string> | undefined;
}

function getSqlJsCache(): Map<string, SqliteAdapter> {
  if (!globalThis.__nerveSqlJsAdapters) {
    globalThis.__nerveSqlJsAdapters = new Map();
  }
  return globalThis.__nerveSqlJsAdapters;
}

function getSqlJsPreInitErrorCache(): Map<string, string> {
  if (!globalThis.__nerveSqlJsPreInitErrors) {
    globalThis.__nerveSqlJsPreInitErrors = new Map();
  }
  return globalThis.__nerveSqlJsPreInitErrors;
}
...
function getSqlJsPendingCache(): Map<string, Promise<SqliteAdapter>> {
  if (!globalThis.__nerveSqlJsInitPromises) {
    globalThis.__nerveSqlJsInitPromises = new Map();
  }
  return globalThis.__nerveSqlJsInitPromises;
}
