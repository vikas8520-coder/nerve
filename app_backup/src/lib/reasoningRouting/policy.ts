...
import { splitClaudeEffortSuffix } from "@nerve/open-sse/config/providerModels.ts";
...
export function attachReasoningRuleDirective(
  ...
  body._nerveReasoningRouteTrace = {
...
export function applyReasoningRuleDirective(bodyInput: unknown): unknown {
  const source = asRecord(bodyInput);
  const directive = asRecord(source._nerveReasoningRule);
  if (!directive.id) return bodyInput;
  const body = { ...source };
  delete body._nerveReasoningRule;
