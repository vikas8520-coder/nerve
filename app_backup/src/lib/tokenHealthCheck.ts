...
import {
  ...
  refreshCopilotToken,
} from "@nerve/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("NERVE_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * (e.g. "codex,openai"). Targeted alternative to NERVE_DISABLE_TOKEN_HEALTHCHECK:
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.NERVE_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("NERVE_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
function getHCState() {
  if (!globalThis.__nerveTokenHC) {
    globalThis.__nerveTokenHC = { initialized: false, interval: null, sweeping: false };
  }
  return globalThis.__nerveTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in Nerve
