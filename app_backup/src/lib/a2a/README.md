...
> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use Nerve as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes Nerve as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through Nerve intelligent pipeline",
...
Routes prompts through Nerve's intelligent pipeline with full observability.
...
Discovers Nerve agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use Nerve A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to Nerve, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via Nerve A2A
    ...
    # Step 2: Review the code via Nerve A2A (different model)
...
Multiple agents share quota through Nerve, using the quota skill to coordinate.
...
```
Authorization: Bearer YOUR_NERVE_API_KEY
```

If no API key is configured on the server (`NERVE_API_KEY` is empty), authentication is bypassed.
