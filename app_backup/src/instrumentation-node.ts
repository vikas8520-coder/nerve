...
/**
 * Rename a Node process title so Nerve is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `nerve${currentTitle.slice("next-server".length)}`;
}
...
function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.NERVE_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  ...
  // Rename the process title so Nerve is identifiable in ps/htop instead
  ...
  await import("@nerve/open-sse/index.ts");
  ...
  // See: https://github.com/diegosouzapw/Nerve/issues/3625 (Part A)
  try {
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@nerve/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } = await import("@nerve/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } =
      await import("@nerve/open-sse/services/thinkingBudget.ts");
    ...
    const { hydrateTaskRoutingConfig } =
      await import("@nerve/open-sse/services/taskAwareRouter.ts");
  ...
  // NERVE_DISABLE_CREDENTIAL_HEALTH_CHECK and its cadence is tunable via
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    await Promise.allSettled([
      ...
      import("@nerve/open-sse/services/autoRefreshDaemon")
      ...
      // Reads the schedule written by `nerve backup auto enable` and fires
      ...
      // flag-gated auto-start (NERVE_ENABLE_LIVE_WS, default ON).
