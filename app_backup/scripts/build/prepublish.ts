...
console.log("🔨 Nerve — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "nerve.ts");
const cliDestFile = join(ROOT, "bin", "nerve.mjs");
  ...
  try {
    execFileSync(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/nerve.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/nerve.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @nerve/opencode-plugin ──────────────
// The plugin ships bundled inside the nerve npm package (see root
// package.json "files": ["@nerve/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `nerve setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@nerve", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @nerve/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @nerve/opencode-plugin bundled to @nerve/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @nerve/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @nerve/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @nerve/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @nerve/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @nerve/opencode-plugin not found in workspace (skipping build)");
}
