 ...
 *
 * When `NERVE_BUILD_BACKEND_ONLY=1` (or `NERVE_BUILD_PROFILE=backend`) is set,
 * `build-next-isolated.mjs` calls `stubDashboardPages()` BEFORE `next build` and
 * `restoreDashboardPages()` in a `finally` afterward.
 *
 * WHY: Nerve embedders that only consume the HTTP API (`/api/*`, `/v1/*`, `/v1beta/*`)
...
export const BACKEND_ONLY_STUB_MARKER = "/* nerve:backend-only-stub (auto-restored after build) */";
...
/**
 * Strip a leading `"use server"` module directive. Some Nerve API Route Handlers
...
export function isBackendOnlyBuild(env = process.env) {
  return env.NERVE_BUILD_BACKEND_ONLY === "1" || env.NERVE_BUILD_PROFILE === "backend";
}
