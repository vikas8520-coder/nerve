...
const uninstallAlreadyInProgress =
  process.env.NERVE_SKIP_UNINSTALL_HOOK === "1" || process.env.npm_lifecycle_event === "uninstall";

console.log("🛑 Nerve Uninstaller");
console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

// 1. Stop PM2 process if it exists
try {
  console.log("Stopping and removing background PM2 processes...");
  execSync("pm2 delete nerve 2>/dev/null", { stdio: "ignore" });
} catch {
...
const dataDir = process.env.DATA_DIR || path.join(os.homedir(), ".nerve");
...
if (uninstallAlreadyInProgress) {
  ...
  try {
    execSync("npm uninstall -g nerve", {
      stdio: "inherit",
      env: {
        ...process.env,
        NERVE_SKIP_UNINSTALL_HOOK: "1",
      },
    });
    console.log("\n✅ Nerve has been successfully uninstalled from your system.");
    if (!fullUninstall) {
