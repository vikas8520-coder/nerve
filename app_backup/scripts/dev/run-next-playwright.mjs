...
const bootstrapMode = process.env.NERVE_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.NERVE_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "nerve-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_NERVE_E2E_MODE: process.env.NEXT_PUBLIC_NERVE_E2E_MODE || "1",
  NERVE_DISABLE_BACKGROUND_SERVICES: process.env.NERVE_DISABLE_BACKGROUND_SERVICES || "true",
  NERVE_DISABLE_TOKEN_HEALTHCHECK: process.env.NERVE_DISABLE_TOKEN_HEALTHCHECK || "true",
  NERVE_DISABLE_LOCAL_HEALTHCHECK: process.env.NERVE_DISABLE_LOCAL_HEALTHCHECK || "true",
  NERVE_HIDE_HEALTHCHECK_LOGS: process.env.NERVE_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        NERVE_E2E_PASSWORD: "",
    ...
    : {
        ...
        NERVE_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.NERVE_USE_TURBOPACK
    ? {
        NERVE_USE_TURBOPACK: process.env.NERVE_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.NERVE_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.NERVE_PLAYWRIGHT_SKIP_BUILD === "1") return;
