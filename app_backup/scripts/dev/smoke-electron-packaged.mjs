...
export const LINUX_EXECUTABLE_NAMES = ["nerve-desktop", "nerve", "Nerve"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "Nerve.app", "Contents", "MacOS", "Nerve"),
    join(distDir, "mac-arm64", "Nerve.app", "Contents", "MacOS", "Nerve"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "Nerve.exe"),
    join(distDir, "win-x64-unpacked", "Nerve.exe"),
    join(distDir, "win-arm64-unpacked", "Nerve.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["Nerve.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing Nerve process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["nerve-desktop", "Nerve", "nerve"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged Nerve executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    process.env.ELECTRON_SMOKE_DATA_DIR || (await mkdtemp(join(tmpdir(), "nerve-electron-smoke-")));
  const removeDataDir =
