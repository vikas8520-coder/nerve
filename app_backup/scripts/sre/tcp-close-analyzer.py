#!/usr/bin/env python3
"""
tcp-close-analyzer.py — dependency-free pcap analyzer for Nerve<->Caddy
...
(the nerve-dev container)?

Why this exists: the dashboard's HTTP-level status (499 "Request aborted")
only tells us Nerve's own executor detected a dropped connection — it
doesn't tell us whether the underlying TCP socket was actually closed by the
far end, or whether Nerve itself is the one tearing it down (e.g. an
idle/read timeout on Nerve's side that then gets misreported as a client
...
container's OWN namespace via its PID instead:
    ...
    PID=$(podman inspect nerve-dev --format '{{.State.Pid}}')
    sudo nsenter -t "$PID" -n tcpdump -i any -w /tmp/nerve-capture.pcap \\
        'host <nerve-container-ip> and port 20128'

Find the container's IP first with:
    podman inspect nerve-dev --format '{{.NetworkSettings.Networks}}'

That captures all traffic between Caddy and the nerve-dev container on
...
again), then Ctrl+C the tcpdump. Make the file readable:
    ...
    sudo chmod 644 /tmp/nerve-capture.pcap

Then run this script against it:

    python3 scripts/sre/tcp-close-analyzer.py /tmp/nerve-capture.pcap
...
<pcap>.streams.jsonl next to the input file), each with:
  ...
  - correlationId / requestId, if an `x-correlation-id:` / `x-request-id:`
    ...
    Nerve doesn't echo these on every hop, so this is a bonus, not the
...
That captures all traffic between Caddy and the nerve-dev container —
...
Then run:
    ...
    python3 scripts/sre/tcp-close-analyzer.py /tmp/nerve-capture.pcap --find "<marker>"
"""
    )
