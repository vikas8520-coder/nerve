...
# If NERVE_MEMORY_MB is set, build NODE_OPTIONS dynamically so the
# user can tune heap size via environment without editing the Dockerfile.
if [ -n "$NERVE_MEMORY_MB" ]; then
  export NODE_OPTIONS="${NODE_OPTIONS:-} --max-old-space-size=${NERVE_MEMORY_MB}"
fi

# Hard Rule #13: never interpolate NERVE_BASE_PATH (or any runtime path)
...
if [ -d "$DATA_PATH" ] && [ ! -w "$DATA_PATH" ]; then
  ...
  if [ "${CONTAINER_HOST:-}" = "podman" ]; then
    ...
    echo "  https://github.com/diegosouzapw/Nerve/blob/main/contrib/podman/README.md#data-directory-permissions-by-topology"
  else
