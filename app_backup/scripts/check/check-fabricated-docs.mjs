...
//   3. CLI commands `nerve ...`     → must exist in bin/cli/commands/ or bin/
...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not Nerve-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  "NERVE_BUILD_PROFILE", // build-time only
  "NERVE_BUILD_SHA",
  "NERVE_URL", // used by ad-hoc tooling, validated elsewhere
  "NERVE_KEY", // ditto
  ...
  // subprocess — never read via `process.env.X` in Nerve's own source, so the
  ...
  "OPENAI_BASE_URL", // env var Nerve passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not Nerve routes.
...
// Env var helper wrappers used across Nerve — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented Nerve env var (including external-CLI / docker / electron vars
  ...
  // Set of `nerve <subcommand>` strings that exist in bin/
...
const COARSE_PATTERNS = {
  ...
  // nerve <verb> <sub> ... — only on the same line, captures first 2 tokens
  cliCmd: /\bnerve\s+([a-z][a-z0-9-]+)(?:\s+([a-z][a-z0-9-]+))?/g,
...
export function scanDocFile(absPath, index, root = ROOT) {
  ...
  // 3) CLI commands: `nerve foo bar` — only flag when the line is in
  //    a code-like context (inside backticks or a shell block). Bare prose
  //    like "we use nerve and..." is not a command claim.
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.cliCmd)) {
    ...
    const isShellLike = /^[ \t]*\$\s|^```sh|^```bash|^```shell|`nerve/.test(lineText);
    ...
    findings.push({
      ...
      value: `nerve ${sub}`,
      line: ln,
      msg: `nerve subcommand '${sub}' not registered in bin/`,
    });
  }
  ...
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.fileRef)) {
    ...
    // (`@nerve/opencode-provider/src/index.ts`). Neither resolves from repo root.
...
export function formatHumanReport(result) {
  ...
  const KIND_LABELS = {
    ...
    "cli-cmd": "nerve subcommands not registered",
