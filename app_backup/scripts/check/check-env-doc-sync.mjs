 ...
 *
 * Enforces that every env var referenced in Nerve source code appears in
...
const IGNORE_FROM_CODE = new Set([
  ...
  "LOCALAPPDATA",
  "XDG_CONFIG_HOME",
  // XDG Base Directory cache root — read (never defined by Nerve) so the
  // Android/Termux serve path can honor an operator-set cache location (#8519).
  "XDG_CACHE_HOME",
  "USERPROFILE",
  "PREFIX",
  // X11 display server — set by the OS/session manager, not Nerve config.
  "DISPLAY",
  ...
  // bundler is active — framework-internal. The Nerve-facing knob is
  // NERVE_USE_TURBOPACK (scripts/dev/run-next.mjs), which IS documented.
  "TURBOPACK",
  "NODE_TEST_CONTEXT",
  "VITEST",
  // Instruction snippet shown to users (Traffic Inspector HttpProxySnippetCard) — not Nerve config.
  ...
  // points at Nerve). A downstream client-tool var, not an Nerve server
  ...
  // (scripts/check/check-openapi-breaking.mjs) — a build/check signal, not Nerve runtime config.
  "BASE_REF",
  ...
  // pull_request.{head,base}.sha — never Nerve runtime config, and meaningless in a .env.
  "HEAD_REF",
  "HEAD_SHA",
  "BASE_SHA",
  ...
  // a CI-only signal, never an Nerve runtime config (Phase 7.10).
  "PR_BODY",
  // CLI machine-id token opt-out (server-side flag; not user-configurable via .env).
  "NERVE_DISABLE_CLI_TOKEN",
  ...
  // ("http://192.168.0.15:20128" / null), never Nerve runtime config (#5151).
  "COMBO_LIVE_BASE_URL",
  "COMBO_LIVE_API_KEY",
  ...
  // These are external signals set by the host OS or cloud provider — not Nerve config.
  "CODESPACES",
  ...
  // internals, never Nerve runtime config.
  "VERIFY_DEADLINE_S",
  "VERIFY_PORT",
  "VERIFY_VERSION",
  // Doctor diagnostic flags (no runtime behavior yet — placeholders).
  "NERVE_DOCTOR_HOST",
  "NERVE_DOCTOR_LIVENESS_URL",
  "NERVE_PROVIDER_CATALOG_PATH",
  "NERVE_PROVIDER_TEST_MODEL",
  // Test-only opt-out: instructs bin/nerve.mjs to skip auto-loading the
  // repository .env so isolation tests get a deterministic environment.
  "NERVE_CLI_SKIP_REPO_ENV",
  ...
  // A dev/ops measurement tool, never Nerve runtime config.
  "NERVE_EVAL_CREDENTIALS",
  ...
  "NERVE_INTERNAL_SCHEME",
  // Source typo / placeholder.
  "OMNIROUT",
  // Static config alias path (the canonical var is NERVE_PAYLOAD_RULES_PATH).
  "PAYLOAD_RULES_PATH",
  // Node.js module resolution path — OS/Node internal, not an Nerve config var.
  ...
  "NVIDIA_BASE_URL",
  "NVIDIA_MODEL",
  // XDG standard data directory — set by OS/desktop session, not Nerve config.
