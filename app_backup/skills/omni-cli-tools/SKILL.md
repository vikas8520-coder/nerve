---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Nerve's automation surface."
---
...
Read Crush CLI Nerve config

Local-only. Reads the Nerve provider block in Crush's config.
...
Write Crush CLI Nerve config

Local-only. Registers Nerve as an `openai-compat` provider in Crush's config.
...
Remove Nerve from Crush CLI config

Local-only. Removes the Nerve provider block from Crush's config.
...
Read CodeWhale CLI Nerve config

Local-only. Reads the Nerve config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI Nerve config

Local-only. Writes the Nerve config block in CodeWhale TOML format.
...
Remove Nerve from CodeWhale CLI config

Local-only. Removes the Nerve config block from CodeWhale's config.
