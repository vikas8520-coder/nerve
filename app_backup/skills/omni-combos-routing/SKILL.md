...
<!-- skill:custom-start -->
<!-- Migrated from skills/nerve-routing/SKILL.md (preserved curated content) -->

# Nerve — Routing & Combos
...
```bash
curl -X POST $NERVE_URL/api/combos \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-combo",
...
```bash
# Activate
curl -X PUT $NERVE_URL/api/combos/{id}/toggle \
  -H "Authorization: Bearer $NERVE_KEY" \
  -d '{ "enabled": true }'
```
...
```bash
curl $NERVE_URL/api/combos/{id}/metrics \
  -H "Authorization: Bearer $NERVE_KEY"
```
...
```bash
curl -X POST $NERVE_URL/api/routing/simulate \
  -H "Authorization: Bearer $NERVE_KEY" \
  -H "Content-Type: application/json" \
...
```
nerve_list_combos     → list all combos
nerve_switch_combo    → enable/disable a combo
nerve_set_routing_strategy → change strategy at runtime
nerve_simulate_route  → dry-run routing decision
nerve_best_combo_for_task → get recommendation by task type
```
