/**
 * Nerve Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead nerve:// protocol (no handler existed)
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. Nerve.app/Contents/MacOS/Nerve). Spawning it with
  ...
  if (process.platform === "darwin" && !isDev) {
    ...
    // ("Nerve"), whereas app.getName() reads package.json `name` ("nerve-desktop")
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "nerve");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "nerve");

  return path.join(require("os").homedir(), ".nerve");
}
    ...
    new Promise((r) =>
      setTimeout(() => {
        try {
          ...
          // bare SIGKILL of the direct child, keeping nerve.exe locked).
...
function installUpdate() {
  if (nextServer) {
    ...
    // grandchildren) keeps nerve.exe locked and the updater fails with "file in use".
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* wss://localhost:* wss://127.0.0.1:* https://*.nerve.online https://*.nerve.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.NERVE_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by Nerve bootstrap",
        "",
  ...
  // NERVE_MEMORY_MB or a pre-set --max-old-space-size still wins. Mirrors
  ...
  const serverNodeOptions = (() => {
    ...
    const explicit = parseInt(serverEnv.NERVE_MEMORY_MB, 10);
    let heapMb;
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      const isHeadless =
        ...
        process.env.NERVE_HEADLESS === "true";
      if (isHeadless && !global.loggedHeadlessReady) {
        global.loggedHeadlessReady = true;
        console.log("\n\x1b[32m✔ Nerve Headless CLI Server is ready and listening!\x1b[0m");
...
function stopNextServer() {
  if (nextServer) {
    ...
    // (nerve.exe-as-node) spawns grandchildren that a bare SIGTERM leaves alive,
    // holding a lock on nerve.exe and blocking updates.
    killProcessTree(nextServer, { signal: "SIGTERM" });
    nextServer = null;
  }
}
...
function enableLinuxDesktopAutostart() {
  try {
    ...
    fs.writeFileSync(path.join(autostartDir, "nerve-desktop.desktop"), desktopFileContent, {
      mode: 0o644,
    });
    return true;
  } catch (err) {
...
function disableLinuxDesktopAutostart() {
  try {
    ...
    const fs = require("fs");
    const path = require("path");
    const desktopPath = path.join(os.homedir(), ".config", "autostart", "nerve-desktop.desktop");
...
function isLinuxDesktopAutostartEnabled() {
  try {
    ...
    const fs = require("fs");
    const path = require("path");
    return fs.existsSync(path.join(os.homedir(), ".config", "autostart", "nerve-desktop.desktop"));
  } catch {
    return false;
  }
}
