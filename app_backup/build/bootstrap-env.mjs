 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.nerve/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by Nerve bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/nerve.mjs`.
  //
  ...
  if (needsPersist) {
    merged.NERVE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "nerve-field-encryption-v1", 32)
              const legacySalt = "nerve-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.nerve/server.env or ~/.nerve/.env,"
                );
                log(
                  "   or run: nerve reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
