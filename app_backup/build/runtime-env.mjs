 ...
 * `NERVE_MEMORY_MB`, mirroring `nerve serve`. Clamped to [64, 16384];
 * invalid/unset → fallback (512). The standalone launcher uses this so
 * NERVE_MEMORY_MB can override the Docker image's NODE_OPTIONS fallback
 ...
 * when `NERVE_MEMORY_MB` is unset. A fixed 512MB default crashed boxes with
 ...
 * explicit NERVE_MEMORY_MB override always wins.
 ...
 * (`--max-old-space-size=…`). Used to decide whether `nerve serve` may
 ...
 * the user already exported. #5238: `nerve serve` used to UNCONDITIONALLY
