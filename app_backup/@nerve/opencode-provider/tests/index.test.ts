...
import {
  buildNerveOpenCodeConfig,
  createNerveAgentBlock,
  createNerveComboConfig,
  createNerveMCPEntry,
  createNerveModesBlock,
  createNerveProvider,
  fetchLiveModels,
  listCombos,
  mergeIntoExistingConfig,
  normalizeBaseURL,
  NERVE_DEFAULT_MODEL_CAPABILITIES,
  NERVE_DEFAULT_MODEL_CONTEXT_LENGTHS,
  NERVE_DEFAULT_OPENCODE_MODELS,
  NERVE_MCP_DEFAULT_SCOPES,
  NERVE_PROVIDER_NPM,
...
test("createNerveProvider validates required fields", () => {
  assert.throws(
    () => createNerveProvider({ baseURL: "", apiKey: "x" } as never),
    /baseURL is required/
  );
  assert.throws(
    () => createNerveProvider({ baseURL: "http://x", apiKey: "" } as never),
    /apiKey is required/
  );
});

test("createNerveProvider produces the OpenCode-compatible shape", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  });

  assert.equal(provider.npm, NERVE_PROVIDER_NPM);
  assert.equal(provider.name, "Nerve");
  assert.equal(provider.options.baseURL, "http://localhost:20128/v1");
  assert.equal(provider.options.apiKey, "sk_nerve");
  assert.equal(typeof provider.models, "object");
});

test("createNerveProvider seeds the default model catalog", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  });
  ...
  const defaultIds = [...NERVE_DEFAULT_OPENCODE_MODELS].sort();
...
test("createNerveProvider honours a custom models list and labels", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
...
test("createNerveProvider deduplicates and trims model ids", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
...
test("createNerveProvider honours displayName override", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
    displayName: "Local Nerve",
  });
  assert.equal(provider.name, "Local Nerve");
});

test("buildNerveOpenCodeConfig wraps the provider with the OpenCode schema", () => {
  const doc = buildNerveOpenCodeConfig({
    baseURL: "http://localhost:20128/v1",
    apiKey: "sk_nerve",
  });

  assert.equal(doc.$schema, OPENCODE_CONFIG_SCHEMA);
  assert.equal(typeof doc.provider.nerve, "object");
  assert.equal(doc.provider.nerve.options.baseURL, "http://localhost:20128/v1");
});
...
test("buildNerveOpenCodeConfig emits model and small_model prefixed with provider key", () => {
  const doc = buildNerveOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  ...
  assert.equal(doc.model, "nerve/claude-sonnet-4-5-thinking");
  assert.equal(doc.small_model, "nerve/gemini-3-flash");
});

test("buildNerveOpenCodeConfig omits model and small_model when not supplied", () => {
  const doc = buildNerveOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  });
...
test("buildNerveOpenCodeConfig ignores blank model strings", () => {
  const doc = buildNerveOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
    model: "   ",
...
test("mergeIntoExistingConfig preserves existing provider entries", () => {
  ...
  assert.ok("nerve" in (result.provider as Record<string, unknown>));
  assert.deepEqual((result as Record<string, unknown>).keybinds, { submit: "enter" });
});

test("mergeIntoExistingConfig overwrites existing nerve entry", () => {
  const existing = {
    provider: {
      nerve: {
  ...
  const nerve = (result.provider as Record<string, unknown>).nerve as { name: string };
  assert.equal(nerve.name, "NEW");
});
...
test("mergeIntoExistingConfig writes model and small_model when supplied", () => {
  ...
  assert.equal(result.model, "nerve/claude-sonnet-4-5-thinking");
  assert.equal(result.small_model, "nerve/gemini-3-flash");
});

test("mergeIntoExistingConfig does not add model keys when not supplied", () => {
  const result = mergeIntoExistingConfig(
    {},
    { baseURL: "http://localhost:20128", apiKey: "sk_nerve" }
  );
  assert.ok(!("model" in result));
  assert.ok(!("small_model" in result));
});

test("NERVE_MCP_DEFAULT_SCOPES contains 7 read-only scopes", () => {
  assert.equal(NERVE_MCP_DEFAULT_SCOPES.length, 7);
  assert.ok(NERVE_MCP_DEFAULT_SCOPES.every((s) => s.startsWith("read:")));
});

test("createNerveMCPEntry defaults to tsx runtime", () => {
  const entry = createNerveMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_nerve",
  });
  assert.equal(entry.command, "npx");
  assert.deepEqual(entry.args, ["tsx", "/path/to/server.ts"]);
  assert.equal(entry.env.NERVE_API_KEY, "sk_nerve");
  assert.ok(!("NERVE_MCP_ENFORCE_SCOPES" in entry.env));
  assert.ok(!("NERVE_MANAGEMENT_API_KEY" in entry.env));
});

test("createNerveMCPEntry uses node runtime when specified", () => {
  const entry = createNerveMCPEntry({
...
test("createNerveMCPEntry sets management key and scopes when supplied", () => {
  const entry = createNerveMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_nerve",
  ...
  assert.equal(entry.env.NERVE_MANAGEMENT_API_KEY, "sk_manage");
  assert.equal(entry.env.NERVE_MCP_ENFORCE_SCOPES, "true");
  assert.equal(entry.env.NERVE_MCP_SCOPES, "read:health,read:combos,execute:completions");
});

test("createNerveMCPEntry rejects missing required fields", () => {
  assert.throws(
    () => createNerveMCPEntry({ serverPath: "", apiKey: "x" }),
    /serverPath is required/
  );
  assert.throws(() => createNerveMCPEntry({ serverPath: "/p", apiKey: "" }), /apiKey is required/);
});
...
test("createNerveComboConfig builds minimal payload", () => {
  const payload = createNerveComboConfig({ name: "my-combo", strategy: "priority" });
...
test("createNerveComboConfig includes optional fields when supplied", () => {
...
test("NERVE_DEFAULT_OPENCODE_MODELS includes cc/ prefixed models", () => {
  const defaults = [...NERVE_DEFAULT_OPENCODE_MODELS];
...
test("NERVE_DEFAULT_MODEL_CONTEXT_LENGTHS covers every default model id", () => {
  for (const id of NERVE_DEFAULT_OPENCODE_MODELS) {
    const ctx = NERVE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.ok(
...
test("createNerveProvider emits limit.context on default model entries", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  });
...
test("createNerveProvider omits limit.context for unknown model ids", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
...
test("createNerveProvider reads contextLength from a live model entry for ids absent from the static map", () => {
  // #3298 regression guard: the static NERVE_DEFAULT_MODEL_CONTEXT_LENGTHS
...
test("createNerveProvider: a live model contextLength wins over the static default map", () => {
...
test("createNerveProvider serialises limit.context to JSON", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  });
  ...
  for (const id of NERVE_DEFAULT_OPENCODE_MODELS) {
    const expectedContext = NERVE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.equal(
...
test("NERVE_DEFAULT_MODEL_CAPABILITIES covers every default model id", () => {
  for (const id of NERVE_DEFAULT_OPENCODE_MODELS) {
    const caps = NERVE_DEFAULT_MODEL_CAPABILITIES[id];
...
test("createNerveProvider emits default capability flags inline with the model entry", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
  });
...
test("createNerveProvider modelCapabilities overrides defaults and merges per id", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
    modelCapabilities: {
...
test("createNerveProvider applies capability overrides to non-default model ids", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
    models: ["custom-model"],
...
test("createNerveProvider modelLabels still works when modelCapabilities omits label", () => {
  const provider = createNerveProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nerve",
...
test("createNerveAgentBlock builds provider-prefixed entries per role", () => {
  const block = createNerveAgentBlock({
    roles: {
  ...
  assert.equal(block.build.model, "nerve/claude-sonnet-4-5-thinking");
  assert.equal(block.build.temperature, 0.2);
  assert.equal(block.plan.model, "nerve/claude-opus-4-5-thinking");
  assert.equal(block.plan.top_p, 0.95);
  assert.equal(block.review.model, "nerve/gemini-3-flash");
  assert.equal(block.review.temperature, 0.0);
});

test("createNerveAgentBlock omits optional fields when not supplied", () => {
  const block = createNerveAgentBlock({
...
test("createNerveAgentBlock skips roles with empty modelId", () => {
  const block = createNerveAgentBlock({
    roles: {
...
test("createNerveAgentBlock emits tools as Record<string, boolean> per OC schema", () => {
  const block = createNerveAgentBlock({
    roles: {
      build: {
...
test("createNerveAgentBlock filters invalid tool entries and omits empty maps", () => {
  const block = createNerveAgentBlock({
    roles: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
test("createNerveModesBlock builds provider-prefixed mode entries", () => {
  const block = createNerveModesBlock({
    modes: {
...
test("createNerveModesBlock skips modes with empty modelId", () => {
  const block = createNerveModesBlock({
    modes: {
      build: { modelId: "claude-sonnet-4-5-thinking" },
      plan: { modelId: "" },
...
test("createNerveModesBlock honours numeric overrides limited to OC schema", () => {
  const block = createNerveModesBlock({
    modes: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
// #3419 — soft-deprecation in favour of @nerve/opencode-plugin. Guard the
...
test("package is marked deprecated in favour of @nerve/opencode-plugin (#3419)", () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const pkg = JSON.parse(readFileSync(join(here, "..", "package.json"), "utf8"));
  assert.match(pkg.description, /DEPRECATED/);
  assert.match(pkg.description, /@nerve\/opencode-plugin/);

  const readme = readFileSync(join(here, "..", "README.md"), "utf8");
  assert.match(readme, /Deprecated/i);
  assert.match(readme, /@nerve\/opencode-plugin/);
});
