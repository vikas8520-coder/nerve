...
export function createNerveProvider(options: NerveProviderOptions): OpenCodeProviderEntry {
  ...
  for (const raw of modelList) {
    ...
    const rawContextLength =
      liveContext ?? options.modelContextLengths?.[id] ?? NERVE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
 ...
 */
export function buildNerveOpenCodeConfig(options: NerveProviderOptions): OpenCodeConfigDocument {
 ...
 * ```
 */
export function createNerveComboConfig(options: NerveComboConfigOptions): Record<string, unknown> {
