/**
 * OpenCode plugin for the Nerve AI Gateway.
 *
 * Implements the official `@opencode-ai/plugin` Plugin contract (auth +
 * provider + config hooks) to drive a running Nerve instance from
 ...
 *                  pass-through (Nerve is the source of truth — no
 ...
 *
 *  1. Single-instance (default `providerId: "nerve"`):
 *
 *     ```json
 *     {
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": ["@nerve/opencode-plugin"]
 *     }
 *     ```
 *
 ...
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": [
 *         ["@nerve/opencode-plugin", { "providerId": "nerve" }],
 *         ["@nerve/opencode-plugin", { "providerId": "nerve-preprod" }]
 *       ]
 *     }
 *     ```
 *
 ...
 *
 * Companion library: `@nerve/opencode-provider` (build-time config generator)
 ...
 * @see https://github.com/diegosouzapw/Nerve for the AI Gateway.
 */
 ...
 *                     different `providerId` ("nerve", "nerve-preprod",
 *                     "nerve-local"). Maps to `ProviderHook.id` and
 ...
 *  - `baseURL`        Override base URL for this Nerve instance. When
 ...
 *                           JSONL file at
 *                           `~/.local/share/opencode/plugins/nerve-debug-{providerId}.jsonl`.
 ...
 *                           (covers Nerve's canonical Anthropic aliases).
 *
 ...
 * import `NervePluginOptions` as before; only the source of truth
 * shifted from a hand-written interface to `z.infer<typeof optionsSchema>`.
 */
export type NervePluginOptions = z.infer<typeof optionsSchema>;
 ...
 */
export const NERVE_FEATURE_DEFAULTS = {
...
/** Union of the boolean feature-flag keys declared in `NERVE_FEATURE_DEFAULTS`. */
export type NerveFeatureFlag = keyof typeof NERVE_FEATURE_DEFAULTS;
...
export function resolveEffectiveFeatureFlags(
  features?: NervePluginOptions["features"]
): Record<NerveFeatureFlag, boolean> {
  const f: Partial<Record<NerveFeatureFlag, boolean>> = features ?? {};
  const out = {} as Record<NerveFeatureFlag, boolean>;
  for (const key of Object.keys(NERVE_FEATURE_DEFAULTS) as NerveFeatureFlag[]) {
    const val = f[key];
    out[key] = typeof val === "boolean" ? val : NERVE_FEATURE_DEFAULTS[key];
  }
  return out;
}
 ...
 */
export function resolveNervePluginOptions(opts?: NervePluginOptions): Required<
  Pick<NervePluginOptions, "providerId" | "displayName" | "modelCacheTtl" | "autoSyncIntervalMs">
> & {
  /**
   * #6859: the UNPREFIXED provider id ("nerve", "nerve-preprod", …).
   ...
   * `provider.<id>` block). `nerveProviderId` MUST be used everywhere an
   * identifier reaches or represents something Nerve's own server parses
   * (model `id` prefix, `ModelV2.providerID`, combo catalog keys in the
   * dynamic provider hook) — Nerve's `parseModel()` has no alias for
   * "opencode-<x>", so a prefixed id there is unrecoverable and credential
   * lookup fails with "No credentials for opencode-<x>".
   */
  nerveProviderId: string;
} & Pick<NervePluginOptions, "baseURL" | "managementReadToken" | "features"> {
  const rawProviderId = opts?.providerId ?? NERVE_PROVIDER_KEY;
  const nerveProviderId = trimLeadingOpencodePrefix(rawProviderId);
  // OC 1.17.8+ native-adapter gate rejects providerID not in
  // {openai, anthropic, opencode*}. Silently prefix so existing
  // configs (providerId: "nerve") keep working.
  const providerId = rawProviderId.startsWith("opencode-")
    ? rawProviderId
    : `opencode-${rawProviderId}`;
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${NERVE_PROVIDER_KEY}` ? "Nerve" : `Nerve (${providerId})`);
  const modelCacheTtl =
...
export type ResolvedNervePluginOptions = ReturnType<typeof resolveNervePluginOptions>;

/**
 * Strip a leading "opencode-" prefix (added only for the OC native-adapter
 * gate — see `resolveNervePluginOptions`) so the returned id is safe to
 * embed in anything Nerve's own server parses. A user-supplied
 * `providerId: "opencode-nerve"` (already prefixed) resolves to the same
 * unprefixed "nerve" as the default, matching `providerId`'s own
 ...
 * direct factory call) into the validated `NervePluginOptions` shape.
 *
 ...
 *
 * Validation happens at plugin invocation time (inside `NervePlugin`),
 ...
 */
export function parseNervePluginOptions(opts: unknown): NervePluginOptions {
  ...
  if (!result.success) {
    const errs = result.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid @nerve/opencode-plugin options: ${errs}`);
  }
  return result.data;
}

/**
 * Internal coercion shim. Delegates to `parseNervePluginOptions` to keep
 ...
 */
function coercePluginOptions(opts?: PluginOptions): NervePluginOptions {
  return parseNervePluginOptions(opts);
}
 ...
 * Covers Nerve's canonical Anthropic aliases: `cc/`, `claude/`,
 ...
 *     the multi-instance bug in opencode-nerve-auth@1.2.1 which pinned
 *     `NERVE_PROVIDER_ID = "nerve"` at module scope).
 *   - `methods[0]` is the `api` flavor (no OAuth flow; Nerve issues bearer
 ...
 */
export function createNerveAuthHook(opts?: NervePluginOptions): AuthHook {
  const { providerId, displayName, baseURL, features } = resolveNervePluginOptions(opts);
        ...
        prompts: [
          {
            type: "text",
            key: "apiKey",
            message: `Nerve API key (${providerId})`,
          },
        ],
      },
    ],
    loader: async (getAuth, _provider) => {
      ...
      if (
        ...
        if (wantFetchInterceptor) {
          composedFetch = createNerveFetchInterceptor({
            apiKey,
 ...
 */
export function invalidateNerveFetchCache(cache: NerveFetchCache, baseURL?: string): number {
  if (!baseURL) {
    const n = cache.size;
    cache.clear();
    return n;
 ...
 */
export async function resolveNerveRuntimeAuth(
  resolved: ResolvedNervePluginOptions,
  readAuthJson?: NerveReadAuthJson
  ...
  if (resolved.nerveProviderId && !lookupKeys.includes(resolved.nerveProviderId)) {
    lookupKeys.push(resolved.nerveProviderId);
  }
...
/**
 * Force-refresh Nerve catalog: clear memory + disk cache, re-fetch /v1/models
 * (and optional management endpoints), and repopulate the shared cache.
 * OpenCode equivalent of Pi `/omni sync`.
 */
export async function forceSyncNerveModels(args: {
  resolved: ResolvedNervePluginOptions;
  cache: NerveFetchCache;
  readAuthJson?: NerveReadAuthJson;
  fetcher?: NerveModelsFetcher;
  combosFetcher?: NerveCombosFetcher;
  autoCombosFetcher?: NerveAutoCombosFetcher;
  enrichmentFetcher?: NerveEnrichmentFetcher;
  compressionMetaFetcher?: NerveCompressionMetaFetcher;
  providersFetcher?: NerveProvidersFetcher;
  now?: () => number;
}): Promise<{
  ok: boolean;
  count: number;
  ...
  const fetcher = args.fetcher ?? defaultNerveModelsFetcher;
  const combosFetcher = args.combosFetcher ?? defaultNerveCombosFetcher;
  const autoCombosFetcher = args.autoCombosFetcher ?? defaultNerveAutoCombosFetcher;
  const enrichmentFetcher = args.enrichmentFetcher ?? defaultNerveEnrichmentFetcher;
  const compressionMetaFetcher = args.compressionMetaFetcher ?? defaultNerveCompressionMetaFetcher;
  const providersFetcher = args.providersFetcher ?? defaultNerveProvidersFetcher;
  ...
  const auth = await resolveNerveRuntimeAuth(resolved, args.readAuthJson ?? defaultReadAuthJson);
  if (!auth) {
    return {
      ok: false,
      count: 0,
      combos: 0,
      provider: resolved.nerveProviderId,
      clearedMemory: 0,
      clearedDisk: false,
      error:
        "No Nerve credentials/baseURL available. Run `opencode connect nerve` or set plugin baseURL.",
    };
  }

  const clearedMemory = invalidateNerveFetchCache(cache, auth.baseURL);
  // Clear residual entries from prior baseURL history as well.
  const clearedAll = invalidateNerveFetchCache(cache);
  let clearedDisk = false;
  if (wantDiskCache) {
    clearedDisk = await clearDiskSnapshot(resolved.providerId);
    if (resolved.nerveProviderId !== resolved.providerId) {
      clearedDisk = (await clearDiskSnapshot(resolved.nerveProviderId)) || clearedDisk;
    }
  }

  try {
    const rawModels = await fetcher(auth.baseURL, auth.apiKey, 10_000);
    let rawCombos: NerveRawCombo[] = [];
    if (wantCombos) {
      try {
        rawCombos = await combosFetcher(auth.baseURL, auth.managementReadToken, 10_000);
      } catch (err) {
        console.warn("[nerve-plugin] force sync: combos fetch failed", err);
      }
    }
    let rawAutoCombos: NerveRawAutoCombo[] = [];
    if (wantAutoCombos) {
      try {
    ...
    let rawEnrichment: NerveEnrichmentMap = new Map();
    if (wantEnrichment) {
      try {
    ...
    let rawCompressionCombos: NerveCompressionCombo[] = [];
    ...
    let rawConnections: NerveProviderConnection[] = [];
    if (wantUsableOnly) {
      try {
    ...
    console.warn(
      `[nerve-plugin] force sync ok providerId=${resolved.providerId} ` +
...
export function createNerveSyncModelsTool(args: {
  resolved: ResolvedNervePluginOptions;
  cache: NerveFetchCache;
  ...
  return tool({
    description:
      "Force-refresh the Nerve model catalog (OpenCode equivalent of Pi `/omni sync`). " +
    ...
    async execute(toolArgs) {
      const result = await forceSyncNerveModels({ resolved, cache });
      const reason = toolArgs.reason ? ` reason=${toolArgs.reason}` : "";
      if (!result.ok) {
        return {
          title: "Nerve sync failed",
          output:
            `Nerve model sync failed for provider=${result.provider}.${reason}\n` +
            `${result.error ?? "unknown error"}`,
          metadata: result,
        };
      }
      return {
        title: `Nerve sync: ${result.count} models`,
        output:
          `Nerve models synced.` +
 ...
 */
export function startNerveAutoSync(args: {
  resolved: ResolvedNervePluginOptions;
  cache: NerveFetchCache;
  intervalMs?: number;
}): () => void {
  ...
  const tick = () => {
    ...
    inFlight = (async () => {
      ...
      if (!result.ok) {
        console.warn(
          `[nerve-plugin] auto-sync failed providerId=${resolved.providerId}: ${result.error}`
        );
        return;
      }
      ...
      if (result.count !== lastCount) {
        console.warn(
          `[nerve-plugin] auto-sync catalog size changed ${lastCount} → ${result.count} ` +
      ...
      .catch((err) => {
        console.warn("[nerve-plugin] auto-sync tick error", err);
      })
      .finally(() => {
        inFlight = null;
      });
  ...
  console.warn(
    `[nerve-plugin] auto-sync enabled intervalMs=${intervalMs} providerId=${resolved.providerId}`
  );
...
export const NervePlugin: Plugin = async (_input, options) => {
  const resolved = resolveNervePluginOptions(coercePluginOptions(options));
  ...
  // Each `NervePlugin(...)` invocation gets its OWN cache via closure,
  // so prod + preprod side-by-side instances do NOT collide.
  const sharedCache: NerveFetchCache = new Map();
  ...
  startNerveAutoSync({ resolved, cache: sharedCache });

  const syncTool = createNerveSyncModelsTool({ resolved, cache: sharedCache });
  const bareProviderId = resolved.nerveProviderId;
  ...
  const baseConfigHook = createNerveConfigHook(resolved, { cache: sharedCache });
  const configWithSyncCommand = async (input: Config) => {
    ...
    if (!cfg.command["omni-sync"]) {
      cfg.command["omni-sync"] = {
        description: "Force-refresh Nerve model catalog (like Pi /omni sync)",
        template:
          `Force-refresh the Nerve model catalog now using the nerve_sync_models tool ` +
    ...
    if (!cfg.command["omni-autosync"]) {
      cfg.command["omni-autosync"] = {
        description: "Show Nerve auto-sync / cache status",
        template:
          `Report Nerve discovery status for provider ${bareProviderId}: ` +
          `autoSyncIntervalMs=${resolved.autoSyncIntervalMs}, modelCacheTtl=${resolved.modelCacheTtl}. ` +
          `If the user asked to refresh now, call nerve_sync_models.`,
      };
    }
  };

  return {
    auth: createNerveAuthHook(resolved),
    provider: createNerveProviderHook(resolved, { cache: sharedCache }),
    config: configWithSyncCommand,
    tool: {
      nerve_sync_models: syncTool,
    },
  };
};
 ...
 * constants (NERVE_PROVIDER_KEY, DEFAULT_MODEL_CACHE_TTL_MS) + types +
 ...
 */
const NerveV1Plugin = {
  id: "@nerve/opencode-plugin",
  server: NervePlugin,
};

export default NerveV1Plugin;
...
/**
 * Raw shape of a `/v1/models` entry from Nerve. Captured verbatim from
 ...
 */
export interface NerveRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * Nerve instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@nerve/opencode-provider`'s
 ...
 */
export type NerveModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<NerveRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope Nerve
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future Nerve build trims the wrapper). Anything that
 * isn't an object with a string `id` is filtered out silently.
 */
export const defaultNerveModelsFetcher: NerveModelsFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@nerve/opencode-plugin: apiKey required to fetch /v1/models");
  if (!baseURL) throw new Error("@nerve/opencode-plugin: baseURL required to fetch /v1/models");
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`@nerve/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`);
    }
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: NerveRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as NerveRawModelEntry);
      }
    }
    return out;
  } finally {
 ...
 *   5. `thinking` from Nerve has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from Nerve has no ModelV2 slot — dropped (the
 ...
 *   7. `temperature: true` per Nerve convention (OpenAI-compat mode
 ...
 *      Unknown strings (future Nerve additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — Nerve doesn't tier models alpha/beta on
...
/**
 * Raw shape of a single combo entry as returned by Nerve's `/api/combos`.
 *
 * Schema established via a live probe against
 * an Nerve `/api/combos` endpoint with a management-scoped key
 ...
 * bare array envelope so the plugin keeps working if a future Nerve
 * build trims the wrapper (mirrors the same pattern in the sibling
 * `@nerve/opencode-provider#listCombos`).
 *
 ...
 */
export interface NerveRawComboMemberRef {
...
export interface NerveRawCombo {
  id: string;
  name?: string;
  ...
  models?: NerveRawComboMemberRef[];
  /** Hidden combos are excluded from the OC model picker. */
  isHidden?: boolean;
  /** When Nerve attaches a lifecycle hint we forward it; today it doesn't. */
  release_date?: string;
  /**
 ...
 * `NerveModelsFetcher` so unit tests can inject a stub instead of
 * monkey-patching global `fetch`.
 */
export type NerveCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<NerveRawCombo[]>;
 ...
 * plugin working if a future Nerve build trims the wrapper).
 *
 * Differences from `defaultNerveModelsFetcher`:
 ...
 */
export const defaultNerveCombosFetcher: NerveCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@nerve/opencode-plugin: apiKey required to fetch /api/combos");
  if (!baseURL) throw new Error("@nerve/opencode-plugin: baseURL required to fetch /api/combos");
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ...
        : [];
    const out: NerveRawCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as NerveRawCombo);
      }
    }
    return out;
  } finally {
...
/**
 * Raw shape of an auto combo entry as returned by Nerve's
 ...
 */
export interface NerveRawAutoCombo {
  ...
  /** MAX of candidates' context windows, served by newer Nerve builds.
 ...
 */
export type NerveAutoCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<NerveRawAutoCombo[]>;
 ...
 */
export const defaultNerveAutoCombosFetcher: NerveAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000
) => {
  ...
  try {
    ...
    if (res.status === 404) {
      console.warn(`[nerve-plugin] /api/combos/auto not available (404) — auto combos disabled`);
      return [];
    }
    if (!res.ok) {
      console.warn(
        `[nerve-plugin] /api/combos/auto failed: ${res.status} ${res.statusText} — auto combos disabled`
      );
      return [];
    }
    const body = (await res.json()) as unknown;
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ? ((body as { combos: unknown[] }).combos as unknown[])
        : [];
    const out: NerveRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as NerveRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
    ...
    console.warn(
      `[nerve-plugin] /api/combos/auto fetch failed: ${err instanceof Error ? err.message : String(err)} — auto combos disabled`
    );
    return [];
  } finally {
    clearTimeout(timer);
 ...
 * Nerve builds). MUST be positive: OpenCode's overflow guard treats
 ...
 */
export function mapAutoComboToStaticEntry(autoCombo: NerveRawAutoCombo): NerveStaticModelEntry {
...
/**
 * Per-model enrichment overlay derived from Nerve's
 ...
 */
export interface NerveEnrichmentEntry {
   ...
   * Populated by `defaultNerveEnrichmentFetcher` from
...
export type NerveEnrichmentMap = Map<string, NerveEnrichmentEntry>;

export type NerveEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<NerveEnrichmentMap>;
 ...
 * other.
 */
export const defaultNerveEnrichmentFetcher: NerveEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const out: NerveEnrichmentMap = new Map();
    ...
    if (res.ok) {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          // Nerve hasn't curated a label for this slot.
          ...
          for (const m of models) {
            ...
            const entry: NerveEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<NerveEnrichmentEntry["pricing"]> = {};
            // Nerve `/api/pricing` keys:
            //   input         → cost.input
            //   output        → cost.output
...
async function writeStartupDiagnostics(params: {
  ...
  enrichment: NerveEnrichmentMap;
  autoCombos: NerveRawAutoCombo[];
  features?: NervePluginOptions["features"];
}): Promise<void> {
  const {
    providerId,
  ...
  lines.push(
    `features(effective): ` +
      (Object.keys(effectiveFlags) as NerveFeatureFlag[])
  ...
  try {
    ...
    const diagPath = path.join(diagDir, "plugins", "nerve-startup-diagnostics.log");
    let existing = "";
    try {
...
// Re-export from naming.ts — thin wrapper preserving NerveEnrichmentEntry signature
export function shortProviderLabel(
  enrichment: NerveEnrichmentEntry | undefined
): string | undefined {
 ...
 *
 * Nerve's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: NerveEnrichmentMap | undefined
): Map<string, string> {
...
): NerveEnrichmentEntry | undefined {
...
export function canonicalDedupSet(
  rawModels: ReadonlyArray<NerveRawModelEntry>,
  canonicalToAlias: Map<string, string>
): Set<string> {
 ...
 *
 * Real example: Nerve's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `NerveEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: NerveEnrichmentMap | undefined
): Map<string, NerveEnrichmentEntry> {
  const out = new Map<string, NerveEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: NerveEnrichmentEntry | undefined,
  aliasIndex: Map<string, NerveEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): NerveEnrichmentEntry | undefined {
  if (direct) {
...
export interface NerveCompressionStep {
...
export interface NerveCompressionCombo {
  id: string;
  name?: string;
  description?: string;
  pipeline: NerveCompressionStep[];
  isDefault?: boolean;
}

export type NerveCompressionMetaFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<NerveCompressionCombo[]>;
 ...
 */
export const defaultNerveCompressionMetaFetcher: NerveCompressionMetaFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: NerveCompressionCombo[] = [];
  ...
  try {
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { combos?: unknown[] })?.combos)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: NerveCompressionCombo[] = [];
    for (const raw of list) {
      ...
      const steps: NerveCompressionStep[] = [];
      for (const step of pipeline) {
        ...
        const entry: NerveCompressionStep = { engine };
        if (typeof intensity === "string" && intensity.length > 0) {
          entry.intensity = intensity;
        }
        steps.push(entry);
      }
      const combo: NerveCompressionCombo = { id, pipeline: steps };
 ...
 * Nerve knows but the plugin doesn't.
 *
 ...
 */
export function formatCompressionPipeline(pipeline: NerveCompressionStep[]): string {
...
// `features.usableOnly` filter. Returns the operator's installed Nerve
...
export interface NerveProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type NerveProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<NerveProviderConnection[]>;
 ...
 */
export const defaultNerveProvidersFetcher: NerveProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: NerveProviderConnection[] = [];
  if (!baseURL || !apiKey) return empty;
  const root = baseURL.replace(/\/v1\/?$/, "").replace(/\/$/, "");
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ? (body as { data: unknown[] }).data
          : [];
    const out: NerveProviderConnection[] = [];
    for (const raw of list) {
      if (!raw || typeof raw !== "object") continue;
 ...
 * (`claude/sonnet-4`) depending on the Nerve deployment's `/v1/models`
 * surface shape.
 *
...
export function usableProviderAliasSet(
  connections: NerveProviderConnection[],
  enrichment: NerveEnrichmentMap | undefined
): {
  aliases: Set<string>;
  ...
  if (enrichment) {
    ...
    // populated by `defaultNerveEnrichmentFetcher`. Every entry carries
...
export function isUsableCombo(
  ...
  // The provider id is folded INTO the full model string by Nerve's
 ...
 * (e.g. `nerve/MASTER`, `nerve/MASTER-LIGHT`), guaranteeing uniqueness
 ...
 *
 * NOTE: the key MUST carry the OWNING provider prefix (`nerve/…`), never a
 ...
 * "Unable to determine provider", whereas `nerve/MASTER` resolves provider=
 * `nerve` and the openai-compatible adapter strips the prefix and sends the
 ...
 */
export function buildComboKey(combo: NerveRawCombo, used: Set<string>, providerId: string): string {
 ...
 * OC user may register prod + preprod Nerve side-by-side with distinct
 ...
 *   - **Combo / model ID collisions: combos win.** Nerve treats combos
 ...
 *     `/api/combos` requires a management-scoped key and Nerve may
 ...
 *             outside tests today).
 */
export function createNerveProviderHook(
  opts?: NervePluginOptions,
  deps: {
    fetcher?: NerveModelsFetcher;
    combosFetcher?: NerveCombosFetcher;
    autoCombosFetcher?: NerveAutoCombosFetcher;
  ...
  const resolved = resolveNervePluginOptions(opts);
  const fetcher = deps.fetcher ?? defaultNerveModelsFetcher;
  ...
  const combosFetcher = deps.combosFetcher ?? defaultNerveCombosFetcher;
  const autoCombosFetcher = deps.autoCombosFetcher ?? defaultNerveAutoCombosFetcher;
  const enrichmentFetcher = deps.enrichmentFetcher ?? defaultNerveEnrichmentFetcher;
  const compressionMetaFetcher = deps.compressionMetaFetcher ?? defaultNerveCompressionMetaFetcher;
  const providersFetcher = deps.providersFetcher ?? defaultNerveProvidersFetcher;
  ...
  const cache: NerveFetchCache = deps.cache ?? new Map();
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      if (!baseURL) {
        console.warn(
          `[nerve-plugin] provider.models(${resolved.providerId}): ` +
      ...
      let rawModels: NerveRawModelEntry[];
      let rawCombos: NerveRawCombo[];
      let rawAutoCombos: NerveRawAutoCombo[];
      let rawEnrichment: NerveEnrichmentMap;
      let rawCompressionCombos: NerveCompressionCombo[];
      let rawConnections: NerveProviderConnection[];
      if (cached && cached.expiresAt > t) {
        ...
        // management-scoped key and Nerve may not have any combos
        ...
        if (wantCombos) {
          try {
            ...
            console.warn(
              "[nerve-plugin] combos fetch failed, falling back to models-only catalog",
              err
            );
          }
        }
        ...
        // may not exist yet on older Nerve versions).
        rawAutoCombos = [];
        if (wantAutoCombos) {
          try {
        ...
        if (wantEnrichment) {
          try {
            ...
            console.warn("[nerve-plugin] enrichment fetch failed, falling back to raw ids", err);
          }
        }
        ...
        if (wantCompressionMeta) {
          try {
            ...
            console.warn("[nerve-plugin] compression-metadata fetch failed", err);
          }
        }
        ...
        if (wantUsableOnly) {
          try {
            ...
            console.warn(
              "[nerve-plugin] /api/providers fetch failed; usableOnly filter disabled for this refresh",
              err
            );
          }
        }
        ...
        // the dynamic pipeline fired and how much catalog Nerve returned.
        // Emitted once per cache miss (TTL refresh) — quiet on cache hits.
        console.warn(
          `[nerve-plugin] catalog refreshed for providerId=${resolved.providerId} baseURL=${baseURL}: ` +
      ...
      const rawModelById = new Map<string, NerveRawModelEntry>();
      ...
      for (const entry of rawModels) {
        ...
        const model = mapRawModelToModelV2(entry, {
          ...
          providerId: resolved.nerveProviderId,
          baseURL,
        ...
        // prefixed id on `model.id` (e.g. `nerve/claude-primary`), so we
      ...
      // compression metadata is present). Nerve returns at most one
      ...
      for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
        ...
        for (const combo of pending) {
          ...
          const memberEntries: NerveRawModelEntry[] = [];
          ...
          for (const step of memberSteps) {
            ...
            if (stepKind === "combo-ref") {
              ...
              } as unknown as NerveRawModelEntry);
              continue;
            }
          ...
          const mapped = mapComboToModelV2(
            ...
            memberEntries,
            // #6859: server-facing id — NOT the OC-gate-prefixed `resolved.providerId`.
            resolved.nerveProviderId,
            baseURL,
            features.apiFormat
          );
          const hasMembers = memberEntries.length > 0;

          // Apply enrichment overlay to combos too (Nerve's
          ...
          const comboKey = buildComboKey(combo, usedComboKeys, resolved.nerveProviderId);
          ...
          if (Object.prototype.hasOwnProperty.call(models, comboKey)) {
            ...
            if (!isIntentionalDedup) {
              ...
              if (!collisionWarned.has(dedupeKey)) {
                ...
                console.warn(
                  `[nerve-plugin] combo key "${comboKey}" collides with a model id; combo wins.`
                );
              }
            }
          }
      ...
      if (pending.length > 0) {
        console.warn(
          `[nerve-plugin] ${pending.length} combo(s) could not resolve all nested combo-refs after ${MAX_COMBO_PASSES} passes; they will advertise context=0 to avoid over-claiming.`
        );
      }
...
// same-origin Nerve inference requests only
// ────────────────────────────────────────────────────────────────────────────
 ...
 * a secret bound to both the configured Nerve origin and these intended
 * inference endpoints, and MUST NOT leak elsewhere.
 *
 * Ported from Alph4d0g's `opencode-nerve-auth@1.2.1` `createFetchInterceptor`
 ...
 *     any caller-supplied Bearer for the same Nerve provider.
 *
 ...
 */
export function createNerveFetchInterceptor(config: {
  apiKey: string;
  baseURL: string;
}): typeof fetch {
 ...
 * validator throws on them, breaking Nerve → Gemini chains transparently.
 *
 * Source: behavioural reverse-engineering from Alph4d0g's
 * opencode-nerve-auth@1.2.1 (dist/src/plugin.js:517).
 */
 ...
 *              interceptor from `createNerveFetchInterceptor`).
 */
    ...
    try {
      ...
      if (typeof initBody === "string") {
        ...
        if (!geminiStreamingWarningEmitted) {
          ...
          console.warn(
            "[nerve-plugin] sanitizeGemini: streaming Request body, skipping schema strip (Gemini may reject)"
          );
        }
        return inner(input, init);
      } else if (
...
// `@nerve/opencode-provider/src/index.ts` (`createNerveProvider`,
 ...
 * `OpenCodeModelEntry` exported by `@nerve/opencode-provider`. Stripped
 ...
 * when Nerve didn't surface a value, NOT emitted as `undefined` — the
 * resulting JSON must be diffable across Nerve deployments without
 * `undefined` noise.
 */
/** Modalities accepted by OC's static catalog reader (see `@opencode-ai/sdk`). */
export type NerveModalityKind = "text" | "audio" | "image" | "video" | "pdf";

const STATIC_MODALITY_VALUES: ReadonlySet<NerveModalityKind> = new Set([
  "text",
  "audio",
  "image",
  "video",
...
function normaliseModalities(raw: unknown): NerveModalityKind[] {
  if (!Array.isArray(raw)) return [];
  const out: NerveModalityKind[] = [];
  const seen = new Set<string>();
  for (const v of raw) {
    if (typeof v !== "string") continue;
    const lower = v.toLowerCase() as NerveModalityKind;
...
export interface NerveStaticModelEntry {
   ...
   * model KEY already carries the provider prefix (e.g. `nerve/MASTER`),
  ...
  tool_call?: boolean;
  /**
   * Per-million-token cost. Maps from Nerve `/api/pricing` shape:
   ...
   * Nerve's `input_modalities` / `output_modalities` on `/v1/models`.
   ...
   */
  modalities?: {
    input: NerveModalityKind[];
    output: NerveModalityKind[];
  };
}

/**
 * Static `provider.<id>` block written to `input.provider` by the config hook.
 * Mirrors `OpenCodeProviderEntry` from `@nerve/opencode-provider`.
 *
 *   - `npm` is always `"@ai-sdk/openai-compatible"` — Nerve exposes an
 ...
 */
export interface NerveStaticProviderEntry {
  npm: "@ai-sdk/openai-compatible";
  name: string;
  options: {
    baseURL: string;
  ...
  models: Record<string, NerveStaticModelEntry>;
}
 ...
 *
 * Mapping rules (per the sibling `createNerveProvider` output spec):
 *
...
export function buildStaticProviderEntry(
  rawModels: NerveRawModelEntry[],
  rawCombos: NerveRawCombo[],
  opts: ReturnType<typeof resolveNervePluginOptions>,
  baseURL: string,
  apiKey: string,
  enrichment?: NerveEnrichmentMap,
  compressionCombos?: NerveCompressionCombo[],
  connections?: NerveProviderConnection[],
  rawAutoCombos?: NerveRawAutoCombo[]
): NerveStaticProviderEntry {
  const models: Record<string, NerveStaticModelEntry> = {};
  ...
  // Build a name-set of every non-hidden combo from `/api/combos`. Nerve
  ...
  for (const raw of rawModels) {
    ...
    // model KEY (e.g. `nerve/claude-opus-4`) is what OC uses to recover
    // (providerID, modelID) when the user selects a model.
    const entry: NerveStaticModelEntry = { name: displayName };
    ...
    // Modalities — emit when Nerve surfaced any. Without this field
  ...
  const resolvedStaticCombosByName = new Map<string, NerveStaticModelEntry>();
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      // provider and the KEY prefix (`nerve/<slug>`) is what OC parses.
      const entry: NerveStaticModelEntry = { name: displayName };
      ...
      if (hasMembers) {
        ...
        const intersect = (sets: Set<NerveModalityKind>[]): NerveModalityKind[] => {
      ...
      // `opts.providerId` reaches Nerve's server doubled
      // (`opencode-nerve/opencode-nerve/<slug>`), and `parseModel()`
      // resolves credentials for the nonexistent provider `opencode-nerve`
      // instead of `nerve`. See #7976.
      models[buildComboKey(combo, usedComboKeys, opts.nerveProviderId)] = entry;
  ...
  if (pendingStatic.length > 0) {
    console.warn(
      `[nerve-plugin] ${pendingStatic.length} combo(s) in the static catalog could not resolve all nested combo-refs after ${MAX_STATIC_COMBO_PASSES} passes; they will be omitted.`
    );
  }
    ...
    for (const autoCombo of rawAutoCombos) {
      ...
      if (models[key]) {
        ...
        if (!reportedCollisions.has(key)) {
          ...
          console.warn(
            `[nerve-plugin] auto combo key "${key}" collides with an existing model; auto combo wins.`
          );
        }
      }
      models[key] = entry;
 ...
 *
 * Exported as a dependency-injectable function on `createNerveConfigHook`
...
// `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/nerve-<providerId>.json`.
...
interface NerveDiskSnapshot {
  ...
  rawEnrichment: Array<[string, NerveEnrichmentEntry]>;
  rawCompressionCombos: NerveCompressionCombo[];
  rawConnections: NerveProviderConnection[];
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return path.join(dir, "plugins", `nerve-${providerId}.json`);
}
...
export type NerveDiskSnapshotWriter = (
  providerId: string,
  entry: Omit<NerveFetchCacheEntry, "expiresAt">,
  identityFingerprint: string
) => Promise<void>;

export type NerveDiskSnapshotReader = (
  providerId: string,
  identityFingerprint: string
) => Promise<Omit<NerveFetchCacheEntry, "expiresAt"> | undefined>;
...
export const defaultDiskSnapshotWriter: NerveDiskSnapshotWriter = async (
  providerId,
  entry,
  identityFingerprint
) => {
  try {
    ...
    const snapshot: NerveDiskSnapshot = {
      v: 2,
      identityFingerprint,
...
export const defaultDiskSnapshotReader: NerveDiskSnapshotReader = async (
  providerId,
  identityFingerprint
) => {
  try {
    ...
    const parsed = JSON.parse(body) as Partial<NerveDiskSnapshot>;
    if (
      !parsed ||
      parsed.v !== 2 ||
...
export const noopDiskSnapshotWriter: NerveDiskSnapshotWriter = async () => {};
...
function debugLogPath(providerId: string): string {
  return join(debugLogDir(), `nerve-debug-${providerId}.jsonl`);
}

function debugStatePath(providerId: string): string {
  return join(debugLogDir(), `nerve-debug-${providerId}.state.json`);
}
...
export function debugLogSetEnabled(providerId: string, enabled: boolean): void {
  try {
    ...
    console.warn(`[nerve-plugin] debugLogSetEnabled failed: ${(err as Error).message}`);
  }
}
...
export function debugLogAppend(entry: DebugLogEntry): void {
  try {
    ...
    console.warn(`[nerve-plugin] debugLogAppend failed: ${(err as Error).message}`);
  }
}
...
export function debugLogClear(providerId: string): void {
  try {
    ...
    console.warn(`[nerve-plugin] debugLogClear failed: ${(err as Error).message}`);
  }
}
...
export const noopDiskSnapshotReader: NerveDiskSnapshotReader = async () => undefined;

export type NerveReadAuthJson = () => Promise<AuthJsonShape | undefined | null>;

export const defaultReadAuthJson: NerveReadAuthJson = async () => {
  const dir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode");
 ...
 *   - `fetcher`          — replaces `defaultNerveModelsFetcher`.
 *   - `combosFetcher`    — replaces `defaultNerveCombosFetcher`.
 *   - `now`              — clock for cache TTL (default `Date.now`).
 *   - `cache`            — shared fetch-result cache (see
 *                          `NerveFetchCache`). Pass the same Map the
 *                          provider hook owns to dedupe round-trips.
 *   - `logger`           — `{warn}` sink for breadcrumb capture in tests.
 *                          Defaults to `console`.
 */
export function createNerveConfigHook(
  opts?: NervePluginOptions,
  deps: {
    readAuthJson?: NerveReadAuthJson;
    fetcher?: NerveModelsFetcher;
    ...
    compressionMetaFetcher?: NerveCompressionMetaFetcher;
    providersFetcher?: NerveProvidersFetcher;
    diskSnapshotReader?: NerveDiskSnapshotReader;
    diskSnapshotWriter?: NerveDiskSnapshotWriter;
    now?: () => number;
    cache?: NerveFetchCache;
  ...
  return async (input: Config) => {
    ...
    if (existingProviders && existingProviders[resolved.providerId] !== undefined) {
      logger.warn(
        `[nerve-plugin] config shim skipped: provider.${resolved.providerId} already set by user`
      );
      return;
    }
    ...
    if (authJson === null) {
      logger.warn("[nerve-plugin] config shim: auth.json failed to parse; treating as missing");
      authJson = undefined;
    }

    // Try both prefixed (e.g. opencode-nerve) and unprefixed (e.g. nerve)
    // keys so a user who ran `/connect nerve` before the auto-prefix fix
    ...
    if (!apiKey) {
      ...
      logger.warn(
        `[nerve-plugin] config shim skipped: no apiKey for providerId=${resolved.providerId}`
      );
      return;
    }
    // Management-plane catalog reads may use a narrower read-only token.
    ...
    if (!baseURL) {
      logger.warn(
        `[nerve-plugin] config shim skipped: no baseURL for providerId=${resolved.providerId}`
      );
      return;
    }
    ...
    if (cached && cached.expiresAt > t) {
      ...
      try {
        ...
        logger.warn(
          "[nerve-plugin] config shim: /v1/models fetch failed; publishing stub provider entry",
          err
        );
        rawModels = [];
      ...
      try {
        ...
        logger.warn(
          "[nerve-plugin] config shim: /api/combos fetch failed; publishing models-only static catalog",
          err
        );
      }
      ...
      if (wantEnrichment) {
        try {
          ...
          logger.warn(
            "[nerve-plugin] config shim: /api/pricing/models fetch failed; publishing raw-id static catalog",
            err
          );
        }
      }
      ...
      if (wantCompressionMeta) {
        try {
          ...
          logger.warn(
            "[nerve-plugin] config shim: /api/context/combos fetch failed; publishing combos without compression suffix",
            err
          );
        }
      }
      ...
      if (wantUsableOnly) {
        try {
          ...
          logger.warn(
            "[nerve-plugin] config shim: /api/providers fetch failed; usableOnly filter disabled for this refresh",
            err
          );
        }
      }
      ...
      if (modelsFetchThrew && wantDiskCache) {
        ...
        if (snapshot && snapshot.rawModels.length > 0) {
          logger.warn(
            `[nerve-plugin] config shim: /v1/models unreachable; using stale disk cache (${snapshot.rawModels.length} models)`
          );
    ...
    if (features.mcpAutoEmit === true) {
      ...
      if (!mcpKey) {
        logger.warn(
          `[nerve-plugin] mcp auto-emit skipped: no Bearer token for providerId=${resolved.providerId}`
        );
      } else {
        ...
        if (inputWithMcp.mcp[resolved.providerId] !== undefined) {
          logger.warn(
            `[nerve-plugin] mcp auto-emit skipped: mcp.${resolved.providerId} already set by user`
          );
        } else {
