...
import {
  ...
  defaultNerveEnrichmentFetcher,
  defaultNerveCompressionMetaFetcher,
...
test("parseNervePluginOptions: empty features object → preserved", () => {
  const r = parseNervePluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseNervePluginOptions: all boolean features set → preserved", () => {
  const r = parseNervePluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseNervePluginOptions: mcpToken string → preserved", () => {
  const r = parseNervePluginOptions({
...
test("parseNervePluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseNervePluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @nerve\/opencode-plugin options/
  );
});

test("parseNervePluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseNervePluginOptions({
...
test("parseNervePluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseNervePluginOptions({ features: { mcpToken: "" } }),
    /Invalid @nerve\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: NerveRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  let called = 0;
  const enrichment: NerveEnrichmentMap = new Map([
    ["claude-sonnet-4-6", { name: "Claude Sonnet 4.6", pricing: { input: 3, output: 15 } }],
  ]);
  const hook = createNerveProviderHook(
    { providerId: "nerve", baseURL: "https://or.example.com/v1" },
    {
  ...
  // #6859: dynamic-hook catalog keys use the unprefixed nerveProviderId.
  const m = out["nerve/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(out["nerve/claude-sonnet-4-6"].name, "claude-sonnet-4-6", "raw id preserved");
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: NerveCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["nerve/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-nerve": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-nerve"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-nerve"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-nerve"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-nerve": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-nerve"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createNerveConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-nerve-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-nerve-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultNerveEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultNerveEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultNerveEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultNerveEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultNerveCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultNerveCompressionMetaFetcher("", "sk", 100);
...
test("defaultNerveEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultNerveEnrichmentFetcher("https://or.example.com/v1", "sk-test", 5_000);
    assert.ok(
...
test("defaultNerveEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultNerveEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultNerveEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
function makeEnrichmentMap(
  ...
  const map: NerveEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: NerveRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as NerveRawModelEntry,
    { id: "claude/claude-opus-4-7" } as NerveRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: NerveRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as NerveRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: NerveRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as NerveRawModelEntry,
    { id: "claude/claude-opus-4-7" } as NerveRawModelEntry,
    { id: "cx/gpt-5.5" } as NerveRawModelEntry,
    { id: "codex/gpt-5.5" } as NerveRawModelEntry,
    { id: "pol/openai-large" } as NerveRawModelEntry,
    { id: "pollinations/openai-large" } as NerveRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
