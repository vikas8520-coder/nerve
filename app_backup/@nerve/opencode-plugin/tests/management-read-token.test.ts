...
import {
  ...
  type NerveCompressionMetaFetcher,
...
const RAW_MODELS: NerveRawModelEntry[] = [
  {
    id: "openai/gpt-test",
...
test("options: managementReadToken is accepted and preserved", () => {
  const parsed = parseNervePluginOptions({ managementReadToken: MANAGEMENT_READ_TOKEN });
...
test("provider hook: management GET fetchers use managementReadToken while /v1 uses apiKey", async () => {
  ...
  const enrichmentFetcher: NerveEnrichmentFetcher = async (_baseURL, token) => {
    calls.push(["pricing", token]);
    return new Map();
  };
  const compressionMetaFetcher: NerveCompressionMetaFetcher = async (_baseURL, token) => {
    calls.push(["context", token]);
    return [];
  };
  const providersFetcher: NerveProvidersFetcher = async (_baseURL, token) => {
...
test("config hook: managementReadToken stays out of provider inference and MCP config", async () => {
  ...
  const hook = createNerveConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-nerve": { type: "api" as const, key: API_KEY },
      }),
      fetcher: async (_baseURL, token) => {
        calls.push(["models", token]);
        return RAW_MODELS;
  ...
  assert.equal(input.provider?.["opencode-nerve"]?.options?.apiKey, API_KEY);
  assert.equal(
    input.mcp?.["opencode-nerve"]?.headers?.Authorization,
    `Bearer ${API_KEY}`,
...
test("disk cache: snapshot written under management token A is rejected under token B", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "nerve-token-snapshot-"));
  ...
  try {
    ...
    const tokenAHook = createNerveConfigHook(
    ...
    const tokenBHook = createNerveConfigHook(
    ...
    assert.deepEqual(
      input.provider?.["opencode-nerve"]?.models,
      {},
