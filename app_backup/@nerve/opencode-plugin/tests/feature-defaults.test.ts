 ...
 *
 * `NERVE_FEATURE_DEFAULTS` declares those defaults explicitly and
...
import { NERVE_FEATURE_DEFAULTS, resolveEffectiveFeatureFlags } from "../src/index.js";
...
test("NERVE_FEATURE_DEFAULTS: declares each flag with its documented default", () => {
  for (const key of DEFAULT_ON) {
    assert.equal(NERVE_FEATURE_DEFAULTS[key], true, `${key} defaults ON`);
  }
  for (const key of DEFAULT_OFF) {
    assert.equal(NERVE_FEATURE_DEFAULTS[key], false, `${key} defaults OFF`);
  }
});
...
test("resolveEffectiveFeatureFlags: non-boolean sibling keys do not leak into flags", () => {
  ...
  assert.equal(Object.keys(flags).length, Object.keys(NERVE_FEATURE_DEFAULTS).length);
