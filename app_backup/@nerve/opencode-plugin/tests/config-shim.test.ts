 ...
 *
 * Covers `createNerveConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@nerve/opencode-provider`).
...
import {
  ...
  createNerveConfigHook,
  createNerveProviderHook,
  NervePlugin,
  resolveNervePluginOptions,
  type NerveCombosFetcher,
  type NerveEnrichmentEntry,
  type NerveEnrichmentFetcher,
  type NerveEnrichmentMap,
  type NerveFetchCache,
  type NerveModelsFetcher,
  type NerveProviderConnection,
  type NerveProvidersFetcher,
  type NerveRawCombo,
  type NerveRawModelEntry,
  type NerveReadAuthJson,
  type NerveStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: NerveRawModelEntry = {
...
const MODEL_GEMINI: NerveRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: NerveRawCombo = {
...
): NerveReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: NerveReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): NerveReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: NerveReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: NerveRawModelEntry[]
): NerveModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: NerveModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: NerveRawCombo[]
): NerveCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: NerveCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): NerveModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: NerveModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): NerveCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: NerveCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: NerveEnrichmentMap): NerveEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: NerveEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): NerveEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: NerveEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve" },
  ...
  const provider = (input as { provider: Record<string, NerveStaticProviderEntry> }).provider;
  const entry = provider["opencode-nerve"];
  assert.ok(entry, "input.provider['opencode-nerve'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "Nerve");
  ...
  const claude = entry.models["opencode-nerve/claude-sonnet-4-6"];
  ...
  const combo = entry.models["nerve/claude-tier"];
...
//     prefixed (`opencode-nerve`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `nerve` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-nerve`.
  const readAuthJson = stubReadAuthJson({
    nerve: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve" }, // resolves to opencode-nerve internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    nerve: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, NerveStaticProviderEntry> }).provider[
    "opencode-nerve"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  ...
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-nerve": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-nerve"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createNerveConfigHook(
    { providerId: "nerve", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, ["opencode-nerve/claude-sonnet-4-6", "opencode-nerve/gemini-3-flash"]);
  assert.equal(entry.models["nerve/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  ...
  const hookA = createNerveConfigHook(
    { providerId: "nerve-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createNerveConfigHook(
    { providerId: "nerve-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, NerveStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-nerve-prod"], "prod block present");
  assert.ok(provider["opencode-nerve-preprod"], "preprod block present");
  assert.equal(provider["opencode-nerve-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-nerve-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-nerve-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(provider["opencode-nerve-preprod"].options.baseURL, "https://preprod.example/v1");
  assert.notEqual(
    provider["opencode-nerve-prod"],
    provider["opencode-nerve-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createNerveConfigHook(
    { providerId: "nerve", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createNerveProviderHook(
    { providerId: "nerve", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: NerveFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @nerve/opencode-provider", () => {
  ...
  const claude = block.models["opencode-nerve/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: empty fetch results → stub block with models: {}", () => {
  const resolved = resolveNervePluginOptions({ providerId: "nerve" });
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["nerve/claude-tier"], undefined);
  assert.ok(block.models["opencode-nerve/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveNervePluginOptions({ providerId: "nerve" });
  const withDate: NerveRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-nerve/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-nerve/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveNervePluginOptions({ providerId: "nerve" });
  const TEXT_ONLY: NerveRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["nerve/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: NervePlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("NervePlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await NervePlugin({} as never, { providerId: "nerve" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-nerve": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-nerve"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, NerveEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-nerve/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-nerve/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["nerve/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const combosFetcher = stubCombosFetcher([]);
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, NerveEnrichmentEntry>([["claude-sonnet-4-6", { name: "Claude Sonnet 4.6" }]])
  );
  const logger = captureWarn();

  const hook = createNerveConfigHook(
    { providerId: "nerve", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-nerve/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(payload: NerveProviderConnection[]): NerveProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: NerveProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): NerveProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: NerveProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-nerve/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-nerve/claude-sonnet-4-6"].name, "Claude - Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-nerve/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot Nerve hasn't curated a human label for yet. We still
  ...
  assert.equal(entry.models["opencode-nerve/claude-sonnet-4-6"].name, "CC - Claude Sonnet 4.6");
});
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createNerveConfigHook(
    { providerId: "nerve", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, NerveStaticProviderEntry> }).provider[
    "opencode-nerve"
  ];
  assert.equal(
    entryA.models["opencode-nerve/claude-sonnet-4-6"].name,
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, NerveStaticProviderEntry> }).provider[
    "opencode-nerve"
  ];
  assert.equal(
    entryB.models["opencode-nerve/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveNervePluginOptions({ providerId: "nerve" });
  const rawModels: NerveRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const parent = block.models["nerve/parent"];
