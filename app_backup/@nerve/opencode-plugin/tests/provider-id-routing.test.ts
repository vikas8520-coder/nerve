 ...
 *
 * `resolveNervePluginOptions()` auto-prefixes `providerId` with
 ...
 * NEVER leak into the identifiers Nerve's own server parses to resolve
 * credentials (`mapRawModelToModelV2`'s `id`/`providerID`,
 * `mapComboToModelV2`'s `providerID`, and the dynamic-hook catalog keys).
 *
 * Nerve's server-side `parseModel()` (open-sse/services/model.ts) splits
 ...
 * the server looks up credentials for a provider named "opencode-nerve"
 * (which never exists in `src/shared/constants/providers.ts`) instead of
 * "nerve" — producing the exact "No credentials for opencode-nerve" /
 * "No active credentials for provider: opencode-nerve" errors reported
 * in #6859.
 */
...
/**
 * Minimal stand-in for Nerve's own `parseModel()` (open-sse/services/
 ...
 * self-contained test suite (`cd @nerve/opencode-plugin && npm test`)
...
test("#6859: server-facing model id/providerID must resolve to the unprefixed provider name", () => {
  ...
  assert.equal(resolved.providerId, "opencode-nerve");

  // A second, unprefixed id must be exposed for anything that reaches
  // Nerve's own server (model id prefix, ModelV2.providerID, combo keys).
  assert.equal(
    resolved.nerveProviderId,
    "nerve",
    "resolveNervePluginOptions() must expose an unprefixed nerveProviderId"
  );

  // A bare raw /v1/models entry (no existing "/" in its id — the common
  // case for Nerve's catalog) mapped with the server-facing id.
  const model = mapRawModelToModelV2(
    { id: "claude-opus-4-7" },
    { providerId: resolved.nerveProviderId, baseURL: "http://localhost:20128" }
  );

  assert.equal(model.providerID, "nerve");
  assert.equal(model.id, "nerve/claude-opus-4-7");

  // OpenCode dispatches back to Nerve using `providerID/modelKey`
  // (matches the issue's own repro: `-m opencode-nerve/oc/big-pickle`).
  ...
  assert.equal(
    parsedProvider,
    "nerve",
    `server-side provider split resolved '${parsedProvider}', expected 'nerve' — ` +
      `credentials lookup would fail for an OC-gate-prefixed provider id`
  );
});

test("#6859: createNerveProviderHook end-to-end — catalog keys/providerID never carry the OC-gate prefix", async () => {
  ...
  const model = out["nerve/claude-opus-4-7"];
...
// key built with the OC-gated `providerId` reaches Nerve's server
// doubled and fails credential lookup for the nonexistent provider
// `opencode-nerve`. Confirmed against the issue's own curl repro
// (`model: "opencode-nerve/hermes-smart-stack"` → "No active
// credentials for provider: opencode-nerve").
test("#7976: buildStaticProviderEntry keys bare-slug combo ids with the unprefixed nerveProviderId (no double OC-gate prefix)", () => {
  const resolved = resolveNervePluginOptions({ providerId: "nerve" });
  assert.equal(resolved.providerId, "opencode-nerve");
  assert.equal(resolved.nerveProviderId, "nerve");
  ...
  const combo = {
    ...
    isHidden: false,
    models: [],
  } as unknown as NerveRawCombo;

  const block = buildStaticProviderEntry([], [combo], resolved, "https://or.example/v1", "sk-test");

  assert.deepEqual(Object.keys(block.models), ["nerve/hermes-smart-stack"]);
  assert.equal(
    block.models["opencode-nerve/hermes-smart-stack"],
    undefined,
