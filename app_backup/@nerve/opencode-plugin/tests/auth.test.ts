 ...
 *
 * Covers the `createNerveAuthHook(opts)` factory and its loader behaviour
...
import { createNerveAuthHook } from "../src/index.js";

test("createNerveAuthHook: default providerId is 'nerve'", () => {
  const hook = createNerveAuthHook();
  assert.equal(hook.provider, "opencode-nerve");
});

test("createNerveAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createNerveAuthHook({ providerId: "nerve-preprod" });
  assert.equal(hook.provider, "opencode-nerve-preprod");
});

test("createNerveAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createNerveAuthHook();
  ...
  assert.equal(m.label, "Nerve API Key");

  const custom = createNerveAuthHook({ providerId: "nerve-preprod" });
  assert.equal(custom.methods[0].label, "Nerve (opencode-nerve-preprod) API Key");
});

test("createNerveAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("nerve"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createNerveAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(
