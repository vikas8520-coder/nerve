...
If you discover a security vulnerability in Nerve, please report it responsibly:

1. **DO NOT** open a public GitHub issue
2. Use [GitHub Security Advisories](https://github.com/diegosouzapw/Nerve/security/advisories/new)
...
Nerve implements a multi-layered security model:
...
Nerve ships a hot-reloadable **guardrails registry** (`src/lib/guardrails/`) with 3 built-in guardrails ordered by priority:
...
Custom guardrails register via `registerGuardrail(new MyGuardrail())`. The model is fail-open (exceptions never block traffic). Per-request opt-out via `x-nerve-disabled-guardrails` header. → See [`docs/security/GUARDRAILS.md`](docs/security/GUARDRAILS.md).
...
The published `nerve` npm artifact bundles the Next.js `output: "standalone"`
...
`NERVE_BUILD_PROFILE=minimal npm run build`. That replaces the four
