...
> **TL;DR**: Nerve's compression engine is **pluggable** — you can register custom engines, ship language packs for new languages, and compose stacked pipelines. This guide shows how.
...
Caveman-style compression uses **language-specific rule packs** to handle fillers, hedging, and verbose patterns in each natural language. Nerve ships with **6 language packs**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
Nerve selects **ONE mode per request** based on configuration, auto-trigger thresholds, and combo overrides.
...
Nerve's compression engines credit several upstream projects in the README
...
pack, how does that reach Nerve?** This section is the authoritative answer.
...
rule/filter in Nerve's own format** (see
...
Nerve's own `smartcrusher` / `toon` / `tabular` layers. There is no live
...
1. **Don't vendor** — re-express the upstream rule/filter in Nerve's format.
