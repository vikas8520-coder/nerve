...
**Nerve** — samlet AI proxy/router. Én endpoint, 160+ LLM udbydere, auto-fallback.
...
Nerve har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@nerve/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@nerve/open-sse` → `open-sse/`, `@nerve/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.nerve/`
