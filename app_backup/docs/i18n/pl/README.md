...
# 🚀 Nerve — Darmowa bramka AI

<img src="../../../docs/diagrams/readme-hero.svg" width="100%" alt="Nerve — Nigdy nie przestawaj kodować. Każde narzędzie AI → 268 dostawców — ponad 90 darmowych — przez jeden punkt końcowy. Claude Code, Codex, Cursor, Cline, Copilot i Antigravity w DARMOWE Claude / GPT / Gemini z automatycznym przekierowaniem (auto-fallback). Kaskadowa kompresja RTK + Caveman oszczędza 15–95% tokenów (średnio ~89%) — nigdy nie przekraczaj limitów. 268 dostawców AI · ponad 90 darmowych pakietów · ~1,4 mld darmowych tokenów/mies. · 18 strategii routingu · 0 USD na start."/>
...
⭐ Dodaj gwiazdkę do repozytorium, jeśli NERVE pomógł Ci zaoszczędzić pieniądze i ułatwił pracę.
...
**Pytania, wskazówki dotyczące dostawców, plany rozwoju (roadmap) i wsparcie → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/nerveOficial) · WhatsApp [🌍 Global](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 Brasil](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[**🚀 Szybki start**](#-szybki-start) • [**🎯 Komba**](#-komba-combos--flagowa-funkcja) • [**🌐 Dostawcy**](#-268-dostawc%C3%B3w-ai--ponad-90-darmowych) • [**🔌 CLI & MCP**](#-pe%C5%82ne-cli--a2a-i-mcp) • [**🗜️ Kompresja**](#%EF%B8%8F-oszcz%C4%99dzaj-1595-token%C3%B3w--automatycznie) • [**🌍 Strona WWW**](https://nerve.online)

[💥 Obietnica](#-obietnica) • [🤔 Dlaczego](#-dlaczego-nerve) • [🏆 Co wyróżnia Nerve](#-co-wyr%C3%B3%C5%BCnia-nerve) • [🤖 Zgodne CLI](#-zgodne-cli-i-agenci-koduj%C4%85cy) • [🖥️ Gdzie to działa](#%EF%B8%8F-gdzie-dzia%C5%82a-nerve--wsz%C4%99dzie) • [🔒 Prywatność](#-prywatno%C5%9B%C4%87-i-lokalne-dzia%C5%82anie-local-first) • [🎬 W akcji](#-nerve-w-akcji) • [📸 Zrzuty ekranu](#-zrzuty-ekranu-z-panelu) • [📧 Wsparcie](#-wsparcie-i-spo%C5%82eczno%C5%9B%C4%87)
...
> **Kombo** to łańcuch modeli, po których Nerve nawiguje **automatycznie**. Wybucha limit, dostawca ulega awarii lub koszty gwałtownie rosną — kombo bezgłośnie przełącza się na kolejny model. **To właśnie sprawia, że Nerve jest niezawodny.** 🛡️

### ⚡ Zero konfiguracji — po prostu użyj `auto`

Nie musisz tworzyć żadnego komba. Ustaw swój model na `auto` (lub jego wariant), a Nerve zbuduje wirtualne kombo z Twoich połączonych dostawców, oceniane na żywo:
...
<sub>📊 Szczegółowe porównanie z LiteLLM, OpenRouter i Portkey → [`docs/comparison/NERVE_VS_ALTERNATIVES.md`](../../../docs/comparison/NERVE_VS_ALTERNATIVES.md)</sub>
...
- **🤖 Konfiguracja CLI/agenta jednym poleceniem** — `setup-*` konfiguruje ponad 12 narzędzi programistycznych; `nerve launch` / `launch-codex` działają bez konfiguracji. → [Integracje CLI](../../../docs/guides/CLI-INTEGRATIONS.md)
- **🛰️ Tryb zdalny** — steruj zdalną instancją Nerve za pomocą tokenów o ograniczonym zakresie (`connect` / `contexts` / `tokens`) + pomocnika OAuth `antigravity` dla instalacji na VPS. → [Tryb zdalny](../../../docs/guides/REMOTE-MODE.md)
...
- **💸 Telemetria kosztów wszędzie** — nagłówki kosztu/użycia `X-Nerve-*` w każdym punkcie końcowym, nagłówek oszczędności z trafień w cache (cache-HIT), limity wydatków USD na klucz. → [Referencja API](docs/reference/API_REFERENCE.md)
- **🧠 Pamięć pod Twoją kontrolą** — domyślnie wyłączona, opcjonalna kwantyzacja wektorowa int8 + stopniowe wygaszanie (typed decay), nagłówek `x-nerve-no-memory` na żądanie. → [Pamięć](../../../docs/frameworks/MEMORY.md)
...
<sub>📖 Konfiguracja per narzędzie dla wszystkich 33 narzędzi (25 z CLI Code + 8 z CLI Agents) → [`docs/reference/CLI-TOOLS.md`](docs/reference/CLI-TOOLS.md) · 🧩 Wtyczka OpenCode → [`@nerve/opencode-provider`](https://www.npmjs.com/package/@nerve/opencode-provider)</sub>
...
<table>
  <tr>
    ...
    <td align="center" width="127"><img src="https://cdn.jsdelivr.net/npm/@lobehub/icons-static-svg@1.91.0/icons/nvidia-color.svg" width="44" alt="NVIDIA NIM"/><br/><b>NVIDIA NIM</b><br/><sub>129 modeli<br/>~40 RPM darmowo</sub><br/><sub>                                     </sub></td>
    <td align="center" width="127"><img src="https://cdn.jsdelivr.net/npm/@lobehub/icons-static-svg@1.91.0/icons/cerebras-color.svg" width="44" alt="Cerebras"/><br/><b>Cerebras</b><br/><sub>Qwen3 235B<br/>1M tokenów/dzień</sub><br/><sub>                                     </sub></td>
  </tr>
</table>
...
| ------------------------- | ---------------------------------------- | --------------------------------------------------------- |
| 📦 **npm (globalnie)** | `npm install -g nerve` | Jedno polecenie, dowolny system operacyjny |
| 🐳 **Docker** | `docker run … diegosouzapw/nerve` | Wielonatywność architektur **AMD64 + ARM64** |
...
| 📱 **Android (Termux)** | `pkg install nodejs && npx -y nerve` | Działa **na Twoim telefonie**, 24/7, bez roota |
| 📲 **PWA** | "Dodaj do ekranu głównego" | Pełny ekran, offline, instalacja z poziomu przeglądarki |
| 🧩 **Wtyczka OpenCode** | `@nerve/opencode-provider` | Natywna integracja z OpenCode |
...
> Nerve to nie tylko serwer — to **kompletny kokpit w wierszu poleceń** z **ponad 80 poleceniami**, plus otwarte protokoły agentów, dzięki którym agent AI może samodzielnie sterować Nerve.

### ⌨️ Prawdziwe CLI (nie tylko `start`)

```bash
nerve               # uruchom bramkę + panel (port 20128)
nerve chat          # interaktywny klient czatu TUI (polecenia ukośnika: /model /combo /skill /memory)
nerve setup         # kreator pierwszej konfiguracji
nerve doctor        # diagnozuj dostawców, porty, natywne zależności
```

### 🛰️ Tryb zdalny — uruchom CLI lokalnie, Nerve na VPS

Nerve na serwerze? Steruj nim ze swojego laptopa za pomocą **tego samego CLI**. Zaloguj się raz
za pomocą tokenu dostępu o ograniczonym zakresie; każde kolejne polecenie będzie skierowane do zdalnej maszyny.

```bash
nerve connect 192.168.0.15            # hasło → token o ograniczonym zakresie, zapisany jako kontekst
nerve models list                     # ← działa na ZDALNYM serwerze
nerve configure codex                 # ← wybiera zdalny model, zapisuje lokalny profil Codex
nerve tokens create --name ci --scope read   # generuj węższe tokeny dla innych maszyn
nerve contexts use default            # ← przełącz z powrotem na serwer lokalny
```
...
<img src="../../../docs/diagrams/cli-terminal.svg" width="100%" alt="Animowany terminal demonstrujący CLI Nerve — nerve providers list, nerve combo list, nerve health — przechodzący przez obszar ponad 80 poleceń: dostawcy · oauth · klucze · kombo · węzły · modele · pamięć podręczna · kompresja · koszt · użycie · limity · stan techniczny · odporność · telemetria · logi · audyt · mcp · a2a · chmura · pamięć · umiejętności · ewaluacja · tunel · kopia zapasowa · synchronizacja · webhooki · polityka · cennik · tłumacz · symulacja …"/>

</div>

### 🤝 Połącz agenta — i pozwól mu kontrolować samo Nerve

Udostępnij Nerve przez **MCP** lub **A2A**, a każdy zdolny do tego agent autonomicznie otrzyma klucze do całej bramki — routingu, dostawców, kombinacji (combos), pamięci podręcznej, kompresji i pamięci.

| Protokół | Punkt końcowy | Zastosowanie |
| ------------------ | ----------------------------------------------- | ------------------------------------------------------- |
| 🧰 **MCP (stdio)** | `nerve --mcp`                               | Podłącz do Claude Desktop, Cursor, dowolnego klienta MCP |
...
```bash
# Daj Claude Code pełny zestaw narzędzi Nerve przez MCP:
...
<img src="../../../docs/diagrams/compression-pipeline.svg" width="100%" alt="Potok kompresji Nerve: żądanie klienta o rozmiarze 10 000 tokenów przechodzi przez 11 ułożonych kaskadowo silników — Session-Dedup, CCR, RTK, Headroom, Relevance, Caveman, LLMLingua-2, Omniglyph, Lite, Aggressive, Ultra — i dociera do dostawcy mając około 1080 tokenów, co daje do 95% oszczędności. Kod, adresy URL i JSON są zawsze zachowywane z dokładnością co do bajtu."/>
...
Powinieneś zobaczyć listę połączonych modeli. 🎉 To wszystko — zacznij kodować, a Nerve automatycznie zajmie się routingiem i przełączaniem awaryjnym.

Jeśli Twój klient nie może wysyłać niestandardowych nagłówków, Nerve udostępnia również stokenizowane aliasy zgodności:
...
docker run -d --name nerve --restart unless-stopped --stop-timeout 40 \
  -p 20128:20128 -v nerve-data:/app/data diegosouzapw/nerve:latest
```
...
```bash
NERVE_SKIP_POSTINSTALL=1 npm install -g nerve   # CI=1 również to pomija
```
...
> 🎬 **Nagrałeś film o Nerve?** Otwórz [zgłoszenie (issue)](https://github.com/diegosouzapw/Nerve/issues/new) lub [dyskusję (discussion)](https://github.com/diegosouzapw/Nerve/discussions) z linkiem — umieścimy go tutaj.
...
- 🌍 **Strona internetowa**: [nerve.online](https://nerve.online)
- 🐙 **GitHub**: [github.com/diegosouzapw/Nerve](https://github.com/diegosouzapw/Nerve)
- 🐛 **Zgłoszenia (Issues)**: [zgłoś błąd](https://github.com/diegosouzapw/Nerve/issues) (dołącz wynik działania komendy `npm run system-info`)
...
- **Strona WWW**: [nerve.online](https://nerve.online)
- **Pakiet**: [npmjs.com/package/nerve](https://www.npmjs.com/package/nerve)
- **Docker**: [hub.docker.com/r/diegosouzapw/nerve](https://hub.docker.com/r/diegosouzapw/nerve)
...
| [Tryb zdalny](../../../docs/guides/REMOTE-MODE.md) | Steruj zdalnym Nerve (VPS) z poziomu CLI na swoim laptopie za pomocą tokenów o ograniczonym zakresie |
| [Konfiguracja Claude Code](../../../docs/guides/CLAUDE-CODE-CONFIGURATION.md) | Skieruj Claude Code na Nerve (lokalnie/zdalnie) za pomocą polecenia launch + profili dla poszczególnych modeli |
...
| [Podręcznik Termux](../../../docs/guides/TERMUX_GUIDE.md) | Uruchamianie Nerve na systemie Android za pomocą Termux |
...
> Nerve jest kształtowane przez pasjonatów ze społeczności open-source. Te osoby wniosły wyjątkowy wkład, który bezpośrednio wpływa na jakość, stabilność i zasięg projektu. **Dziękujemy.**
...
> 🙏 Funkcje, poprawki błędów i ulepszenia infrastruktury wprowadzone przez tych autorów stanowią **kluczową część** tego, co czyni Nerve niezawodnym i bogatym w funkcje. Każde żądanie ściągnięcia (pull request), każdy przypadek testowy i każdy plik tłumaczenia i18n ma znaczenie. Open source tworzą ludzie tacy jak oni.
...
Nerve stoi na barkach gigantów. Projekt powstał jako fork **[9router](https://github.com/decolua/9router)** oraz port na TypeScript projektu w Go **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** — a stamtąd każdy z poniższych podsystemów był inspirowany projektem open-source, który powstał wcześniej. Każdy z nich ukształtował konkretny element Nerve. To jest nasze podziękowanie dla nich wszystkich. 🙏

> ⭐ liczba gwiazdek na lipiec 2026 r. — zachęcamy do dodania gwiazdki tym projektom.

### 🧬 Rodowód i bramka (gateway)

| Projekt | ⭐ | Jak zainspirował Nerve |
| ------------------------------------------------------------------------------- | ----: | ------------------------------------------------------------------------------------------------------------------------------------- |
...
| Projekt | Jak współgra z Nerve |
| ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[CodeWebChat](https://github.com/robertpiosik/CodeWebChat)** · robertpiosik | Towarzysz po stronie edytora — rozszerzenie VS Code + przeglądarki, które automatycznie uzupełnia ponad 15 interfejsów webowych chatbotów kontekstem z edytora. Obsługuje ścieżkę darmowego UI webowego obok ścieżki API Nerve; pozwala skierować swój tryb API na Nerve. |

## 💖 Sponsorzy

> **Chcesz się tutaj pojawić?** Nerve prezentuje sponsorów przed jedną z najszybciej rozwijających się społeczności bramki AI na GitHubie — i publicznie informujemy, dokąd trafia każdy sponsorowany token. Skontaktuj się z nami: [diegosouza.pw@outlook.com](mailto:diegosouza.pw@outlook.com)
...
Nerve jest darmowym projektem o otwartym kodzie źródłowym, tworzonym i utrzymywanym jawnie. Jeśli pozwala Ci zaoszczędzić czas lub pieniądze, rozważ wsparcie rozwoju:
...
- 🐛 **Zgłaszaj błędy i dziel się opiniami** w [Dyskusjach](https://github.com/diegosouzapw/Nerve/discussions)
...
**[⬆ Powrót do góry](#-nerve--darmowa-bramka-ai)** · Stworzono z ❤️ dla społeczności AI open-source.

<sub>Nerve v3.8.49 · Node ≥22.22.2 · Licencja MIT · <a href="https://nerve.online">nerve.online</a></sub>
