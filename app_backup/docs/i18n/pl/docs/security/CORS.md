...
Nerve kontroluje, które **origin przeglądarki** mogą odczytywać odpowiedzi cross-origin
...
traktować poważnie, ale ekspozycja Nerve jest **węższa niż w przypadku ogólnym**,
...
> `Access-Control-Allow-Credentials: true`. Wspólna ścieżka CORS Nerve nigdy
> tego nie robi.
...
- Jeśli Nerve działa za reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
```bash
# Vite (5173) + Next.js (3000) dev servers calling a local Nerve
...
## Przykład: reverse proxy przed Nerve

CORS jest egzekwowany przez samo Nerve, więc proxy generalnie **nie powinno** dodawać ani
przepisywać nagłówków `Access-Control-*` (podwójne nagłówki psują przeglądarki). Terminuj TLS
i forwarduj — niech Nerve odpowiada na preflight:

```nginx
# nginx — forward to Nerve; do NOT inject Access-Control-* here
location / {
...
Ustaw dozwolone originy przeglądarki w Nerve (`CORS_ALLOWED_ORIGINS` lub
