...
między Nerve a upstream providerami. Każdy guardrail może sprawdzać (oraz
...
- nagłówek `x-nerve-disabled-guardrails` (lub legacy
...
- **`garak` (advisory)** — uruchamia garak `--probes promptinject,dan,leakreplay`
  przeciw lokalnej instancji Nerve (`http://localhost:20128/v1`). Bramkowany
