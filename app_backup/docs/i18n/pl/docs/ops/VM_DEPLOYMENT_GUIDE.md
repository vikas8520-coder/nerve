---
title: "Nerve — Przewodnik wdrożenia na VM z Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Nerve — Przewodnik wdrożenia na VM z Cloudflare
...
Kompletny przewodnik instalacji i konfiguracji Nerve na VM (VPS) z domeną zarządzaną przez Cloudflare.
...
NERVE_WS_BRIDGE_SECRET=REPLACE-WITH-WS-BRIDGE-SECRET  # REQUIRED em produção: usado pelo Codex Responses WS bridge
...
# NERVE_PUBLIC_BASE_URL=https://llms.seudominio.com

# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.nerve.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.nerve.online
EOF
```
...
```bash
docker pull diegosouzapw/nerve:latest

docker run -d \
  --name nerve \
  --restart unless-stopped \
  --env-file /opt/nerve/.env \
  -p 20128:20128 \
...
```bash
cat > /etc/nginx/sites-available/nerve << 'NGINX'
...
Utrzymuj limity czasu strumienia reverse proxy w zgodzie ze zmiennymi timeout Nerve. Jeśli podniesiesz
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, podnieś też `proxy_read_timeout` / `proxy_send_timeout`
powyżej tego samego progu.

Nerve używa `NEXT_PUBLIC_BASE_URL` jako kanonicznego, przeglądarkowego originu dla callbacków OAuth
...
`NERVE_TRUST_PROXY` tylko wtedy, gdy Nerve nie jest bezpośrednio osiągalny przez klientów, a Twój proxy
...
# Enable Nerve
ln -sf /etc/nginx/sites-available/nerve /etc/nginx/sites-enabled/nerve
...
```bash
docker pull diegosouzapw/nerve:latest
docker stop nerve && docker rm nerve
docker run -d --name nerve --restart unless-stopped \
  --env-file /opt/nerve/.env \
  -p 20128:20128 \
  -v nerve-data:/app/data \
  diegosouzapw/nerve:latest
...
```bash
docker logs -f nerve          # Real-time stream
docker logs nerve --tail 50   # Last 50 lines
```
...
docker cp nerve:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v nerve-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/nerve-data-$(date +%F).tar.gz /data
```
...
docker stop nerve
docker run --rm -v nerve-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/nerve-data-YYYY-MM-DD.tar.gz -C /"
docker start nerve
```
...
Zobacz też [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) — przewodnik po Cloudflare Tunnel w tym repozytorium. Samodzielny worker `nerveCloud/` znajduje się w osobnym repozytorium towarzyszącym.
...
| 20128 | Nerve   | Tylko localhost (przez nginx) |
