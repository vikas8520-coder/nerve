...
Nerve może udostępnić swój lokalny serwer (`http://localhost:20128`) w publicznym
...
Wszystkie trzy backendy są zarządzane w procesie — Nerve uruchamia/zatrzymuje
...
- **Auto-install.** Przy pierwszym użyciu Nerve pobiera najnowszą binarkę
...
login` + `cloudflared tunnel route dns ...`) nie są orkiestrowane przez
  Nerve. URL-e są efemeryczne i zmieniają się przy każdym restarcie.
...
| Variable                                             | Purpose                                                                                           |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Nadpisuje ścieżkę do binarki. Jeśli ustawiona i poprawna, Nerve używa jej zamiast pobierania. |
...
trzeba skonfigurować w dashboardzie ngrok — sam Nerve tylko przekazuje
...
1. Zainstaluj Tailscale (albo pozwól Nerve to zrobić — patrz endpoint `install` poniżej).
2. Zaloguj się (`tailscale login` lub przez endpoint `login` Nerve).
...
cache haseł MITM Nerve (`getCachedPassword` / `setCachedPassword`) na
...
Gdy udostępniasz Nerve przez tunel, dashboard i przepływy OAuth muszą
...
i zrestartuj Nerve przed rozpoczęciem OAuth. Dla efemerycznych Cloudflare Quick
...
uruchomienie więcej niż jednego backendu jest dozwolone; Nerve śledzi każdy
niezależnie.
...
Nerve próbuje auto-instalacji przy pierwszym użyciu. Jeśli instalacja jest zablokowana
