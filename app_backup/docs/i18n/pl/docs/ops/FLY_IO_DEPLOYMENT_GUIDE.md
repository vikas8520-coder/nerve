---
title: "Przewodnik wdrażania Nerve na Fly.io"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Przewodnik wdrażania Nerve na Fly.io

Ten dokument opisuje rzeczywisty proces wdrażania Nerve na Fly.io i obejmuje dwa scenariusze:
...
Przewodnik opiera się na zweryfikowanej, działającej konfiguracji bieżącego projektu. Nazwa aplikacji to `nerve`.
...
- Adres dostępu: `https://nerve.fly.dev/`
...
- `app = 'nerve'` określa, do której aplikacji Fly kierowane jest wdrożenie
...
- Jeśli wcześniej używałeś innej nazwy, np. `oroute`, nie myl jej z `nerve`
...
Jeśli zmieniłeś nazwę aplikacji, zastąp `nerve` wybraną nazwą.
...
Te parametry były używane w rzeczywistych wdrożeniach bieżącej aplikacji `nerve`:
...
- `NERVE_WS_BRIDGE_SECRET` (wymagany w produkcji — służy do uwierzytelniania mostu WebSocket)
...
| `NERVE_WS_BRIDGE_SECRET`     | Wymagana w produkcji | Sekret uwierzytelniania mostu WebSocket                    |
...
1. **Ustaw `NEXT_PUBLIC_BASE_URL` na publiczną domenę HTTPS**
   ...
   Jeśli używasz własnej domeny, zastąp ją odpowiednią domeną (np. `https://nerve.yourdomain.com`).
...
- Przeznaczone dla bieżącego projektu `nerve`
...
- `NERVE_WS_BRIDGE_SECRET` jest wymagany w produkcji; jego brak zepsuje handshake mostu WebSocket
...
- Czy przeglądasz aplikację `nerve`
...
Jeśli bieżące repozytorium jest forkiem i chcesz synchronizować aktualizacje z upstream `https://github.com/diegosouzapw/Nerve`, postępuj według poniższego przepływu.
...
- Przeglądasz inną aplikację (np. `oroute` zamiast `nerve`)
