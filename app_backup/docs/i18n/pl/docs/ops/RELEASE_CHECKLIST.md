...
1. `npm stage list nerve` — znajdź stage id (wypisywany też w podsumowaniu workflow).
...
`nerve` w trybie stage-only, żeby wycieknięty długotrwały token nie mógł `npm publish`
bezpośrednio skądkolwiek — CI może tylko stage'ować; tylko 2FA właściciela wypuszcza.

**Playbook zepsutego artefaktu (bez zmian):** `npm deprecate nerve@<bad> "<reason> — use <fixed>"`
...
- [ ] Jeśli źródłowe angielskie docs się zmieniły, uruchom `npm run i18n:run` (wymaga `NERVE_TRANSLATION_API_KEY` w `.env`) przed tagowaniem
...
- [ ] Test na świeżej instalacji: usuń `~/.nerve/nerve.db` i uruchom `npm run dev`
...
> **Notatka operatorska:** zdalny katalog obrazu VPS pozostaje `/usr/lib/node_modules/nerve/app/`.
...
- [ ] `nerve --tray` bootuje na macOS (systray2 instalowany do `~/.nerve/runtime/`)
- [ ] `nerve --tray` bootuje na Linux (wymaga DISPLAY; graceful error jeśli nie ustawione)
- [ ] `nerve --tray` bootuje na Windows (PowerShell NotifyIcon, bez dodatkowych binarek)
- [ ] `nerve config tray enable` tworzy wpis autostart; disable go usuwa
- [ ] `npm install -g nerve@<this-version>` uruchamia postinstall bez fatalnego wyjścia
- [ ] Ścieżka update zachowuje optional deps: `nerve update --apply` i auto-updater
...
- [ ] `nerve status` działa bez `.env` (ścieżka tokenu CLI, tylko loopback)
...
- [ ] Wszystkie 10 plików `skills/nerve*/SKILL.md` są publicznie pobieralne przez raw GitHub URL
