...
> **TL;DR**: Nerve używa **SQLite z journalingiem WAL** jako głównego magazynu, z szyfrowaniem **AES-256-GCM** w spoczynku dla wrażliwych pól. Ten przewodnik obejmuje schemat, migracje, kopie zapasowe/odzyskiwanie oraz runbooki operacyjne.
...
Nerve wybrał SQLite zamiast PostgreSQL/MySQL z kilku powodów:
...
Dla wdrożeń **jednoużytkownikowych, jednainstancyjnych** (główny przypadek użycia Nerve) SQLite jest prostszy i szybszy.
...
| OS      | Ścieżka                                                      |
| ------- | ------------------------------------------------------------ |
| Linux   | `~/.nerve/storage.sqlite`                                |
| macOS   | `~/.nerve/storage.sqlite`                                |
| Windows | `%USERPROFILE%\.nerve\storage.sqlite`                    |
...
Baza Nerve ma **94 moduły domenowe** w `src/lib/db/`. Każdy moduł:
...
Nerve ma **94 pliki modułów** w `src/lib/db/`. Poniżej próbka kluczowych modułów; pełna lista w listingu katalogu:
...
Nerve używa **wersjonowanych, idempotentnych migracji** w `src/lib/db/migrations/`. Każda migracja to pojedynczy plik SQL o nazwie `NNN_description.sql`.
...
1. Tworzy tabelę `_nerve_migrations`, jeśli nie istnieje
...
> **Zmiany niekompatybilne wstecz** (np. usuwanie kolumn) są trudne. Nerve NIE wspiera downgrade — po zastosowaniu migracji zmiana schematu jest trwała. Planuj odpowiednio.
...
Nerve używa **`migrateLegacyEncryptedString()`**, aby przezroczyście obsługiwać dwa schematy szyfrowania:
...
nerve backup create --name pre-migration
...
nerve backup auto enable --cron "0 2 * * *" --retention 7
```
...
| ------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------ |
| `NERVE_BACKUP_SCHEDULE_JOB_INTERVAL_MS` | `30000`   | Interwał ticka w ms (min `5000`). Musi być krótszy niż 60 s, aby niezawodnie trafiać w pasującą minutę cron. |
...
```bash
sqlite3 ~/.nerve/storage.sqlite ".backup /backups/nerve-hot.db"
```

Używa online backup API SQLite — bezpieczne podczas działania Nerve.
...
Długo działające instancje Nerve zyskują na okazjonalnym `VACUUM`:

```bash
sqlite3 ~/.nerve/storage.sqlite "VACUUM;"
```
...
```bash
sqlite3 ~/.nerve/storage.sqlite "PRAGMA integrity_check;"
# Should print: ok
```
...
```bash
sqlite3 ~/.nerve/storage.sqlite ".recover" > recovered.sql
sqlite3 recovered.db < recovered.sql
mv recovered.db ~/.nerve/storage.sqlite
```
...
```bash
nerve sync pull --merge   # or: nerve backup restore <backup-id>
```
...
sqlite3 ~/.nerve/storage.sqlite "PRAGMA wal_checkpoint(TRUNCATE);"
```
...
```bash
sqlite3 ~/.nerve/storage.sqlite "SELECT * FROM api_keys LIMIT 5;"
```

### Liczba wierszy we wszystkich tabelach

```bash
sqlite3 ~/.nerve/storage.sqlite <<EOF
...
- Zabij drugi proces
- Jeśli problem się utrzymuje, zrestartuj Nerve
...
1. **Zatrzymaj Nerve** (zapobiegaj dalszym próbom)
2. **Sprawdź stan DB** przez `sqlite3`
3. **Napraw ręcznie** częściową migrację
4. **Uruchom ponownie** Nerve (migracja zostanie ponowiona)
