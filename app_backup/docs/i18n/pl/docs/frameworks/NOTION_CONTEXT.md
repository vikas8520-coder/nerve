...
Nerve może połączyć się z workspace'em **Notion** jako **źródłem kontekstu** — bazą
...
oraz dopisywać nowe bloki — wszystko proksowane przez Nerve (z retry, timeoutem
...
> strony/bazy, do których Nerve ma mieć dostęp (model uprawnień Notion opiera się
...
`POST` Nerve zapisuje token i od razu wykonuje testowe wyszukiwanie z 1 wynikiem;
...
`open-sse/mcp-server/server.ts` tylko gdy `NERVE_MCP_ENFORCE_SCOPES=true`;
dozwolone scope'y wywołującego pochodzą z `NERVE_MCP_SCOPES` (rozdzielone
