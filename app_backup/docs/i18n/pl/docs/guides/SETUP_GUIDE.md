---
title: "📖 Przewodnik instalacji — Nerve"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 📖 Przewodnik instalacji — Nerve

> Kompletne odniesienie do instalacji Nerve. Szybka wersja: [Szybki start w README](../README.md#-quick-start).
...
[Pakiet AUR](https://aur.archlinux.org/packages/nerve-bin) instaluje Nerve i udostępnia usługę użytkownika systemd.
...
Nerve dostarcza opakowanie desktopowe oparte na Electron 41 + electron-builder 26.10. Dostępne skrypty (katalog główny workspace):
...
nerve providers test-batch
```

W połączeniu ze zmiennymi środowiskowymi (`INITIAL_PASSWORD`, `NERVE_WS_BRIDGE_SECRET` itd.) pozwala to w pełni skryptowalnie uruchomić instancję Nerve.

### Opcje CLI

| Polecenie               | Opis                                                                 |
| ----------------------- | -------------------------------------------------------------------- |
| `nerve`             | Uruchom serwer (`PORT=20128`, API i dashboard na tym samym porcie)   |
| `nerve setup`       | Prowadzony onboarding CLI: hasło i pierwszy provider                 |
| `nerve doctor`      | Lokalne testy zdrowia bez uruchamiania serwera                       |
| `nerve providers`   | Odkrywaj, listuj, waliduj i testuj providerów z CLI                  |
| `nerve config`      | Konfiguracja narzędzi CLI — list, get, set, validate                 |
| `nerve status`      | Offline status dashboard — wersja, DB, narzędzia, config             |
| `nerve logs`        | Strumień logów użycia z API (obsługuje `--follow`)                   |
| `nerve update`      | Sprawdź lub zastosuj aktualizacje Nerve                          |
| `nerve provider`    | Zarządzaj połączeniami providerów — add, list, remove, test, default |
| `nerve --port 3000` | Ustaw kanoniczny/API port na 3000                                    |
| `nerve --mcp`       | Uruchom serwer MCP (transport stdio)                                 |
| `nerve --no-open`   | Nie otwieraj przeglądarki automatycznie                              |
| `nerve --help`      | Pokaż pomoc                                                          |

Konfigurację headless można zautomatyzować flagami lub zmiennymi środowiskowymi:

```bash
nerve setup --non-interactive --password "$NERVE_PASSWORD"
nerve setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY"
nerve setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY" --test-provider
```
...
nerve providers available --search openai
nerve providers available --category api-key
nerve providers list
nerve providers test <id-or-name>
...
Zamiast ręcznie wklejać base URL i klucz, pozwól Nerve zapisać konfigurację
każdego narzędzia na podstawie żywego katalogu modeli. Jedno polecenie na narzędzie:

```bash
nerve setup-codex        # ~/.codex/<name>.config.toml profiles
nerve setup-claude       # ~/.claude/profiles/<name>/settings.json
nerve setup-opencode     # ~/.config/opencode/opencode.json (openai-compatible)
nerve setup-cline        # Cline CLI + VS Code extension settings
nerve setup-kilo         # Kilo Code
nerve setup-continue     # ~/.continue/config.yaml (Continue / cn)
nerve setup-cursor       # prints Cursor's in-app steps
nerve setup-roo          # Roo Code import + autoImport pointer
nerve setup-crush        # ~/.config/crush/crush.json
nerve setup-goose        # ~/.config/goose/config.yaml
nerve setup-aider        # ~/.aider.conf.yml
nerve setup-qwen         # ~/.qwen/settings.json + ~/.qwen/.env
```

Każde przyjmuje `--remote <url> --api-key <key>`, aby skonfigurować lokalne narzędzie względem
**zdalnego** Nerve, oraz `--dry-run` do podglądu. Launchery
`nerve launch` (Claude Code) i `nerve launch-codex` (Codex) uruchamiają CLI
...
nerve_get_health        # Should return system health
nerve_list_combos       # Should return active combos
...
| `STREAM_IDLE_TIMEOUT_MS` | dziedziczy `REQUEST_TIMEOUT_MS` | Maksymalna przerwa między chunkami streamu, po której Nerve przerywa strumień SSE                                                          |
...
Dla upstreamów zgodnych z Claude Code (`anthropic-compatible-cc-*`) Nerve wyprowadza nagłówek wychodzący `X-Stainless-Timeout` z rozstrzygniętego timeoutu fetch, aby timeouty odczytu po stronie providera pozostały zsynchronizowane z konfiguracją env.

Dla zewnętrznych reverse proxy zgodnych z Claude Code Nerve utrzymuje domyślny zestaw `anthropic-beta` konserwatywny i, gdy `Client Cache Control` jest na `Auto`, przekazuje tylko markery `cache_control` dostarczone przez klienta. Włącz przełącznik per połączenie „Enable redact-thinking beta” tylko wtedy, gdy upstream wymaga zredagowanych strumieni myślenia Claude.
...
> **Uwaga:** Przy żądaniach streamingowych `FETCH_TIMEOUT_MS` obejmuje tylko nawiązanie połączenia / oczekiwanie na pierwszą odpowiedź upstream. Gdy stream jest aktywny, Nerve przerywa tylko przy rzeczywistym zastoju (`STREAM_IDLE_TIMEOUT_MS`) lub bezczynności body Undici (`FETCH_BODY_TIMEOUT_MS`).

### Zgodność z reverse proxy

Jeśli uruchamiasz Nerve za Nginx, Caddy, Cloudflare lub innym reverse proxy, upewnij się, że timeouty proxy są też wyższe niż timeouty stream/fetch Nerve.
...
Dla użytkowników Void Linux możesz zbudować natywny pakiet przez `xbps-src`. Zapisz ten blok jako `srcpkgs/nerve/template`:
...
#   curl -L -o /tmp/nerve.tar.gz "https://github.com/diegosouzapw/Nerve/archive/refs/tags/v${version}.tar.gz" && sha256sum /tmp/nerve.tar.gz
