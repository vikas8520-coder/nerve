...
Typowe problemy i rozwiązania dla Nerve.

---

## Szybki przewodnik

**Nowy w Nerve?** Zacznij tutaj — te wskazówki rozwiązują 90% problemów:

| Widzę to                | Co to oznacza                      | Co zrobić                                                                             |
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------------------------- |
| "Can't connect"         | Nerve nie działa               | Uruchom `nerve` lub `docker restart nerve`                                    |
...
| EACCES: permission denied                                  | Ustaw `DATA_DIR=/path/to/writable/dir`, by nadpisać `~/.nerve`                                                                                                         |
...
| `dlopen` / `slice is not valid mach-o file` (macOS)        | Uruchom `cd $(npm root -g)/nerve/app && npm rebuild better-sqlite3 && nerve` — zobacz [przebudowa natywnego modułu na macOS](#macos-native-module-rebuild) poniżej |
...
linków wyglądających jak żądania HTTP. `README.md` Nerve jest w paczce npm (jest
wymieniony w `package.json` → `files`), więc ląduje w `node_modules/nerve/README.md` przy
...
`%LOCALAPPDATA%\Programs\Nerve` (w tym katalogi paczek z sufiksem hasha z
...
1. **Najpierw zweryfikuj pobranie** (wyklucza spreparowany plik). Każde wydanie publikuje
   `latest.yml`, którego pole `sha512` (base64) obejmuje instalator `Nerve.Setup.<version>.exe`.
   W PowerShell, z folderu z instalatorem:
   ```powershell
   $b = [System.Security.Cryptography.SHA512]::Create().ComputeHash(
     [System.IO.File]::ReadAllBytes("$PWD\Nerve.Setup.<version>.exe"))
   [Convert]::ToBase64String($b)
   ```
   Wynik musi pasować do `latest.yml` → `sha512`. Jeśli nie, usuń plik i
   pobierz ponownie wyłącznie ze [strony wydań GitHub](https://github.com/diegosouzapw/Nerve/releases).
2. **Przywróć + wyklucz** — przywróć cofnięte elementy z kwarantanny i dodaj wykluczenie
   dla `%LOCALAPPDATA%\Programs\Nerve` (Kaspersky → Settings → Threats and Exclusions),
...
**Przyczyna:** Uruchamiasz wersję Node.js poza zatwierdzonym bezpiecznym progiem runtime Nerve. Najczęstszy przypadek to starszy patch Node 22 lub 24 poniżej wymaganego przez Nerve zabezpieczonego progu.
...
3. Zainstaluj ponownie Nerve: `npm install -g nerve`
4. Uruchom ponownie: `nerve`
...
**Przyczyna:** Po globalnym `npm install -g nerve` natywne binarium `better-sqlite3` w paczce mogło zostać skompilowane pod inną architekturę lub ABI Node.js niż lokalnie uruchomione. To częste na macOS (Apple Silicon i Intel), gdy prebuilt nie pasuje do środowiska.
...
```
dlopen(/Users/<user>/.nvm/versions/node/v24.14.1/lib/node_modules/nerve/app/node_modules/better-sqlite3/build/Release/better_sqlite3.node, 0x0001): tried: '...' (slice is not valid mach-o file)
```

**Naprawa — przebuduj pod lokalne środowisko (bez obniżania Node.js):**

```bash
cd $(npm root -g)/nerve/app
...
**Naprawa (v3.5.5+):** Nerve używa teraz własnego `fetch()` z undici, gdy aktywny jest dispatcher proxy, co daje spójne zachowanie. Zaktualizuj do v3.5.5+.

### Proxy MITM pod WSL: aplikacje desktop na hoście Windows nie są przechwytywane

**Przyczyna:** Proxy MITM i jego certyfikat CA instalują się w środowisku, w którym działa Nerve. Pod WSL to gość Linux, a aplikacje AI desktop (Kiro, Trae, Copilot, Zed, …) działają na hoście Windows. Aplikacje hosta nie ufają magazynowi certyfikatów gościa i nie idą przez systemowe proxy gościa, więc przechwytywanie desktop tam nie działa.

**Rekomendacja:** Uruchamiaj Nerve natywnie na tym samym OS co aplikacje desktop, które chcesz przechwytywać (Windows dla aplikacji Windows; analogicznie macOS/Linux). Trzymanie Nerve w WSL przy celowaniu w aplikacje hosta wymaga ręcznego zaufania wygenerowanemu certyfikatowi CA na hoście Windows i wskazania ustawień sieci/proxy każdej aplikacji hosta na endpoint proxy WSL — to nieobsługiwana, krucha konfiguracja.
...
Nerve automatycznie odświeża tokeny. Jeśli problemy trwają:
...
2. Sprawdź, czy `CLOUD_URL` wskazuje na endpoint chmury (np. `https://nerve.dev`)
...
Gdy wiele równoległych żądań trafia w rate-limitowanego providera, Nerve używa mutex + auto rate-limiting, by serializować żądania i zapobiec kaskadowym awariom. To automatyczne dla providerów z kluczem API.

---

## Opcjonalna taksonomia awarii RAG / LLM (16 problemów)

Część użytkowników Nerve stawia gateway przed stackami RAG lub agentów. W takich setupach często widać dziwny wzorzec: Nerve wygląda na zdrowy (providerzy w górze, profile routingu OK, brak alertów rate limit), a końcowa odpowiedź i tak jest zła.
...
1. Przy badaniu złej odpowiedzi zbierz:
   ...
   - route lub combo providera w Nerve
...
3. Zapisz numer we własnym dashboardzie, runbooku lub trackerze incydentów obok logów Nerve.
...
Możesz pominąć tę sekcję, jeśli nie uruchamiasz pipeline’ów RAG ani agentów za Nerve.
...
2. Zrestartuj Nerve, by nowe wartości env weszły w życie
...
3. Zrestartuj Nerve i przetestuj ponownie z **Dashboard → CLI Tools**
...
- Uruchom `nerve providers` z CLI, by ponownie odpalić flow OAuth, albo
...
### Brak `NERVE_WS_BRIDGE_SECRET` w produkcji
...
**Przyczyna:** Brak zmiennej env `NERVE_WS_BRIDGE_SECRET` w środowisku produkcyjnym.

**Naprawa:**

1. Wygeneruj losowy sekret: `openssl rand -hex 32`
2. Ustaw `NERVE_WS_BRIDGE_SECRET=<random-secret>` w env serwera produkcyjnego (i każdego klienta łączącego się z mostem)
3. Zrestartuj Nerve
...
- **GitHub Issues**: [github.com/diegosouzapw/Nerve/issues](https://github.com/diegosouzapw/Nerve/issues)
