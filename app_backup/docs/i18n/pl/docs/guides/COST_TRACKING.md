...
Jak Nerve szacuje, rejestruje i raportuje koszt każdego żądania — oraz dlaczego
...
Nerve przypisuje koszt w USD do każdego completion, mnożąc liczbę tokenów przez
stawki cenowe modelu. Te liczby zasilają dashboard **Costs**, CLI
`nerve cost` / `nerve usage`, eksporty CSV/JSON oraz budżety per klucz API.

> **„Koszt” na dashboardzie to tracker oszczędności, a nie rachunek.** Nerve nigdy
...
- Zależy od tabeli cen Nerve dla każdego modelu. Model bez wpisu cenowego
...
3. **Zakodowane na stałe domyślne** — dostarczane z Nerve.
...
- Wydatki kluczy API są buforowane i flushowane partiami przez
  ...
  | Env var                             | Default | Cel                                             |
  | ----------------------------------- | ------- | ----------------------------------------------- |
  | `NERVE_SPEND_FLUSH_INTERVAL_MS` | `60000` | Interwał flush w milisekundach.                 |
  | `NERVE_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Maks. liczba buforowanych wpisów przed flushem. |
...
CLI Nerve udostępnia komendy cost, usage i pricing (zarejestrowane w
...
```bash
nerve cost                          # last 30d, grouped by provider
nerve cost --period 7d              # last 7 days
nerve cost --group-by model         # group by provider | model | combo | api-key | day
nerve cost --since 2026-06-01 --until 2026-06-13
nerve cost --api-key <key> --limit 50
```
...
```bash
nerve usage analytics --period 30d [--provider <id>]   # per-provider cost summary
nerve usage logs [--limit 100] [--follow] [--api-key <k>] [--search <q>]
nerve usage quota [--provider <id>] [--check]
nerve usage utilization [--api-key <k>]
nerve usage history [--limit 100]
nerve usage proxy-logs [--limit 100]

# Budgets
nerve usage budget list
nerve usage budget get [scope]
nerve usage budget set <amount> [--scope global] [--period monthly]
nerve usage budget reset [scope]
```

### `nerve pricing`

```bash
nerve pricing list [--provider <p>] [--model <m>] [--limit 200]
nerve pricing get <model>
nerve pricing sync [--provider <p>] [--force]   # POST /api/pricing/sync
nerve pricing diff [--model <m>]
nerve pricing defaults show
nerve pricing defaults set [--input <p>] [--output <p>] [--cache-read <p>] [--cache-write <p>]
```
...
- **Wszystkie koszty pokazują $0 / „Legacy / Free”.** Używane modele nie mają wpisu
  ...
  `nerve pricing sync`, albo ustaw ceny ręcznie na stronie Pricing / przez
  `PATCH /api/pricing`.
...
- **Wydatki opóźniają się względem czasu rzeczywistego.** Spend per klucz jest batchowany;
  obniż `NERVE_SPEND_FLUSH_INTERVAL_MS`, jeśli potrzebujesz świeższych liczb.
