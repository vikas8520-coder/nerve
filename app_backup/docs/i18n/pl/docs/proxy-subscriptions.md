...
> proxy w Nerve. To jest cięcie v1: pojedynczy operator wkleja linki
> subskrypcji, wybiera tryb (global lub rule), a Nerve wiąże wynikową pulę
...
Dziś pula proxy Nerve jest ręcznie kuratorowana: każdy węzeł żyje w
...
Nerve i ponownie je wiązać przy każdej zmianie listy upstream.

Celem v1 jest uczynienie Nerve first-class dla subskrypcji
...
| U5  | Operator | podać lokalny endpoint SOCKS5 sing-box/clash                          | węzły SS/VMess/Trojan/VLESS (których dispatcher Nerve nie mówi natywnie) stawały się używalne przez lokalny mostek kernela |
