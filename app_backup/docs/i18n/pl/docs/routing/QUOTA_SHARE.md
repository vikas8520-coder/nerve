...
**Problem, który rozwiązuje:** Nerve proxy’uje wiele kluczy API względem
...
- Cała persystencja w istniejącej bazie SQLite Nerve (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("nerve:quota-share:pools")` jest niepuste.
