# Runbook reagowania na incydenty — Nerve (2026-06-18)
...
| **SEV-3** | Uśpiony błąd lub near-miss; brak bieżącego wpływu na użytkownika, ale budżet błędów zagrożony.               | Wyciek pamięci w trendzie wzrostowym; circuit breaker wyłącza jednego dostawcę. | Slack `#nerve-ops` (następny standup) | 7 d                          |
...
| Panele SLO w Grafana              | Panele burn-rate SLO                 | Slack `#nerve-ops`                |
...
Alerty **nie** idą na prywatne DM. Domyślny kanał to `#nerve-ops`; PagerDuty
...
2. **Zadeklaruj**. Opublikuj w `#nerve-ops`:
   ```
...
3. **Stabilizuj** przed diagnozą główną przyczyny. Preferowana kolejność:
   - Odetnij zły deploy: `kubectl rollout undo deploy/nerve` (lub
   ...
   - Włącz tryb degradacji: ustaw
     `NERVE_DEGRADATION_MODE=lite` (pomija niekrytyczne middleware).
...
- **Wewnętrzna**: `#nerve-ops` jest źródłem prawdy. Wątek na incydent na
  deklarację; wszystkie aktualizacje w wątku.
- **Zewnętrzna** (gdy dotyczy użytkowników zewnętrznych): status page
  (status.nerve.example — zastąp prawdziwym URL, gdy będzie live). SEV-1
