...
> **TL;DR**: Silnik kompresji Nerve jest **podłączalny (pluggable)** — możesz rejestrować własne silniki, dostarczać language packi dla nowych języków i składać stacked pipelines. Ten przewodnik pokazuje jak.
...
````ts
import type { CompressionEngine } from "nerve/compression/engines/types";
import { registerCompressionEngine } from "nerve/compression/engines/registry";
...
```
~/.nerve/compression/engines/my-engine.ts    # User-level
...
} from "@nerve/open-sse/services/compression/engines/registry";
...
Kompresja w stylu Caveman używa **pakietów reguł zależnych od języka**, aby obsługiwać wypełniacze (fillers), hedging i rozwlekłe wzorce w każdym języku naturalnym. Nerve dostarcza **6 language packów**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "nerve/compression/ruleLoader";
...
```
~/.nerve/compression/rules/hi/filler.json  # User-level
...
Nerve wybiera **JEDEN tryb na żądanie** na podstawie konfiguracji, progów auto-trigger i override'ów combo.
...
`@nerve/open-sse/services/compression/strategySelector`):

```ts
import { applyStackedCompression } from "@nerve/open-sse/services/compression/strategySelector";
...
Silniki kompresji Nerve w README przypisują zasługi kilku projektom upstream
(„inspired by RTK, Caveman, LLMLingua-2, Troglodita”). Częste pytanie kontrybutorów:
**gdy upstream RTK doda nowy filtr toola albo Caveman doda rule pack, jak to trafia do Nerve?**
...
reguła/filtr w formacie Nerve** (zob.
...
własne warstwy Nerve `smartcrusher` / `toon` / `tabular`. Nie ma żywego
...
1. **Nie vendoruj** — wyraź regułę/filtr upstream w formacie Nerve.
