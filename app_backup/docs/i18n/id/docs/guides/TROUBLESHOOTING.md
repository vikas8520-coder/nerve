...
Masalah umum dan solusinya untuk Nerve.
...
| EACCES: permission denied                           | Atur `DATA_DIR=/path/to/writable/dir` untuk mengganti `~/.nerve`                                                                                                    |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Jalankan `cd $(npm root -g)/nerve/app && npm rebuild better-sqlite3 && nerve` — lihat [Pembangunan ulang modul native macOS](#macos-native-module-rebuild) di bawah |
...
**Penyebab:** Anda menjalankan versi Node.js di luar batas runtime aman yang disetujui Nerve. Kasus paling umum adalah menjalankan Node 20, 22, atau 24 versi patch lama yang berada di bawah batas keamanan yang diperlukan Nerve.
...
3. Instal ulang Nerve: `npm install -g nerve`
4. Mulai ulang: `nerve`
...
**Solusi (v3.5.5+):** Nerve sekarang menggunakan fungsi `fetch()` milik undici sendiri ketika dispatcher proxy aktif, memastikan perilaku yang konsisten. Perbarui ke v3.5.5+.
...
Nerve memperbarui token secara otomatis. Jika masalah berlanjut:
...
2. Pastikan `CLOUD_URL` mengarah ke endpoint cloud Anda (misalnya, `https://nerve.dev`)
...
Ketika banyak permintaan bersamaan mengenai penyedia yang dibatasi lajunya, Nerve menggunakan mutex + auto rate-limiting untuk membuat serialisasi permintaan dan mencegah kegagalan berantai. Ini berjalan otomatis untuk penyedia dengan API key.

---

## Taksonomi Kegagalan RAG / LLM Opsional (16 masalah)

Beberapa pengguna Nerve menempatkan gateway di depan tumpukan RAG atau agen. Dalam pengaturan tersebut, umum terjadi pola yang aneh: Nerve terlihat sehat (penyedia aktif, profil routing baik, tidak ada peringatan batas laju) tetapi jawaban akhir masih salah.
...
1. Saat Anda menyelidiki respons yang buruk, kumpulkan:
   ...
   - route atau combo penyedia di Nerve
...
3. Simpan nomornya di dashboard, runbook, atau pelacak insiden Anda sendiri di samping log Nerve.
...
Anda dapat mengabaikan bagian ini jika Anda tidak menjalankan pipeline RAG atau agen di belakang Nerve.
