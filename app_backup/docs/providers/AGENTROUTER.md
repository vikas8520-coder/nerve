...
For most users, **no special setup is required**. Nerve ships a built-in
...
Nerve also supports AgentRouter (and similar relays) through the **Claude Code
...
- Nerve running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting Nerve:
...
**`Invalid JSON response from provider (reset after Ns)` from the nerve logs** —
...
model actually resolves in the nerve logs (`ROUTING` tag shows
