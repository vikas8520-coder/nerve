...
Common problems and solutions for Nerve.

---

## Quick Reference

**New to Nerve?** Start here — these solve 90% of problems:

| I see this              | What it means                       | What to do                                                                                        |
| ----------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | Nerve isn't running                 | Run `nerve` or `docker restart nerve`                                                             |
...
| EACCES: permission denied                                  | Set `DATA_DIR=/path/to/writable/dir` to override `~/.nerve`                                                                                               |
...
| `dlopen` / `slice is not valid mach-o file` (macOS)        | Run `cd $(npm root -g)/nerve/app && npm rebuild better-sqlite3 && nerve` — see [macOS native module rebuild](#macos-native-module-rebuild) below          |
...
HTTP-request-looking links. Nerve's `README.md` ships inside the npm package (it is
listed in `package.json` → `files`), so it lands at `node_modules/nerve/README.md` on
...
`%LOCALAPPDATA%\Programs\Nerve` (including hash-suffixed package directories from the
...
1. **Verify your download first** (rules out a tampered file). Every release publishes
   `latest.yml`, whose `sha512` field (base64) covers the `Nerve.Setup.<version>.exe`
   ...
   re-download only from the [GitHub releases page](https://github.com/diegosouzapw/Nerve/releases).
2. **Restore + exclude** — restore the rolled-back items from quarantine and add an exclusion
   for `%LOCALAPPDATA%\Programs\Nerve` (Kaspersky → Settings → Threats and Exclusions),
   then reinstall.
...
**Cause:** You are running a Node.js version outside Nerve's approved secure runtime floor. The most common case is running an older Node 22 or 24 patch level that falls below the patched security floor Nerve requires.
...
3. Reinstall Nerve: `npm install -g nerve`
4. Restart: `nerve`
...
**Cause:** After a global `npm install -g nerve`, the `better-sqlite3` native binary inside the package may have been compiled for a different architecture or Node.js ABI than what is running locally. This is common on macOS (both Apple Silicon and Intel) when the pre-built binary does not match your environment.
...
**Fix (v3.5.5+):** Nerve now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.

### MITM proxy under WSL: desktop apps on the Windows host are not intercepted

**Cause:** The MITM proxy and its CA certificate install into the environment where Nerve runs. Under WSL that environment is the Linux guest, while the AI desktop apps (Kiro, Trae, Copilot, Zed, …) run on the Windows host. The host apps do not trust the guest's certificate store and do not route through the guest's system proxy, so desktop interception does not engage there.

**Recommendation:** Run Nerve natively on the same OS as the desktop apps you want to intercept (Windows for Windows apps; macOS/Linux likewise). Keeping Nerve inside WSL while targeting host apps requires manually trusting the generated CA certificate on the Windows host and pointing each host app's network/proxy settings at the WSL proxy endpoint — an unsupported, fragile setup.
...
Nerve auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://nerve.dev`)
...
When many concurrent requests hit a rate-limited provider, Nerve uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some Nerve users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: Nerve looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in Nerve
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the Nerve logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind Nerve.
...
2. Restart Nerve so the new env values are picked up
...
3. Restart Nerve and re-test from **Dashboard → CLI Tools**
...
- Run `nerve providers` from the CLI to re-trigger the OAuth flow, or
...
### NERVE_WS_BRIDGE_SECRET missing in production
...
**Cause:** The `NERVE_WS_BRIDGE_SECRET` env var is missing from the production environment.

**Fix:**

1. Generate a random secret: `openssl rand -hex 32`
2. Set `NERVE_WS_BRIDGE_SECRET=<random-secret>` in the production server env (and any client that talks to the bridge)
3. Restart Nerve
