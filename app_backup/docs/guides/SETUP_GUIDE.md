...
> Complete setup reference for Nerve. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/nerve-bin) installs Nerve and provides a systemd user service.
...
Nerve ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
Combined with env vars (`INITIAL_PASSWORD`, `NERVE_WS_BRIDGE_SECRET`, etc.), this lets you spin up an Nerve instance fully scriptable.
...
| `nerve setup`       | Guided CLI onboarding for password and first provider          |
| `nerve doctor`      | Run local health checks without starting the server            |
| `nerve providers`   | Discover, list, validate, and test providers from CLI          |
| `nerve config`      | CLI tool configuration — list, get, set, validate configs      |
| `nerve status`      | Offline status dashboard — version, DB, tools, config          |
| `nerve logs`        | Stream usage logs from the API (supports `--follow`)           |
| `nerve update`      | Check for or apply Nerve updates                               |
| `nerve provider`    | Manage provider connections — add, list, remove, test, default |
...
Instead of pasting the base URL and key by hand, let Nerve write each tool's
...
**remote** Nerve, plus `--dry-run` to preview. The launchers
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), Nerve derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, Nerve keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, Nerve will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run Nerve behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your Nerve stream/fetch timeouts.
