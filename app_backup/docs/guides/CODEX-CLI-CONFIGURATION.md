---
title: "Codex CLI — Configuration with Nerve"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — Configuration with Nerve

Complete guide for using the Codex CLI pointed at Nerve as an OpenAI-compatible backend.
...
**Nerve solves this transparently:**
...
You never need a separate translation proxy when using Nerve. **All models use `wire_api = "responses"`** — Nerve handles the rest.
...
| Model                                | Nerve ID                             | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in Nerve use the `cx/` prefix:

| Codex CLI name          | Nerve model        |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the Nerve provider alias.
...
## Generating profiles automatically with `nerve setup-codex`

If you run Nerve on a VPS, you can auto-generate profile files from the live model catalog:
...
Nerve can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `NERVE_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `nerve launch-codex`

Health-checks your Nerve instance before launching Codex:
...
Two Nerve defaults can silently sabotage multi-hour Codex CLI sessions. Neither is a Codex CLI setting — both live on the Nerve side. Users migrating a config from upstream proxies that pin accounts and disable idle cutoffs often hit both and conclude Nerve “cannot sustain a long session.”
...
Related discussions: [#7126](https://github.com/diegosouzapw/Nerve/discussions/7126) (long task drops), [#5718](https://github.com/diegosouzapw/Nerve/discussions/5718) (why affinity defaults off). Tracking: [#7287](https://github.com/diegosouzapw/Nerve/issues/7287).
...
Critical detail: Nerve’s synthetic SSE **heartbeat does not reset** the idle clock. Only a real upstream body chunk updates `lastChunkTime`. A quiet model that is still “thinking” looks identical to a stalled upstream from the watchdog’s point of view.
...
In the Nerve process environment (`.env` / compose / systemd):
...
Restart Nerve after changing these env vars.
...
2. **Raise / disable idle cutoffs** in Nerve’s environment:
...
3. Keep the usual Codex `config.toml` (`wire_api = "responses"`, correct `base_url`, `NERVE_API_KEY`) — no Codex-side affinity/idle knobs exist for these two behaviors.
4. Restart Nerve, then start the long Codex task.
...
When the idle watchdog fires, Nerve logs a line shaped like:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grep for `Idle timeout: no data from` (or the code `stream_idle_timeout` / error name `StreamIdleTimeoutError`). The provider segment is whatever Nerve used for that request (`codex`, another provider id, or `provider` if unknown) — it is not always the literal string `codex`.
...
Verify the model exists in Nerve with the correct prefix. Use `nerve models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `NERVE_API_KEY` is exported: `echo $NERVE_API_KEY`.

**`Connection refused`**
Verify Nerve is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
...
See [Long-running tasks](#long-running-tasks). Enable session affinity (TTL above task length) and raise or disable `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grep Nerve logs for `Idle timeout: no data from`.
