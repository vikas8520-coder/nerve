...
> **TL;DR**: Nerve ranks its **free** providers by model quality using **Arena AI
...
Nerve aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to Nerve task categories:

| Arena leaderboard | Nerve task categories                             |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, Nerve keeps
...
stripped and a small alias map expands canonical names into the variants Nerve uses
