...
Nerve ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
- Windows: `%APPDATA%\nerve`
- Linux: `$XDG_CONFIG_HOME/nerve` or `~/.nerve`
- macOS: `~/.nerve`
...
- Close button minimizes to tray; the tray menu has **Open Nerve**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
Uses `electron-updater` with the GitHub provider (`diegosouzapw/Nerve`).
...
Releases are published to GitHub Releases (`diegosouzapw/Nerve`), which is also where `electron-updater` checks for new versions.
