...
Nerve is a local AI routing gateway and dashboard built on Next.js.
...
Nerve can install, supervise, and route to locally-running AI tool processes
...
- Nerve response metadata builder: `src/domain/nerveResponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/nerve/storage.sqlite` when set, else `~/.nerve/storage.sqlite`)
...
> Nerve v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by Nerve; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/nerve` -> `~/.nerve`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@nerve/open-sse` **npm workspace package**. Source code imports it via `@nerve/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t nerve .`
