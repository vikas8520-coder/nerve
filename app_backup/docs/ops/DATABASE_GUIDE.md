...
> **TL;DR**: Nerve uses **SQLite with WAL journaling** as its primary store, with **AES-256-GCM** encryption at rest for sensitive fields. This guide covers the schema, migrations, backup/recovery, and operational runbooks.
...
Nerve chose SQLite over PostgreSQL/MySQL for several reasons:
...
For **single-user, single-instance** deployments (the primary Nerve use case), SQLite is simpler and faster.
...
Nerve's database has **94 domain modules** in `src/lib/db/`. Each module:
...
Nerve has **94 module files** in `src/lib/db/`. Below is a sampling of core modules; see the directory listing for the complete list:
...
Nerve uses **versioned, idempotent migrations** in `src/lib/db/migrations/`. Each migration is a single SQL file named `NNN_description.sql`.
...
1. Creates `_nerve_migrations` table if not exists
...
> **Backwards-incompatible changes** (e.g., dropping columns) are tricky. Nerve does NOT support downgrade — once a migration is applied, the schema change is permanent. Plan accordingly.
...
Nerve uses **`migrateLegacyEncryptedString()`** to handle two encryption schemes transparently:
...
| --------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------- |
| `NERVE_BACKUP_SCHEDULE_JOB_INTERVAL_MS` | `30000` | Tick interval in ms (min `5000`). Must be shorter than 60 s to reliably land inside the matching cron minute. |
...
This uses SQLite's online backup API — safe to run while Nerve is running.
...
Long-running Nerve instances benefit from occasional `VACUUM`:
...
1. **Stop Nerve** (prevent further attempts)
2. **Check the DB state** with `sqlite3`
3. **Manually fix** the partial migration
4. **Re-run** Nerve (the migration will be retried)
