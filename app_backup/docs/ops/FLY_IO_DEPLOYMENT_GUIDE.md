---
title: "Nerve Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Nerve Fly.io Deployment Guide

This document describes the actual deployment process for Nerve on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `nerve`.
...
- Access URL: `https://nerve.fly.dev/`
...
- `app = 'nerve'` determines which Fly application the deployment targets
...
- If you previously used a different name, such as `oroute`, do not confuse it with `nerve`
...
```powershell
flyctl apps create nerve
```

If you changed the application name, replace `nerve` with your chosen name.
...
These parameters have been used in actual deployments on the current `nerve` application:
...
- `NERVE_WS_BRIDGE_SECRET` (required in production — used for WebSocket bridge authentication)
...
| `NERVE_WS_BRIDGE_SECRET`      | Required in production | WebSocket bridge authentication secret                |
...
| ---------------------- | ----------------------- |
| `DATA_DIR`             | `/data`                 |
| `NEXT_PUBLIC_BASE_URL` | `https://nerve.fly.dev` |
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://nerve.fly.dev -a nerve
   ```

   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://nerve.yourdomain.com`).
...
- Intended for the current project `nerve`
...
flyctl secrets set `
  ...
  NERVE_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://nerve.fly.dev `
  -a nerve
```
...
```bash
flyctl secrets set NERVE_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a nerve
```

Notes:

- `NERVE_WS_BRIDGE_SECRET` is required in production; missing it will break the WebSocket bridge handshake

If you also want to set an initial password:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a nerve
```
...
```powershell
flyctl secrets set KEY=value -a nerve
```
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/diegosouzapw/Nerve`, follow the workflow below.
...
```powershell
git remote add upstream https://github.com/diegosouzapw/Nerve.git
```
...
5. `flyctl deploy`
6. `flyctl status -a nerve`
7. `flyctl logs --no-tail -a nerve`
...
```powershell
flyctl status -a nerve
```

### 10.2 View Startup Logs

```powershell
flyctl logs --no-tail -a nerve
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://nerve.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- You are viewing a different application (e.g. `oroute` instead of `nerve`)
...
1. `flyctl auth login`
2. `flyctl apps create nerve`
3. `flyctl secrets set ... -a nerve`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a nerve`
