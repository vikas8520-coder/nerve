...
Nerve integrates with providers whose edges actively fingerprint non-official clients (TLS JA3/JA4, header ordering, JSON body shape, integrity tokens). This page documents the stealth surfaces Nerve exposes and where they are implemented.

## Legal and Ethical Notice

Stealth features exist so Nerve can act as a compatibility layer between user-owned official accounts (Claude Code CLI, ChatGPT Desktop/Web, Antigravity, Cursor, etc.) and Nerve's unified API. They are **not** for evading fraud detection, sharing credentials, or violating provider Terms of Service. The maintainers expect operators to comply with the upstream ToS they signed when creating accounts.
...
- Proxy resolution (priority): per-call `proxyUrl` → `NERVE_TLS_PROXY_URL` → `HTTPS_PROXY`/`HTTP_PROXY`/`ALL_PROXY` (the native binding does **not** read these envs itself; it must be threaded through)
...
When `cliCompatMode` is on, Nerve reshapes outgoing Claude requests so they are indistinguishable from `claude-cli` traffic. Three modules collaborate:
...
Server-side integrity check the official Claude Code CLI computes via Bun/Zig. Nerve reimplements with `xxhash-wasm`:
...
Antigravity requests preserve caller text byte-for-byte. Nerve does not insert zero-width characters into prompts or rename/inject tools to imitate an IDE client.
...
The upstream enforcement is on **Google's side**, not anything Nerve can prevent. The env var name and the existing docs make it sound like a safe knob to flip; it isn't.
...
- Multiple Nerve users hitting overage credits in parallel from the same external IP compounds the signal.
...
- Original incident report: Discussion [#1183](https://github.com/diegosouzapw/Nerve/discussions/1183)
...
For CLIs whose binaries cannot be redirected via `OPENAI_BASE_URL`, Nerve runs a local TLS-terminating proxy. Endpoints live under `src/app/api/cli-tools/antigravity-mitm/`.
...
Cert filename: `nerve-mitm.crt`. Fingerprint match via `getCertFingerprint()` (SHA-1 of DER).

Additionally, `updateNssDatabases()` installs into per-user NSS DBs when `certutil` is available: `~/.pki/nssdb`, `~/snap/chromium/.../nssdb`, all Firefox profiles (including snap), under the nickname **`Nerve MITM Root CA`**.
...
Nerve scrubs inbound client headers before forwarding so a request that arrives from Cursor doesn't leak `User-Agent: Cursor/X.Y.Z` to a Claude upstream. See `src/shared/constants/upstreamHeaders.ts` for the denylist, kept in lockstep with the Zod schemas and unit tests.
