...
between Nerve and upstream providers. Each guardrail can inspect (and
...
- Header `x-nerve-disabled-guardrails` (or legacy
...
- **`garak` (advisory)** — runs garak `--probes promptinject,dan,leakreplay`
  against a local Nerve instance (`http://localhost:20128/v1`). Gated on a
