...
Nerve CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-nerve-cli-token`
request header.

This allows CLI subcommands (`nerve status`, `nerve providers`, etc.)
...
3. The CLI sends the token as `x-nerve-cli-token` on every request to
...
Set `NERVE_CLI_SALT` to rotate the derived token without code changes.
...
export NERVE_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
nerve status
```

Default salt: `nerve-cli-auth-v1`
...
**Opt-out:** set `NERVE_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
