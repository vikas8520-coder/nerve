...
Nerve controls which **browser origins** may read cross-origin responses
...
taking seriously, but Nerve's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. Nerve's shared CORS path never
> does.
...
- If Nerve runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
## Example: reverse proxy in front of Nerve

CORS is enforced by Nerve itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let Nerve answer preflight:
...
Set the allowed browser origins in Nerve (`CORS_ALLOWED_ORIGINS` or the
