...
> **TL;DR**: Nerve aggregates free tiers from 50+ providers. Connect multiple free providers for unlimited free AI with automatic fallback.
...
Nerve **aggregates** these free tiers into one endpoint. Instead of signing up for 10 different services, you connect them all to Nerve and use `model: "auto"` to automatically pick the best free option for each request.
...
The magic of Nerve is **stacking free tiers**. Instead of relying on one provider, you connect multiple free providers and let Nerve automatically pick the best one for each request.
...
Then use `model: "auto"` and Nerve will:
...
## How Nerve Makes Free Tiers Better

### 1. Automatic Fallback

If one free provider is busy or down, Nerve automatically tries the next one. You don't need to do anything.

### 2. Smart Routing

Nerve picks the **best free provider** for each request based on:
...
Nerve's **compression** feature saves 15-95% of tokens. This means your free quota lasts **5-20x longer**.

### 4. Multi-Account Support

If you have multiple accounts for the same provider, Nerve treats each as a separate candidate. This doubles or triples your free quota.
...
**Yes!** These are official free tiers from the providers. Nerve just makes it easier to use them all at once.
...
No catch! Providers offer free tiers to attract users. Nerve just makes it easier to use them all at once.
