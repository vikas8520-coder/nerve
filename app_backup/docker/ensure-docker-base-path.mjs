...
/**
 * Compares NERVE_BASE_PATH at container start against BUILD_NERVE_BASE_PATH
...
const SENTINEL = "BUILD_NERVE_BASE_PATH";
...
export function ensureDockerBasePath(opts = {}) {
  ...
  const runtime = normalizeBasePath(opts.env?.NERVE_BASE_PATH ?? process.env.NERVE_BASE_PATH);
  ...
  if (!runtime && baked) {
    throw new Error(
      `This Nerve image was built for subpath ${baked}, but NERVE_BASE_PATH is unset. ` +
        `Set NERVE_BASE_PATH=${baked} or rebuild without the build-arg.`
    );
  }
  ...
  try {
    ...
    console.error(
      "[ensure-docker-base-path] Rebuild the image with the same subpath, e.g.\n" +
        "  docker compose build --build-arg NERVE_BASE_PATH=/nerve"
    );
    process.exit(1);
  }
}
