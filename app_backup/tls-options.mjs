...
//
// Pure, dependency-light helpers for Nerve's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `NERVE_TLS_CERT` and `NERVE_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.NERVE_TLS_CERT === "string" ? env.NERVE_TLS_CERT.trim() : "";
  const keyPath = typeof env?.NERVE_TLS_KEY === "string" ? env.NERVE_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[nerve][tls] HTTPS not enabled: both NERVE_TLS_CERT and NERVE_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[nerve][tls] HTTPS not enabled: could not read TLS cert/key ` +
