...
Run Nerve with **podman compose** on Linux, macOS, or Windows, or with
...
```bash
cd /path/to/nerve
podman build --target runner-base -t nerve:base .
# For web-cookie providers (gemini-web, claude-web, claude-turnstile):
podman build --target runner-web -t nerve:web .
# For CLI tool support:
podman build --target runner-cli -t nerve:cli .
```

### 2. Copy Quadlet files to the systemd directory

```bash
mkdir -p ~/.config/containers/systemd/nerve
cp contrib/podman/*.container ~/.config/containers/systemd/nerve/
cp contrib/podman/*.network ~/.config/containers/systemd/nerve/
cp contrib/podman/*.volume ~/.config/containers/systemd/nerve/
```

### 3. Mount the project .env for secrets

Edit `~/.config/containers/systemd/nerve/nerve.container` and
uncomment/replace the `EnvironmentFile` line with the absolute path to
your project `.env`:

```
EnvironmentFile=/home/USER/code/docker/Nerve/.env
```
...
systemctl --user start nerve-redis
systemctl --user start nerve
```
...
The application profiles use local image names such as `nerve:base`; those
...
```bash
podman volume create nerve-data
podman run -d --name nerve \
  --env-file .env \
  -e DATA_DIR=/app/data \
  -p 20128:20128 \
  -v nerve-data:/app/data \
  docker.io/diegosouzapw/nerve:latest
```
