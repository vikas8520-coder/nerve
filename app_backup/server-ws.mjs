...
// Opt-in native HTTPS (#5242). Resolved once at boot: when both NERVE_TLS_CERT
// and NERVE_TLS_KEY point at readable files we terminate TLS on the same
...
process.env.NERVE_INTERNAL_SCHEME = tlsOptions ? "https" : "http";
if (tlsOptions) {
  console.log(`[nerve][tls] HTTPS enabled — terminating TLS with cert=${tlsOptions.certPath}`);
}

process.env.NERVE_WS_BRIDGE_SECRET ||= randomUUID();
...
http.createServer = function createServerWithResponsesWs(...args) {
  ...
  // (#7003). This wrapper is what `nerve serve` / Docker / Electron actually
